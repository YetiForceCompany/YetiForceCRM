...
<div class="well" id="VtVTEmailTemplateTaskContainer">
	<div class="row-fluid">
		...
		<!--
		<div class="row-fluid padding-bottom1per">
			<span class="span4">{vtranslate('Do you send all attachments', $QUALIFIED_MODULE)}</span>
			<input type="checkbox" name="attachments" class="span4" value="true" {if $TASK_OBJECT->attachments eq 'true'}checked{/if}>
		</div>
		-->
		<div class="row-fluid padding-bottom1per">
			<span class="span4">{vtranslate('Select e-mail address', $QUALIFIED_MODULE)}</span>
			<select class="chzn-select span4" name="email" data-placeholder="{vtranslate('LBL_SELECT_FIELD',$QUALIFIED_MODULE)}" data-validation-engine='validate[required]'>
		...
		<div class="row-fluid padding-bottom1per">
			<span class="span4">{vtranslate('Send a copy to email', $QUALIFIED_MODULE)}</span>
			<input class="span4 marginLeftZero" data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="copy_email" value="{$TASK_OBJECT->copy_email}">
