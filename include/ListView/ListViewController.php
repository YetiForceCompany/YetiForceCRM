...
class ListViewController
{
	...
	public function setupAccessiblePicklistValueList($name)
	{
		...
		if ($this->picklistRoleMap[$name]) {
			$this->picklistValueMap[$name] = App\Fields\Picklist::getAssignedPicklistValues($name, $this->user->roleid);
		}
	}
			...
			foreach ($listViewFields as $fieldName) {
				...
				if ($uitype == 308) {
					...
					if (!empty($value)) {
						...
						$value = vtlib\Functions::suppressHTMLTags(implode(',', \App\Json::decode($temp_val)));
					}
