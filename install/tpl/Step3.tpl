								...
								<tbody>
									{foreach from=$STABILITY_CONF key=key item=item}
										<tr {if $item.status}class="danger"{/if}>
											<td>
												<label>{$key}</label>
												{if isset($item.help)}<a href="#" class="popoverTooltip pull-right" data-placement="rigth" data-content="{App\Language::translate($item.help, 'Settings::ConfReport')}"><i class="fas fa-info-circle"></i></a>{/if}
											</td>
