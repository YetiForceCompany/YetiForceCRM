...
class TextParser extends \Tests\Base
{
	...
	public function testRecord()
	{
		...
		self::$parserRecord->withoutTranslations(true);
		$text = '+ $(record : ChangesListValues)$ +';
		$this->assertNotSame('+  +', self::$parserRecord->setContent($text)->parse()->getContent(), 'Test record changes list values should be not empty(withoutTranslations)');
		self::$parserRecord->withoutTranslations(false);

		$this->assertStringContainsString('TestLead sp. z o.o.', self::$parserRecord->setContent($text)->parse()->getContent(), 'Test record changes list values should contain "TestLead sp. z o.o."');
		$this->assertStringContainsString('autogenerated test lead for \App\TextParser tests', self::$parserRecord->setContent($text)->parse()->getContent(), 'Test record changes list values should contain "autogenerated test lead for \App\TextParser tests"');
		...
		$text = '+ $(record : ChangesListChanges)$ +';
		$this->assertNotFalse(strpos(self::$parserRecord->setContent($text)->parse()->getContent(), 'test'), 'Test record changes list should should contain vat_id info');
		self::$parserRecord->withoutTranslations(true);
		$text = '+ $(record : ChangesListChanges)$ +';
		$this->assertNotFalse(strpos(self::$parserRecord->setContent($text)->parse()->getContent(), 'test'), 'Test record changes list should should contain vat_id info');
		self::$parserRecord->withoutTranslations(false);
		...
		$text = '+ $(record : company)$ +';
		$this->assertSame('+ ' . self::$recordLeads->get('company') . ' +', self::$parserRecord->setContent($text)
			->parse()
