...
$server->register(
	'get_invoice_detail', array('id' => 'xsd:string', 'block' => 'xsd:string', 'contactid' => 'xsd:string', 'sessionid' => 'xsd:string'), array('return' => 'tns:field_details_array'), $NAMESPACE);
...
function get_combo_values($input_array)
{
	...
	if (!vtlib_isModuleActive('ServiceContracts')) {
		...
		if ($showAll == 'true') {
			$servicequery .= ' OR vtiger_servicecontracts.sc_related_to = (SELECT parentid FROM vtiger_contactdetails WHERE contactid=? AND parentid <> 0)
								OR vtiger_servicecontracts.sc_related_to IN
											(SELECT contactid FROM vtiger_contactdetails WHERE parentid =
													(SELECT parentid FROM vtiger_contactdetails WHERE contactid=? AND parentid <> 0))
							';
...
function get_tickets_list($input_array)
{
	...
	if ($only_mine == 'true' || $show_all == 'false') {
		...
		$contactquery = "SELECT contactid, parentid FROM vtiger_contactdetails " .
			...
			" AND vtiger_crmentity.deleted = 0 " .
			" WHERE (parentid = (SELECT parentid FROM vtiger_contactdetails WHERE contactid = ?)  AND parentid != 0) OR contactid = ?";
...
function get_invoice_detail($id, $module, $customerid, $sessionid)
{
	...
	$fieldquery = "SELECT fieldname, columnname, fieldlabel,block,uitype FROM vtiger_field WHERE tabid = ? AND displaytype in (1,2,4,10) ORDER BY block,sequence";
	$fieldres = $adb->pquery($fieldquery, array(getTabid($module)));
	$nooffields = $adb->num_rows($fieldres);
	$query = "select vtiger_invoice.*,vtiger_crmentity.* ,vtiger_invoiceaddress.*,
		vtiger_invoicecf.* from vtiger_invoice
		inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_invoice.invoiceid
		LEFT JOIN vtiger_invoiceaddress ON vtiger_invoice.invoiceid = vtiger_invoiceaddress.invoiceaddressid
	...
	for ($i = 0; $i < $nooffields; $i++) {
		...
		$fieldlabel = Vtiger_Language_Handler::getTranslatedString($adb->query_result($fieldres, $i, 'fieldlabel'), 'Invoice', vglobal('default_language'));
		...
		$output[0][$module][$i]['blockname'] = Vtiger_Language_Handler::getTranslatedString($blocklabel, $module, vglobal('default_language'));
	}
	...
	switch ($module) {
		...
		case 'SalesOrder' :
			$query = "SELECT vtiger_salesorder.salesorderid
				...
				WHERE vtiger_crmentity.deleted=0
					AND vtiger_salesorder.accountid IN (" . generateQuestionMarks($allowed_contacts_and_accounts) . ")
		...
		case 'Invoice' : $query = "SELECT vtiger_invoice.invoiceid
								...
								WHERE vtiger_crmentity.deleted=0
									AND vtiger_invoice.accountid IN (" . generateQuestionMarks($allowed_contacts_and_accounts) . ")
		...
		case 'Project' :
			...
			break;
		case 'ProjectTask' :
			$query = "SELECT vtiger_projecttask.projecttaskid FROM vtiger_projecttask
					Left JOIN vtiger_project ON vtiger_project.projectid = vtiger_projecttask.projectid
					Left JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_project.projectid AND vtiger_crmentity.deleted = 0
					WHERE vtiger_project.linktoaccountscontacts IN (" . generateQuestionMarks($allowed_contacts_and_accounts) . ")
					AND vtiger_projecttask.projecttaskid = ?";
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
		...
		case 'ProjectMilestone' :
			$query = "SELECT vtiger_projectmilestone.projectmilestoneid FROM vtiger_projectmilestone
					Left JOIN vtiger_project ON vtiger_project.projectid = vtiger_projectmilestone.projectid
					Left JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_project.projectid AND vtiger_crmentity.deleted = 0
					WHERE vtiger_project.linktoaccountscontacts IN (" . generateQuestionMarks($allowed_contacts_and_accounts) . ")
					AND vtiger_projectmilestone.projectmilestoneid = ?";
			$res = $adb->pquery($query, array($allowed_contacts_and_accounts, $entityid));
			if ($adb->num_rows($res) > 0) {
				return true;
			}
