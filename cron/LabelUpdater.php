...
WHERE `vtiger_crmentity`.deleted = '0' AND (u_yf_crmentity_label.label IS NULL OR u_yf_crmentity_search_label.searchlabel IS NULL) AND vtiger_tab.`presence` = 0 LIMIT 1000");
...
LEFT JOIN u_yf_crmentity_search_label ON u_yf_crmentity_search_label.crmid = vtiger_crmentity.crmid WHERE `vtiger_crmentity`.deleted = '0' AND (u_yf_crmentity_label.label = '' OR u_yf_crmentity_search_label.searchlabel = '') AND vtiger_tab.`presence` = 0 LIMIT 1000");
while ($row = $adb->getRow($result)) {
