...
(function() {
    ...
    initElement: function(el) {
      if (!el.getContext) {
        ...
        if (attrs.width && attrs.width.specified) {
          // YTTODO: use runtimeStyle and coordsize
  ...
  contextPrototype.beginPath = function() {
    // YTTODO: Branch current matrix so that save/restore has no effect
    //       as per safari docs.
    this.currentPath_ = [];
  };
  ...
  contextPrototype.stroke = function(aFill) {
    ...
    for (var i = 0; i < this.currentPath_.length; i++) {
      ...
      // YTTODO: Following is broken for curves due to
  ...
  function appendFill(ctx, lineStr, min, max) {
    ...
    if (fillStyle instanceof CanvasGradient_) {
      // YTTODO: Gradients transformed with the transformation matrix.
      var angle = 0;
      var focus = {x: 0, y: 0};
      ...
      if (width && height) {
        ...
        lineStr.push('<g_vml_:fill',
                     ...
                     ' type="tile"',
                     // YTTODO: Figure out the correct size to fit the scale.
                     //' size="', w, 'px ', h, 'px"',
                     ' src="', fillStyle.src_, '" />');
       }
    } else {
  ...
  contextPrototype.drawText_ = function(text, x, y, maxWidth, stroke) {
    ...
    if (stroke) {
      ...
      // YTTODO: Fix the min and max params.
