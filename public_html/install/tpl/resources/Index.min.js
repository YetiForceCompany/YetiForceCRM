...
jQuery.Class('Install_Index_Js', {
	fieldsCached: ['db_server', 'db_username', 'db_name', 'currency_name', 'firstname', 'lastname', 'admin_email', 'dateformat', 'default_timezone'],
	checkUsername: function checkUsername(field, rules, i, options) {
		...
		var logins = JSON.parse($('#not_allowed_logins').val());
		if ($.inArray(fieldValue, logins) !== -1) {
			return app.vtranslate('LBL_INVALID_USERNAME_ERROR');
		}
	},
	checkDbName: function checkDbName(field, rules, i, options) {
		var fieldValue = field.val(),
	...
	registerEventForStep1: function registerEventForStep1() {
	...
	registerEventForStep2: function registerEventForStep2() {
		var modalContainer = $('.js-license-modal');
	...
	showBuyModal: function showBuyModal(event) {
		$.get('Install.php?mode=showBuyModal&product=' + $(event.currentTarget).data('product')).done(function (data) {
	...
	registerEventForStepChooseHost: function registerEventForStepChooseHost() {
		var _this = this;

		$('.js-buy-modal').on('click', this.showBuyModal);
		$('.js-product-modal').on('click', function (e) {
			$.get('Install.php?mode=showProductModal&product=' + $(e.currentTarget).data('product')).done(function (data) {
				app.showModalWindow(data, '', function (modalContainer) {
					modalContainer.find('.js-modal__save').on('click', function (_) {
						app.hideModalWindow();
						_this.showBuyModal(e);
					});
				});
			});
		});
	...
	registerEventForStep3: function registerEventForStep3() {
		$('#recheck').on('click', function () {
			window.location.reload();
		});
		var elements = jQuery('.js-wrong-status');
	...
	registerEventForStep4: function registerEventForStep4() {
		var _this2 = this;
		...
		$('form[name="step4"]').on('submit', function (e) {
			if (_this2.checkForm()) {
				e.preventDefault();
			} else {
	...
	registerEventForStep5: function registerEventForStep5() {
	...
	registerEventForStep6: function registerEventForStep6() {
	...
	registerEventForMigration: function registerEventForMigration() {
