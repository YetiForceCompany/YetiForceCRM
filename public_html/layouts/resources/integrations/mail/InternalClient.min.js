 ...
 * @class
 */Integrations_Mail_Base.driver="InternalClient",window.Integrations_Mail_InternalClient=/*#__PURE__*/function(_Integrations_Mail_Ba){function Integrations_Mail_InternalClient(){return _classCallCheck(this,Integrations_Mail_InternalClient),_super.apply(this,arguments)}_inherits(Integrations_Mail_InternalClient,_Integrations_Mail_Ba);var _super=_createSuper(Integrations_Mail_InternalClient);return _createClass(Integrations_Mail_InternalClient,[{key:"sendMail",value:/** @inheritdoc */function sendMail(attr){this.log("|\u25BA| sendMail",attr),attr.record&&app.getRecordId()&&attr.record!==app.getRecordId()&&(attr.crmModule=app.getModuleName(),attr.crmRecord=app.getRecordId()),AppConnector.request({dataType:"json",data:_objectSpread({module:"AppComponents",action:"Mail",mode:"sendMail"},attr)}).done(function(response){response.result.status?(attr.record&&app.getRecordId()&&attr.record!==app.getRecordId()&&(response.result.url+="&crmModule="+attr.crmModule+"&crmRecord="+attr.crmRecord),Vtiger_Index_Js.sendMailWindow(response.result.url,response.result.popup)):app.showError({title:app.vtranslate("JS_UNEXPECTED_ERROR"),text:response.result.text});});}}]),Integrations_Mail_InternalClient}(Integrations_Mail_Base);
//# sourceMappingURL=InternalClient.min.js.map
