 ...
 * @class
 */Integrations_Pbx_Base.driver="GenesysWdeWhirly",window.Integrations_Pbx_GenesysWdeWhirly=/*#__PURE__*/function(_Integrations_Pbx_Bas){/** @inheritdoc */function Integrations_Pbx_GenesysWdeWhirly(container){return _classCallCheck(this,Integrations_Pbx_GenesysWdeWhirly),_super.call(this,container)}/** @inheritdoc */_inherits(Integrations_Pbx_GenesysWdeWhirly,_Integrations_Pbx_Bas);var _super=_createSuper(Integrations_Pbx_GenesysWdeWhirly);return _createClass(Integrations_Pbx_GenesysWdeWhirly,[{key:"performCall",value:function performCall(data){var _this=this;this.log("|\u25BA| performCall",data),AppConnector.request(_objectSpread({module:"AppComponents",action:"Pbx",mode:"performCall"},data)).done(function(response){response.result.status?(app.showNotify({title:response.result.text,type:"info"}),$.ajax({url:response.result.url,headers:{"Token-Api":response.result.token}}).done(function(ajax){_this.log("|\u25C4| performCall",ajax),1==ajax.data.status?app.showNotify({title:ajax.data.description,type:"success"}):app.showError({title:app.vtranslate("JS_UNEXPECTED_ERROR"),text:ajax.data.description});}).fail(function(_jqXHR,textStatus){app.showError({title:app.vtranslate("JS_UNEXPECTED_ERROR"),text:textStatus});})):app.showError({title:app.vtranslate("JS_UNEXPECTED_ERROR"),text:response.result.text});});}}]),Integrations_Pbx_GenesysWdeWhirly}(Integrations_Pbx_Base);
