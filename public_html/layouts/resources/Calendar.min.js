 ...
 */window.Calendar_Js=/*#__PURE__*/function(){/**
	 ...
	 * @returns {object}
	 */},{key:"setBrowserHistoryOptions",value:function setBrowserHistoryOptions(){var historyParams=app.getMainParams("historyParams",!0),options=null;if(historyParams&&(historyParams.length||Object.keys(historyParams).length)){options={start:historyParams.start,end:historyParams.end,time:historyParams.time,hiddenDays:historyParams.hiddenDays.split(",").map(function(x){var parsedValue=parseInt(x);return isNaN(parsedValue)?"":parsedValue}),cvid:historyParams.cvid,defaultView:historyParams.viewType,user:historyParams.user};var dateFormat=CONFIG.dateFormat.toUpperCase(),s=moment(options.start,dateFormat).valueOf(),e=moment(options.end,dateFormat).valueOf();options.defaultDate=moment(moment(s+(e-s)/2).format("YYYY-MM-DD")),Object.keys(options).forEach(function(key){return "undefined"===options[key]&&delete options[key]}),app.moduleCacheSet("browserHistoryEvent",!1),app.setMainParams("showType",options.time),app.setMainParams("usersId",options.user),app.setMainParams("defaultView",options.defaultView);}return window.addEventListener("popstate",function(){app.moduleCacheSet("browserHistoryEvent",!0);},!1),options}/**
	 * Register filters
	 */},{key:"registerFilters",value:function registerFilters(){var thisInstance=this,aDeferred=jQuery.Deferred(),sideBar=thisInstance.getSidebarView();return !sideBar||0>=sideBar.length?(aDeferred.resolve(!0),aDeferred.promise()):(sideBar.find(".js-sidebar-filter-container").each(function(_,row){var formContainer=$(row);thisInstance.registerUsersChange(formContainer),App.Fields.Picklist.showSelect2ElementView(formContainer.find("select")),app.showNewScrollbar(formContainer,{suppressScrollX:!0}),thisInstance.registerFilterForm(formContainer);}),thisInstance.registerSelectAll(sideBar),aDeferred.promise())}/**
	 * Register filter for users and groups
	 * @param {jQuery} container
	 */},{key:"registerFilterForm",value:function registerFilterForm(container){var _this2=this;container.find(".js-filter__search").length&&container.find(".js-filter__search").on("keyup",this.findElementOnList.bind(self)),container.find(".js-calendar__filter__select, .filterField").each(function(_,e){var element=$(e),name=element.data("cache"),cachedValue=app.moduleCacheGet(name);if(0<element.length&&void 0!==cachedValue)"SELECT"==element.prop("tagName")&&element.val(cachedValue);else if(name&&0<element.length&&void 0===cachedValue&&!element.find(":selected").length&&0!==element.data("selected")){var allOptions=[];element.find("option").each(function(i,option){allOptions.push($(option).val());}),element.val(allOptions),app.moduleCacheSet(name,cachedValue);}element.off("change"),App.Fields.Picklist.showSelect2ElementView(element),element.on("change",function(e){var item=$(e.currentTarget),value=item.val();null==value&&(value=""),"checkbox"==item.attr("type")&&(value=element.is(":checked")),app.moduleCacheSet(item.data("cache"),value),_this2.getCalendarView().fullCalendar("getCalendar").view.options.loadView();});}),container.find(".js-filter__container_checkbox_list .js-filter__item__val").off("change").on("change",function(){_this2.getCalendarView().fullCalendar("getCalendar").view.options.loadView();});}},{key:"registerUsersChange",value:function registerUsersChange(formContainer){var _this3=this;formContainer.find(".js-input-user-owner-id-ajax, .js-input-user-owner-id").on("change",function(){_this3.getCalendarView().fullCalendar("getCalendar").view.options.loadView();});}/**
	 * Register change on select all checkbox
	 * @param {jQuery} formContainer
	 */},{key:"registerSelectAll",value:function registerSelectAll(formContainer){var _this4=this;formContainer.find(".js-select-all").on("change",function(e){var checkboxSelectAll=$(e.currentTarget),checkboxes=formContainer.find(".js-input-user-owner-id-ajax, .js-input-user-owner-id");checkboxSelectAll.is(":checked")?checkboxes.prop("checked",!0):(checkboxes.prop("checked",!1),formContainer.find("#ownerId"+CONFIG.userId).prop("checked",!0));var calendar=_this4.getCalendarView().fullCalendar("getCalendar");calendar&&calendar.view.options.loadView();});}/**
	 * Register events.
	 * @returns {object}
	 */},{key:"registerEvents",value:function registerEvents(){this.registerFilters(),this.registerSitebarEvents(),this.renderCalendar(),this.registerButtonSelectAll(),this.registerAddButton();}/**
	 * Invokes fullcalendar with options.
	 */},{key:"renderCalendar",value:function renderCalendar(){this.getCalendarView().fullCalendar(this.calendarOptions);}/**
	 * Register sitebar events.
	 */},{key:"registerSitebarEvents",value:function registerSitebarEvents(){var _this5=this;$(".bodyContents").on("Vtiger.Widget.Load.undefined",function(){_this5.registerSelect2Event();});}/**
	 * Load calendar data
	 */},{key:"loadCalendarData",value:function loadCalendarData(){var _this6=this,defaultParams=this.getDefaultParams();if(this.getCalendarView().fullCalendar("removeEvents"),!defaultParams.emptyFilters){var progressInstance=$.progressIndicator();AppConnector.request(defaultParams).done(function(events){_this6.getCalendarView().fullCalendar("addEventSource",events.result),progressInstance.hide();});}}},{key:"getSidebarView",value:function getSidebarView(){return this.sidebarView.length||(this.sidebarView=this.container.find(".js-calendar-right-panel")),this.sidebarView}},{key:"getCurrentCvId",value:function getCurrentCvId(){return $(".js-calendar__extended-filter-tab .active").parent(".js-filter-tab").data("cvid")}/**
	 * Default params
	 * @returns {{module: *, action: string, mode: string, start: *, end: *, user: *, emptyFilters: boolean}}
	 */},{key:"getDefaultParams",value:function getDefaultParams(){var formatDate=CONFIG.dateFormat.toUpperCase(),view=this.getCalendarView().fullCalendar("getView"),cvid=this.getCurrentCvId(),users=app.moduleCacheGet("calendar-users")||CONFIG.userId,sideBar=this.getSidebarView(),params={module:this.module?this.module:CONFIG.module,action:"Calendar",mode:"getEvents",start:view.start.format(formatDate),end:view.end.format(formatDate),user:users,cvid:cvid,emptyFilters:0===users.length},filters=[];return sideBar.find(".calendarFilters .filterField").each(function(){var name,value,element=$(this);"checkbox"==element.attr("type")?(name=element.val(),value=element.prop("checked")?1:0):(name=element.attr("name"),value=element.val()),filters.push({name:name,value:value});}),filters.length&&(params.filters=filters),sideBar.find(".js-sidebar-filter-container").each(function(_,e){var element=$(e),name=element.data("name"),cacheName=element.data("cache");name&&cacheName&&app.moduleCacheGet(cacheName)&&(params[name]=app.moduleCacheGet(cacheName),params.emptyFilters=!params.emptyFilters&&0===params[name].length);}),sideBar.find(".js-filter__container_checkbox_list").each(function(_,e){var filters=[],element=$(e),name=element.data("name");element.find(".js-filter__item__val:checked").each(function(){filters.push($(this).val());}),name&&(params[name]=filters);}),sideBar.find(".js-calendar__filter__select").each(function(_,e){var element=$(e),name=element.attr("name"),cacheName=element.data("cache");name&&(params[name]=cacheName&&app.moduleCacheGet(cacheName)?app.moduleCacheGet(cacheName):element.val(),params.emptyFilters=!params.emptyFilters&&0===params[name].length);}),params}/**
	 * Register select2 event.
	 */},{key:"registerSelect2Event",value:function registerSelect2Event(){var self=this;$(".siteBarRight .js-calendar__filter__select").each(function(){var element=$(this),name=element.data("cache"),cachedValue=app.moduleCacheGet(name);if(0<element.length&&void 0!==cachedValue)"SELECT"==element.prop("tagName")&&element.val(cachedValue);else if(0<element.length&&void 0===cachedValue&&!element.find(":selected").length){var allOptions=[];element.find("option").each(function(i,option){allOptions.push($(option).val());}),element.val(allOptions),app.moduleCacheSet(name,cachedValue);}});var selectsElements=$(".siteBarRight .select2, .siteBarRight .filterField");selectsElements.off("change"),App.Fields.Picklist.showSelect2ElementView(selectsElements),selectsElements.on("change",function(){var element=$(this),value=element.val();null==value&&(value=""),"checkbox"==element.attr("type")&&(value=element.is(":checked")),app.moduleCacheSet(element.data("cache"),value),self.loadCalendarData();});}/**
	 * Register button select all.
	 */},{key:"registerButtonSelectAll",value:function registerButtonSelectAll(){var selectBtn=$(".selectAllBtn");selectBtn.on("click",function(){var selectAllLabel=$(this).find(".selectAll"),deselectAllLabel=$(this).find(".deselectAll");selectAllLabel.hasClass("d-none")?(selectAllLabel.removeClass("d-none"),deselectAllLabel.addClass("d-none"),$(this).closest(".quickWidget").find("select option").prop("selected",!1)):($(this).closest(".quickWidget").find("select option").prop("selected",!0),deselectAllLabel.removeClass("d-none"),selectAllLabel.addClass("d-none")),$(this).closest(".quickWidget").find("select").trigger("change");});}/**
	 * Register add button.
	 */},{key:"registerAddButton",value:function registerAddButton(){var self=this;$(".js-add").on("click",function(){self.getCalendarCreateView().done(function(data){App.Components.QuickCreate.showModal(data,{callbackFunction:function callbackFunction(data){self.addCalendarEvent(data.result);}});});});}/**
	 ...
	 * @returns {promise}
	 */},{key:"loadCalendarCreateView",value:function loadCalendarCreateView(){var aDeferred=jQuery.Deferred(),moduleName=app.getModuleName();return App.Components.QuickCreate.getForm("index.php?module="+moduleName+"&view=QuickCreateAjax",moduleName).done(function(data){aDeferred.resolve(jQuery(data));}).fail(function(textStatus,errorThrown){aDeferred.reject(textStatus,errorThrown);}),aDeferred.promise()}/**
	 * Add event data to render.
	 */},{key:"getEventRenderData",value:function getEventRenderData(calendarDetails){var calendar=this.getCalendarView(),eventObject={id:calendarDetails._recordId,title:calendarDetails._recordLabel,start:calendar.fullCalendar("moment",calendarDetails.date_start.value+" "+calendarDetails.time_start.value).format(),end:calendar.fullCalendar("moment",calendarDetails.due_date.value+" "+calendarDetails.time_end.value).format(),start_display:calendarDetails.date_start.display_value+" "+calendarDetails.time_start.display_value,end_display:calendarDetails.due_date.display_value+" "+calendarDetails.time_end.display_value,url:"index.php?module=".concat(CONFIG.module,"&view=Detail&record=").concat(calendarDetails._recordId),className:"js-popover-tooltip--record ownerCBg_".concat(calendarDetails.assigned_user_id.value," picklistCBr_").concat(CONFIG.module,"_").concat($(".js-calendar__filter__select[data-cache=\"calendar-types\"]").length&&calendarDetails[this.eventTypeKeyName]?this.eventTypeKeyName+"_"+calendarDetails[this.eventTypeKeyName].value:""),allDay:"undefined"!=typeof calendarDetails.allday&&"on"==calendarDetails.allday.value};return eventObject}},{key:"isNewEventToDisplay",value:function isNewEventToDisplay(eventObject){var ownerSelects=$(".js-calendar__filter__select[data-cache=\"calendar-users\"]").add($(".js-calendar__filter__select[data-cache=\"calendar-groups\"]"));if(0>$.inArray(eventObject.assigned_user_id.value,ownerSelects.val()))return this.refreshFilterValues(eventObject,ownerSelects),!1;var calendarTypes=$(".js-calendar__filter__select[data-cache=\"calendar-types\"]");return !(calendarTypes.length&&(this.eventTypeKeyName||this.setEventTypeKey(eventObject,calendarTypes.data("name")),this.eventTypeKeyName&&calendarTypes.val().length&&0>$.inArray(eventObject[this.eventTypeKeyName].value,calendarTypes.val())))}},{key:"setEventTypeKey",value:function setEventTypeKey(eventObject,keyName){var self=this;keyName&&void 0!==eventObject.keyName?self.eventTypeKeyName=keyName:Object.keys(eventObject).forEach(function(key){key.endsWith("type")&&(self.eventTypeKeyName=key);});}},{key:"refreshFilterValues",value:function refreshFilterValues(eventObject,filtersValues){var _this7=this;if(CONFIG.searchShowOwnerOnlyInList){var allOptions=[];filtersValues.find("option").each(function(i,option){allOptions.push($(option).val());}),0>$.inArray(eventObject.assigned_user_id.value,allOptions)&&AppConnector.request("module=".concat(CONFIG.module,"&view=RightPanel&mode=getUsersList")).done(function(usersData){var filterUsers=$(".js-calendar__filter--users"),filterGroups=$(".js-calendar__filter--groups");filterUsers.html(usersData),usersData&&filterUsers.closest(".js-toggle-panel").removeClass("d-none"),filterGroups.length?AppConnector.request("module=".concat(CONFIG.module,"&view=RightPanel&mode=getGroupsList")).done(function(groupsData){filterGroups.html(groupsData),groupsData&&filterGroups.closest(".js-toggle-panel").removeClass("d-none"),_this7.registerSelect2Event();}):_this7.registerSelect2Event();});}}/**
	 * Add calendar event.
	 */},{key:"addCalendarEvent",value:function addCalendarEvent(eventObject){this.isNewEventToDisplay(eventObject)&&this.getCalendarView().fullCalendar("renderEvent",this.getEventRenderData(eventObject));}/**
	 * Get calendar container.
	 * @returns {(boolean|jQuery)}
	 */},{key:"getCalendarView",value:function getCalendarView(){return !1==this.calendarView&&(this.calendarView=this.container.find(".js-calendar__container")),this.calendarView}}]),_class}(),window.Calendar_Unselectable_Js=/*#__PURE__*/function(_Calendar_Js){function _class2(){return _classCallCheck(this,_class2),_super.apply(this,arguments)}_inherits(_class2,_Calendar_Js);var _super=_createSuper(_class2);return _createClass(_class2,[{key:"setCalendarModuleOptions",value:/**
	 * Set calendar module options.
	 * @returns {{allDaySlot: boolean, dayClick: object, selectable: boolean}}
	 */function setCalendarModuleOptions(){var self=this;return {allDaySlot:!1,dayClick:!!this.eventCreate&&function(date){self.registerDayClickEvent(date.format()),self.getCalendarView().fullCalendar("unselect");},selectable:!1}}/**
	 * Register day click event.
	 * @param {string} date
	 */},{key:"registerDayClickEvent",value:function registerDayClickEvent(date){var self=this;self.getCalendarCreateView().done(function(data){if(!(0>=data.length)){var dateFormat=data.find("[name=\"date_start\"]").data("dateFormat").toUpperCase(),timeFormat=data.find("[name=\"time_start\"]").data("format"),defaultTimeFormat="hh:mm A";24==timeFormat&&(defaultTimeFormat="HH:mm");var endTimeString,startDateInstance=Date.parse(date),startDateString=moment(date).format(dateFormat),startTimeString=moment(date).format(defaultTimeFormat),endDateInstance=Date.parse(date),endDateString=moment(date).format(dateFormat),view=self.getCalendarView().fullCalendar("getView");if("month"==view.name){var diffDays=parseInt((endDateInstance-startDateInstance)/86400000);if(1<diffDays){var defaultFirstHour=app.getMainParams("startHour"),explodedTime=defaultFirstHour.split(":");startTimeString=explodedTime[0];var defaultLastHour=app.getMainParams("endHour");explodedTime=defaultLastHour.split(":"),endTimeString=explodedTime[0];}else {var now=new Date;startTimeString=moment(now).format(defaultTimeFormat),endTimeString=moment(now).add(15,"minutes").format(defaultTimeFormat);}}else endTimeString=moment(endDateInstance).add(30,"minutes").format(defaultTimeFormat);data.find("[name=\"date_start\"]").val(startDateString),data.find("[name=\"due_date\"]").val(endDateString),data.find("[name=\"time_start\"]").val(startTimeString),data.find("[name=\"time_end\"]").val(endTimeString),App.Components.QuickCreate.showModal(data,{callbackFunction:function callbackFunction(data){self.addCalendarEvent(data.result,dateFormat);}});}});}}]),_class2}(Calendar_Js);
