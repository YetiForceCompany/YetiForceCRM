...
if(0==dependentFieldInContext.val().length)continue;var comparedDateVal=value-dependentValue;if(0<comparedDateVal){var errorInfo=fieldLabel+' '+app.vtranslate('JS_SHOULD_BE_LESS_THAN_OR_EQUAL_TO')+' '+dependentFieldLabel+'';return this.setError(errorInfo),!1}}}return !0}},getDateTimeInstance:function getDateTimeInstance(field){var dateFormat=field.data('dateFormat'),fieldValue=field.val();try{var dateTimeInstance=Vtiger_Helper_Js.getDateInstance(fieldValue,dateFormat);}catch(err){return this.setError(err),!1}return dateTimeInstance}}),Vtiger_Base_Validator_Js('Vtiger_Currency_Validator_Js',{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!0!=response)return response;var fieldData=this.getElement().data(),decimalSeparator=fieldData.decimalSeparator?fieldData.decimalSeparator:CONFIG.currencyDecimalSeparator,groupSeparator=fieldData.groupSeparator?fieldData.groupSeparator:CONFIG.currencyGroupingSeparator,strippedValue=this.getFieldValue().replace(decimalSeparator,''),spacePattern=/\s/;(spacePattern.test(decimalSeparator)||spacePattern.test(groupSeparator))&&(strippedValue=strippedValue.replace(/ /g,''));var errorInfo;'$'===groupSeparator&&(groupSeparator='\\$'),'.'===groupSeparator&&(groupSeparator='\\.');var regex=new RegExp(groupSeparator,'g');//Note: Need to review if we should allow only positive values in currencies
/*if(strippedValue < 0){
		 var errorInfo = app.vtranslate('JS_CONTAINS_ILLEGAL_CHARACTERS');//"currency value should be greater than or equal to zero";
		 this.setError(errorInfo);
		 return false;
		 }*/if(strippedValue=strippedValue.replace(regex,''),isNaN(strippedValue))return errorInfo=app.vtranslate('JS_CONTAINS_ILLEGAL_CHARACTERS'),this.setError(errorInfo),!1;if(0>strippedValue)return errorInfo=app.vtranslate('JS_ACCEPT_POSITIVE_NUMBER'),this.setError(errorInfo),!1;var maximumLength='undefined'==typeof fieldData.fieldinfo?null:fieldData.fieldinfo.maximumlength;return !(maximumLength&&strippedValue>parseFloat(maximumLength))||(errorInfo=app.vtranslate('JS_ERROR_MAX_VALUE'),this.setError(errorInfo),!1)}}),Vtiger_Currency_Validator_Js('Vtiger_NumberUserFormat_Validator_Js',{/**
	 *Function which invokes field validation
	 * @param accepts field element as parameter
	 * @return error if validation fails true on success
