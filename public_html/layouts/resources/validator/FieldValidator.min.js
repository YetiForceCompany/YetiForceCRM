...
Vtiger_Base_Validator_Js('Vtiger_Email_Validator_Js',{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!0===response){var fieldValue=this.getFieldValue(),doubleRegex=/(^[-+]?\d+)(\.\d+)?$/;if(!fieldValue.toString().match(doubleRegex)){var errorInfo=app.vtranslate('JS_PLEASE_ENTER_DECIMAL_VALUE');return this.setError(errorInfo),!1}var fieldInfo=this.getElement().data().fieldinfo;if(!fieldInfo||!fieldInfo.maximumlength)return !0;var maximumLength=fieldInfo.maximumlength,minimumLength=-maximumLength;fieldValue=parseFloat(fieldValue);var ranges=maximumLength.split(',');if(2===ranges.length&&(maximumLength=ranges[1],minimumLength=ranges[0]),fieldValue>parseFloat(maximumLength)||fieldValue<parseFloat(minimumLength))return errorInfo=app.vtranslate('JS_ERROR_MAX_VALUE'),this.setError(errorInfo),!1}return response},/**
		 * Overwrites base function to avoid trimming and validate white spaces
		 * @return fieldValue
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();return !0==response?!(100<App.Fields.Double.formatToDb(this.getFieldValue()))||(this.setError(app.vtranslate('JS_PERCENTAGE_VALUE_SHOULD_BE_LESS_THAN_100')),!1):response}}),Vtiger_Base_Validator_Js('Vtiger_Url_Validator_Js',{invokeValidation:function invokeValidation(field){var validatorInstance=new Vtiger_Url_Validator_Js;validatorInstance.setElement(field);var result=validatorInstance.validate();return !0===result?result:validatorInstance.getError()}},{/**
		 * Function to validate the Url
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var fieldValue=this.getFieldValue(),regexp=/(^|\s)((https?:\/\/)?[\w-]+(\.[\w-]+)+\.?(:\d+)?(\/\S*)?)/gi,result=regexp.test(fieldValue);if(result||0!==fieldValue.indexOf('http://')&&0!==fieldValue.indexOf('https://')&&0!==fieldValue.indexOf('ftp://')&&0!==fieldValue.indexOf('ftps://')&&0!==fieldValue.indexOf('telnet://')&&0!==fieldValue.indexOf('smb://')&&0!==fieldValue.indexOf('www.')||(result=!0),!result){var errorInfo=app.vtranslate('JS_CONTAINS_ILLEGAL_CHARACTERS');//"Please enter valid url";
return this.setError(errorInfo),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_MultiSelect_Validator_Js',{invokeValidation:function invokeValidation(field){var validatorInstance=new Vtiger_MultiSelect_Validator_Js;validatorInstance.setElement(field);var result=validatorInstance.validate();return !0==result?result:validatorInstance.getError()}},{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var fieldInstance=this.getElement(),selectElementValue=fieldInstance.val();if(Array.isArray(selectElementValue)){var _iteratorNormalCompletion=!0,_didIteratorError=!1,_iteratorError=void 0;try{for(var _step,value,_iterator=selectElementValue[Symbol.iterator]();!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=!0)if(value=_step.value,value&&!/(?:[0-9a-z\u017F\u212A](?:[\x2D0-9a-z\u017F\u212A]{0,61}[0-9a-z\u017F\u212A])?\.)+[0-9a-z\u017F\u212A][\x2D0-9a-z\u017F\u212A]{0,61}[0-9a-z\u017F\u212A]/i.test(value))return this.setError(app.vtranslate('JS_PLEASE_SELECT_VALID_DOMAIN_NAMES')),!1}catch(err){_didIteratorError=!0,_iteratorError=err;}finally{try{!_iteratorNormalCompletion&&_iterator.return&&_iterator.return();}finally{if(_didIteratorError)throw _iteratorError}}}return !0}}),Vtiger_Double_Validator_Js('Vtiger_GreaterThanZero_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
...
if(0==dependentFieldInContext.val().length)continue;var comparedDateVal=value-dependentValue;if(0<comparedDateVal){var errorInfo=fieldLabel+' '+app.vtranslate('JS_SHOULD_BE_LESS_THAN_OR_EQUAL_TO')+' '+dependentFieldLabel+'';return this.setError(errorInfo),!1}}}return !0}},getDateTimeInstance:function getDateTimeInstance(field){var dateFormat=field.data('dateFormat'),fieldValue=field.val();try{var dateTimeInstance=Vtiger_Helper_Js.getDateInstance(fieldValue,dateFormat);}catch(err){return this.setError(err),!1}return dateTimeInstance}}),Vtiger_Base_Validator_Js('Vtiger_Currency_Validator_Js',{/**
	 ...
	 */invokeValidation:function invokeValidation(field){var instance=new Vtiger_Currency_Validator_Js;if(instance.setElement(field),!0!==instance.validate())return instance.getError()}}),Vtiger_Base_Validator_Js('Vtiger_ReferenceField_Validator_Js',{},{/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var field=this.getElement(),fieldData=field.data(),fieldDateFormat=fieldData.dateFormat,fieldValue=this.getFieldValue();try{if('range'!==fieldData.calendarType)fieldValue=[fieldValue];else if(fieldValue=fieldValue.split(','),2!==fieldValue.length)throw new Error;fieldValue.forEach(function(key){Vtiger_Helper_Js.getDateInstance(key,fieldDateFormat);});}catch(err){var errorInfo=app.vtranslate('JS_PLEASE_ENTER_VALID_DATE');return this.setError(errorInfo),!1}return !0}}),Vtiger_Date_Validator_Js('Vtiger_Datetime_Validator_Js',{},{}),Vtiger_Base_Validator_Js('Vtiger_Time_Validator_Js',{/**
		 * Function which invokes field validation
		 * @param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var response=this._super();if(!0!=response)return response;var fieldValue=this.getFieldValue();if(0==fieldValue){var errorInfo=app.vtranslate('JS_VALUE_SHOULD_BE_GREATER_THAN_ZERO');return this.setError(errorInfo),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_AlphaNumeric_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var field=this.getElement(),fieldValue=field.val(),alphaNumericRegex=/^[a-z0-9 _-]*$/i;if(!fieldValue.match(alphaNumericRegex)){var errorInfo=app.vtranslate('JS_CONTAINS_ILLEGAL_CHARACTERS');return this.setError(errorInfo),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_AlphaNumericWithSlashesCurlyBraces_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){var field=this.getElement(),fieldValue=field.val(),alphaNumericRegex=/^[\/a-z\\0-9{}()$|: _-]*$/i;if(!fieldValue.match(alphaNumericRegex)){var errorInfo=app.vtranslate('JS_CONTAINS_ILLEGAL_CHARACTERS');return this.setError(errorInfo),!1}return !0}}),Vtiger_Base_Validator_Js('Vtiger_InputMask_Validator_Js',{/**
		 *Function which invokes field validation
		 *@param accepts field element as parameter
		 * @return error if validation fails true on success
