  ...
  */
	validateValue: function validateValue(fieldValue) {
		...
		if (fieldData.advancedVerification == 1) {
			...
			var moduleName = form.find('[name="module"]').length ? form.find('[name="module"]').val() : app.getModuleName();
...
Vtiger_Base_Validator_Js('Vtiger_UserName_Validator_Js', {
  ...
  */
	validate: function validate() {
		var fieldValue = this.getFieldValue();
		var fieldData = this.getElement().data();
		var maximumLength = typeof fieldData.fieldinfo !== 'undefined' ? fieldData.fieldinfo.maximumlength : '3,64';
		var ranges = maximumLength.split(',');
...
Vtiger_Base_Validator_Js('Vtiger_Integer_Validator_Js', {
  ...
  */
	validate: function validate() {
		...
		var fieldInfo = this.getElement().data().fieldinfo;
		if (!fieldInfo || !fieldInfo.maximumlength) {
			return true;
		}
		var ranges = fieldInfo.maximumlength.split(',');
		if (ranges.length === 2) {
...
Vtiger_Email_Validator_Js('Vtiger_MultiEmails_Validator_Js', {
	invokeValidation: function invokeValidation(field) {
...
Vtiger_Base_Validator_Js('Vtiger_MultiDomain_Validator_Js', {
	invokeValidation: function invokeValidation(field, rules, i, options) {
  ...
  */
	validate: function validate() {
		...
		if (Array.isArray(selectElementValue)) {
			...
			try {
				for (var _iterator = selectElementValue[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
					var value = _step.value;

					if (value && !/(?:[0-9a-z\u017F\u212A](?:[\x2D0-9a-z\u017F\u212A]{0,61}[0-9a-z\u017F\u212A])?\.)+[0-9a-z\u017F\u212A][\x2D0-9a-z\u017F\u212A]{0,61}[0-9a-z\u017F\u212A]/i.test(value)) {
...
Vtiger_Base_Validator_Js('Vtiger_greaterThanDependentField_Validator_Js', {
  ...
  * @return false if validation error occurs
  */
	validate: function validate(dependentFieldList) {
		var field = this.getElement();
	...
	getDateTimeInstance: function getDateTimeInstance(field) {
...
Vtiger_Base_Validator_Js('Vtiger_dateAndTimeGreaterThanDependentField_Validator_Js', {}, {
  ...
  */
	...
	difference: function difference(fieldDateTimeInstance) {
...
Vtiger_Base_Validator_Js('Vtiger_Currency_Validator_Js', {
  ...
  */
	validate: function validate() {
		...
		var fieldData = this.getElement().data();
		var decimalSeparator = fieldData.decimalSeparator ? fieldData.decimalSeparator : CONFIG.currencyDecimalSeparator;
		var groupSeparator = fieldData.groupSeparator ? fieldData.groupSeparator : CONFIG.currencyGroupingSeparator;

		var strippedValue = this.getFieldValue().replace(decimalSeparator, '');
		var spacePattern = /\s/;
  ...
  return false;
		...
		var maximumLength = typeof fieldData.fieldinfo !== 'undefined' ? fieldData.fieldinfo.maximumlength : null;
...
Vtiger_Currency_Validator_Js('Vtiger_NumberUserFormat_Validator_Js', {
  ...
  */
	invokeValidation: function invokeValidation(field, rules, i, options) {
		var instance = new Vtiger_Currency_Validator_Js();
		instance.setElement(field);
  ...
  */
	validate: function validate() {
		...
		try {
			...
			fieldValue.forEach(function (key) {
...
Vtiger_Email_Validator_Js('Vtiger_MultiEmail_Validator_Js', {
  ...
  */
	invokeValidation: function invokeValidation(field) {
		var validatorInstance = new Vtiger_MultiEmail_Validator_Js();
		validatorInstance.setElement(field);
		var result = validatorInstance.validate();
		if (result == true) {
			return result;
  ...
  */
	validate: function validate() {
		...
		var allFields = $(this.field).closest('div.js-multi-email').eq(0).find('[class*=js-multi-email-row]');
		...
		for (var i = 0; i < arrayLength; ++i) {
			...
			var inputClass1 = $(allFields[i]).closest('div[class*=js-multi-email-row-]').eq(0).attr('class');
			var inputClass2 = $(this.field).closest('div[class*=js-multi-email-row-]').eq(0).attr('class');
...
Vtiger_Base_Validator_Js('Vtiger_MultiImage_Validator_Js', {
	invokeValidation: function invokeValidation(field, rules, i, options) {
		var instance = new Vtiger_MultiImage_Validator_Js();
		instance.setElement(field);
		if (instance.validate() != true) {
			return instance.getError();
		}
...
Vtiger_Base_Validator_Js('Vtiger_MaxSizeInByte_Validator_Js', {
	invokeValidation: function invokeValidation(field, rules, i, options) {
		var instance = new Vtiger_MaxSizeInByte_Validator_Js();
		instance.setElement(field);
		if (instance.validate() != true) {
			return instance.getError();
		}
...
Vtiger_Base_Validator_Js('Vtiger_FieldName_Validator_Js', {
	invokeValidation: function invokeValidation(field, rules, i, options) {
		var instance = new Vtiger_FieldName_Validator_Js();
		instance.setElement(field);
		if (instance.validate() != true) {
			return instance.getError();
		}
