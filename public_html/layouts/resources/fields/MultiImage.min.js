...
var MultiImage=function(){/**
	 ...
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:'submit',value:function submit(e,data){data.formData={hash:data.files[0].hash},App.Fields.MultiImage.currentFileUploads++;}/**
	 * Get file information
	 *
	 * @param {String} hash - file id
	 * @returns {Object}
	 */},{key:'getFileInfo',value:function getFileInfo(hash){for(var file,i=0,len=this.files.length;i<len;i++)if(file=this.files[i],file.hash===hash)return file;app.errorLog('File \''+hash+'\' not found.'),Vtiger_Helper_Js.showPnotify({text:app.vtranslate('JS_INVALID_FILE_HASH')+(' ['+hash+']')});}/**
	 ...
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:'uploadError',value:function uploadError(e,data){var _this=this;app.errorLog('File upload error.');var jqXHR=data.jqXHR,files=data.files;if('undefined'==typeof jqXHR.responseJSON||null===jqXHR.responseJSON)return App.Fields.MultiImage.currentFileUploads--,Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_FILE_UPLOAD_ERROR'));var response=jqXHR.responseJSON;// first try to show error for concrete file
return 'undefined'!=typeof response.result&&'undefined'!=typeof response.result.attach&&Array.isArray(response.result.attach)?(response.result.attach.forEach(function(fileAttach){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(fileAttach.hash,!1),'string'==typeof fileAttach.error?Vtiger_Helper_Js.showPnotify(fileAttach.error+(' ['+fileAttach.name+']')):Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_FILE_UPLOAD_ERROR')+(' ['+fileAttach.name+']'));}),void this.updateFormValues()):void(// else show default upload error
files.forEach(function(file){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(file.hash,!1),Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_FILE_UPLOAD_ERROR')+(' ['+file.name+']'));}),this.updateFormValues())}/**
	 * Success event handler from file upload request
	 *
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:'uploadSuccess',value:function uploadSuccess(e,data){var _this2=this,result=data.result,attach=result.result.attach;attach.forEach(function(fileAttach){var hash=fileAttach.hash;if(!hash)return app.errorLog(new Error(app.vtranslate('JS_INVALID_FILE_HASH')+(' ['+hash+']')));if('undefined'==typeof fileAttach.key)return _this2.uploadError(e,data);'undefined'!=typeof fileAttach.info&&fileAttach.info&&Vtiger_Helper_Js.showPnotify({type:'notice',text:fileAttach.info+(' ['+fileAttach.name+']')});var fileInfo=_this2.getFileInfo(hash);_this2.addFileInfoProperty(hash,'key',fileAttach.key),_this2.addFileInfoProperty(hash,'size',fileAttach.size),_this2.addFileInfoProperty(hash,'name',fileAttach.name),_this2.removePreviewPopover(hash),_this2.addPreviewPopover(fileInfo.file,fileInfo.previewElement,fileInfo.imageSrc),App.Fields.MultiImage.currentFileUploads--;}),this.updateFormValues();}/**
	 * Update form input values
	 */},{key:'updateFormValues',value:function updateFormValues(){this.elements.fileInput.val(null);var formValues=this.files.map(function(file){return {key:file.key,name:file.name,size:file.size}});this.elements.values.val(JSON.stringify(formValues));}/**
	 * Validate file
	 *
	 * @param {Object} file
	 * @returns {boolean}
	 */},{key:'validateFile',value:function validateFile(file){var valid=!1;return this.options.formats.forEach(function(format){file.type==='image/'+format&&(valid=!0);}),valid||Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_INVALID_FILE_TYPE')+' ['+file.name+']\n'+app.vtranslate('JS_AVAILABLE_FILE_TYPES')+'  ['+this.options.formats.join(', ')+']'),valid}/**
	 * Show limit error
	 */},{key:'showLimitError',value:function showLimitError(){this.elements.fileInput.val(''),Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_FILE_LIMIT')+' ['+this.options.limit+']');}/**
	 * Get only valid files from list
	 *
	 * @param {Array} files
	 * @returns {Array}
	 */},{key:'filterValidFiles',value:function filterValidFiles(files){var _this3=this;return files.length+this.files.length>this.options.limit?(this.showLimitError(),[]):files.filter(function(file){return _this3.validateFile(file)})}/**
	 * Set files hash
	 * @param {Array} files
	 * @returns {Array}
	 ...
	 */},{key:'deleteFile',value:function deleteFile(hash){var _this6=this,showConfirmation=!(1<arguments.length&&void 0!==arguments[1])||arguments[1];if(showConfirmation){var fileInfo=this.getFileInfo(hash);bootbox.confirm({title:'<i class="fa fa-trash-alt"></i> '+app.vtranslate('JS_DELETE_FILE'),message:app.vtranslate('JS_DELETE_FILE_CONFIRMATION')+' <span class="font-weight-bold">'+fileInfo.name+'</span>?',callback:function callback(result){result&&_this6.deleteFileCallback(hash);}});}else this.deleteFileCallback(hash);}/**
	 * File change event handler from jQuery-file-upload
	 *
	 * @param {Event} e
	 * @param {object} data
	 */},{key:'change',value:function change(e,data){var _this7=this;data.files=this.filterValidFiles(data.files),data.files=this.setFilesHash(data.files),this.dragLeave(e),data.files.length&&this.generatePreviewElements(data.files,function(){_this7.redraw();});}/**
	 ...
	 */},{key:'loadExistingFiles',value:function loadExistingFiles(){var _this12=this;this.files=this.files.map(function(file){return file.hash=App.Fields.Text.generateRandomHash(CONFIG.userId),file}).slice(0,this.options.limit),this.generatePreviewElements(this.files,function(element){_this12.elements.result.append(element);}),this.updateFormValues();}/**
