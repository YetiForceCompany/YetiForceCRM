...
/* {[The file is published on the basis of YetiForce Public License 4.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */'use strict';function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}function _defineProperties(target,props){for(var descriptor,i=0;i<props.length;i++)descriptor=props[i],descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor);}function _createClass(Constructor,protoProps,staticProps){return protoProps&&_defineProperties(Constructor.prototype,protoProps),staticProps&&_defineProperties(Constructor,staticProps),Constructor}var MultiImage=/*#__PURE__*/function(){/**
	 ...
	 * @param e
	 */return _createClass(MultiImage,[{key:"onFormSubmit",value:function onFormSubmit(e){return !App.Fields.MultiImage.currentFileUploads||(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),bootbox.alert(app.vtranslate("JS_WAIT_FOR_FILE_UPLOAD")),!1)}/**
...
return "undefined"!=typeof response.result&&"undefined"!=typeof response.result.attach&&Array.isArray(response.result.attach)?(response.result.attach.forEach(function(fileAttach){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(fileAttach.hash,!1),"string"==typeof fileAttach.error?app.showNotify({text:fileAttach.error+" [".concat(fileAttach.name,"]"),type:"error"}):app.showNotify({text:app.vtranslate("JS_FILE_UPLOAD_ERROR")+" [".concat(fileAttach.name,"]"),type:"error"});}),void this.updateFormValues()):void(// else show default upload error
files.forEach(function(file){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(file.hash,!1),app.showNotify({text:app.vtranslate("JS_FILE_UPLOAD_ERROR")+" [".concat(file.name,"]"),type:"error"});}),this.updateFormValues())}/**
	 * Success event handler from file upload request
	 *
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:"uploadSuccess",value:function uploadSuccess(e,data){var _this2=this,result=data.result,attach=result.result.attach;attach.forEach(function(fileAttach){var hash=fileAttach.hash;if(!hash)return app.errorLog(new Error(app.vtranslate("JS_INVALID_FILE_HASH")+" [".concat(hash,"]")));if("undefined"==typeof fileAttach.key)return _this2.uploadError(e,data);"undefined"!=typeof fileAttach.info&&fileAttach.info&&app.showNotify({type:"notice",text:fileAttach.info+" [".concat(fileAttach.name,"]")});var fileInfo=_this2.getFileInfo(hash);_this2.addFileInfoProperty(hash,"key",fileAttach.key),_this2.addFileInfoProperty(hash,"size",fileAttach.size),_this2.addFileInfoProperty(hash,"name",fileAttach.name),_this2.removePreviewPopover(hash),_this2.addPreviewPopover(fileInfo.file,fileInfo.previewElement,fileInfo.imageSrc),App.Fields.MultiImage.currentFileUploads--;}),this.updateFormValues();}/**
	 ...
	 * @param {Object} file
	 * @returns {boolean}
	 */},{key:"validateFile",value:function validateFile(file){var valid=!1;return this.options.formats.forEach(function(format){file.type==="image/"+format&&(valid=!0);}),valid||app.showNotify({text:"".concat(app.vtranslate("JS_INVALID_FILE_TYPE")," [").concat(file.name,"]\n").concat(app.vtranslate("JS_AVAILABLE_FILE_TYPES"),"  [").concat(this.options.formats.join(", "),"]"),type:"error"}),valid}/**
	 * Show limit error
	 ...
	 * @param {File} file
	 * @param {function} callback
	 */},{key:"generatePreviewFromFile",value:function generatePreviewFromFile(file,callback){var _this11=this,fr=new FileReader;fr.onload=function(){file.imageSrc=fr.result,_this11.addFileInfoProperty(file.hash,"imageSrc",file.imageSrc),callback("<div class=\"d-inline-block mr-1 js-multi-image__preview\" id=\"js-multi-image__preview-hash-".concat(file.hash,"\" data-hash=\"").concat(file.hash,"\" data-js=\"container|click\">\n\t\t\t\t\t<div class=\"img-thumbnail js-multi-image__preview-img c-multi-image__preview-img\" data-hash=\"").concat(file.hash,"\" data-js=\"drag\" style=\"background-image:url(").concat(fr.result,")\" tabindex=\"0\" title=\"").concat(file.name,"\"></div>\n\t\t\t</div>"),fr.result);},fr.readAsDataURL(file);}/**
