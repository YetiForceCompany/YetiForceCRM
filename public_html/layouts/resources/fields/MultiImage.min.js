...
var MultiImage=function(){/**
	 ...
	 * @returns {Object}
	 */},{key:'getFileInfo',value:function getFileInfo(hash){for(var file,i=0,len=this.files.length;i<len;i++)if(file=this.files[i],file.hash===hash)return file;app.errorLog('File \''+hash+'\' not found.'),app.showNotify({text:app.vtranslate('JS_INVALID_FILE_HASH')+(' ['+hash+']'),type:'error'});}/**
	 ...
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:'uploadError',value:function uploadError(e,data){var _this=this;app.errorLog('File upload error.');var jqXHR=data.jqXHR,files=data.files;if('undefined'==typeof jqXHR.responseJSON||null===jqXHR.responseJSON)return App.Fields.MultiImage.currentFileUploads--,app.showNotify({text:app.vtranslate('JS_FILE_UPLOAD_ERROR'),type:'error'});var response=jqXHR.responseJSON;// first try to show error for concrete file
return 'undefined'!=typeof response.result&&'undefined'!=typeof response.result.attach&&Array.isArray(response.result.attach)?(response.result.attach.forEach(function(fileAttach){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(fileAttach.hash,!1),'string'==typeof fileAttach.error?app.showNotify({text:fileAttach.error+(' ['+fileAttach.name+']'),type:'error'}):app.showNotify({text:app.vtranslate('JS_FILE_UPLOAD_ERROR')+(' ['+fileAttach.name+']'),type:'error'});}),void this.updateFormValues()):void(// else show default upload error
files.forEach(function(file){App.Fields.MultiImage.currentFileUploads--,_this.deleteFile(file.hash,!1),app.showNotify({text:app.vtranslate('JS_FILE_UPLOAD_ERROR')+(' ['+file.name+']'),type:'error'});}),this.updateFormValues())}/**
	 * Success event handler from file upload request
	 *
	 * @param {Event} e
	 * @param {Object} data
	 */},{key:'uploadSuccess',value:function uploadSuccess(e,data){var _this2=this,result=data.result,attach=result.result.attach;attach.forEach(function(fileAttach){var hash=fileAttach.hash;if(!hash)return app.errorLog(new Error(app.vtranslate('JS_INVALID_FILE_HASH')+(' ['+hash+']')));if('undefined'==typeof fileAttach.key)return _this2.uploadError(e,data);'undefined'!=typeof fileAttach.info&&fileAttach.info&&app.showNotify({type:'notice',text:fileAttach.info+(' ['+fileAttach.name+']')});var fileInfo=_this2.getFileInfo(hash);_this2.addFileInfoProperty(hash,'key',fileAttach.key),_this2.addFileInfoProperty(hash,'size',fileAttach.size),_this2.addFileInfoProperty(hash,'name',fileAttach.name),_this2.removePreviewPopover(hash),_this2.addPreviewPopover(fileInfo.file,fileInfo.previewElement,fileInfo.imageSrc),App.Fields.MultiImage.currentFileUploads--;}),this.updateFormValues();}/**
	 ...
	 * @param {Object} file
	 * @returns {boolean}
	 */},{key:'validateFile',value:function validateFile(file){var valid=!1;return this.options.formats.forEach(function(format){file.type==='image/'+format&&(valid=!0);}),valid||app.showNotify({text:app.vtranslate('JS_INVALID_FILE_TYPE')+' ['+file.name+']\n'+app.vtranslate('JS_AVAILABLE_FILE_TYPES')+'  ['+this.options.formats.join(', ')+']',type:'error'}),valid}/**
	 * Show limit error
	 */},{key:'showLimitError',value:function showLimitError(){this.elements.fileInput.val(''),app.showNotify({text:app.vtranslate('JS_FILE_LIMIT')+' ['+this.options.limit+']',type:'error'});}/**
