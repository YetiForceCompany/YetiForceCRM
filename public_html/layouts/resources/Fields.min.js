...
page:params.page}},processResults:function processResults(data){var items=new Array;return !0==data.success&&(selectElement.find("option").each(function(){var currentTarget=$(this);items.push({label:currentTarget.html(),value:currentTarget.val()});}),items=items.concat(data.result.items)),{results:items,pagination:{more:!1}}},cache:!1},params.escapeMarkup=function(markup){if("undefined"!==markup)return markup};var minimumInputLength=3;return "undefined"!==selectElement.data("minimumInput")&&(minimumInputLength=selectElement.data("minimumInput")),params.minimumInputLength=minimumInputLength,params.templateResult=function(data){return "undefined"==typeof data.name?data.text:"optgroup"==data.type?"<strong>"+data.name+"</strong>":"<span>"+data.name+"</span>"},params.templateSelection=function(data){return ""===data.text?data.name:data.text},params},/**
	 ...
	 * MultiReference
	 */MultiReference:/*#__PURE__*/function(){function MultiReference(container){_classCallCheck(this,MultiReference),this.container=container,this.init();}/**
		 * Register function
		 * @param {jQuery} container
		 */return _createClass(MultiReference,[{key:"init",value:/**
		 * Initiation
		 */function init(){var _this15=this;$(".js-clear-selection",this.container).off("click").on("click",function(){_this15.clear();}),$(".js-related-popup",this.container).off("click").on("click",function(){var params={},field=_this15.getField(),url=field.data("url");url&&(params=_this15.convertUrl(url)),app.showRecordsList($.extend(params,_this15.getParams()),function(_modal,instance){instance.setSelectEvent(function(data){_this15.setReferenceFieldValue(data);});});}),this.registerAutoComplete();}/**
		 * Clear selection
		 */},{key:"clear",value:function clear(){var element=this.getField(),fieldName=element.attr("name");element.val(""),this.container.find("#".concat(fieldName,"_display")).removeAttr("readonly").val("");}/**
		 * Function which will handle the reference auto complete event registrations
		 ...
		 * @param {object} data
		 */},{key:"setReferenceFieldValue",value:function setReferenceFieldValue(data){var sourceField=this.getField(),fieldName=sourceField.attr("name"),selectedNames=[],ids=[];for(var index in data)ids.push(index),selectedNames.push(data[index]);this.clear(),sourceField.val(ids.join(",")),this.container.find("#".concat(fieldName,"_display")).val(app.decodeHTML(selectedNames.join(", "))).attr("readonly",!0);}/**
		 * Gets field
	 ...
	 */Password:/*#__PURE__*/function(){function Password(container){_classCallCheck(this,Password),this.container=container,this.init();}/**
		 ...
		 * Initiation
		 */function init(){var _this16=this,field=this.getField();$(".js-pwd-auto-generate",this.container).off("click").on("click",function(e){_this16.getResponse($(e.currentTarget).data("url")).then(function(response){response.success&&response.result&&response.result.pwd&&(_this16.clear(),field.val(response.result.pwd).trigger("keyup").focus());});}),$(".js-pwd-validate",this.container).off("click").on("click",function(e){_this16.getResponse($(e.currentTarget).data("url")+"&password="+field.val()).then(function(response){if(response.success&&response.result){var message=response.result.message;Array.isArray(message)&&(message=message.join("<br>")),field.validationEngine("showPrompt",message,response.result.type,"topLeft",!0),field.validationEngine("updatePromptsPosition");}});}),$(".js-pwd-clear",this.container).off("click").on("click",function(){_this16.clear();}),$(".js-pwd-get",this.container).off("click").on("click",function(){var form=_this16.container.closest("form"),recordId=$("input[name=\"record\"]",form).val()||app.getRecordId();_this16.getResponse({module:field.data("module"),field:field.attr("name"),record:recordId,action:"Password",mode:"getPwd"}).then(function(response){_this16.clear(),field.val(response.result.text);});}),field.data("strengthMeter")&&field.off("keyup").on("keyup",function(e){var score=_this16.strengthMeter(e.target.value||"");field.attr("data-original-title",App.Fields.Password.getStrengthLevels()[score]).tooltip("show").validationEngine("hide");});}/**
		 * Clear data
	 ...
	 */MultiAttachment:/*#__PURE__*/function(){/**
		 ...
		 * @param {Event} e
		 * @param {Object} data
		 */},{key:"progressAll",value:function progressAll(e,data){var _this19=this,progress=parseInt(100*(data.loaded/data.total),10);this.progressBar.css({width:progress+"%"}),100===progress?setTimeout(function(){_this19.progress.addClass("d-none"),_this19.progressBar.css({width:"0%"});},1e3):this.progress.removeClass("d-none");}/**
		 ...
		 * @param {Event} e
		 * @param {Object} data
		 */},{key:"uploadError",value:function uploadError(_e,data){var _this20=this;this.progressInstance.progressIndicator({mode:"hide"}),this.filesActive--,app.errorLog("File upload error.");var jqXHR=data.jqXHR,files=data.files;return "undefined"==typeof jqXHR.responseJSON||null===jqXHR.responseJSON?this.showError({title:app.vtranslate("JS_FILE_UPLOAD_ERROR"),type:"error"}):void(files.forEach(function(file){_this20.showError({title:app.vtranslate("JS_FILE_UPLOAD_ERROR"),text:file.name,type:"error"});}),this.updateFormValues())}/**
		 * Success event handler from file upload request
		 *
		 * @param {Event} e
		 * @param {Object} data
		 */},{key:"uploadSuccess",value:function uploadSuccess(e,data){var _this21=this;this.progressInstance.progressIndicator({mode:"hide"});var result=data.result,attach=result.result.attach;attach.forEach(function(fileAttach){if(_this21.filesActive--,"undefined"==typeof fileAttach.key)return _this21.uploadError(e,data);"undefined"!=typeof fileAttach.info&&fileAttach.info&&app.showNotify({type:"notice",text:fileAttach.info}),_this21.files.push(fileAttach);var fileInfo=_this21.getFileInfo(fileAttach.key);_this21.createItem(fileInfo);}),this.updateFormValues();}/**
		 ...
		 * @param {Event} e
		 * @param {Object} ui
		 */},{key:"sortStop",value:function sortStop(){var _this22=this,actualElements=this.result.find("fieldset").toArray();this.files=actualElements.map(function(element){for(var elementHash,i=0,len=_this22.files.length;i<len;i++)if(elementHash=$(element).data("key"),_this22.files[i].key===elementHash)return _this22.files[i]}),this.updateFormValues();}/**
	 ...
	 */Icon:/*#__PURE__*/function(){function Icon(container){_classCallCheck(this,Icon),this.container=container,this.init();}/**
		 ...
		 * Initiation
		 */function init(){var _this23=this;this.iconElement=$(".js-icon-show span",this.container),$(".js-clear-selection",this.container).off("click").on("click",function(){_this23.clear();}),$(".js-icon-select",this.container).off("click").on("click",function(){App.Components.Icons.modalView().done(function(data){_this23.iconElement.removeClass().addClass(data.name),_this23.setDisplayFieldValue(data.name);});});}/**
		 * Clear selection
		 */},{key:"clear",value:function clear(){var element=this.getField(),fieldName=element.attr("name");element.val(""),this.container.find("#".concat(fieldName,"_display")).val(""),this.iconElement.removeClass();}/**
		 * Set icon name
		 * @param {string} data
		 */},{key:"setDisplayFieldValue",value:function setDisplayFieldValue(name){var sourceField=this.getField(),fieldName=sourceField.attr("name");sourceField.val(name),this.container.find("#".concat(fieldName,"_display")).val(name).attr("readonly",!0);}/**
		 * Gets field
