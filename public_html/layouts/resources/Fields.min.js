...
page:params.page}},processResults:function processResults(data){var items=new Array;return !0==data.success&&(selectElement.find("option").each(function(){var currentTarget=$(this);items.push({label:currentTarget.html(),value:currentTarget.val()});}),items=items.concat(data.result.items)),{results:items,pagination:{more:!1}}},cache:!1},params.escapeMarkup=function(markup){if("undefined"!==markup)return markup};var minimumInputLength=3;return "undefined"!==selectElement.data("minimumInput")&&(minimumInputLength=selectElement.data("minimumInput")),params.minimumInputLength=minimumInputLength,params.templateResult=function(data){return "undefined"==typeof data.name?data.text:"optgroup"==data.type?"<strong>"+data.name+"</strong>":"<span>"+data.name+"</span>"},params.templateSelection=function(data){return ""===data.text?data.name:data.text},params},/**
	 ...
	 */ChangesJson:/*#__PURE__*/function(){function ChangesJson(container){_classCallCheck(this,ChangesJson),this.container=container,this.init();}/**
		 ...
		 * Save data to field
		 */},{key:"saveData",value:function saveData(container,data){var _this13=this,form=container.find("form");container.on("click",".js-modal__save",function(e){if(form.validationEngine("validate")){if(e.preventDefault(),!form.find("input[id^=\"selectRow\"]:checked").length)return void app.showNotify({text:app.vtranslate("JS_NONE_FIELD_MARKED_IN_MASS_EDIT"),type:"error"});var invalidFields=form.data("jqv").InvalidFields;if(0!==invalidFields.length)return;form.find("[id^=\"selectRow\"]").each(function(_,checkbox){checkbox=$(checkbox),checkbox.prop("checked")||checkbox.closest(".js-form-row-container").find(".fieldValue [name]").each(function(_,element){element=$(element),element.attr("data-element-name",element.attr("name")).removeAttr("name");});});var changeData=form.serializeFormData();for(var fieldName in delete changeData._csrf,changeData)if("[]"===fieldName.substr(-2)){var fieldNameShort=fieldName.substr(0,fieldName.length-2);changeData[fieldNameShort]=changeData[fieldName],delete changeData[fieldName];}data.changes=changeData,_this13.getField().val(JSON.stringify(data)),app.hideModalWindow(null,form.closest(".js-modal-container").attr("id"));}});}/**
		 * Gets field
	 ...
	 */Password:/*#__PURE__*/function(){function Password(container){_classCallCheck(this,Password),this.container=container,this.init();}/**
		 ...
		 * Initiation
		 */function init(){var _this15=this,field=this.getField();$(".js-pwd-auto-generate",this.container).off("click").on("click",function(){_this15.getResponse({module:field.data("module"),field:field.attr("name"),action:"Password",mode:"generatePwd"}).then(function(response){response.success&&response.result&&response.result.pwd&&(_this15.clear(),field.val(response.result.pwd).trigger("keyup").focus());});}),$(".js-pwd-validate",this.container).off("click").on("click",function(){_this15.getResponse({module:field.data("module"),field:field.attr("name"),password:field.val(),action:"Password",mode:"validatePwd"}).then(function(response){if(response.success&&response.result){var message=response.result.message;Array.isArray(message)&&(message=message.join("<br>")),field.validationEngine("showPrompt",message,response.result.type,"topLeft",!0),field.validationEngine("updatePromptsPosition");}});}),$(".js-pwd-clear",this.container).off("click").on("click",function(){_this15.clear();}),$(".js-pwd-get",this.container).off("click").on("click",function(){var form=_this15.container.closest("form"),recordId=$("input[name=\"record\"]",form).val()||app.getRecordId();_this15.getResponse({module:field.data("module"),field:field.attr("name"),record:recordId,action:"Password",mode:"getPwd"}).then(function(response){_this15.clear(),field.val(response.result.text);});}),field.data("strengthMeter")&&field.off("keyup").on("keyup",function(e){var score=_this15.strengthMeter(e.target.value||"");field.attr("data-original-title",App.Fields.Password.getStrengthLevels()[score]).tooltip("show").validationEngine("hide");});}/**
		 * Clear data
