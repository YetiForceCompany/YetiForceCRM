...
page:item.page}},processResults:params.ajax&&params.ajax.processResults?params.ajax.processResults:function(data){var items=new Array;return !0==data.success&&(selectElement.find("option").each(function(){var currentTarget=$(this);items.push({label:currentTarget.html(),value:currentTarget.val()});}),items=items.concat(data.result.items)),{results:items,pagination:{more:!1}}},cache:!1},params.escapeMarkup=function(markup){if("undefined"!==markup)return markup},params.minimumInputLength=3,"undefined"!==selectElement.data("minimumInput")&&(params.minimumInputLength=selectElement.data("minimumInput")),params.templateResult=function(data){return "undefined"==typeof data.name?data.text:"optgroup"==data.type?"<strong>"+data.name+"</strong>":"<span>"+data.name+"</span>"},params.templateSelection=function(data){return ""===data.text?data.name:data.text},params},/**
	 ...
	 */MultiAttachment:/*#__PURE__*/function(){/**
		 ...
		 * @returns {Object}
		 */},{key:"getFileInfo",value:function getFileInfo(key){for(var file,i=0,len=this.files.length;i<len;i++)if(file=this.files[i],file.key===key)return file;app.errorLog("File '".concat(key,"' not found.")),app.showNotify({text:app.vtranslate("JS_INVALID_FILE_HASH"),type:"error"});}/**
