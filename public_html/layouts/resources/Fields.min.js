...
window.App.Fields={Date:{months:['JS_JAN','JS_FEB','JS_MAR','JS_APR','JS_MAY_SHORT','JS_JUN','JS_JUL','JS_AUG','JS_SEP','JS_OCT','JS_NOV','JS_DEC'],monthsTranslated:['JS_JAN','JS_FEB','JS_MAR','JS_APR','JS_MAY_SHORT','JS_JUN','JS_JUL','JS_AUG','JS_SEP','JS_OCT','JS_NOV','JS_DEC'].map(function(monthName){return app.vtranslate(monthName)}),fullMonths:['JS_JANUARY','JS_FEBRUARY','JS_MARCH','JS_APRIL','JS_MAY','JS_JUNE','JS_JULY','JS_AUGUST','JS_SEPTEMBER','JS_OCTOBER','JS_NOVEMBER','JS_DECEMBER'],fullMonthsTranslated:['JS_JANUARY','JS_FEBRUARY','JS_MARCH','JS_APRIL','JS_MAY','JS_JUNE','JS_JULY','JS_AUGUST','JS_SEPTEMBER','JS_OCTOBER','JS_NOVEMBER','JS_DECEMBER'].map(function(monthName){return app.vtranslate(monthName)}),days:['JS_SUN','JS_MON','JS_TUE','JS_WED','JS_THU','JS_FRI','JS_SAT'],daysTranslated:['JS_SUN','JS_MON','JS_TUE','JS_WED','JS_THU','JS_FRI','JS_SAT'].map(function(monthName){return app.vtranslate(monthName)}),fullDays:['JS_SUNDAY','JS_MONDAY','JS_TUESDAY','JS_WEDNESDAY','JS_THURSDAY','JS_FRIDAY','JS_SATURDAY'],fullDaysTranslated:['JS_SUNDAY','JS_MONDAY','JS_TUESDAY','JS_WEDNESDAY','JS_THURSDAY','JS_FRIDAY','JS_SATURDAY'].map(function(monthName){return app.vtranslate(monthName)}),/**
		 ...
		 */registerCopyClipboard:function registerCopyClipboard(container){var key=1<arguments.length&&void 0!==arguments[1]?arguments[1]:'.clipboard';if('object'===('undefined'==typeof container?'undefined':_typeof(container))){container=$(container).get(0);var elements=container.querySelectorAll(key);return 0===elements.length&&(elements=key,container=''),new ClipboardJS(elements,{container:container,text:function text(trigger){Vtiger_Helper_Js.showPnotify({text:app.vtranslate('JS_NOTIFY_COPY_TEXT'),type:'success'}),trigger=$(trigger);var element=$(trigger.data('copyTarget'),container),val=void 0;return val='undefined'==typeof trigger.data('copyType')?'undefined'==typeof trigger.data('copy-attribute')?element.val():trigger.data(trigger.data('copy-attribute')):element.is('select')?element.find('option:selected').data(trigger.data('copyType')):element.data(trigger.data('copyType')),val}})}},Editor:function(){function Editor(parentElement,params){classCallCheck(this,Editor);var elements;if(parentElement='undefined'==typeof parentElement?$('body'):$(parentElement),elements=parentElement.hasClass('js-editor')&&!parentElement.prop('disabled')?parentElement:$('.js-editor:not([disabled])',parentElement),0!==elements.length&&'undefined'!=typeof elements)if(this.isModal=elements.closest('.js-modal-container').length,this.isModal){var self=this;this.progressInstance=$.progressIndicator({blockInfo:{enabled:!0,onBlock:function onBlock(){self.loadEditor(elements,params);}}});}else App.Fields.Text.destroyEditor(elements),this.loadEditor(elements,params);}/*
			 ...
			 */},{key:'getMentionUsersData',value:function getMentionUsersData(opts,callback){App.Fields.Text.getMentionData(opts,callback,'owners');}}]),Editor}(),/**
		 ...
		 */Completions:function(){/**
			 ...
			 * @param params
			 */function Completions(){var inputDiv=0<arguments.length&&void 0!==arguments[0]?arguments[0]:$('.js-completions').eq(0),params=1<arguments.length&&void 0!==arguments[1]?arguments[1]:{emojiPanel:!0};if(classCallCheck(this,Completions),'undefined'!=typeof inputDiv&&0!==inputDiv.length){void 0===inputDiv.length&&(inputDiv=$(inputDiv));this.params=Object.assign({completionsCollection:{records:!0,users:!0,emojis:!0}},inputDiv.data(),params),this.inputDiv=inputDiv,this.collection=[],this.params.completionsCollection.records&&this.collection.push(this.registerMentionCollection('#')),this.params.completionsCollection.users&&this.collection.push(this.registerMentionCollection('@','owners')),this.params.completionsCollection.emojis&&this.collection.push(this.registerEmojiCollection()),this.register(inputDiv);}}/**
			 ...
			 */},{key:'registerCompletionsButtons',value:function registerCompletionsButtons(){var _this=this,completionsContainer=this.inputDiv.parents().eq(3);completionsContainer.find('.js-completions__users').on('click',function(){_this.completionsCollection.showMenuForCollection(_this.inputDiv[0],1);}),completionsContainer.find('.js-completions__records').on('click',function(){_this.completionsCollection.showMenuForCollection(_this.inputDiv[0],0);});}/**
		 ...
		 */changeSelectElementView:function changeSelectElementView(parent,view,viewParams){var _this2=this;if('undefined'==typeof parent&&(parent=$('body')),'undefined'==typeof view){var select2Elements=$('select.select2',parent).toArray();return void select2Elements.forEach(function(elem){_this2.changeSelectElementView($(elem),'select2',viewParams);})}//If view is select2, This will convert the ui of select boxes to select2 elements.
...
page:params.page}},processResults:function processResults(data){var items=new Array;return !0==data.success&&(selectElement.find('option').each(function(){var currentTarget=$(this);items.push({label:currentTarget.html(),value:currentTarget.val()});}),items=items.concat(data.result.items)),{results:items,pagination:{more:!1}}},cache:!1},params.escapeMarkup=function(markup){if('undefined'!==markup)return markup};var minimumInputLength=3;return 'undefined'!==selectElement.data('minimumInput')&&(minimumInputLength=selectElement.data('minimumInput')),params.minimumInputLength=minimumInputLength,params.templateResult=function(data){return 'undefined'==typeof data.name?data.text:'optgroup'==data.type?'<strong>'+data.name+'</strong>':'<span>'+data.name+'</span>'},params.templateSelection=function(data){return ''===data.text?data.name:data.text},params},/**
	 ...
	 */Tree:function(){function Tree(container){classCallCheck(this,Tree),this.container=container,this.init();}/**
		 ...
		 */},{key:'modalEvent',value:function modalEvent(){var _this5=this;$('.js-tree-modal',this.container).off('click').on('click',function(){var sourceFieldElement=_this5.container.find('input.sourceField'),fieldDisplayElement=_this5.container.find('input[name="'+sourceFieldElement.attr('name')+'_display"]');AppConnector.request({module:sourceFieldElement.data('modulename'),view:'TreeModal',template:sourceFieldElement.data('treetemplate'),fieldName:sourceFieldElement.attr('name'),multiple:sourceFieldElement.data('multiple'),value:sourceFieldElement.val()}).done(function(requestData){app.modalEvents.treeModal=function(modal,instance){instance.setSelectEvent(function(responseData){sourceFieldElement.val(responseData.id),fieldDisplayElement.val(responseData.name).attr('readonly',!0),sourceFieldElement.trigger('change');});},app.showModalWindow(requestData,{modalId:'treeModal'});});});}/**
		 * Function which will handle the reference auto complete event registrations
		 */},{key:'autoCompleteEvent',value:function autoCompleteEvent(){var autoCompleteElement=$('input.treeAutoComplete',this.container);autoCompleteElement.hasClass('ui-autocomplete-input')&&autoCompleteElement.autocomplete('destroy'),autoCompleteElement.autocomplete({delay:'600',minLength:'3',source:function source(request,response){var inputElement=$(this.element[0]),searchValue=request.term.toLowerCase(),parentElem=inputElement.closest('.js-tree-container'),sourceFieldElement=$('input.sourceField',parentElem),fieldInfo=sourceFieldElement.data('fieldinfo'),allValues=fieldInfo.picklistvalues,responseDataList=[];for(var id in allValues)0<=allValues[id].toLowerCase().indexOf(searchValue)&&responseDataList.push({label:allValues[id],value:id,id:id});0>=responseDataList.length&&($(inputElement).val(''),responseDataList.push({label:app.vtranslate('JS_NO_RESULTS_FOUND'),type:'no results'})),response(responseDataList);},select:function select(event,ui){var selectedItemData=ui.item;if('undefined'!=typeof selectedItemData.type&&'no results'==selectedItemData.type)return !1;selectedItemData.name=selectedItemData.value,this.value=selectedItemData.label;var element=$(this).attr('readonly',!0);return element.closest('.js-tree-container').find('input.sourceField').val(selectedItemData.id),!1},change:function change(){},open:function open(){$(this).data('ui-autocomplete').menu.element.css('z-index','100001');}});}/**
		 * Function which will register reference field clear event
		 */},{key:'clearSelectionEvent',value:function clearSelectionEvent(){var _this6=this;$('.clearTreeSelection',this.container).off('click').on('click',function(e){var fieldElement=_this6.container.find('.sourceField');$('input[name="'+fieldElement.attr('name')+'_display"]',_this6.container).removeAttr('readonly').val(''),fieldElement.val('').trigger('change'),e.preventDefault();});}}],[{key:'register',value:function register(container){if(container.hasClass('js-tree-container'))return new Tree(container);var instances=[];return container.find('.js-tree-container').each(function(n,e){instances.push(new Tree($(e)));}),instances}}]),Tree}(),/**
	 * TimePeriod class
	 *
	 ...
	 */MultiCurrency:function(){function MultiCurrency(container){classCallCheck(this,MultiCurrency),this.container=container,this.init();}/**
		 ...
		 * Initiation
		 */value:function init(){var _this7=this;$('.js-multicurrency-event',this.container).off('click').on('click',function(){var modal=$('<form>').append(_this7.container.find('.js-currencies-container .js-currencies-modal').clone());_this7.registerEnableCurrencyEvent(modal),_this7.registerResetCurrencyEvent(modal),_this7.loadData(modal),_this7.calculateConversionRate(modal),app.showModalWindow({data:modal,css:{},cb:function cb(data){var form=data.parent();form.validationEngine(app.validationEngineOptionsForRecord),form.on('submit',function(e){if(e.preventDefault(),form.validationEngine('validate')&&_this7.saveCurrencies(form)){var id=form.closest('.js-modal-container').attr('id');app.hideModalWindow(null,id);}});}});}),this.getField().on('focusout',function(e){var element=$(e.currentTarget);element.formatNumber(),_this7.setPrice(element.val());});}/**
		 * Loading data
	 ...
	 */MeetingUrl:function(){function MeetingUrl(container){classCallCheck(this,MeetingUrl),this.container=container,this.init();}/**
		 ...
		 * Initiation
		 */value:function init(){var _this10=this,addButton=$('.js-meeting-add',this.container);if(!addButton.length)return !1;var valElement=$('.js-meeting-val',this.container);addButton.off('click').on('click',function(e){var progressIndicatorElement=$.progressIndicator({blockInfo:{enabled:!0}});AppConnector.request(_this10.getUrl(e)).done(function(data){var result=data.result;result&&result.success&&result.url?valElement.attr('readonly',!0).val(result.url):Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_ERROR')),progressIndicatorElement.progressIndicator({mode:'hide'});}).fail(function(){Vtiger_Helper_Js.showPnotify(app.vtranslate('JS_ERROR')),progressIndicatorElement.progressIndicator({mode:'hide'});});}),$('.js-meeting-clear',this.container).off('click').on('click',function(){valElement.attr('readonly',!1).val('');}),this.addEventsForDependentFields();}/**
		 * Gets URL
