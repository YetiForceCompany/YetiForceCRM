...
form.removeData('submit'),$.progressIndicator({mode:'hide'});e.preventDefault();}),form.find('.js-full-editlink').on('click',function(e){var form=$(e.currentTarget).closest('form'),editViewUrl=$(e.currentTarget).data('url');goToFullFormCallBack(form),_this4.goToFullForm(form,editViewUrl);});},/**
		 ...
		 * @param {Object} params
		 */desktop:function desktop(params){var type='error';return params.modules=new Map([].concat(toConsumableArray(PNotify.defaultModules),[[PNotifyDesktop,{fallback:!1,icon:params.icon}]])),'undefined'!=typeof params.type&&(type=params.type,'error'!=params.type&&(params.hide=!0)),PNotify[type](params)}},Clipboard:function(){function Clipboard(container){classCallCheck(this,Clipboard),this.text=null,this.oClipboard=null,this.container=container;}/**
		 * Register
		 * @param {jQuery} params
		 */return createClass(Clipboard,[{key:'load',/**
		 * Initiation
		 */value:function load(){var _this6=this,aDeferred=$.Deferred(),url=this.container.data('url');return url?this.getTextFromUrl(url).then(function(response){_this6.text=response.result.text,aDeferred.resolve(_this6);}):aDeferred.resolve(this),aDeferred.promise()}/**
		 * Create ClipboardJS
		 */},{key:'createClipboard',value:function createClipboard(){var _this7=this,id=this.container.attr('id');this.oClipboard=new ClipboardJS('#'+id,{text:function text(){return _this7.text}});}/**
