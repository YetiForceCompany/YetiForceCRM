...
form.removeData('submit'),$.progressIndicator({mode:'hide'});e.preventDefault();}),form.find('.js-full-editlink').on('click',function(e){var form=$(e.currentTarget).closest('form'),editViewUrl=$(e.currentTarget).data('url');goToFullFormCallBack(form),_this3.goToFullForm(form,editViewUrl);}),this.registerTabEvents(form);},/**
			 ...
			 */registerHelpInfo:function registerHelpInfo(){var container=0<arguments.length&&arguments[0]!==void 0?arguments[0]:$('form[name="QuickCreate"]');app.showPopoverElementView(container.find('.js-help-info'));},/**
...
quickCreateTabOnHide(tabElements.not('[aria-expanded="false"]').attr('data-target')),quickCreateTabOnShow($(this).attr('data-target')),form.data('jqv').InvalidFields=[];}),tabElements.filter('a:not(.active)').each(function(){quickCreateTabOnHide($(this).attr('data-target'));});},/**
			 ...
			 *
			 * @return  {Promise}        aDeferred
			 */save:function save(form){var aDeferred=$.Deferred(),quickCreateSaveUrl=form.serializeFormData();return AppConnector.request(quickCreateSaveUrl).done(function(data){aDeferred.resolve(data);},function(textStatus,errorThrown){aDeferred.reject(textStatus,errorThrown);}),aDeferred.promise()}},QuickEdit:{/**
			 * Show modal
			 *
			 * @param   {string}  html
			 * @param   {object}  params
			 */showModal:function showModal(){var params=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{},self=this;params.view='QuickEditModal',AppConnector.request(params).done(function(html){app.showModalWindow(html,function(container){var form=container.find('form[name="QuickEdit"]'),moduleName=form.find('[name="module"]').val(),editViewInstance=Vtiger_Edit_Js.getInstanceByModuleName(moduleName),moduleClassName=moduleName+'_QuickEdit_Js';editViewInstance.registerBasicEvents(form),'undefined'!=typeof window[moduleClassName]&&new window[moduleClassName]().registerEvents(container),form.validationEngine(app.validationEngineOptions),'undefined'!=typeof params.callbackPostShown&&params.callbackPostShown(form,params),self.registerPostLoadEvents(form,params);});});},/**
			 * Register post load events
			 *
			 * @param   {object}  form jQuery
			 * @param   {object}  params
...
prettySelect:!0,usePrefix:'s2id_',validateNonVisibleFields:!0,onBeforePromptType:function onBeforePromptType(field){var block=field.closest('.js-toggle-panel');block.find('.blockContent').is(':hidden')&&block.find('.blockHeader').click();}},/**
...
var dropdown=tabs.find('> li.dropdown');dropdown.appendTo(tabs),tab.on('click',function(){setTimeout(function(){$(window).trigger('resize');},500);}),$(window).trigger('resize');}},getScreenHeight:function getScreenHeight(percantage){return 'undefined'==typeof percantage&&(percantage=100),$(window).height()*percantage/100},clearBrowsingHistory:function clearBrowsingHistory(){AppConnector.request({module:'Home',action:'BrowsingHistory'}).done(function(){$('.historyList').html('<a class="item dropdown-item" href="#" role="listitem">'+app.vtranslate('JS_NO_RECORDS')+'</a>');});},/**
	 ...
	 */convertUrlToObject:function convertUrlToObject(url){var urlObject={};return url.split('index.php?')[1].split('&').forEach(function(el){if(el.includes('=')){var values=el.split('=');urlObject[values[0]]=values[1];}}),urlObject},/**
...
PNotify.defaults.textTrusted=!0,PNotify.defaults.titleTrusted=!0,PNotify.defaults.styling='bootstrap4',PNotify.defaults.icons='fontawesome5';},/**
