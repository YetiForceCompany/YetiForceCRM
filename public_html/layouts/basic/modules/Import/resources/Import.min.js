...
if (typeof ImportJs === 'undefined') {
  ...
  */
	var ImportJs = {
		toogleMergeConfiguration: function toogleMergeConfiguration() {
		...
		handleFileTypeChange: function handleFileTypeChange() {
		...
		uploadFilter: function uploadFilter(elementId, allowedExtensions) {
		...
		uploadFileSize: function uploadFileSize(elementId) {
		...
		validateMergeCriteria: function validateMergeCriteria() {
			var mergeChecked = jQuery('#auto_merge').is(':checked');
			if (mergeChecked) {
		...
		convertOptionsToJSONArray: function convertOptionsToJSONArray(objName, targetObjName) {
		...
		copySelectedOptions: function copySelectedOptions(source, destination) {
		...
		removeSelectedOptions: function removeSelectedOptions(objName) {
			var obj = jQuery(objName);
			if (!obj.length) {
				return;
			}
		...
		sanitizeAndSubmit: function sanitizeAndSubmit() {
			if (!ImportJs.sanitizeFieldMapping()) return false;
			if (!ImportJs.validateCustomMap()) return false;
			return true;
		},
		sanitizeFieldMapping: function sanitizeFieldMapping() {
		...
		checkIfMappedFieldExist: function checkIfMappedFieldExist(selectedFieldName, mappedFields, selectedFieldElement) {
		...
		validateCustomMap: function validateCustomMap() {
			var errorMessage;
			var params = {};
		...
		loadSavedMap: function loadSavedMap() {
		...
		deleteMap: function deleteMap(module) {
		...
		loadDefaultValueWidget: function loadDefaultValueWidget(rowIdentifierId) {
		...
		loadDefaultValueWidgetForMappedFields: function loadDefaultValueWidgetForMappedFields() {
			var fieldsList = jQuery('.fieldIdentifier');
			fieldsList.each(function (i, element) {
				var fieldElement = jQuery(element);
		...
		openListInModal: function openListInModal() {
			$('.js-open-list-in-modal').on('click', function () {
				...
				app.showModalWindow(null, 'index.php?module=' + moduleName + '&view=List&mode=getImportDetails' + type + '&start=1&foruser=' + forUser + '&forModule=' + forModule, function (data) {
					var container = data.find('.listViewEntriesDiv'),
