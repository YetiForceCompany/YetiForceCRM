	...
	{},
	{
		...
		registerResetCurrencyEvent: function(container) {
			container.on('click', '.js-currency-reset', e => {
				...
				let price =
					this.getUnitPrice().getNumberFromValue() * parentElem.find('.js-conversion-rate').getNumberFromValue();
				$('.js-converted-price', parentElem).val(App.Fields.Double.formatToDisplay(price));
			});
		},
		calculateConversionRate: function(container) {
			let baseCurrencyConvestationRate = container
		...
		registerEnableCurrencyEvent: function(container) {
			container.on('change', '.js-enable-currency', e => {
				...
				if (element.is(':checked')) {
					...
					let price =
						this.getUnitPrice().getNumberFromValue() * parentRow.find('.js-conversion-rate').getNumberFromValue();
					$('input', parentRow)
						.attr('disabled', true)
						.removeAttr('disabled');
					parentRow
						.find('.js-currency-reset')
						.attr('disabled', true)
						.removeAttr('disabled');
		...
		saveCurrencies: function(modalContainer) {
			...
			$('.js-base-currency-check-id', thisInstance.getForm()).attr(
				'name',
				thisInstance.baseCurrencyName.replace('name', '_') + '_check'
			);
			this.getUnitPrice()
				.closest('.fieldValue')
				.find('.js-currencies-container')
				.html('')
				.append(modalContainer.children().clone());
			return true;
		},
		/**
