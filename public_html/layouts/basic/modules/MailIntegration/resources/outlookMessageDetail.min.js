...
/* {[The file is published on the basis of YetiForce Public License 4.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */window.MailIntegration_Start={iframe:{},/**
	 ...
	 */registerLoginEvents:function registerLoginEvents(){var loader,_this3=this,reloadPanelAfterLogin=function(){_this3.isUserLoggedIn()?window.location.reload():loader.progressIndicator({mode:"hide"});};this.iframe.on("load",reloadPanelAfterLogin),$(this.iframe[0].contentWindow).on("unload",function showLoader(){loader=$.progressIndicator({blockInfo:{enabled:!0},message:!1,blockOverlayCSS:{"background-color":"white",opacity:1}});});var src=this.iframe[0].getAttribute("src-a");src&&this.iframe[0].getAttribute("src")==null&&(this.iframe[0].removeAttribute("src-a"),this.iframe[0].setAttribute("src",src));},/**
	 * Is user logged in.
	 *
	 * @return  {boolean}
