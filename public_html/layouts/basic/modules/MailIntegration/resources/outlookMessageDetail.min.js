...
/* {[The file is published on the basis of YetiForce Public License 3.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */var MailIntegration_Start={iframe:{},/**
	 ...
	 */registerLogoutEvents:function registerLogoutEvents(){var _this2=this;this.setIframe();this.iframe.on('load',function reloadPanelAfterLogout(){_this2.isUserLoggedIn()||(console.log('registerLogoutEvents = reload'),window.location.reload());});},/**
	 * Register login events.
	 */registerLoginEvents:function registerLoginEvents(){var _this3=this,loader=void 0,reloadPanelAfterLogin=function(){_this3.isUserLoggedIn()?(console.log('registerLoginEvents = reload'),window.location.reload()):loader.progressIndicator({mode:'hide'});};this.iframe.on('load',reloadPanelAfterLogin),$(this.iframe[0].contentWindow).on('unload',function showLoader(){loader=$.progressIndicator({blockInfo:{enabled:!0},message:!1,blockOverlayCSS:{"background-color":'white',opacity:1}});});},/**
	 * Is user logged in.
	 *
	 * @return  {boolean}
	 */isUserLoggedIn:function isUserLoggedIn(){return console.log(this.iframe[0].contentWindow.document.body.dataset),'Users'!=this.iframe[0].contentWindow.document.body.dataset.module||'Login'!=this.iframe[0].contentWindow.document.body.dataset.view},showConsole:function showConsole(){var s='',x='';for(var p in navigator)s+=p+' : '+navigator[p]+'<br>',x+=p+' : '+navigator[p]+'\n';console.log(x),document.body.innerHTML+='<div style="position:absolute;width:100%;height:100%;z-index:100;background:#fff;left: 0px; top: 50%;overflow-y: auto;">'+s+'</div>';},/**
	 * Register events.
	 *
