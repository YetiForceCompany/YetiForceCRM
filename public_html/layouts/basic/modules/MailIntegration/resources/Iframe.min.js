...
var MailIntegration_Iframe={mailId:0,container:{},iframe:{},iframeWindow:{},moduleSelect:{},addRecordBtn:{},loaderParams:{blockInfo:{enabled:!0},message:!1},/**
	 ...
	 */fillNewRecordData:function fillNewRecordData(moduleName){var _this4=this,data={email:this.mailItem.from.emailAddress,email1:this.mailItem.from.emailAddress,relationOperation:!0,relatedRecords:$.map(this.container.find(".js-list-item-click"),function(record){return {module:record.dataset.module,id:record.dataset.id}})},fillNameFields=function(){var nameData=_this4.mailItem.from.displayName.split(" "),firstName=nameData.shift(),lastName=nameData.join(" ");data.firstname=firstName,data.lastname=lastName;};switch(moduleName){case"Leads":data.company=this.mailItem.from.displayName,fillNameFields();break;case"Contacts":fillNameFields();break;case"Project":data.projectname=this.mailItem.subject;break;case"HelpDesk":data.ticket_title=this.mailItem.subject;break;case"Products":data.productname=this.mailItem.subject;break;case"Services":data.servicename=this.mailItem.subject;break;}return this.asyncGetMailBody(function mailBodyCallback(body){return data.description=body,data})},/**
	 ...
	 */registerImportClick:function registerImportClick(){var _this7=this;this.container.on("click",".js-import-mail",function(){_this7.showIframeLoader(),_this7.getMailDetails().then(function(mails){_this7.connector(Object.assign({module:"MailIntegration",action:"Import"},mails,window.PanelParams)).done(function(response){_this7.hideIframeLoader(),_this7.showResponseMessage(response.success,app.vtranslate("JS_IMPORT")),_this7.reloadView(response.success);});});});},/**
	 * Get mail details
	 *
	 ...
	 *
	 * @return  {object}            Promise
	 */asyncGetMailBody:function asyncGetMailBody(callback){var _this8=this;return new Promise(function(resolve,reject){_this8.mailItem.body.getAsync(Office.CoercionType.Html,function(body){"succeeded"===body.status?resolve(callback(body.value)):reject(body);});})},/**
	 ...
	 */registerModulesSelect:function registerModulesSelect(){var _this9=this;this.moduleSelect=App.Fields.Picklist.showSelect2ElementView(this.container.find(".js-modules")),this.moduleSelect.on("change",this.registerModulesSelectChange.bind(this)),this.container.find(".js-select-record").on("click",function(){var params={module:_this9.moduleSelect[0].value,src_module:"OSSMailView"};app.showRecordsList(params,function(modal,instance){instance.setSelectEvent(function(responseData){_this9.addRelation(responseData.id,params.module);});});});},/**
	 ...
	 * Register events
	 */registerEvents:function registerEvents(){this.container=$("#page"),this.iframe=$("#js-iframe"),this.iframeWindow=this.iframe[0].contentWindow,this.addRecordBtn=this.container.find(".js-add-record"),this.mailId=this.container.find(".js-iframe-container").data("mailId"),this.mailItem=Office.context.mailbox.item,this.iframe.length&&(this.registerListItemEvents(),this.registerIframeEvents(),this.setIframeHeight(),this.mailId?(this.registerModulesSelect(),this.registerAddRecord()):this.registerImportClick());}};(function(){MailIntegration_Iframe.registerEvents();})($);
