...
Vtiger_RelatedList_Js(
	'Campaigns_RelatedList_Js',
	{
		 ...
		 */ triggerSendEmail: function triggerSendEmail() {
			var params = Vtiger_RelatedList_Js.relatedListInstance.getDefaultParams();
	...
	},
	{
		getCompleteParams: function getCompleteParams() {
			var params = this._super(),
				container = this.getRelatedContainer();
			return (
		...
		changeCustomFilterElementView: function changeCustomFilterElementView() {
			...
			if (0 < filterSelectElement.length) {
		...
		registerChangeCustomFilterEvent: function registerChangeCustomFilterEvent() {
			var thisInstance = this,
				relatedContainer = thisInstance.getRelatedContainer(),
				filterSelectElement = relatedContainer.find('.loadFormFilterButton'),
				customFilter = relatedContainer.find('#customFilter');
			filterSelectElement.on('click', function () {
				...
				Vtiger_Helper_Js.showConfirmationBox({ message: message }).done(function () {
					var cvId = customFilter.val(),
						relatedModuleName = relatedContainer.find('.relatedModuleName').val(),
						params = {
							sourceRecord: app.getRecordId(),
					...
					AppConnector.request(params)
						.done(function (responseData) {
							if (
								...
								Vtiger_Helper_Js.showMessage({ text: message, type: 'info' });
							} else Vtiger_Detail_Js.reloadRelatedList();
						})
						.fail(function () {
							progressIndicatorElement.progressIndicator({ mode: 'hide' });
						});
		 ...
		 */ registerEventToEditRelatedStatus: function registerEventToEditRelatedStatus() {
			var thisInstance = this,
				relatedContainer = thisInstance.getRelatedContainer();
			...
			statusDropdown.on('click', 'a', function (e) {
				e.stopImmediatePropagation();
				var element = jQuery(e.currentTarget),
					liContainer = element.closest('li'),
					currentStatus = element.closest('.currentStatus'),
					selectedStatusId = liContainer.attr('id'),
					selectedStatusValue = liContainer.data('status'),
					relatedRecordId = element.closest('tr').data('id'),
					params = {
