...
jQuery.Class('Vtiger_List_Js', {
	...
	getInstance: function getInstance() {
		if (Vtiger_List_Js.listInstance === false) {
			var module = app.getModuleName(),
			    ...
			    moduleClassName = void 0,
			    fallbackClassName = void 0,
			    instance = void 0;
  ...
  */
	triggerSendEmail: function triggerSendEmail(params) {
  ...
  * @params: send email url , module name.
  */
	triggerSendSms: function triggerSendSms(massActionUrl, module) {
		var listInstance = Vtiger_List_Js.getInstance();
		var validationResult = listInstance.checkListRecordSelected();
		if (validationResult != true) {
	...
	triggerQuickExportToExcel: function triggerQuickExportToExcel(module) {
  ...
  */
	...
	triggerMassAction: function triggerMassAction(massActionUrl, callBackFunction, beforeShowCb, css) {
		if (typeof beforeShowCb === 'undefined') {
			beforeShowCb = function beforeShowCb() {
				return true;
			};
		}
		if ((typeof beforeShowCb === 'undefined' ? 'undefined' : _typeof(beforeShowCb)) == 'object') {
			css = beforeShowCb;
			beforeShowCb = function beforeShowCb() {
				return true;
			};
	...
	triggerMassEdit: function triggerMassEdit(massEditUrl) {
	...
	getSelectedRecordCount: function getSelectedRecordCount() {
		var count;
		var listInstance = Vtiger_List_Js.getInstance();
  ...
  * returns UI
  */
	triggerExportAction: function triggerExportAction(exportActionUrl) {
		var listInstance = Vtiger_List_Js.getInstance();
		var params = listInstance.getSearchParams();
		if ('undefined' === typeof params.viewname) exportActionUrl += '&selected_ids=' + params.selected_ids + '&excluded_ids=' + params.excluded_ids + '&page=' + params.page;else exportActionUrl += '&selected_ids=' + params.selected_ids + '&excluded_ids=' + params.excluded_ids + '&viewname=' + params.viewname + '&page=' + params.page + '&entityState=' + params.entityState;
  ...
  */
	...
	getSelectedRecordsParams: function getSelectedRecordsParams(checkList) {
		var listInstance = Vtiger_List_Js.getInstance();
	...
	triggerGenerateRecords: function triggerGenerateRecords() {
	...
	triggerReviewChanges: function triggerReviewChanges(reviewUrl) {
		var listInstance = Vtiger_List_Js.getInstance();
		var validationResult = listInstance.checkListRecordSelected();
		if (validationResult != true) {
			...
			Vtiger_Helper_Js.showConfirmationBox({ message: message, title: title }).done(function (e) {
				var params = listInstance.getSearchParams();
				var url = reviewUrl + '&viewname=' + params.viewname + '&selected_ids=' + params.selected_ids + '&excluded_ids=' + params.excluded_ids + '&entityState=' + params.entityState;
				if (listInstance.getListSearchInstance()) {
	...
	getListSearchInstance: function getListSearchInstance(events) {
		if (events != undefined) {
	...
	getListViewTopMenuContainer: function getListViewTopMenuContainer() {
	...
	getListViewContentContainer: function getListViewContentContainer() {
	...
	getFilterBlock: function getFilterBlock() {
	...
	calculatePages: function calculatePages() {
		...
		if (totalPageNumber == '') {
			...
			if (totalRecordCount != '') {
				...
				var pageCount = Math.ceil(totalRecordCount / pageLimit);
				if (pageCount == 0) {
					pageCount = 1;
				}
  ...
  */
	noRecordSelectedAlert: function noRecordSelectedAlert() {
		var text = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'JS_PLEASE_SELECT_ONE_RECORD';

		return Vtiger_Helper_Js.showPnotify({ text: app.vtranslate(text) });
	},
	massActionSave: function massActionSave(form, isMassEdit) {
		...
		AppConnector.request(massActionUrl).done(function (data) {
			progressIndicatorElement.progressIndicator({
				mode: 'hide'
			});
			app.hideModalWindow();
	...
	readSelectedIds: function readSelectedIds(decode) {
		...
		if (decode == true) {
			if ((typeof selectedIds === 'undefined' ? 'undefined' : _typeof(selectedIds)) == 'object') {
				return JSON.stringify(selectedIds);
			}
		}
		return selectedIds;
	},
	readExcludedIds: function readExcludedIds(decode) {
		var cvId = this.getCurrentCvId();
		...
		if (decode == true) {
			if ((typeof excludedIds === 'undefined' ? 'undefined' : _typeof(excludedIds)) == 'object') {
				return JSON.stringify(excludedIds);
			}
		}
		return excludedIds;
	},
	writeSelectedIds: function writeSelectedIds(selectedIds) {
		var cvId = this.getCurrentCvId();
	...
	writeExcludedIds: function writeExcludedIds(excludedIds) {
		var cvId = this.getCurrentCvId();
  ...
  * @returns {boolean}
  */
	checkListRecordSelected: function checkListRecordSelected() {
		var minNumberOfRecords = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;

		var selectedIds = this.readSelectedIds();
		if ((typeof selectedIds === 'undefined' ? 'undefined' : _typeof(selectedIds)) === 'object' && selectedIds.length < minNumberOfRecords) {
			return true;
		}
		return false;
	},

	inactiveFieldValidation: function inactiveFieldValidation(field) {
		...
		if (field.is('select') && field.hasClass('select2')) {
			var selectElement = app.getSelect2ElementFromSelect(field);
	...
	activeFieldValidation: function activeFieldValidation(field) {
	...
	postMassEdit: function postMassEdit(massEditContainer) {
		var thisInstance = this;
		var editInstance = Vtiger_Edit_Js.getInstance();
  ...
  * @param {int} page
  */
	paginationGoToPage: function paginationGoToPage(page) {
		var self = this,
  ...
  */
	registerPageNavigationEvents: function registerPageNavigationEvents() {
		var _this = this;

		var listViewPageDiv = this.getListViewContainer();
		listViewPageDiv.find('.js-next-page').on('click', function (e) {
			_this.jumpToNextPage(e);
		});
		listViewPageDiv.find('.js-page--previous').on('click', function () {
			_this.jumpToPreviousPage();
		});
		listViewPageDiv.find('.pageNumber').on('click', function (e) {
			_this.jumpToClickedPage($(e.currentTarget));
		});
		listViewPageDiv.find('.js-count-number-records').on('click', function () {
			_this.updatePaginationAjax(true);
		});
		listViewPageDiv.find('.js-page--jump-drop-down').on('click', 'li', function (e) {
  ...
  */
	jumpToNextPage: function jumpToNextPage(element) {
  ...
  */
	jumpToPreviousPage: function jumpToPreviousPage() {
		var pageNumber = this.getListViewContainer().find('#pageNumber');
		if (pageNumber.val() > 1) {
  ...
  * @param {jQuery} element
  */
	jumpToClickedPage: function jumpToClickedPage(element) {
	...
	jumpToPage: function jumpToPage(e) {
		...
		if (13 === e.which) {
			...
			if (typeof response !== 'undefined') {
				...
				var currentPageElement = listViewPageDiv.find('#pageNumber'),
  ...
  */
	updatePagination: function updatePagination(pageNumber) {
		...
		AppConnector.request(Object.assign(this.getDefaultParams(), {
			...
			_this2.registerPageNavigationEvents();
		});
	},
	/**
  ...
  */
	registerChangeCustomFilterEvent: function registerChangeCustomFilterEvent(event) {
		var _this3 = this;
		...
		if ((typeof selectOption === 'undefined' ? 'undefined' : _typeof(selectOption)) === 'object') {
		...
		this.getListViewRecords(urlParams).done(function () {
			_this3.breadCrumbsFilter(selectOption.text());
			_this3.ListViewPostOperation();
			_this3.updatePagination(1);
		});
  ...
  */
	registerChangeCustomFilterEventListeners: function registerChangeCustomFilterEventListeners() {
		var _this4 = this;

		var filterSelect = this.getFilterSelectElement();
		filterSelect.on('select2:selecting', function (event) {
			...
			_this4.registerChangeCustomFilterEvent(event);
		});
		...
		this.getListViewTopMenuContainer().find('.js-filter-tab').on('click', function (e) {
			var cvId = $(e.currentTarget).data('cvid');
			var selectOption = filterSelect.find('[value=' + cvId + ']');
	...
	ListViewPostOperation: function ListViewPostOperation() {
		return true;
	},
	/*
  * Function to register the click event for list view main check box.
  */
	registerMainCheckBoxClickEvent: function registerMainCheckBoxClickEvent() {
		var listViewPageDiv = this.getListViewContainer();
		var thisInstance = this;
  ...
  */
	registerCheckBoxClickEvent: function registerCheckBoxClickEvent() {
		var listViewPageDiv = this.getListViewContainer();
		var thisInstance = this;
  ...
  */
	registerSelectAllClickEvent: function registerSelectAllClickEvent() {
		var listViewPageDiv = this.getListViewContainer();
		var thisInstance = this;
  ...
  */
	registerDeselectAllClickEvent: function registerDeselectAllClickEvent() {
		var listViewPageDiv = this.getListViewContainer();
		var thisInstance = this;
  ...
  */
	registerHeadersClickEvent: function registerHeadersClickEvent() {
		...
		YetiForce_ListSearch_Js.registerSearch(this.getListViewContainer(), function (data) {
			_this5.getListViewRecords(data);
		});
	},
	/*
  * function to register the click event event for create filter
  */
	createFilterClickEvent: function createFilterClickEvent(event) {
		//to close the dropdown
		this.getFilterSelectElement().data('select2').close();
		new CustomView($(event.currentTarget).find('#createFilter').data('createurl'));
	},
	registerFeaturedFilterClickEvent: function registerFeaturedFilterClickEvent() {
		var thisInstance = this;
		...
		if (listViewFilterBlock != false) {
			listViewFilterBlock.on('mouseup', '.js-filter-favorites', function (event) {
				var liElement = $(this).closest('.select2-results__option');
  ...
  */
	registerDuplicateFilterClickEvent: function registerDuplicateFilterClickEvent() {
		var thisInstance = this;
		var listViewFilterBlock = this.getFilterBlock();
		if (listViewFilterBlock != false) {
  ...
  */
	registerEditFilterClickEvent: function registerEditFilterClickEvent() {
		var thisInstance = this;
		var listViewFilterBlock = this.getFilterBlock();
		if (listViewFilterBlock != false) {
  ...
  */
	registerDeleteFilterClickEvent: function registerDeleteFilterClickEvent() {
		var thisInstance = this;
		var listViewFilterBlock = this.getFilterBlock();
		if (listViewFilterBlock != false) {
			...
			listViewFilterBlock.on('mouseup', '.js-filter-delete', function (event) {
				//to close the dropdown
				thisInstance.getFilterSelectElement().data('select2').close();
				var liElement = $(event.currentTarget).closest('.select2-results__option');
  ...
  */
	registerApproveFilterClickEvent: function registerApproveFilterClickEvent() {
		var thisInstance = this;
		var listViewFilterBlock = this.getFilterBlock();
		if (listViewFilterBlock != false) {
  ...
  */
	registerDenyFilterClickEvent: function registerDenyFilterClickEvent() {
		var thisInstance = this;
		var listViewFilterBlock = this.getFilterBlock();
		if (listViewFilterBlock != false) {
  ...
  */
	...
	registerRecordEvents: function registerRecordEvents() {
		var thisInstance = this;
		var listViewContentDiv = this.getListViewContentContainer();
	...
	registerMassRecordsEvents: function registerMassRecordsEvents() {
		var self = this;
  ...
  */
	massUpdatePagination: function massUpdatePagination() {
		var urlParams = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];

		var self = this,
		    listViewPageDiv = this.getListViewContainer();
		var paginationObject = listViewPageDiv.find('.js-pagination-list'),
  ...
  */
	inactiveFieldsValidation: function inactiveFieldsValidation(form) {
	...
	registerEventForTabClick: function registerEventForTabClick(form) {
	...
	registerSlimScrollMassEdit: function registerSlimScrollMassEdit() {
  ...
  */
	registerMassActionSubmitEvent: function registerMassActionSubmitEvent() {
		var _this7 = this;

		$('body').on('submit', '#massSave', function (e) {
			var form = jQuery(e.currentTarget),
			...
			_this7.massActionSave(form).done(function (data) {
				Vtiger_List_Js.clearList();
			});
			e.preventDefault();
		});
  ...
  */
	...
	registerUnreviewedCountEvent: function registerUnreviewedCountEvent() {
		var _this8 = this;
	...
	registerLastRelationsEvent: function registerLastRelationsEvent() {
		var _this9 = this;
		...
		AppConnector.request({
			...
			$.each(data, function (id, value) {
				if (value.type) {
					listViewContentDiv.find('tr[data-id="' + id + '"] .timeLineIconList').addClass(value.color + ' userIcon-' + value.type).removeClass('d-none').on('click', function (e) {
						var element = jQuery(e.currentTarget);
	...
	registerSummationEvent: function registerSummationEvent() {
		var self = this;
		var listContainer = this.getListViewContentContainer();
	...
	registerListScroll: function registerListScroll(container) {
	...
	registerFixedThead: function registerFixedThead(container) {
	...
	getFloatTheadContainer: function getFloatTheadContainer() {
		var container = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.getListViewContentContainer();
	...
	reflowThead: function reflowThead() {
		if (Quasar.plugins.Platform.is.desktop) {
			this.getFloatTheadContainer().floatThead('reflow');
		}
	},
	registerMassActionsBtnEvents: function registerMassActionsBtnEvents() {
		var _this10 = this;
		...
		this.getListViewContainer().on('click', '.js-mass-action', function (e) {
			...
			if (typeof url != 'undefined') {
				if (_this10.checkListRecordSelected() !== true) {
					Vtiger_List_Js.triggerMassAction(url);
				} else {
					_this10.noRecordSelectedAlert();
				}
			}
			e.stopPropagation();
		});
	},
	registerMassActionsBtnMergeEvents: function registerMassActionsBtnMergeEvents() {
		var _this11 = this;

		this.getListViewContainer().on('click', '.js-mass-action--merge', function (e) {
			var url = $(e.target).data('url');
			if (typeof url !== 'undefined') {
				if (_this11.checkListRecordSelected(2) !== true) {
					Vtiger_List_Js.triggerMassAction(url);
				} else {
					_this11.noRecordSelectedAlert('JS_SELECT_ATLEAST_TWO_RECORD_FOR_MERGING');
				}
			}
		});
	},
	registerMassActionModalEvents: function registerMassActionModalEvents() {
		app.event.on('MassEditModal.AfterLoad', function (data, container) {
  ...
  */
	registerDesktopEvents: function registerDesktopEvents(listViewContainer) {
		if (Quasar.plugins.Platform.is.desktop) {
			this.registerListScroll(listViewContainer);
			this.registerFixedThead(listViewContainer);
		}
	},
	registerPostLoadDesktopEvents: function registerPostLoadDesktopEvents(listViewContainer) {
		if (Quasar.plugins.Platform.is.desktop) {
  ...
  */
	postMassDeleteRecords: function postMassDeleteRecords() {
		var aDeferred = jQuery.Deferred();
		var listInstance = Vtiger_List_Js.getInstance();
		app.hideModalWindow();
