...
jQuery.Class('Vtiger_GenerateModal_Js', {}, {
	registerGenetateButton: function registerGenetateButton(container) {
		...
		container.find('button.js-genetate-button').on('click', function (e) {
			...
			var currentTarget = container.find('.js-generate-mapping option:selected'),
	...
	summary: function summary(container, data) {
		container.find('.modal-title').text(app.vtranslate('JS_SUMMARY'));
		container.find('.modal-body').html('<div>' + app.vtranslate('JS_SELECTED_RECORDS') + ': <strong>' + data.all + '</strong></div><div>' + app.vtranslate('JS_SUCCESSFULLY_PERFORMED_ACTION_FOR') + ': <strong>' + data.ok.length + '</strong></div><div>' + app.vtranslate('JS_ACTION_FAILED_FOR') + ': <strong>' + data.fail.length + '</strong></div>');
	},
