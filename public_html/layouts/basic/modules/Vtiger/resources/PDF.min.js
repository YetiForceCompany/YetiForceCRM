...
$.Class('Vtiger_PDF_Js', {
  ...
  */
	proceedSubmit: function proceedSubmit(mode) {
  ...
  */
	registerPreSubmitEvent: function registerPreSubmitEvent(container) {
		var self = this;
  ...
  */
	registerListViewCheckRecords: function registerListViewCheckRecords(container) {
		var templateElements = container.find('[name="pdf_template[]"]');
		templateElements.on('change', function () {
			...
			var templateContainer = $(this).closest('.js-pdf-template-content');
			...
			AppConnector.request(params).done(function (data) {
				...
				if (data.success) {
					...
					var info = container.find('.js-records-info').text(response.message).removeClass('d-none');
					if (valid) {
  ...
  */
	registerSelectCustomColumnsChange: function registerSelectCustomColumnsChange() {
		var _this = this;

		this.container.find('[name="isCustomMode"]').on('change', function (ev) {
			if ($(ev.target).is(':checked')) {
				_this.container.find('[name="inventoryColumns[]"]').prop('disabled', null);
				_this.container.find('.js-save-scheme').prop('disabled', null);
			} else {
				_this.container.find('[name="inventoryColumns[]"]').prop('disabled', 'disabled');
				_this.container.find('.js-save-scheme').prop('disabled', 'disabled');
			}
		});
	},
  ...
  */
	registerSaveInventoryColumnSchemeClick: function registerSaveInventoryColumnSchemeClick() {
		var _this2 = this;

		this.container.find('.js-save-scheme').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			var loader = $.progressIndicator({
				position: 'html',
			...
			var params = _this2.container.find('form').serializeFormData();
			params.mode = 'saveInventoryColumnScheme';
			params.isCustomMode = _this2.container.find('[name="isCustomMode"]').is(':checked');
  ...
  */
	registerEvents: function registerEvents() {
		var container = this.container = $('#pdfExportModal').closest('.js-modal-container');
