...
for(var i=0,len=dataset._meta[prop].data.length;i<len;i++){var metaDataItem=dataset._meta[prop].data[i],label=metaDataItem._view.label;if(13<label.length&&(chart.options=shortenYTicks(chart.data,chart.options),!dataset._updated)){dataset._updated=!0,chart.update(),chart.data.datasets.forEach(function(dataset,index){dataset._meta[prop].data.forEach(function(metaDataItem,dataIndex){"undefined"!=typeof metaDataItem._xScale&&(metaDataItem._view.x=metaDataItem._xScale.getPixelForValue(index,dataIndex),metaDataItem._view.base=metaDataItem._xScale.getBasePixel(),metaDataItem._view.width=metaDataItem._xScale.width/dataset._meta[prop].data.length*metaDataItem._xScale.options.categoryPercentage*metaDataItem._xScale.options.barPercentage);});});break}}dataset._updated=!0;}};for(var prop in dataset._meta)_loop2(prop);});}}},/**
		 ...
		 */parseOptionsObject:function(options,original){var afterInit=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2],result={};for(var propertyName in options){var _value=options[propertyName];result[propertyName]=afterInit?"_"===propertyName.substr(0,1)?_value:Array.isArray(_value)?this.parseOptionsArray(_value,original,afterInit):"object"===babelHelpers.typeof(_value)&&null!==_value?this.parseOptionsObject(_value,original,afterInit):_value:"_"===propertyName.substr(0,1)?_value:this.isReplacementString(_value)?this.getFunctionFromReplacementString(_value,afterInit,original):Array.isArray(_value)?this.parseOptionsArray(_value,original,afterInit):"object"===babelHelpers.typeof(_value)&&null!==_value?this.parseOptionsObject(_value,original,afterInit):_value;}return result},/**
		 ...
		 * @return {Array}
		 */parseOptionsArray:function(arr,original){var _this=this,afterInit=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2];return arr.map(function(item){if(_this.isReplacementString(item))return _this.getFunctionFromReplacementString(value);return Array.isArray(item)?_this.parseOptionsArray(item,original,afterInit):"object"===babelHelpers.typeof(item)&&null!==item?_this.parseOptionsObject(item,original,afterInit):item})},/**
		 ...
		 * @return {Object}
		 */parseOptions:function(options,original){var afterInit=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2];if(Array.isArray(options))return this.parseOptionsArray(options,original,afterInit);return "object"===babelHelpers.typeof(options)&&null!==options?this.parseOptionsObject(options,original,afterInit):void app.errorLog(new Error("Unknown options format ["+babelHelpers.typeof(options)+"] - should be object."))},/**
...
return data.datasets=data.datasets.map(function(dataset,index){return dataset.datasetIndex=index,_this5.parseOptions(dataset,dataset,!0)}),chart},/**
		 ...
		 * @return {Array}
		 */mergeOptionsArray:function(to,fromArray){var _this9=this;if("undefined"!=typeof to)return to;to=[];var result=fromArray.map(function(from,index){return Array.isArray(from)&&!to.hasOwnProperty(key)?_this9.mergeOptionsArray(to[index],from):"object"===babelHelpers.typeof(from)&&null!==from&&("undefined"==typeof to[index]||"object"===babelHelpers.typeof(to[index])&&null!==to[index])?_this9.mergeOptionsObject(to[index],from):to[index]}).filter(function(item){return "undefined"!=typeof item});return result},/**
		 ...
		 */mergeOptionsObject:function(to,from){for(var _key in "undefined"==typeof to&&(to={}),from)from.hasOwnProperty(_key)&&(Array.isArray(from[_key])?!to.hasOwnProperty(_key)&&(to[_key]=this.mergeOptionsArray(void 0,from[_key])):"object"!==babelHelpers.typeof(from[_key])||null===from[_key]||to.hasOwnProperty(_key)&&("object"!==babelHelpers.typeof(to[_key])||null===to[_key]||Array.isArray(to[_key]))?!to.hasOwnProperty(_key)&&(to[_key]=from[_key]):to[_key]=this.mergeOptionsObject(to[_key],from[_key]));return to},/**
		 * Merge two objects with options and do not override existing properties
		 *
		 * @param  {Object} to
		 * @param  {Array|arguments} fromArray
		 * @return {object}
		 */mergeOptions:function(){for(var to=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{},_len=arguments.length,fromArray=new Array(1<_len?_len-1:0),_key2=1;_key2<_len;_key2++)fromArray[_key2-1]=arguments[_key2];for(var i=0,len=fromArray.length;i<len;i++)"object"!==babelHelpers.typeof(fromArray[i])||Array.isArray(fromArray[i])?app.errorLog(new Error("Options argument should be an object! Chart subType: "+this.getSubType()+" ["+fromArray[i].toString()+"]")):to=this.mergeOptionsObject(to,fromArray[i]);return to},/**
