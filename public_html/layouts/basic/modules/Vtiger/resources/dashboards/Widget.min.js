...
for(var i=0,len=dataset._meta[prop].data.length;i<len;i++){var metaDataItem=dataset._meta[prop].data[i],label=metaDataItem._view.label;if(13<label.length&&(chart.options=shortenYTicks(chart.data,chart.options),!dataset._updated)){dataset._updated=!0,chart.update(),chart.data.datasets.forEach(function(dataset,index){dataset._meta[prop].data.forEach(function(metaDataItem,dataIndex){"undefined"!=typeof metaDataItem._xScale&&(metaDataItem._view.x=metaDataItem._xScale.getPixelForValue(index,dataIndex),metaDataItem._view.base=metaDataItem._xScale.getBasePixel(),metaDataItem._view.width=metaDataItem._xScale.width/dataset._meta[prop].data.length*metaDataItem._xScale.options.categoryPercentage*metaDataItem._xScale.options.barPercentage);});});break}}dataset._updated=!0;}};for(var prop in dataset._meta)_loop2(prop);});}}},/**
		 ...
		 * @return {Array}
		 */parseOptionsArray:function(arr,original){var _this=this,afterInit=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2];return arr.map(function(item){if(_this.isReplacementString(item))return _this.getFunctionFromReplacementString(value);return Array.isArray(item)?_this.parseOptionsArray(item,original,afterInit):"object"===_typeof(item)&&null!==item?_this.parseOptionsObject(item,original,afterInit):item})},/**
		 ...
		 * @return {Object}
		 */parseOptions:function(options,original){var afterInit=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2];if(Array.isArray(options))return this.parseOptionsArray(options,original,afterInit);return "object"===_typeof(options)&&null!==options?this.parseOptionsObject(options,original,afterInit):void app.errorLog(new Error("Unknown options format ["+_typeof(options)+"] - should be object."))},/**
...
return data.datasets=data.datasets.map(function(dataset,index){return dataset.datasetIndex=index,_this5.parseOptions(dataset,dataset,!0)}),chart},/**
		 ...
		 * @return {object}
		 */mergeOptions:function(){for(var to=0<arguments.length&&arguments[0]!==void 0?arguments[0]:{},_len=arguments.length,fromArray=new Array(1<_len?_len-1:0),_key2=1;_key2<_len;_key2++)fromArray[_key2-1]=arguments[_key2];for(var i=0,len=fromArray.length;i<len;i++)"object"!==_typeof(fromArray[i])||Array.isArray(fromArray[i])?app.errorLog(new Error("Options argument should be an object! Chart subType: "+this.getSubType()+" ["+fromArray[i].toString()+"]")):to=this.mergeOptionsObject(to,fromArray[i]);return to},/**
