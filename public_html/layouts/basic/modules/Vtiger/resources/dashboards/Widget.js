			...
			eventRender: function (event, element, view) {
				...
				for (var key in event.event) {
					element += '<a class="" href="javascript:;"' +
						...
						'<span class="' + event.event[key].className + ((event.width <= 20) ? ' small-badge' : '') + ((event.width >= 24) ? ' big-badge' : '') + ' badge badge-secondary u-font-size-95per"><span>' + event.event[key].count + '</span></span>' +
						'</a>\n';
				}
				element += '</div>';
				return element;
