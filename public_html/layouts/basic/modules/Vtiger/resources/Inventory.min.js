...
if(isGroupTax){var parameters=parentRow.closest(".inventoryItems").data("taxParam");parameters&&(taxParam=JSON.parse(parameters));}else recordData.taxes&&(taxParam={aggregationType:recordData.taxes.type},taxParam[recordData.taxes.type+"Tax"]=recordData.taxes.value);for(var field in recordData.taxes&&parentRow.find(".js-tax").attr("data-default-tax",recordData.taxes.value),thisInstance.setPurchase(parentRow,recordData.purchase),thisInstance.setTaxParam(parentRow,taxParam),thisInstance.setTax(parentRow,0),thisInstance.setTaxPercent(parentRow,0),recordData.autoFields){var inputField=parentRow.find("input.".concat(field,",select.").concat(field));"checkbox"===inputField.attr("type")?inputField.prop("checked",recordData.autoFields[field]):inputField.is("select")?inputField.val(recordData.autoFields[field]).trigger("change"):inputField.val(recordData.autoFields[field]),recordData.autoFields[field+"Text"]&&parentRow.find("."+field+"Text").text(recordData.autoFields[field+"Text"]);}void 0!==recordData.price&&thisInstance.setUnitPrice(parentRow,recordData.price),void 0!==unitPriceValuesJson&&$("input.unitPrice",parentRow).attr("list-info",unitPriceValuesJson);var commentElement=$("textarea.js-inventory-item-comment",parentRow.next()),editorInstance=CKEDITOR.instances[commentElement.attr("id")];editorInstance?editorInstance.setData(description):commentElement.val(description),"undefined"!=typeof recordData.autoFields.unit&&(unit=recordData.autoFields.unit),app.event.trigger("Inventory.SelectionItem",thisInstance,parentRow,recordData,referenceModule),this.triggerQtyParam(unit,recordData.qtyPerUnit,parentRow);}"Products"===referenceModule&&thisInstance.loadSubProducts(parentRow,!0),thisInstance.quantityChangeActions(parentRow);},/**
		 ...
		 */setRowData:function setRowData(row,rowData){this.setName(row,rowData.name,rowData.info.name),this.setQuantity(row,App.Fields.Double.formatToDisplay(rowData.qty)),this.setUnit(row,rowData.info.autoFields.unit,rowData.info.autoFields.unitText),"undefined"!=typeof rowData.info.autoFields&&"undefined"!=typeof rowData.info.autoFields.subunit&&this.setSubUnit(row,rowData.info.autoFields.subunit,rowData.info.autoFields.subunitText),this.setComment(row,rowData.comment1),this.setUnitPrice(row,App.Fields.Double.formatToDisplay(rowData.price)),this.setNetPrice(row,App.Fields.Double.formatToDisplay(rowData.net)),this.setGrossPrice(row,App.Fields.Double.formatToDisplay(rowData.gross)),this.setTotalPrice(row,App.Fields.Double.formatToDisplay(rowData.total)),this.setDiscountParam(row,rowData.discountparam?JSON.parse(discountParam):[]),this.setDiscount(row,App.Fields.Double.formatToDisplay(rowData.discount)),this.setTaxParam(row,rowData.taxparam?JSON.parse(rowData.taxparam):[]),this.setTax(row,App.Fields.Double.formatToDisplay(rowData.tax)),this.setTaxPercent(row,App.Fields.Double.formatToDisplay(rowData.tax_percent));},/**
		 ...
		 * @returns
		 */addHeaderItem:function addHeaderItem(){var data=0<arguments.length&&void 0!==arguments[0]?arguments[0]:{},items=this.getInventoryItemsContainer(),newRow=this.getBasicRow();return data.grouplabel&&newRow.find(".js-grouplabel").val(data.grouplabel),newRow=newRow.find("tr.inventoryRowGroup").appendTo(items.find(".js-inventory-items-body")),this.initItem(newRow),newRow},/**
		 * Get next block ID
		 * @returns int
		 ...
		 * @returns Promise
		 */loadInventoryData:function loadInventoryData(recordId,sourceModule){var _this15=this,success=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2],fail=!!(3<arguments.length&&arguments[3]!==void 0)&&arguments[3],progressLoader=$.progressIndicator({blockInfo:{enabled:!0}});return new Promise(function(resolve,reject){AppConnector.request({module:app.getModuleName(),src_module:sourceModule,src_record:recordId,action:"Inventory",mode:"getTableData",record:app.getRecordId()}).done(function(response){var activeModules=[];_this15.getContainer().find(".js-inv-add-item").each(function(_,addBtn){activeModules.push($(addBtn).data("module"));}),progressLoader.progressIndicator({mode:"hide"});var oldCurrencyChangeAction=_this15.currencyChangeActions;_this15.currencyChangeActions=function(select,option){this.currencyConvertValues(select,option),select.data("oldValue",select.val());};var first=response.result[Object.keys(response.result)[0]];_this15.setCurrencyParam(first.currencyparam),_this15.setCurrency(first.currency),_this15.setDiscountMode(first.discountmode),_this15.setTaxMode(first.taxmode),_this15.currencyChangeActions=oldCurrencyChangeAction,_this15.clearInventory(),$.each(response.result,function(_,row){-1===activeModules.indexOf(row.moduleName)?Vtiger_Helper_Js.showMessage({type:"error",textTrusted:!1,text:app.vtranslate("JS_INVENTORY_ITEM_MODULE_NOT_FOUND").replace("${sourceModule}",row.moduleName).replace("${position}",row.info.name)}):(row.groupid&&row.add_header&&_this15.addHeaderItem(row),_this15.addItem(row.moduleName,row.basetableid,row));}),_this15.summaryCalculations(),resolve(response.result),"function"==typeof success&&success(response.result);}).fail(function(error,err){progressLoader.progressIndicator({mode:"hide"}),reject(error,err),"function"==typeof fail&&fail(error,err);});})},/**
