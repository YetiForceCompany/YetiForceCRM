...
if(isGroupTax){var parameters=parentRow.closest(".inventoryItems").data("taxParam");parameters&&(taxParam=JSON.parse(parameters));}else recordData.taxes&&(taxParam={aggregationType:recordData.taxes.type},taxParam[recordData.taxes.type+"Tax"]=recordData.taxes.value);for(var field in recordData.taxes&&parentRow.find(".js-tax").attr("data-default-tax",recordData.taxes.value),thisInstance.setPurchase(parentRow,recordData.purchase),thisInstance.setTaxParam(parentRow,taxParam),thisInstance.setTax(parentRow,0),thisInstance.setTaxPercent(parentRow,0),recordData.autoFields){var inputField=parentRow.find("input."+field);"checkbox"===inputField.attr("type")?inputField.prop("checked",recordData.autoFields[field]):inputField.val(recordData.autoFields[field]),recordData.autoFields[field+"Text"]&&parentRow.find("."+field+"Text").text(recordData.autoFields[field+"Text"]);}void 0!==recordData.price&&thisInstance.setUnitPrice(parentRow,recordData.price),void 0!==unitPriceValuesJson&&$("input.unitPrice",parentRow).attr("list-info",unitPriceValuesJson);var commentElement=$("textarea.js-inventory-item-comment",parentRow.next()),editorInstance=CKEDITOR.instances[commentElement.attr("id")];editorInstance?editorInstance.setData(description):commentElement.val(description),"undefined"!=typeof recordData.autoFields.unit&&(unit=recordData.autoFields.unit),app.event.trigger("Inventory.SelectionItem",thisInstance,parentRow,recordData,referenceModule),this.triggerQtyParam(unit,recordData.qtyPerUnit,parentRow);}"Products"===referenceModule&&thisInstance.loadSubProducts(parentRow,!0),thisInstance.quantityChangeActions(parentRow);},/**
		 ...
		 */addItem:function addItem(module,baseTableId){var rowData=!!(2<arguments.length&&void 0!==arguments[2])&&arguments[2],group=3<arguments.length&&void 0!==arguments[3]?arguments[3]:null,items=group||this.getInventoryItemsContainer(),newRow=this.getBasicRow(),sequenceNumber=this.getNextLineItemRowNumber(),replaced=newRow.html().replace(/\_NUM_/g,sequenceNumber),moduleLabels=newRow.data("moduleLbls");return newRow.html(replaced),newRow=newRow.children().appendTo(items.find(".js-inventory-items-body")),newRow.find(".rowName input[name=\"popupReferenceModule\"]").val(module).data("field",baseTableId),newRow.find(".js-module-icon").removeClass().addClass("yfm-".concat(module)),newRow.find(".rowName span.input-group-text").attr("data-content",moduleLabels[module]),newRow.find(".colPicklistField select").each(function(_,select){select=$(select),select.find("option").each(function(_,option){option=$(option),option.data("module")!==module&&option.remove();});}),this.initItem(newRow),Vtiger_Edit_Js.getInstance().registerAutoCompleteFields(newRow),rowData&&(this.setRowData(newRow,rowData),this.quantityChangeActions(newRow)),newRow},/**
		 * Add block
		 * @returns
		 */addBlock:function addBlock(){var block=this.getContainer().find(".js-inv-container-group:last"),newBlock=block.clone(!0,!0);return newBlock.find(".grouplabel").val(""),newBlock.find(".js-inventory-items-body").empty(),newBlock.find(".js-groupid").val(this.getNextBlockId()),block.after(newBlock),this.registerSortableItems(),newBlock},/**
		 * Get next block ID
		 * @returns int
		 ...
		 * @returns Promise
		 */loadInventoryData:function loadInventoryData(recordId,sourceModule){var _this10=this,success=!!(2<arguments.length&&arguments[2]!==void 0)&&arguments[2],fail=!!(3<arguments.length&&arguments[3]!==void 0)&&arguments[3],progressLoader=$.progressIndicator({blockInfo:{enabled:!0}});return new Promise(function(resolve,reject){AppConnector.request({module:app.getModuleName(),src_module:sourceModule,src_record:recordId,action:"Inventory",mode:"getTableData",record:app.getRecordId()}).done(function(response){var activeModules=[];_this10.getInventoryHeadContainer().find(".js-inv-add-item").each(function(_,addBtn){activeModules.push($(addBtn).data("module"));}),progressLoader.progressIndicator({mode:"hide"});var oldCurrencyChangeAction=_this10.currencyChangeActions;_this10.currencyChangeActions=function(select,option){this.currencyConvertValues(select,option),select.data("oldValue",select.val());};var first=response.result[Object.keys(response.result)[0]];_this10.setCurrencyParam(first.currencyparam),_this10.setCurrency(first.currency),_this10.setDiscountMode(first.discountmode),_this10.setTaxMode(first.taxmode),_this10.currencyChangeActions=oldCurrencyChangeAction,_this10.clearInventory(),$.each(response.result,function(_,row){-1===activeModules.indexOf(row.moduleName)?Vtiger_Helper_Js.showMessage({type:"error",text:app.vtranslate("JS_INVENTORY_ITEM_MODULE_NOT_FOUND").replace("${sourceModule}",row.moduleName).replace("${position}",row.info.name)}):_this10.addItem(row.moduleName,row.basetableid,row);}),_this10.summaryCalculations(),resolve(response.result),"function"==typeof success&&success(response.result);}).fail(function(error,err){progressLoader.progressIndicator({mode:"hide"}),reject(error,err),"function"==typeof fail&&fail(error,err);});})},/**
		 * Clear inventory data
		 */clearInventory:function clearInventory(){var _this11=this;this.getInventoryItemsContainer().find(".inventoryRow").each(function(_,e){var num=$(e).attr("numrow");_this11.deleteLineItem(num);});},/**
		 * Function which will register all the events
		 */registerEvents:function registerEvents(container){this.form=container,this.loadConfig(),this.registerInventorySaveData(),this.registerAddItem(),this.registerAddBlock(),this.registerMassAddItem(),this.initItem(),this.registerSortableItems(),this.registerSubProducts(),this.registerChangeDiscount(),this.registerChangeTax(),this.registerClearReferenceSelection(),this.registerShowHideExpanded(),this.registerChangeCurrency(),this.registerChangeDiscountAggregation(),this.setDefaultGlobalTax(container),app.registerBlockToggleEvent(container);}});
