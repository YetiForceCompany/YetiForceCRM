...
$.Class('Vtiger_Inventory_Js', {
  ...
  */
	getInventoryInstance: function getInventoryInstance(container) {
		if (this.inventoryInstance === false) {
			var moduleClassName = container.find('[name="module"]').val() + '_Inventory_Js';
  ...
  */
	getInventoryItemsContainer: function getInventoryItemsContainer() {
		if (this.inventoryContainer === false) {
			this.inventoryContainer = $('.inventoryItems');
		}
		return this.inventoryContainer;
	},
	getInventoryHeadContainer: function getInventoryHeadContainer() {
		if (this.inventoryHeadContainer === false) {
			this.inventoryHeadContainer = $('.inventoryHeader');
		}
		return this.inventoryHeadContainer;
	},
	getInventorySummaryDiscountContainer: function getInventorySummaryDiscountContainer() {
	...
	getInventorySummaryTaxesContainer: function getInventorySummaryTaxesContainer() {
		if (this.summaryTaxesContainer === false) {
			this.summaryTaxesContainer = $('.inventorySummaryTaxes');
		}
		return this.summaryTaxesContainer;
	},
	getInventorySummaryCurrenciesContainer: function getInventorySummaryCurrenciesContainer() {
	...
	getNextLineItemRowNumber: function getNextLineItemRowNumber() {
	...
	checkDeleteIcon: function checkDeleteIcon() {
	...
	showLineItemsDeleteIcon: function showLineItemsDeleteIcon() {
		this.getInventoryItemsContainer().find('.deleteRow').removeClass('d-none');
	},
	hideLineItemsDeleteIcon: function hideLineItemsDeleteIcon() {
		this.getInventoryItemsContainer().find('.deleteRow').addClass('d-none');
	},
	getClosestRow: function getClosestRow(element) {
  ...
  */
	...
	isIndividualTaxMode: function isIndividualTaxMode(row) {
	...
	isGroupTaxMode: function isGroupTaxMode() {
	...
	showIndividualTax: function showIndividualTax(row) {
		...
		if (thisInstance.isIndividualTaxMode()) {
			...
			var taxParam = { aggregationType: 'global' };
	...
	setDefaultGlobalTax: function setDefaultGlobalTax(row) {
		var thisInstance = this;
		var parentRow = thisInstance.getInventoryItemsContainer();
		var taxDefaultValue = thisInstance.getInventorySummaryTaxesContainer().find('.js-default-tax').data('tax-default-value');
	...
	getDiscountModeSelectElement: function getDiscountModeSelectElement(row) {
		var items = this.getInventoryHeadContainer();
	...
	isIndividualDiscountMode: function isIndividualDiscountMode(row) {
		var discountModeElement = this.getDiscountModeSelectElement(row);
		var selectedOption = discountModeElement.find('option:selected');
		return selectedOption.val() == '1';
	},
	showIndividualDiscount: function showIndividualDiscount(row) {
		var thisInstance = this;
	...
	getCurrency: function getCurrency() {
		var currency = $('.js-currency', this.getInventoryHeadContainer());
		return currency.find('option:selected').val();
	},
	getTax: function getTax(row) {
	...
	getTaxPercent: function getTaxPercent(row) {
		var taxParams = row.find('.taxParam').val();
		if (taxParams == '' || taxParams == '[]' || taxParams == undefined) return 0;
		taxParams = JSON.parse(taxParams);
		var taxPercent = taxParams[taxParams.aggregationType + 'Tax'];
		return taxPercent ? taxPercent : 0;
	},
	getTaxParams: function getTaxParams(row) {
		var taxParams = row.find('.taxParam').val();
		if (taxParams == '' || taxParams == '[]' || taxParams == undefined) return false;
		return JSON.parse(taxParams);
	},
	getQuantityValue: function getQuantityValue(row) {
		return $('.qty', row).getNumberFromValue();
	},
	getUnitPriceValue: function getUnitPriceValue(row) {
		return $('.unitPrice', row).getNumberFromValue();
	},
	getDiscount: function getDiscount(row) {
		var discountParams = row.find('.discountParam').val();
		var aggregationType = $('.aggregationTypeDiscount').val();
		if (discountParams == '' || discountParams == 'null' || discountParams == '[]' || discountParams == undefined) return 0;
	...
	getNetPrice: function getNetPrice(row) {
		return this.getTotalPrice(row) - this.getDiscount(row);
	},
	getTotalPrice: function getTotalPrice(row) {
		return this.getQuantityValue(row) * this.getUnitPriceValue(row);
	},
	getGrossPrice: function getGrossPrice(row) {
		return $('.grossPrice', row).getNumberFromValue();
	},
	getPurchase: function getPurchase(row) {
	...
	getSummaryGrossPrice: function getSummaryGrossPrice() {
		var thisInstance = this;
		var price = 0;
  ...
  */
	setCurrencyParam: function setCurrencyParam(val) {
  ...
  * @param {int} val
  */
	setDiscountMode: function setDiscountMode(val) {
  ...
  * @param {int} val
  */
	setTaxMode: function setTaxMode(val) {
  ...
  */
	setName: function setName(row, val, display) {
  ...
  * @param {int} val
  */
	setQuantity: function setQuantity(row, val) {
  ...
  * @param {string} display
  */
	setUnit: function setUnit(row, val, display) {
  ...
  * @param {string} display
  */
	setSubUnit: function setSubUnit(row, val, display) {
  ...
  * @param {string} val
  */
	setComment: function setComment(row, val) {
		row.parent().find('[numrowex=' + row.attr('numrow') + ']').find('.comment').val(val).trigger('change');
	},

	setUnitPrice: function setUnitPrice(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		row.find('.unitPrice').val(val).attr('title', val);
		return this;
	},
	setPurchase: function setPurchase(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		row.find('.purchase').val(val);
		return this;
	},
	setNetPrice: function setNetPrice(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.netPriceText', row).text(val);
		$('.netPrice', row).val(val);
	},
	setGrossPrice: function setGrossPrice(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.grossPriceText', row).text(val);
		$('.grossPrice', row).val(val);
	},
	setTotalPrice: function setTotalPrice(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.totalPriceText', row).text(val);
		$('.totalPrice', row).val(val);
	},
	setMargin: function setMargin(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.margin', row).val(val);
	},
	setMarginP: function setMarginP(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.marginp', row).val(val);
	},
	setDiscount: function setDiscount(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.discount', row).val(val);
	},
	setDiscountParam: function setDiscountParam(row, val) {
		$('.discountParam', row).val(JSON.stringify(val));
	},
	setTax: function setTax(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.tax', row).val(val);
	},
	setTaxPercent: function setTaxPercent(row, val) {
		val = App.Fields.Double.formatToDisplay(val);
		$('.js-tax-percent', row).val(val);
	},
	setTaxParam: function setTaxParam(row, val) {
	...
	rowCalculations: function rowCalculations(row) {
	...
	rowsCalculations: function rowsCalculations() {
		var thisInstance = this;
		this.getInventoryItemsContainer().find(thisInstance.rowClass).each(function (index) {
			thisInstance.quantityChangeActions($(this));
		});
	...
	calculateDiscounts: function calculateDiscounts(row) {
		this.setDiscount(row, this.getDiscount(row));
	},
	calculateTaxes: function calculateTaxes(row) {
	...
	calculatSummary: function calculatSummary(element, field) {
		var thisInstance = this;
		var sum = 0;
		this.getInventoryItemsContainer().find(thisInstance.rowClass).each(function (index) {
	...
	calculatMarginPSummary: function calculatMarginPSummary() {
		var sumRow = this.getInventoryItemsContainer().find('tfoot'),
		    totalPriceField = sumRow.find('[data-sumfield="netPrice"]').length > 0 ? sumRow.find('[data-sumfield="netPrice"]') : sumRow.find('[data-sumfield="totalPrice"]'),
		    sumPrice = totalPriceField.getNumberFromText(),
		    purchase = 0,
		    marginp = 0;
		this.getInventoryItemsContainer().find(this.rowClass).each(function (index) {
			var qty = $(this).find('.qty').getNumberFromValue(),
		...
		var subtraction = sumPrice - purchase;
	...
	calculatDiscountSummary: function calculatDiscountSummary() {
		var thisInstance = this;
	...
	calculatCurrenciesSummary: function calculatCurrenciesSummary() {
		var container = this.getInventorySummaryCurrenciesContainer(),
	...
	calculatTaxSummary: function calculatTaxSummary() {
		var thisInstance = this;
	...
	getAllTaxs: function getAllTaxs() {
		var thisInstance = this;
		var tax = [];
	...
	calculateNetPrice: function calculateNetPrice(row) {
		this.setNetPrice(row, this.getNetPrice(row));
	},
	calculateGrossPrice: function calculateGrossPrice(row) {
	...
	calculateTotalPrice: function calculateTotalPrice(row) {
		this.setTotalPrice(row, this.getTotalPrice(row));
	},
	calculateMargin: function calculateMargin(row) {
		var netPrice;
	...
	calculateDiscount: function calculateDiscount(row, modal) {
	...
	calculateTax: function calculateTax(row, modal) {
		...
		if (netPriceWithoutTax) {
			var taxValue = (valuePrices - netPriceWithoutTax) / netPriceWithoutTax * 100;
	...
	registerInventorySaveData: function registerInventorySaveData() {
		...
		thisInstance.form.on(Vtiger_Edit_Js.recordPreSave, function (e, data) {
			...
			var table = thisInstance.form.find('#blackIthemTable');
			table.find('[name]').removeAttr('name');
		});
	},
	syncHeaderData: function syncHeaderData() {
		var header = this.getInventoryHeadContainer();
  ...
  */
	pricebooksModalHandler: function pricebooksModalHandler(element) {
		var thisInstance = this;
		var lineItemRow = element.closest(this.rowClass);
	...
	subProductsCashe: [],
	loadSubProducts: function loadSubProducts(parentRow, indicator) {
		var thisInstance = this;
	...
	removeSubProducts: function removeSubProducts(parentRow) {
		var subProductsContainer = $('.subProductsContainer ul', parentRow);
		subProductsContainer.find('li').remove();
	},
	addSubProducts: function addSubProducts(parentRow, responseData) {
		var subProductsContainer = $('.subProductsContainer ul', parentRow);
	...
	mapResultsToFields: function mapResultsToFields(referenceModule, parentRow, responseData) {
		var unit = void 0,
		    taxParam = [];
		var thisInstance = this;
		var isGroupTax = thisInstance.isGroupTaxMode();
  ...
  */
	triggerQtyParam: function triggerQtyParam(unit, perUnit, parentRow) {
		var validationEngine = void 0;
		switch (unit) {
			default:
	...
	saveDiscountsParameters: function saveDiscountsParameters(parentRow, modal) {
		var thisInstance = this;
		var info = {};
		var typeName = 'aggregationType';
		var panels = modal.find('[name="' + typeName + '"]:checked');
		info[typeName] = [];
		panels.each(function (i) {
			...
			container.find('[name="' + type + 'Discount"]').each(function () {
				...
				if ('global' === type) {
					...
					var name = 'individualDiscountType';
	...
	saveTaxsParameters: function saveTaxsParameters(parentRow, modal) {
		var thisInstance = this;
		var info = {};
	...
	showExpandedRow: function showExpandedRow(row) {
		var thisInstance = this;
		var items = thisInstance.getInventoryItemsContainer();
	...
	hideExpandedRow: function hideExpandedRow(row) {
		var thisInstance = this;
		var items = thisInstance.getInventoryItemsContainer();
		var inventoryRowExpanded = items.find('[numrowex="' + row.attr('numrow') + '"]');
		var element = row.find('.toggleVisibility');
	...
	initDiscountsParameters: function initDiscountsParameters(parentRow, modal) {
		var thisInstance = this;
	...
	initTaxParameters: function initTaxParameters(parentRow, modal) {
		var thisInstance = this;
		...
		$.each(thisInstance.taxModalFields, function (index, param) {
			...
			if (field.attr('type') === 'checkbox' || field.attr('type') === 'radio') {
				...
				var input = modal.find('[name="' + param + '"]');
				input.val(parameter);
	...
	currencyChangeActions: function currencyChangeActions(select, option) {
	...
	showCurrencyChangeModal: function showCurrencyChangeModal(select, option) {
		var thisInstance = this;
	...
	currencyConvertValues: function currencyConvertValues(select, selected) {
		var thisInstance = this;
  ...
  */
	setRowData: function setRowData(row, rowData) {
		...
		var discountParam = rowData.discountparam || null;
  ...
  */
	addItem: function addItem(module, baseTableId) {
		var rowData = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

		var items = this.getInventoryItemsContainer();
		var newRow = this.getBasicRow();
		var sequenceNumber = this.getNextLineItemRowNumber();
		var replaced = newRow.html().replace(/\_NUM_/g, sequenceNumber);
		var moduleLbls = newRow.data('moduleLbls');
  ...
  * @param {jQuery} container
  */
	registerAddItem: function registerAddItem() {
		var thisInstance = this;
		var itemsHeader = thisInstance.getInventoryHeadContainer();
	...
	registerSortableItems: function registerSortableItems() {
		var thisInstance = this;
		var items = thisInstance.getInventoryItemsContainer();
		items.sortable({
			handle: '.dragHandle',
	...
	registerPriceBookModal: function registerPriceBookModal(container) {
		var thisInstance = this;
	...
	registerRowChangeEvent: function registerRowChangeEvent(container) {
		var _this = this;

		container.on('focusout', '.qty', function (e) {
			var element = $(e.currentTarget);
			element.formatNumber();
			_this.quantityChangeActions(_this.getClosestRow(element));
		});
		...
		headContainer.on('change', '.js-taxmode', function (e) {
			var element = $(e.currentTarget);
			_this.showIndividualTax(_this.getClosestRow(element));
			_this.rowsCalculations();
		});
		headContainer.on('change', '.js-discountmode', function (e) {
			var element = $(e.currentTarget);
			_this.showIndividualDiscount(_this.getClosestRow(element));
			_this.rowsCalculations();
		});
	},
	registerSubProducts: function registerSubProducts() {
		var thisInstance = this;
  ...
  */
	registerClearReferenceSelection: function registerClearReferenceSelection() {
		var _this2 = this;

		this.form.on('click', '.clearReferenceSelection', function (e) {
			var referenceGroup = $(e.currentTarget).closest('div.referenceGroup');
			if (referenceGroup.length) {
				referenceGroup.find('input[id$="_display"]').val('').removeAttr('readonly');
			} else {
				var row = _this2.getClosestRow($(e.currentTarget));
				_this2.removeSubProducts(row);
				...
				if (!_this2.isGroupTaxMode()) {
					_this2.setTaxParam(row, []);
				}
				_this2.quantityChangeActions(row);
			}
		});
	},

	registerDeleteLineItemEvent: function registerDeleteLineItemEvent(container) {
		var thisInstance = this;
		container.on('click', '.deleteRow', function (e) {
			var num = thisInstance.getClosestRow($(e.currentTarget)).attr('numrow');
	...
	registerChangeDiscountModal: function registerChangeDiscountModal(modal, parentRow, params) {
		var thisInstance = this;
	...
	registerChangeTax: function registerChangeTax() {
		var thisInstance = this;
		thisInstance.form.on('click', '.changeTax', function (e) {
			...
			if (element.hasClass('groupTax')) {
				...
				parentRow = element.closest(thisInstance.rowClass);
				var sourceRecord = parentRow.find('.rowName .sourceField').val();
	...
	registerChangeCurrency: function registerChangeCurrency() {
		var _this3 = this;

		this.getInventoryHeadContainer().on('change', '.js-currency', function (e) {
			var element = $(e.currentTarget),
			    symbol = element.find('option:selected').data('conversionSymbol');
			_this3.currencyChangeActions(element, element.find('option:selected'));
			_this3.form.find('.currencySymbol').text(symbol);
		});
	},

	registerChangeTaxModal: function registerChangeTaxModal(modal, parentRow, params) {
		var thisInstance = this;
	...
	registerRowAutoComplete: function registerRowAutoComplete(container) {
		var thisInstance = this;
		var sourceFieldElement = container.find('.sourceField.js-name');
	...
	calculateItemNumbers: function calculateItemNumbers() {
		var thisInstance = this;
		var items = this.getInventoryItemsContainer();
		var i = 1;
	...
	initItem: function initItem(container) {
		var thisInstance = this;
  ...
  * @returns Promise
  */
	loadInventoryData: function loadInventoryData(recordId, sourceModule) {
		var _this4 = this;

		var success = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
		var fail = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
		...
		return new Promise(function (resolve, reject) {
			AppConnector.request({
				...
				var activeModules = [];
				_this4.getInventoryHeadContainer().find('.js-inv-add-item').each(function (index, addBtn) {
					activeModules.push($(addBtn).data('module'));
				});
				progressLoader.progressIndicator({ mode: 'hide' });
				var oldCurrencyChangeAction = _this4.currencyChangeActions;
				_this4.currencyChangeActions = function changeCurrencyActions(select, option) {
					this.currencyConvertValues(select, option);
					select.data('oldValue', select.val());
				};
				var first = response.result[Object.keys(response.result)[0]];
				_this4.setCurrencyParam(first.currencyparam);
				_this4.setCurrency(first.currency);
				_this4.setDiscountMode(first.discountmode);
				_this4.setTaxMode(first.taxmode);
				_this4.currencyChangeActions = oldCurrencyChangeAction;
				$.each(response.result, function (index, row) {
					if (activeModules.indexOf(row.moduleName) !== -1) {
						_this4.addItem(row.moduleName, row.basetableid, row);
					} else {
