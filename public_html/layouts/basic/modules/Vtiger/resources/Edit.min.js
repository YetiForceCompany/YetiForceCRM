...
document.progressLoader.progressIndicator({mode:"hide"}),editViewForm.removeData("submit"),app.formAlignmentAfterValidation(editViewForm);});},/*
		 ...
		 * @param {object} data
		 */triggerRecordEditEvents:function triggerRecordEditEvents(data){var self=this,form=this.getForm();"object"==babelHelpers.typeof(data.changeValues)&&$.each(data.changeValues,function(key,field){self.setFieldValue(field);}),"undefined"!=typeof data.hoverField&&form.find("[name=\"".concat(data.hoverField,"\"]")).focus(),"undefined"!=typeof data.showNotify&&app.showNotify(data.showNotify),"undefined"!=typeof data.showModal&&app.showModalWindow(null,data.showModal.url),"undefined"!=typeof data.showFields&&$.each(data.showFields,function(key,fieldName){form.find(".js-field-block-column[data-field=\"".concat(fieldName,"\"]")).removeClass("d-none"),self.checkVisibilityBlocks();}),"undefined"!=typeof data.hideFields&&$.each(data.hideFields,function(key,fieldName){form.find(".js-field-block-column[data-field=\"".concat(fieldName,"\"]")).addClass("d-none"),self.checkVisibilityBlocks();});},/**
		 * Register change value handler events
		 * @param {jQuery} container
		 */registerChangeValueHandlerEvent:function registerChangeValueHandlerEvent(container){var event=container.find(".js-change-value-event");if(!(0>=event.length)){var self=this,fields=JSON.parse(event.val());$.each(fields,function(key,fieldName){var fieldElement=container.find("[name=\"".concat(fieldName,"\"]"));fieldElement.on("change ".concat(Vtiger_Edit_Js.referenceSelectionEvent," ").concat(Vtiger_Edit_Js.referenceDeSelectionEvent),function(){var formData=container.serializeFormData();formData.action="ChangeValueHandler",delete formData.view,AppConnector.request(formData).done(function(response){$.each(response.result,function(key,data){self.triggerRecordEditEvents(data);});});});});}},/**
