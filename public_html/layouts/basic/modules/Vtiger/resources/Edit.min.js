...
var currentElement=$(e.target);13!=e.which||currentElement.is("textarea")||e.preventDefault();});},registerTimeFields:function registerTimeFields(container){app.registerEventForClockPicker(),App.Fields.Date.register(container),App.Fields.DateTime.register(container);},referenceCreateHandler:function referenceCreateHandler(container){var _this4=this,formData=this.getForm().serializeFormData();for(var i in formData)formData[i]&&-1==$.inArray(i,["_csrf","action"])||delete formData[i];App.Components.QuickCreate.createRecord(this.getReferencedModuleName(container),{data:{sourceModule:formData.module,sourceRecordData:formData},callbackFunction:function callbackFunction(data){_this4.setReferenceFieldValue(container,{name:data.result._recordLabel,id:data.result._recordId});},noCache:!0});},/**
...
"undefined"!=typeof response[i].showModal&&"undefined"!=typeof response[i].showModal.url?app.showModalWindow(null,response[i].showModal.url,function(modalContainer){app.registerModalController(void 0,modalContainer,function(_,instance){instance.formContainer=form;});}):app.showNotify({text:response[i].message?response[i].message:app.vtranslate("JS_ERROR"),type:"error"});response[i].hoverField!=null&&form.find("[name=\""+response[i].hoverField+"\"]").focus();}};for(var i in response)if(_loop2(i))break;aDeferred.resolve(0>=data.result.length);}).fail(function(textStatus,errorThrown){document.progressLoader.progressIndicator({mode:"hide"}),app.showNotify({text:app.vtranslate("JS_ERROR"),type:"error"}),app.errorLog(textStatus,errorThrown),aDeferred.resolve(!1);});}else aDeferred.resolve(!0);return aDeferred.promise()},registerLeavePageWithoutSubmit:function registerLeavePageWithoutSubmit(form){if("undefined"!=typeof CKEDITOR&&"undefined"!=typeof CKEDITOR.instances&&Object.keys(CKEDITOR.instances).length)CKEDITOR.on("instanceReady",function(e){var initialFormData=form.serialize();window.onbeforeunload=function(e){if(initialFormData!=form.serialize()&&"true"!=form.data("submit"))return app.vtranslate("JS_CHANGES_WILL_BE_LOST")};});else {var initialFormData=form.serialize();window.onbeforeunload=function(e){if(initialFormData!=form.serialize()&&"true"!=form.data("submit"))return app.vtranslate("JS_CHANGES_WILL_BE_LOST")};}},stretchCKEditor:function stretchCKEditor(){var row=$(".js-editor").parents(".fieldRow"),td=$(".js-editor").parent();$(row).find(".fieldLabel").remove(),$(td).removeClass("col-md-10"),$(td).addClass("col-md-12");},/**
		 ...
		 * Set field value
		 * @param {object} params
		 */setFieldValue:function setFieldValue(params){var fieldElement=this.getForm().find("[name=\"".concat(params.fieldName,"\"]")),fieldInfo=fieldElement.data("fieldinfo");fieldElement.is("select")?fieldElement.find("option[value=\"".concat(params.value,"\"]")).length?fieldElement.val(params.value).trigger("change"):fieldInfo.picklistvalues.hasOwnProperty(params.value)&&fieldElement.append(new Option(params.value,params.value,!0,!0)).trigger("change"):"checkbox"==fieldElement.attr("type")?fieldElement.prop("checked","1"==params.value).trigger("change"):"reference"===fieldElement.data("fieldtype")?this.setReferenceFieldValue(fieldElement.closest(".js-field-block-column"),{id:params.value,name:params.display}):fieldElement.val(params.value);},/**
		 * Set field options
		 ...
		 * @param {jQuery} container
		 */registerChangeValueHandlerEvent:function registerChangeValueHandlerEvent(container){var _this9=this,event=container.find(".js-change-value-event");if(!(0>=event.length||"[]"===event.val())){var fields=JSON.parse(event.val());$.each(fields,function(_,fieldName){container.find("[name=\"".concat(fieldName,"\"],[name=\"").concat(fieldName,"[]\"]")).on("change ".concat(Vtiger_Edit_Js.referenceSelectionEvent," ").concat(Vtiger_Edit_Js.referenceDeSelectionEvent),function(){_this9.sendChangeValueHandlerEvent(container.serializeFormData());});}),this.sendChangeValueHandlerEvent(container.serializeFormData());}},/**
		 * Send change value handler events
		 * @param {object} formData
		 */sendChangeValueHandlerEvent:function sendChangeValueHandlerEvent(formData){var _this10=this;formData.action="ChangeValueHandler",delete formData.view;var progress=$.progressIndicator({position:"html",blockInfo:{enabled:!0}});AppConnector.request(formData).done(function(response){$.each(response.result,function(_,data){_this10.triggerRecordEditEvents(data);}),progress.progressIndicator({mode:"hide"});});},/**
