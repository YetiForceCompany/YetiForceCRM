...
document.progressLoader.progressIndicator({mode:"hide"}),editViewForm.removeData("submit"),app.formAlignmentAfterValidation(editViewForm);});},/*
		 ...
		 */registerKeyboardShortcutsEvent:function registerKeyboardShortcutsEvent(){document.addEventListener("keydown",function(event){if(event.altKey&&"KeyS"===event.code){var _form=event.target.closest("form");_form?$(_form).trigger("submit"):(_form=$(event.target).find("form"),_form.length&&_form.last().trigger("submit"));}});},/**
