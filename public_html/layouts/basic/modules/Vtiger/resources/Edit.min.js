...
recordPreSave:'Vtiger.Record.PreSave',editInstance:!1,inventoryController:!1,/**
		 ...
		 */registerAutoCompleteReferenceFields:function registerAutoCompleteReferenceFields(container){var thisInstance=this;container.find('input.referenceAutoComplete').autocomplete({delay:'600',minLength:'3',source:function source(request,response){//element will be array of dom elements
...
$(this).data('ui-autocomplete').menu.element.css('z-index','100001');}});},registerAutoCompleteFields:function registerAutoCompleteFields(container){container.find('input.autoComplete').autocomplete({delay:600,minLength:3,source:function source(request,response){var element=$(this.element[0]),name=element.prop('name'),module=app.getModuleName(),term=request.term;AppConnector.request({module:module,action:'Autocomplete',field:name,term:term}).done(function(data){var items=data.result;response(items);});},open:function open(){$(this).data('ui-autocomplete').menu.element.css('z-index','100001');}});},/**
...
var currentElement=$(e.target);13!=e.which||currentElement.is('textarea')||e.preventDefault();});},registerTimeFields:function registerTimeFields(container){app.registerEventForClockPicker(),App.Fields.Date.register(container),App.Fields.DateTime.register(container);},referenceCreateHandler:function referenceCreateHandler(container){var thisInstance=this,params={callbackFunction:function postQuickCreateSave(data){thisInstance.setReferenceFieldValue(container,{name:data.result._recordLabel,id:data.result._recordId});}};if('Edit'===app.getViewName()&&!app.getRecordId()){var formElement=this.getForm(),formData=formElement.serializeFormData();for(var i in formData)formData[i]&&-1==$.inArray(i,['_csrf','action'])||delete formData[i];params.data={},params.data.sourceRecordData=formData;}var referenceModuleName=this.getReferencedModuleName(container);Vtiger_Header_Js.getInstance().quickCreateModule(referenceModuleName,params);},/**
		 ...
		 */registerEventForCopyAddress:function registerEventForCopyAddress(){var thisInstance=this,account_id=!1,contact_id=!1,lead_id=!1,vendor_id=!1;this.formElement.find('.js-toggle-panel:not(.inventoryHeader):not(.inventoryItems) .fieldValue, .js-toggle-panel:not(.inventoryHeader):not(.inventoryItems) .fieldLabel').each(function(){var block=$(this),referenceModulesList=!1,relatedField=block.find('[name="popupReferenceModule"]').val();'Accounts'==relatedField&&(account_id=block.find('.sourceField').attr('name')),'Contacts'==relatedField&&(contact_id=block.find('.sourceField').attr('name')),'Leads'==relatedField&&(lead_id=block.find('.sourceField').attr('name')),'Vendors'==relatedField&&(vendor_id=block.find('.sourceField').attr('name')),referenceModulesList=block.find('.referenceModulesList'),0<referenceModulesList.length&&$.each(referenceModulesList.find('option'),function(key,data){'Accounts'==data.value&&(account_id=block.find('.sourceField').attr('name')),'Contacts'==data.value&&(contact_id=block.find('.sourceField').attr('name')),'Leads'==data.value&&(lead_id=block.find('.sourceField').attr('name')),'Vendors'==data.value&&(vendor_id=block.find('.sourceField').attr('name'));});}),!1==account_id?this.formElement.find('.copyAddressFromAccount').addClass('d-none'):this.formElement.find('.copyAddressFromAccount').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label'),recordRelativeAccountId=$('[name="'+account_id+'"]').val();if(''==recordRelativeAccountId||'0'==recordRelativeAccountId)app.showNotify({text:app.vtranslate('JS_PLEASE_SELECT_AN_ACCOUNT_TO_COPY_ADDRESS'),type:'error'});else {var recordRelativeAccountName=$('#'+account_id+'_display').val();thisInstance.copyAddressDetails(from,to,{record:recordRelativeAccountId,selectedName:recordRelativeAccountName,module:'Accounts'},element.closest('.js-toggle-panel')),element.attr('checked','checked');}}),!1==contact_id?this.formElement.find('.copyAddressFromContact').addClass('d-none'):this.formElement.find('.copyAddressFromContact').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label'),recordRelativeAccountId=$('[name="'+contact_id+'"]').val();if(''==recordRelativeAccountId||'0'==recordRelativeAccountId)app.showNotify({text:app.vtranslate('JS_PLEASE_SELECT_AN_CONTACT_TO_COPY_ADDRESS'),type:'error'});else {var recordRelativeAccountName=$('#'+contact_id+'_display').val();thisInstance.copyAddressDetails(from,to,{record:recordRelativeAccountId,selectedName:recordRelativeAccountName,module:'Contacts'},element.closest('.js-toggle-panel')),element.attr('checked','checked');}}),!1==lead_id?this.formElement.find('.copyAddressFromLead').addClass('d-none'):this.formElement.find('.copyAddressFromLead').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label'),recordRelativeAccountId=$('[name="'+lead_id+'"]').val();if(''==recordRelativeAccountId||'0'==recordRelativeAccountId)app.showNotify({text:app.vtranslate('JS_PLEASE_SELECT_AN_LEAD_TO_COPY_ADDRESS'),type:'error'});else {var recordRelativeAccountName=$('#'+lead_id+'_display').val();thisInstance.copyAddressDetails(from,to,{record:recordRelativeAccountId,selectedName:recordRelativeAccountName,module:'Leads'},element.closest('.js-toggle-panel')),element.attr('checked','checked');}}),!1==vendor_id?this.formElement.find('.copyAddressFromVendor').addClass('d-none'):this.formElement.find('.copyAddressFromVendor').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label'),recordRelativeAccountId=$('[name="'+vendor_id+'"]').val();if(''==recordRelativeAccountId||'0'==recordRelativeAccountId)app.showNotify({text:app.vtranslate('JS_PLEASE_SELECT_AN_VENDOR_TO_COPY_ADDRESS'),type:'error'});else {var recordRelativeAccountName=$('#'+vendor_id+'_display').val();thisInstance.copyAddressDetails(from,to,{record:recordRelativeAccountId,selectedName:recordRelativeAccountName,module:'Vendors'},element.closest('.js-toggle-panel')),element.attr('checked','checked');}}),this.formElement.find('.js-toggle-panel').each(function(){var hideCopyAddressLabel=!0;$(this).find('.adressAction button').each(function(){!1==$(this).hasClass('d-none')&&(hideCopyAddressLabel=!1);}),hideCopyAddressLabel&&$(this).find('.copyAddressLabel').addClass('d-none');}),this.formElement.find('.copyAddressFromMain').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);}),this.formElement.find('.copyAddressFromMailing').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);}),this.formElement.find('.copyAddressFromDelivery').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);});},/**
