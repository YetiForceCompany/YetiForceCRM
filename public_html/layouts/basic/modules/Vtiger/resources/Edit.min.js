...
var currentElement=$(e.target);13!=e.which||currentElement.is('textarea')||e.preventDefault();});},registerTimeFields:function registerTimeFields(container){app.registerEventForClockPicker(),App.Fields.Date.register(container),App.Fields.DateTime.register(container);},referenceCreateHandler:function referenceCreateHandler(container){var thisInstance=this,params={callbackFunction:function postQuickCreateSave(data){thisInstance.setReferenceFieldValue(container,{name:data.result._recordLabel,id:data.result._recordId});}};if('Edit'===app.getViewName()&&!app.getRecordId()){var formElement=this.getForm(),formData=formElement.serializeFormData();for(var i in formData)formData[i]&&-1==$.inArray(i,['_csrf','action'])||delete formData[i];params.data={},params.data.sourceRecordData=formData;}var referenceModuleName=this.getReferencedModuleName(container);Vtiger_Header_Js.getInstance().quickCreateModule(referenceModuleName,params);},/**
		 ...
		 */registerEventForCopyAddress:function registerEventForCopyAddress(){var thisInstance=this;this.formElement.find('.js-toggle-panel:not(.inventoryHeader):not(.inventoryItems) .fieldValue, .js-toggle-panel:not(.inventoryHeader):not(.inventoryItems) .fieldLabel').each(function(){var block=$(this),referenceModulesList=!1;referenceModulesList=block.find('.referenceModulesList'),0<referenceModulesList.length&&(referenceModulesList.on('change',function(){thisInstance.formElement.find('[class*="copyAddressFrom"]:not(.copyAddressFromMain, .copyAddressFromMailing)').addClass('d-none');var moduleName=$(this).val();'Accounts'==moduleName?thisInstance.enableCopyAddressFromModule(moduleName,thisInstance.formElement,'copyAddressFromAccount',block.find('.sourceField').attr('name'),'JS_PLEASE_SELECT_AN_ACCOUNT_TO_COPY_ADDRESS'):'Contacts'==moduleName?thisInstance.enableCopyAddressFromModule(moduleName,thisInstance.formElement,'copyAddressFromContact',block.find('.sourceField').attr('name'),'JS_PLEASE_SELECT_AN_CONTACT_TO_COPY_ADDRESS'):'Leads'==moduleName?thisInstance.enableCopyAddressFromModule(moduleName,thisInstance.formElement,'copyAddressFromLead',block.find('.sourceField').attr('name'),'JS_PLEASE_SELECT_AN_LEAD_TO_COPY_ADDRESS'):'Vendors'==moduleName&&thisInstance.enableCopyAddressFromModule(moduleName,thisInstance.formElement,'copyAddressFromVendor',block.find('.sourceField').attr('name'),'JS_PLEASE_SELECT_AN_VENDOR_TO_COPY_ADDRESS');}),referenceModulesList.trigger('change'));}),this.formElement.find('.js-toggle-panel').each(function(){var hideCopyAddressLabel=!0;$(this).find('.adressAction button').each(function(){!1==$(this).hasClass('d-none')&&(hideCopyAddressLabel=!1);}),hideCopyAddressLabel&&$(this).find('.copyAddressLabel').addClass('d-none');}),this.formElement.find('.copyAddressFromMain').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);}),this.formElement.find('.copyAddressFromMailing').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);}),this.formElement.find('.copyAddressFromDelivery').on('click',function(){var element=$(this),block=element.closest('.js-toggle-panel'),from=element.data('label'),to=block.data('label');thisInstance.copyAddress(from,to,!1,!1);});},/**
