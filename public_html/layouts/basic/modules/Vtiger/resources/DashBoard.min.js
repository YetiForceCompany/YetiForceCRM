...
$.Class('Vtiger_DashBoard_Js', {
	...
	restrictContentDrag: function restrictContentDrag(container) {
	...
	getCurrentDashboard: function getCurrentDashboard() {
		var dashboardId = $('.selectDashboard li a.active').closest('li').data('id');
		if (!dashboardId) {
			dashboardId = 1;
		}
		return dashboardId;
	},
	getWidgetInstance: function getWidgetInstance(widgetContainer) {
	...
	registerGrid: function registerGrid() {
		...
		if (this.getContainer().width() !== this.getContainer().parent().width()) {
			var parentWidth = thisInstance.getContainer().parent().width();
			this.getContainer().css('width', parentWidth + 'px');
		}
	},
	savePositions: function savePositions(widgets) {
	...
	updateLazyWidget: function updateLazyWidget() {
	...
	loadWidgets: function loadWidgets() {
		...
		thisInstance.getContainer().find('.dashboardWidget').Lazy({
			...
			widgetLoader: function widgetLoader(element) {
				thisInstance.loadWidget(element);
			}
		});
		this.updateLazyWidget(this.scrollContainer);
	},
	loadWidget: function loadWidget(widgetContainer) {
		var self = this;
		var urlParams = widgetContainer.data('url');
		var mode = widgetContainer.data('mode');
		var moduleName = app.getModuleName();
		var sourceModule = $('a.active', 'ul.selectDashboradView').parent().data('module');
		...
		if (mode === 'open') {
			...
			AppConnector.request(urlParams).done(function (data) {
				...
				widgetContainer.closest('.grid-stack').on('gsresizestop', function (event, elem) {
  ...
  */
	adjustHeightWidget: function adjustHeightWidget(widgetContainer) {
	...
	registerRefreshWidget: function registerRefreshWidget() {
		var thisInstance = this;
				...
				AppConnector.request(url).done(function (response) {
					if (response.success) {
						...
						if ($.inArray(widgetName, nonReversableWidgets) == -1) {
							...
							data += '</a>';
							var divider = $('.js-widget-list .dropdown-divider');
							if (divider.length) {
	...
	registerSelectDashboard: function registerSelectDashboard() {
		var thisInstance = this;
	...
	registerDatePickerHideInitiater: function registerDatePickerHideInitiater() {
	...
	registerShowMailBody: function registerShowMailBody() {
		var container = this.getContainer();
	...
	registerChangeMailUser: function registerChangeMailUser() {
		var container = this.getContainer();
	...
	registerChartFilterWidget: function registerChartFilterWidget() {
		var thisInstance = this;
			...
			app.showModalWindow(null, 'index.php?module=Home&view=ChartFilter&step=step1', function (wizardContainer) {
				...
				moduleNameSelect2.on('change', function () {
					...
					AppConnector.request({
						...
						footer.hide();
						var filtersIdElement = step2.find('.filtersId');
						var valueTypeElement = step2.find('.valueType');
				...
				form.on('submit', function (e) {
					...
					if (save) {
						...
						var selectedFiltersId = form.find('.filtersId').val();
	...
	saveChartFilterWidget: function saveChartFilterWidget(data, element, moduleNameLabel, filtersId, filterLabel, groupFieldName, form) {
		var thisInstance = this;
		var label = moduleNameLabel;
	...
	registerMiniListWidget: function registerMiniListWidget() {
		var thisInstance = this;
		$('.dashboardHeading').off('click', '.addFilter').on('click', '.addFilter', function (e) {
			...
			app.showModalWindow(null, 'index.php?module=Home&view=MiniListWizard&step=step1', function (wizardContainer) {
				...
				var moduleNameSelectDOM = $('select[name="module"]', wizardContainer);
				var filteridSelectDOM = $('select[name="filterid"]', wizardContainer);
				var fieldsSelectDOM = $('select[name="fields"]', wizardContainer);
				var filterFieldsSelectDOM = $('select[name="filter_fields"]', wizardContainer);
				...
				filteridSelect2.on('change', function () {
					...
					AppConnector.request({
						...
						var fieldsHTML = responseHTML.find('select[name="fields"]').html();
						var filterFieldsHTML = responseHTML.find('select[name="filter_fields"]').html();
				...
				form.on('submit', function (e) {
					...
					if (form.validationEngine('validate') === true) {
						...
						}, element, moduleNameSelect2.find(':selected').text(), filteridSelect2.val(), filteridSelect2.find(':selected').text(), form);
					}
				});
			});
		});
	},
	saveMiniListWidget: function saveMiniListWidget(data, element, moduleNameLabel, filterid, filterLabel, form) {
		var thisInstance = this,
		    paramsForm = {
			data: JSON.stringify(data),
			blockid: element.data('block-id'),
	...
	saveWidget: function saveWidget(form, mode, sourceModule, linkid) {
	...
	registerTabModules: function registerTabModules() {
		var thisInstance = this;
  ...
  */
	removeWidgetFromList: function removeWidgetFromList() {
		var thisInstance = this;
  ...
  */
	registerTabletScrollEvent: function registerTabletScrollEvent() {
		var _this = this;
		...
		var scollbarContainer = $('.js-tablet-scroll');
		scollbarContainer.parent().removeClass('d-none');

		var scollbarContainerH = scollbarContainer.outerHeight(),
		    scollbarOffsetTop = scollbarContainer.offset().top,
		    maxOffset = $('.js-header').outerHeight() + 8;

		this.scrollContainer.on('scroll', function () {
			if (_this.scrollContainer.scrollTop() + maxOffset >= scollbarOffsetTop) {
				scollbarContainer.css({ top: maxOffset });
			} else {
				scollbarContainer.css({
					top: scollbarOffsetTop - _this.scrollContainer.scrollTop(),
					height: scollbarContainerH + _this.scrollContainer.scrollTop()
				});
			}
		});
	},
  ...
  */
	registerUpdatePredefinedWidgets: function registerUpdatePredefinedWidgets() {
		var _this2 = this;

		var container = $('.js-predefined-widgets');
		container.off('click', '.js-widget-list__item');
		container.on('click', '.js-widget-list__item', function (e) {
			if (!$(e.target).hasClass('removeWidgetFromList')) {
				_this2.addWidget($(e.currentTarget), $(e.currentTarget).data('widgetUrl'));
			}
		});
		AppConnector.request({
			view: 'BasicAjax',
	...
	addWidget: function addWidget(element, url) {
		element = $(element);
