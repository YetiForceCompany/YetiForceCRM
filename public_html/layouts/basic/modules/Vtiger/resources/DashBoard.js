...
$.Class(
	...
	},
	{
				...
				AppConnector.request({
					...
					app.showModalWindow(data, function(data) {
						data.find('.js-modal__save').on('click', e => {
							...
							let selectedTrackerActions = [];
							$.each(data.find('.js-tracker-action:checked'), function() {
								selectedTrackerActions.push($(this).val());
							});
							AppConnector.request({
								action: 'Widget',
		...
		registerUpdatesConfigSearch: function() {
			$('body').on('keyup', '.js-updates-config-search', function() {
				let value = $(this)
					.val()
					.toLowerCase();
				$('.updatesWidgetConfig .js-updates-config-search-item span').filter(function() {
					let item = $(this).closest('.js-updates-config-search-item');
					if (
						$(this)
							.text()
							.toLowerCase()
				...
				$('.js-updates-config-search-block').hide();
				$('.js-updates-config-search-item')
					.not('.d-none')
					.each(function() {
						$(this)
							.closest('.js-updates-config-search-block')
							.show();
					});
			});
		},
