...
/* {[The file is published on the basis of YetiForce Public License 4.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */'use strict';$.Class("Base_InterestsConflictModal_JS",{},{/**
		 ...
		 * @param {jQuery} modalContainer
		 */registerUsersEvents:function registerUsersEvents(modalContainer){modalContainer.find(".js-ic-canceled-btn").click(function(e){var btn=$(e.currentTarget),tr=btn.closest("tr"),icon=tr.find(".js-change-icon");bootbox.prompt({title:app.vtranslate("JS_ENTER_A_REASON"),required:!0,callback:function callback(result){result&&AppConnector.request($.extend({module:app.getModuleName(),action:"InterestsConflict",mode:"usersCancel",id:tr.data("id"),comment:result},modalContainer.find(".js-modal-form").serializeFormData())).done(function(data){data.result&&(btn.hide(),icon.length&&icon.removeClass("fa-times text-danger").addClass("fa-slash text-dark"),app.showNotify({text:data.result.message,type:data.result.type}));});}});});},/**
