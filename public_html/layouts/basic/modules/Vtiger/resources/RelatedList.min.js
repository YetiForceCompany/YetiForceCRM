...
jQuery.Class('Vtiger_RelatedList_Js', {
	...
	getInstance: function getInstance(parentId, parentModule, selectedRelatedTabElement, relatedModuleName) {
	...
	triggerMassAction: function triggerMassAction(massActionUrl, type) {
		var self = this.relatedListInstance;
		var validationResult = self.checkListRecordSelected();
  ...
  * @return boolean
  */
	verifyFileExist: function verifyFileExist(selectedIds) {
  ...
  * @param type
  */
	triggerMassDownload: function triggerMassDownload(massActionUrl, type) {
		var self = this.relatedListInstance,
		    thisInstance = this;
	...
	setSelectedTabElement: function setSelectedTabElement(tabElement) {
		this.selectedRelatedTabElement = tabElement;
	},
	getSelectedTabElement: function getSelectedTabElement() {
		return this.selectedRelatedTabElement;
	},
	getParentId: function getParentId() {
		return this.parentRecordId;
	},
	getRelatedContainer: function getRelatedContainer() {
		return this.content;
	},
	setRelatedContainer: function setRelatedContainer(container) {
	...
	setCurrentPageNumber: function setCurrentPageNumber(pageNumber) {
	...
	getCompleteParams: function getCompleteParams() {
		var container = this.getRelatedContainer();
		var params = {
			view: 'Detail',
	...
	showSelectRelation: function showSelectRelation(extendParams) {
		var _this = this;

		var params = $.extend(this.getRecordsListParams(), extendParams);
		app.showRecordsList(params, function (modal, instance) {
			instance.setSelectEvent(function (responseData) {
				_this.addRelations(Object.keys(responseData)).done(function () {
					app.event.trigger('RelatedListView.AfterSelectRelation', responseData, _this, instance, params);
					var detail = Vtiger_Detail_Js.getInstance();
					_this.loadRelatedList().done(function () {
						detail.registerRelatedModulesRecordCount();
					});
					if (_this.getSelectedTabElement().data('link-key') === 'LBL_RECORD_SUMMARY') {
						detail.loadWidgets();
						detail.registerRelatedModulesRecordCount();
					}
				});
	...
	getRecordsListParams: function getRecordsListParams() {
		return {
	...
	addRelations: function addRelations(idList) {
		var aDeferred = jQuery.Deferred();
		AppConnector.request({
			module: this.parentModuleName,
			action: 'RelationAjax',
	...
	deleteRelation: function deleteRelation(target) {
		var _this2 = this;
		...
		if (target.data('url')) {
			...
			var id = target.data('id') ? target.data('id') : target.closest('tr').data('id');
			params = {
				module: this.parentModuleName,
				action: 'RelationAjax',
				mode: 'deleteRelation',
		...
		var progressInstance = $.progressIndicator({ position: 'html', blockInfo: { enabled: true } });
		AppConnector.request(params).done(function (response) {
			progressInstance.progressIndicator({ mode: 'hide' });
			if (response.result) {
				var widget = target.closest('.widgetContentBlock');
				var detail = Vtiger_Detail_Js.getInstance();
				if (widget.length) {
					detail.loadWidget(widget);
					var updatesWidget = _this2.getContentHolder().find('[data-type="Updates"]');
					if (updatesWidget.length > 0) {
						detail.loadWidget(updatesWidget);
					}
				} else {
					_this2.loadRelatedList();
				}
				detail.registerRelatedModulesRecordCount();
			} else {
  ...
  * Function to handle next page navigation
  */
	previousPageHandler: function previousPageHandler() {
		var thisInstance = this,
  ...
  */
	selectPageHandler: function selectPageHandler(pageNumber) {
		var thisInstance = this,
		    aDeferred = jQuery.Deferred();
		this.loadRelatedList({
			page: pageNumber
  ...
  */
	pageJumpHandler: function pageJumpHandler(e) {
		var aDeferred = jQuery.Deferred();
		var thisInstance = this;
		if (e.which == 13) {
  ...
  */
	addRelatedRecord: function addRelatedRecord(element, callback) {
		var aDeferred = jQuery.Deferred();
		var thisInstance = this;
	...
	getRelatedPageCount: function getRelatedPageCount() {
		var aDeferred = jQuery.Deferred();
	...
	favoritesRelation: function favoritesRelation(relcrmId, state) {
		var aDeferred = jQuery.Deferred();
		if (relcrmId) {
			AppConnector.request({
				module: this.parentModuleName,
	...
	registerRowsEvent: function registerRowsEvent() {
		var thisInstance = this;
	...
	getDefaultSplitSizes: function getDefaultSplitSizes() {
		var thWidth = (this.listColumnFirstWidth + this.listColumnSecondWidth + 82) / this.windowW * 100;
		return [thWidth, 100 - thWidth];
	},
	getSplitSizes: function getSplitSizes() {
		var cachedParams = app.moduleCacheGet('userRelatedSplitSet');
	...
	registerListPreviewEvents: function registerListPreviewEvents() {
		var _this3 = this;
		...
		this.list.on('click', '.listViewEntries', function () {
			if (_this3.split.getSizes()[1] < 10) {
				var defaultGutterPosition = _this3.getDefaultSplitSizes();
				_this3.split.setSizes(defaultGutterPosition);
				_this3.preview.show();
				_this3.sideBlockRight.removeClass('d-block');
	...
	registerScrollEvent: function registerScrollEvent() {
		...
		var mainViewPortHeightCss = { height: this.mainBody.height() };
		var mainViewPortWidthCss = { width: this.mainBody.height() };
		var fixedElements = this.mainBody.find('.js-fixed-scroll');
		if (!this.mainBody.length) {
			...
			var iframe = $(top.document).find('.js-detail-preview');
		...
		var onScroll = function onScroll() {
			if (scrollContainer.scrollTop() >= listOffsetTop) {
				...
				_this4.sideBlocks.css({ top: scrollContainer.scrollTop() - listOffsetTop + 56 });
				fixedElements.css(mainViewPortHeightCss);
				_this4.rotatedText.css(mainViewPortHeightCss);
				_this4.rotatedText.css(mainViewPortWidthCss);
			} else {
				fixedElements.css({ top: 'initial' });
				fixedElements.css({ height: initialH + scrollContainer.scrollTop() });
				_this4.rotatedText.css({
  ...
  */
	registerSplitEvents: function registerSplitEvents(container, split) {
		...
		this.gutter.on('dblclick', function () {
			var gutterRelatedMidPosition = app.moduleCacheGet('gutterRelatedMidPosition');
			if (isNaN(_this5.split.getSizes()[0])) {
				_this5.split.setSizes(gutterRelatedMidPosition);
			}
			if (split.getSizes()[0] < 7) {
				_this5.gutter.removeClass('js-gutter-corr-left');
				...
				if (gutterRelatedMidPosition[0] > 11) {
					...
					split.setSizes(_this5.getDefaultSplitSizes());
				}
				...
				_this5.gutter.removeClass('js-gutter-corr-right');
				...
				_this5.gutter.addClass('js-gutter-corr-left');
				...
				_this5.gutter.addClass('js-gutter-corr-right');
	...
	registerSplit: function registerSplit(container) {
		...
		var split = Split([this.list[0], this.preview[0]], {
			...
			onDrag: function onDrag() {
				...
				if (split.getSizes()[0] < 7) {
					_this6.sideBlockLeft.addClass('d-block');
					_this6.list.addClass('u-hide-underneath');
				} else {
					_this6.gutter.removeClass('js-gutter-corr-left');
					_this6.sideBlockLeft.removeClass('d-block');
					_this6.list.removeClass('u-hide-underneath');
				}
				if (split.getSizes()[1] < 10) {
					_this6.sideBlockRight.addClass('d-block');
					_this6.preview.hide();
					_this6.list.width(_this6.list.width() - 10);
				} else {
					_this6.gutter.removeClass('js-gutter-corr-right');
		...
		if (!container.closest('.mainBody').length) {
			var mainBody = $(top.document).find('.mainBody').height();
			var iframe = $(top.document).find('.js-detail-preview');
		...
		$(window).on('resize', function () {
			if ($(window).width() < 993) {
				if (container.find('.gutter').length) {
					...
					_this7.sideBlockRight.removeClass('d-block');
					_this7.sideBlockLeft.removeClass('d-block');
				}
			} else {
				if (container.find('.gutter').length !== 1) {
					_this7.split = _this7.registerSplit(container);
					var gutter = container.find('.gutter');
					if (_this7.mainBody.scrollTop() >= _this7.list.offset().top + commactHeight) {
						gutter.addClass('gutterOnScroll');
						gutter.css('left', _this7.preview.offset().left - 8);
	...
	registerListScroll: function registerListScroll() {
		var container = $('.listViewEntriesDiv');
	...
	readSelectedIds: function readSelectedIds(decode) {
		var selectedIdsDataAttr = this.getCurrentCvId() + 'selectedIds',
		...
		if (decode == true && (typeof selectedIds === 'undefined' ? 'undefined' : _typeof(selectedIds)) == 'object') {
	...
	readExcludedIds: function readExcludedIds(decode) {
		var excludedIdsDataAttr = this.getCurrentCvId() + 'Excludedids',
		...
		if (decode == true && (typeof excludedIds === 'undefined' ? 'undefined' : _typeof(excludedIds)) == 'object') {
	...
	registerMainCheckBoxClickEvent: function registerMainCheckBoxClickEvent() {
		...
		this.getRelatedContainer().on('click', '#relatedListViewEntriesMainCheckBox', function () {
			...
			if ($('#relatedListViewEntriesMainCheckBox').is(':checked')) {
				var recordCountObj = self.getRecordsCount();
	...
	registerRelatedEvents: function registerRelatedEvents() {
		var _this8 = this;
		...
		YetiForce_ListSearch_Js.registerSearch(this.content, function (data) {
			_this8.loadRelatedList(data);
		});
	}
});
