...
jQuery.Class('Vtiger_RelatedList_Js',{relatedListInstance:!1,getInstance:function getInstance(parentId,parentModule,selectedRelatedTabElement,relatedModuleName){if(!1===Vtiger_RelatedList_Js.relatedListInstance||Vtiger_RelatedList_Js.relatedListInstance.moduleName!==relatedModuleName){var instance,moduleClassName=app.getModuleName()+'_RelatedList_Js',fallbackClassName=Vtiger_RelatedList_Js;instance='undefined'==typeof window[moduleClassName]?new fallbackClassName:new window[moduleClassName],instance.parentRecordId=parentId,instance.parentModuleName=parentModule,instance.selectedRelatedTabElement=selectedRelatedTabElement,instance.moduleName=relatedModuleName,instance.relatedTabsContainer=selectedRelatedTabElement.closest('div.related'),instance.content=$('div.contents',instance.relatedTabsContainer.closest('div.detailViewContainer')),instance.relatedView=instance.content.find('input.relatedView').val(),Vtiger_RelatedList_Js.relatedListInstance=instance;}return Vtiger_RelatedList_Js.relatedListInstance},triggerMassAction:function triggerMassAction(massActionUrl,type){var self=this.relatedListInstance,validationResult=self.checkListRecordSelected();if(!0!=validationResult){var progressIndicatorElement=$.progressIndicator(),selectedIds=self.readSelectedIds(!0),excludedIds=self.readExcludedIds(!0),cvId=self.getCurrentCvId(),postData=self.getCompleteParams();delete postData.mode,delete postData.view,postData.viewname=cvId,postData.selected_ids=selectedIds,postData.excluded_ids=excludedIds;'sendByForm'===type?(app.openUrlMethodPost(massActionUrl,postData),progressIndicatorElement.progressIndicator({mode:'hide'})):AppConnector.request({type:'POST',url:massActionUrl,data:postData}).done(function(responseData){if(progressIndicatorElement.progressIndicator({mode:'hide'}),responseData&&null!==responseData.result&&(responseData.result.notify&&Vtiger_Helper_Js.showMessage(responseData.result.notify),responseData.result.reloadList&&Vtiger_Detail_Js.reloadRelatedList(),responseData.result.procesStop))return progressIndicatorElement.progressIndicator({mode:'hide'}),!1}).fail(function(){progressIndicatorElement.progressIndicator({mode:'hide'});});}else self.noRecordSelectedAlert();},/**
		 ...
		 */addRelatedRecord:function addRelatedRecord(element,callback){var aDeferred=jQuery.Deferred(),thisInstance=this,referenceModuleName=this.moduleName,parentId=this.getParentId(),parentModule=this.parentModuleName,quickCreateParams={},relatedParams={},relatedField=element.data('name'),fullFormUrl=element.data('url');relatedField&&(relatedParams[relatedField]=parentId);var eliminatedKeys=new Array('view','module','mode','action'),preQuickCreateSave=function(data){var index,queryParam,queryParamComponents,queryParameters=[];//To handle switch to task tab when click on add task from related list of activities
