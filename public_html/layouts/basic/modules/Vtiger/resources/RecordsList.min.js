...
$.Class('Base_RecordsList_JS', {}, {
  ...
  * @returns {*}
  */
	loadRecordList: function loadRecordList(params) {
		var _this = this;

		var body = this.container.find('.js-modal-body');
		...
		AppConnector.request($.extend(this.getParams(), params)).done(function (responseData) {
			progressIndicatorElement.progressIndicator({ mode: 'hide' });
			body.html($(responseData).html());
			_this.registerBasicEvents();
  ...
  */
	registerHeadersClickEvent: function registerHeadersClickEvent() {
		var _this2 = this;

		YetiForce_ListSearch_Js.registerSearch(this.container, function (data) {
			_this2.loadRecordList(data);
		});
	},
	/**
  ...
  */
	updatePagination: function updatePagination(countNumberRecords) {
		var _this3 = this;
		...
		AppConnector.request(params).done(function (responseData) {
			_this3.container.find('.js-pagination-container').html(responseData);
			var totalCount = _this3.container.find('.js-pagination-list').data('totalCount');
			if (totalCount) {
				_this3.container.find('.js-total-count').val(totalCount);
			}
			_this3.registerPaginationEvents();
		});
	},
	/**
  * Register pagination events
  */
	registerPaginationEvents: function registerPaginationEvents() {
		var self = this;
		this.container.find('.js-next-page').on('click', function () {
			...
			if (self.container.find('.js-no-entries').val() == self.container.find('.js-page-limit').val()) {
				var pageNumber = self.container.find('.js-page-number');
				var nextPageNumber = parseInt(parseFloat(pageNumber.val())) + 1;
  ...
  */
	registerListEvents: function registerListEvents() {
		var self = this;
		var additional = this.container.find('.js-additional-informations').val() == 1;
