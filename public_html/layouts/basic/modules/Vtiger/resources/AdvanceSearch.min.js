...
Vtiger_BasicSearch_Js('Vtiger_AdvanceSearch_Js',{//cache will store the search data
...
if(searchModule in Vtiger_AdvanceSearch_Js.cache)return aDeferred.resolve(Vtiger_AdvanceSearch_Js.cache[searchModule]),aDeferred.promise();var searchableModulesParams={module:app.getModuleName(),searchModule:searchModule,view:'BasicAjax',mode:'showAdvancedSearch'};app.getParentModuleName()&&(searchableModulesParams.parent=app.getParentModuleName());var progressInstance=$.progressIndicator();return AppConnector.request(searchableModulesParams).done(function(data){progressInstance.hide(),Vtiger_AdvanceSearch_Js.cache[searchModule]=data,aDeferred.resolve(data);}).fail(function(error){aDeferred.reject(error);}),aDeferred.promise()},/**
		 ...
		 */showSearchResults:function showSearchResults(data){var aDeferred=$.Deferred(),postLoad=function(data){aDeferred.resolve(data);},jQhtml=$('<div class="row"><span class="col-md-4 searchHolder"></span><span class="col-md-8 filterHolder marginLeftZero d-none"></span></div>');$('.searchHolder',jQhtml).html(data),data=jQhtml;var params={};return params.data=data,params.cb=postLoad,app.showModalWindow(params),aDeferred.promise()},/**
		 * Function which will save the filter
		 */saveFilter:function saveFilter(params){var aDeferred=$.Deferred();return params.source_module=this.getSearchModule(),params.status=1,params.advfilterlist=JSON.stringify(this.advanceFilter.getConditions()),params.module='CustomView',params.action='Save',AppConnector.request(params).done(function(data){if(!data.success){var params={title:app.vtranslate('JS_MESSAGE'),text:data.error.message,type:'error'};Vtiger_Helper_Js.showPnotify(params);}aDeferred.resolve(data);}),aDeferred.promise()},/**
		 * Function which will save the filter and show the list view of new custom view
		 */saveAndViewFilter:function saveAndViewFilter(params){this.saveFilter(params).done(function(response){var url=response.result.listviewurl;window.location.href=url;},function(){});},/**
		 ...
		 */performSearch:function performSearch(){var thisInstance=this,isSearchResultsAndFilterShown=this.isSearchAndFilterComponentsShown();this.search().done(function(data){thisInstance.setContainer(thisInstance.getContainer().detach()),thisInstance.showSearchResults(data).done(function(){thisInstance.registerShowFiler(),isSearchResultsAndFilterShown&&thisInstance.showFilter();});});},/**
		 * Function which will show the advance filter next to search results
		 */showFilter:function showFilter(){var thisInstance=this;app.hideModalWindow(function callback(){app.showModalWindow(thisInstance.getContainer());});},/**
		 ...
		 */performValidation:function performValidation(){var thisInstance=this;this.formValidationDeferred=$.Deferred();var controlForm=this.getFilterForm();//To perform validation registration only once
return this.filterValidationRegistered||(this.filterValidationRegistered=!0,controlForm.validationEngine({onValidationComplete:function validationDone(form,status){status?thisInstance.formValidationDeferred.resolve():thisInstance.formValidationDeferred.reject();}})),controlForm.submit(),this.formValidationDeferred.promise()},/**
		 * Function which will register the show filer invocation
		 */registerShowFiler:function registerShowFiler(){var thisInstance=this;$('#showFilter').on('click',function(){thisInstance.showFilter();});},/**
...
container.on('change','#searchModuleList',function(e){var selectElement=$(e.currentTarget),selectedModuleName=selectElement.val();thisInstance.setSearchModule(selectedModuleName),thisInstance.initiateSearch();}),$('#advanceSearchButton').on('click',function(){var searchModule=thisInstance.getSearchModule();//If no module is selected
