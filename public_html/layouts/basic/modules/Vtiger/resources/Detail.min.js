...
jQuery.Class('Vtiger_Detail_Js', {
	...
	getInstance: function getInstance() {
		if (Vtiger_Detail_Js.detailInstance == false) {
			var moduleClassName = app.getModuleName() + '_' + app.getViewName() + '_Js',
			    instance = void 0;
  ...
  */
	triggerDetailViewAction: function triggerDetailViewAction(detailActionUrl, callBackFunction) {
  ...
  */
	triggerSendSms: function triggerSendSms(detailActionUrl, module) {
		Vtiger_Detail_Js.triggerDetailViewAction(detailActionUrl);
	},
	triggerTransferOwnership: function triggerTransferOwnership(massActionUrl) {
	...
	transferOwnershipSave: function transferOwnershipSave(form) {
  ...
  */
	getRelatedModuleContainer: function getRelatedModuleContainer() {
	...
	reloadRelatedList: function reloadRelatedList() {
		var detailInstance = Vtiger_Detail_Js.getInstance();
		var params = {};
	...
	showWorkflowTriggerView: function showWorkflowTriggerView(instance) {
		$(instance).popover('hide');
		var detailInstance = Vtiger_Detail_Js.getInstance(),
		    callback = function callback(data) {
			var treeInstance = data.find('#treeWorkflowContents');
			treeInstance.jstree({
				core: {
			...
			data.find('[type="submit"]').on('click', function () {
				var tasks = {};
				var selected = treeInstance.jstree('getCategory', true);
	...
	//constructor
	init: function init() {},
	loadWidgetsEvents: function loadWidgetsEvents() {
		var thisInstance = this;
	...
	loadWidget: function loadWidget(widgetContainer, params) {
		var thisInstance = this,
		    contentContainer = $('.js-detail-widget-content', widgetContainer);
		var relatedModuleName = void 0;
		...
		if (params === undefined) {
			...
			for (index = 0; index < queryParameters.length; index++) {
				var queryParamComponents = queryParameters[index].split('=');
	...
	widgetRelatedRecordView: function widgetRelatedRecordView(container, load) {
	...
	loadContents: function loadContents(url, data) {
		var thisInstance = this;
	...
	getUpdateFieldsArray: function getUpdateFieldsArray() {
  ...
  */
	getTabByLabel: function getTabByLabel(tabLabel) {
	...
	getTabByModule: function getTabByModule(moduleName) {
		var relationId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';

		var tabs = this.getTabs();
		var targetTab = false;
		tabs.each(function (index, element) {
			var tab = jQuery(element);
			if (tab.data('reference') == moduleName && (!relationId || relationId && relationId == tab.data('relation-id'))) {
				targetTab = tab;
				return false;
			}
		});
	...
	selectModuleTab: function selectModuleTab() {
	...
	deSelectAllrelatedTabs: function deSelectAllrelatedTabs() {
		this.getTabs().removeClass('active');
	},
	markTabAsSelected: function markTabAsSelected(tabElement) {
		tabElement.addClass('active');
		$('.related .dropdown [data-reference="' + tabElement.data('reference') + '"][data-relation-id="' + tabElement.data('relation-id') + '"]').addClass('active');
	},
	reloadTabContent: function reloadTabContent() {
	...
	getContentHolder: function getContentHolder() {
  ...
  */
	...
	getRelatedModuleName: function getRelatedModuleName() {
	...
	getRelatedListCurrentPageNum: function getRelatedListCurrentPageNum() {
		return jQuery('input[name="currentPageNum"]', this.getContentHolder()).val();
	},
	/**
  * function to remove comment block if its exists.
  */
	removeCommentBlockIfExists: function removeCommentBlockIfExists() {
  ...
  */
	getCommentThread: function getCommentThread(url) {
		var aDeferred = jQuery.Deferred();
  ...
  */
	saveCommentAjax: function saveCommentAjax(element, commentMode, commentContentValue, editCommentReason, commentId, parentCommentId, aDeferred) {
		var thisInstance = this;
		var progressIndicatorElement = jQuery.progressIndicator({});
  ...
  * return html
  */
	getCommentUI: function getCommentUI(commentId) {
		var aDeferred = jQuery.Deferred();
		var postData = {
			view: 'DetailAjax',
			module: 'ModComments',
  ...
  * return jQuery Obj.
  */
	getCommentBlock: function getCommentBlock() {
		var clonedCommentBlock = jQuery('.basicAddCommentBlock', this.getContentHolder()).clone(true, true).removeClass('basicAddCommentBlock d-none').addClass('js-add-comment-block');
  ...
  * return jQuery Obj.
  */
	getEditCommentBlock: function getEditCommentBlock() {
		var clonedCommentBlock = jQuery('.basicEditCommentBlock', this.getContentHolder()).clone(true, true).removeClass('basicEditCommentBlock d-none').addClass('js-add-comment-block');
		clonedCommentBlock.find('.commentcontenthidden').removeClass('commentcontenthidden').addClass('js-comment-content');
  ...
  */
	registerSendSmsSubmitEvent: function registerSendSmsSubmitEvent() {
		var thisInstance = this;
  ...
  */
	SendSmsSave: function SendSmsSave(form) {
  ...
  */
	registerNameAjaxEditEvent: function registerNameAjaxEditEvent() {
		var thisInstance = this;
	...
	updateHeaderNameFields: function updateHeaderNameFields() {
		var thisInstance = this;
		var detailContentsHolder = thisInstance.getContentHolder();
		var form = thisInstance.getForm();
		var nameFields = form.data('nameFields');
	...
	registerAjaxEditEvent: function registerAjaxEditEvent() {
		var thisInstance = this;
		var detailContentsHolder = thisInstance.getContentHolder();
	...
	updateHeaderValues: function updateHeaderValues(currentElement) {
		var thisInstance = this;
  ...
  */
	registerEmailFieldClickEvent: function registerEmailFieldClickEvent() {
		var detailContentsHolder = this.getContentHolder();
  ...
  */
	registerPhoneFieldClickEvent: function registerPhoneFieldClickEvent() {
		var detailContentsHolder = this.getContentHolder();
  ...
  */
	registerUrlFieldClickEvent: function registerUrlFieldClickEvent() {
		var detailContentsHolder = this.getContentHolder();
  ...
  */
	registerRelatedRowClickEvent: function registerRelatedRowClickEvent() {
		var detailContentsHolder = this.getContentHolder();
	...
	loadRelatedList: function loadRelatedList(params) {
		var aDeferred = jQuery.Deferred();
		if (params == undefined) {
			params = {};
		}
  ...
  */
	...
	registerBlockAnimationEvent: function registerBlockAnimationEvent() {
		var thisInstance = this;
		var detailContentsHolder = this.getContentHolder();
	...
	registerBlockStatusCheckOnLoad: function registerBlockStatusCheckOnLoad() {
		var blocks = this.getContentHolder().find('.js-toggle-panel');
		var module = app.getModuleName();
		blocks.each(function (index, block) {
			var currentBlock = jQuery(block);
			var dynamicAttr = currentBlock.attr('data-dynamic');
			if ((typeof dynamicAttr === 'undefined' ? 'undefined' : _typeof(dynamicAttr)) !== ('undefined') && dynamicAttr !== false) {
  ...
  */
	ajaxEditHandling: function ajaxEditHandling(currentTdElement) {
		var thisInstance = this;
		var readRecord = $('.setReadRecord'),
		...
		$(fieldElement).each(function (index, element) {
			var fieldName = $(element).val(),
			    ...
			    elementName = $.inArray(elementTarget.data('type'), ['taxes', 'sharedOwner', 'multipicklist', 'multiListFields', 'multiDomain', 'mailScannerFields', 'mailScannerActions']) != -1 ? fieldName + '[]' : fieldName;
			var fieldElement = $('[name="' + elementName + '"]:not([type="hidden"])', editElement);
			...
			var saveHandler = function saveHandler(e) {
				...
				if (element.closest('.fieldValue').is(currentTdElement) || element.hasClass('select2-selection__choice__remove') || element.closest('.select2-container--open').length || element.parents('.clockpicker-popover').length) {
					return;
				}
				...
				if (toStr(previousValue) === toStr(ajaxEditNewValue)) {
					...
					thisInstance.saveFieldValues(fieldNameValueMap).done(function (response) {
						...
						Vtiger_Helper_Js.showPnotify({
							...
							text: '<b>' + fieldInfo.data.label + '</b><br>' + '<b>' + app.vtranslate('JS_SAVED_FROM') + '</b>: ' + prevDisplayValue + '<br> ' + '<b>' + app.vtranslate('JS_SAVED_TO') + '</b>: ' + displayValue,
							type: 'info',
							textTrusted: true
						});
						if (postSaveRecordDetails['isViewable'] === false) {
							var urlObject = app.convertUrlToObject(window.location.href);
  ...
  */
	addElementsToQuickCreateForCreatingRelation: function addElementsToQuickCreateForCreatingRelation(container, customParams) {
  ...
  */
	registerEventForActivityWidget: function registerEventForActivityWidget() {
		var thisInstance = this;
   ...
   */
	...
	getSingleEventType: function getSingleEventType(modDay, id, type) {
  ...
  */
	registerFilterForAddingModuleRelatedRecordFromSummaryWidget: function registerFilterForAddingModuleRelatedRecordFromSummaryWidget(container) {
		var thisInstance = this;
		container.find('.createRecordFromFilter').on('click', function (e) {
			...
			var relatedParams = {};
			var postQuickCreateSave = function postQuickCreateSave(data) {
		...
		container.find('button.selectRelation').on('click', function (e) {
			var summaryWidgetContainer = jQuery(e.currentTarget).closest('.js-detail-widget');
			var referenceModuleName = summaryWidgetContainer.data('moduleName');
	...
	registerAddingInventoryRecords: function registerAddingInventoryRecords() {
	...
	registerEmailEvent: function registerEmailEvent() {
	...
	getFiltersDataAndLoad: function getFiltersDataAndLoad(e, params) {
		var data = this.getFiltersData(e, params);
		this.loadWidget(data['container'], data['params']);
	},
	getFiltersData: function getFiltersData(e, params) {
		if (e.currentTarget) {
			var currentElement = jQuery(e.currentTarget);
		} else {
			currentElement = e;
		...
		var urlNewParams = [];
		summaryWidgetContainer.find('.js-detail-widget .js-switch, .js-detail-widget-header .js-filter_field').each(function (n, item) {
			var value = '';
	...
	registerChangeFilterForWidget: function registerChangeFilterForWidget() {
		var thisInstance = this;
	...
	registerChangeSwitchForWidget: function registerChangeSwitchForWidget(summaryViewContainer) {
		var thisInstance = this;
  ...
  */
		...
		summaryViewContainer.on('click', '.editDefaultStatus', function (e) {
			...
			if (url) {
				if (currentTarget.hasClass('showEdit')) {
					...
					headerInstance.getQuickCreateForm(url, 'Calendar', { noCache: true }).done(function (data) {
   ...
   */
		jQuery('.createRecord').on('click', function (e) {
			...
			var goToFullFormcallback = function goToFullFormcallback(data) {
				thisInstance.addElementsToQuickCreateForCreatingRelation(data, customParams);
			};
	...
	addRelationBetweenRecords: function addRelationBetweenRecords(relatedModule, relatedModuleRecordId, selectedTabElement) {
		var aDeferred = jQuery.Deferred();
		var thisInstance = this;
  ...
  * summary view widget
  */
	postSummaryWidgetAddRecord: function postSummaryWidgetAddRecord(data, currentElement) {
		var thisInstance = this;
		var summaryWidgetContainer = currentElement.closest('.js-detail-widget');
		var widgetHeaderContainer = summaryWidgetContainer.find('.js-detail-widget-header');
		var referenceModuleName = widgetHeaderContainer.find('[name="relatedModule"]').val();
	...
	registerChangeEventForModulesList: function registerChangeEventForModulesList() {
	...
	registerEventForRelatedTabClick: function registerEventForRelatedTabClick() {
		var thisInstance = this;
		var detailContentsHolder = thisInstance.getContentHolder();
  ...
  */
	registerEventForPicklistDependencySetup: function registerEventForPicklistDependencySetup(container) {
		var thisInstance = this;
  ...
  */
	getChildComments: function getChildComments(commentId) {
		var aDeferred = jQuery.Deferred();
  ...
  * @returns {string}
  */
	getParentComments: function getParentComments(commentId) {
		var aDeferred = $.Deferred(),
  ...
  * of pageNumber text
  */
	registerEventForTotalRecordsCount: function registerEventForTotalRecordsCount() {
		var thisInstance = this;
		var detailContentsHolder = this.getContentHolder();
	...
	showPagingInfo: function showPagingInfo() {
		var totalNumberOfRecords = jQuery('#totalCount').val();
	...
	getCustomFieldNameValueMap: function getCustomFieldNameValueMap(fieldNameValueMap) {
		return fieldNameValueMap;
	},
	registerSetReadRecord: function registerSetReadRecord(detailContentsHolder) {
		var thisInstance = this;
	...
	registerFastEditingFiels: function registerFastEditingFiels() {
		var thisInstance = this;
  ...
  */
	registerRelatedModulesRecordCount: function registerRelatedModulesRecordCount(tabContainer) {
		...
		relationContainer.each(function (n, item) {
			item = $(item);
			var relationId = item.data('relationId'),
  ...
  */
	addComment: function addComment(currentTarget, data) {
		var self = this;
		...
		if (mode == 'add') {
			...
			commentHtml.done(function (data) {
				var commentBlock = closestAddCommentBlock.closest('.js-comment-details'),
				...
				if (commentBlock.length > 0) {
					...
					if (childComments.length <= 0) {
						var currentChildCommentsCount = commentInfoBlock.find('.js-view-thread-block').data('data-child-comments-count'),
						    newChildCommentCount = currentChildCommentsCount + 1;
						commentInfoBlock.find('.js-child-comments-count').text(newChildCommentCount);
						var parentCommentId = commentInfoBlock.find('.js-comment-info-header').data('commentid');
						...
						$('<li class="js-comment-details commentDetails">' + data + '</li>').appendTo(commentBlock.find('.js-comments-body'));
					}
				} else {
					$('<li class="js-comment-details commentDetails">' + data + '</li>').prependTo(closestAddCommentBlock.closest('.contents').find('.commentsList'));
				}
				commentInfoBlock.find('.js-comment-container').show();
				app.event.trigger('DetailView.SaveComment.AfterLoad', commentInfoBlock, data);
			});
		} else if (mode == 'edit') {
			var modifiedTime = commentInfoBlock.find('.js-comment-modified-time'),
  ...
  */
	registerCommentEvents: function registerCommentEvents(detailContentsHolder) {
		var self = this;
		detailContentsHolder.on('click', '.js-close-comment-block', function (e) {
			var commentInfoBlock = $(e.currentTarget.closest('.js-comment-single'));
			commentInfoBlock.find('.js-comment-container').show();
  ...
  * @param {jQuery} detailContentsHolder
  */
	submitSearchForm: function submitSearchForm(detailContentsHolder) {
		var searchTextDom = detailContentsHolder.find('.js-comment-search'),
		...
		if (searchTextDom.data('container') === 'widget' && !searchTextDom.val()) {
			var request = widgetContainer.data('url');
  ...
  */
	registerCommentEventsInDetail: function registerCommentEventsInDetail(widgetContainer) {
	...
	registerMailPreviewWidget: function registerMailPreviewWidget(container) {
		var self = this;
	...
	loadMailPreviewWidget: function loadMailPreviewWidget(widgetContent) {
		var thisInstance = this;
	...
	registerEmailEvents: function registerEmailEvents(detailContentsHolder) {
		Vtiger_Index_Js.registerMailButtons(detailContentsHolder);
	},
	registerMapsEvents: function registerMapsEvents(container) {
		if (container.find('#coordinates').length) {
			var mapView = new OpenStreetMap_Map_Js();
			mapView.registerDetailView(container);
		}
	},
	registerSocialMediaEvents: function registerSocialMediaEvents(container) {
		var socialMediaContainer = container.find('.tpl-Detail-SocialMedia');
		if (socialMediaContainer.length) ;
	},

	registerShowSummary: function registerShowSummary(container) {
  ...
  */
	registerProgress: function registerProgress() {
		var self = this;
		$('.js-header-progress-bar').each(function (index, element) {
			var picklistName = $(element).data('picklistName');
			$(element).find('.js-access').on('click', function (e) {
				self.showProgressConfirmation(e.currentTarget, picklistName);
			});
		});
	},
	loadChat: function loadChat() {
		var chatVue = $('#ChatRecordRoomVue', this.detailViewContentHolder);
		if (chatVue.length) {
			var chatContainer = this.detailViewContentHolder.find('.js-chat-container');
			var padding = 10;
	...
	registerBasicEvents: function registerBasicEvents() {
		...
		detailContentsHolder.on('click', '.js-fab__btn', function (e) {
			$(e.currentTarget).closest('.js-fab__container').toggleClass('q-fab--opened');
		});
		detailContentsHolder.find('.js-fab__container').on('clickoutside', function (e) {
		...
		detailContentsHolder.on('click', '.detailViewThread', function (e) {
			...
			var commentLoad = function commentLoad(data) {
		...
		detailContentsHolder.on('click', '.moreRecentRecords', function (e) {
			e.preventDefault();
			var recentCommentsTab = thisInstance.getTabByModule($(this).data('label-key'), $(this).data('relation-id'));
			if (recentCommentsTab.length) {
				...
				if (container.length) {
					var page = container.find('[name="page"]:last').val(),
		...
		detailContentsHolder.on('click', '.moreRecentUpdates', function (e) {
			...
			var newChange = container.find('#newChange').val(),
	...
	reloadWidgetActivitesStats: function reloadWidgetActivitesStats(container) {
	...
	refreshCommentContainer: function refreshCommentContainer(commentId) {
		var thisInstance = this;
	...
	updateRecordsPDFTemplateBtn: function updateRecordsPDFTemplateBtn(form) {
		var btnToolbar = $('.js-btn-toolbar .js-pdf');
	...
	updateWindowHeight: function updateWindowHeight(currentHeight, frame) {
		frame.height(currentHeight);
	},
	loadSubProducts: function loadSubProducts(parentRow) {
		var thisInstance = this;
		var recordId = parentRow.data('product-id'),
		    subProrductParams = {
			module: 'Products',
			action: 'SubProducts',
			record: recordId
	...
	addSubProducts: function addSubProducts(parentRow, responseData) {
		var subProductsContainer = $('.js-subproducts-container ul', parentRow);
		for (var id in responseData) {
			var productText = $('<li>').text(responseData[id]);
			subProductsContainer.append(productText);
		}
	},
	registerSubProducts: function registerSubProducts(container) {
		var thisInstance = this;
		container.find('.inventoryItems .js-inventory-row').each(function (index) {
			thisInstance.loadSubProducts($(this), false);
		});
	},
	registerCollapsiblePanels: function registerCollapsiblePanels(detailViewContainer) {
		var _this2 = this;

		var panels = detailViewContainer.find('.js-detail-widget-collapse');
		var storageName = 'yf-' + app.getModuleName() + '-detail-widgets';
		if (Quasar.plugins.LocalStorage.has(storageName)) {
			this.setPanels({ panels: panels, storageName: storageName });
		} else {
		...
		panels.on('hidden.bs.collapse shown.bs.collapse', function (e) {
			_this2.updatePanelsStorage({ id: e.target.dataset.storageKey, type: e.type, storageName: storageName });
		});
	...
	setPanels: function setPanels(_ref) {
		var panels = _ref.panels,
		    storageName = _ref.storageName;

		var panelsStorage = Quasar.plugins.LocalStorage.getItem(storageName);
		panels.each(function (i, item) {
			if (panelsStorage[item.dataset.storageKey] === 'shown' || undefined === panelsStorage[item.dataset.storageKey]) {
				$(item).collapse('show');
				$(item).siblings('.js-detail-widget-header').toggleClass('collapsed');
			}
		});
	},
	updatePanelsStorage: function updatePanelsStorage(_ref2) {
		var id = _ref2.id,
		    type = _ref2.type,
		    storageName = _ref2.storageName;
