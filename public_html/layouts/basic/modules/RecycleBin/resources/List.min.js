...
/* {[The file is published on the basis of YetiForce Public License 5.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */"use strict";Vtiger_List_Js("RecycleBin_List_Js",{/**
		 * Mass activation trigerred on the list
		 */massActivation:function massActivation(){var self=this;Vtiger_Helper_Js.showConfirmationBox({message:app.vtranslate("JS_ACTIVATE_RECORD_DESC"),title:app.vtranslate("JS_MASS_ACTIVATE")}).done(function(){var params=self.getSelectedRecordsParams(),listInstance=Vtiger_List_Js.getInstance(),container=listInstance.getListViewContainer();params.module=container.find(".js-source-module").val(),params.state="Active",params.entityState="Trash",params.action="MassState",AppConnector.request(params).done(function(data){data&&data.result&&data.result.notify&&Vtiger_Helper_Js.showMessage(data.result.notify),listInstance.getListViewRecords({module:app.getModuleName(),view:"List",sourceModule:container.find(".js-source-module").val()});});});},/**
