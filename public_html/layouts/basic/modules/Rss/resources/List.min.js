...
Vtiger_List_Js('Rss_List_Js',{},{/**
		 ...
		 */deleteRecord:function deleteRecord(container){var thisInstance=this,recordId=container.find('#recordId').val(),message=app.vtranslate('LBL_DELETE_CONFIRMATION');Vtiger_Helper_Js.showConfirmationBox({message:message}).done(function(){var module=app.getModuleName(),deleteMessage=app.vtranslate('JS_RECORD_GETTING_DELETED'),progressIndicatorElement=jQuery.progressIndicator({message:deleteMessage,position:'html',blockInfo:{enabled:!0}});AppConnector.request({module:module,action:'DeleteAjax',record:recordId}).done(function(data){if(progressIndicatorElement.progressIndicator({mode:'hide'}),data.success)thisInstance.getRssFeeds();else{var params={text:app.vtranslate(data.error.message),title:app.vtranslate('JS_LBL_PERMISSION')};Vtiger_Helper_Js.showPnotify(params);}},function(){});},function(){});},/**
