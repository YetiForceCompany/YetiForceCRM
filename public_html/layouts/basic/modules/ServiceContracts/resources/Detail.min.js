 ...
 */'use strict';Vtiger_Detail_Js("ServiceContracts_Detail_Js",{},{/**
		 * Hide all settings
		 *
		 * @return  {self}
		 ...
		 *
		 * @returns {String} HTML
		 */getTemplateTableHtml:function getTemplateTableHtml(rows){var somethingChecked=!1;return rows.forEach(function(row){if(row.checked)return somethingChecked=!0,!1}),somethingChecked||"undefined"==typeof rows[0]||(rows[0].checked=!0),"<div class=\"col-12\"><table class=\"table js-sla-policy-template-table\">\n\t\t<thead>\n\t\t\t<tr>\n\t\t\t\t<th></th>\n\t\t\t\t<th>".concat(app.vtranslate("JS_POLICY_NAME"),"</th>\n\t\t\t\t<th>").concat(app.vtranslate("JS_OPERATIONAL_HOURS"),"</th>\n\t\t\t\t<th>").concat(app.vtranslate("JS_REACTION_TIME"),"</th>\n\t\t\t\t<th>").concat(app.vtranslate("JS_IDLE_TIME"),"</th>\n\t\t\t\t<th>").concat(app.vtranslate("JS_RESOLVE_TIME"),"</th>\n\t\t\t</tr>\n\t\t</thead>\n\t\t<tbody>\n\t\t").concat(rows.map(function(row){return "<tr>\n\t\t\t\t<td><input type=\"radio\" name=\"policy_id\" value=\"".concat(row.id,"\"").concat(row.checked?"checked=\"checked\"":"","></td>\n\t\t\t\t<td>").concat(row.name,"</td>\n\t\t\t\t<td>").concat(row.operational_hours,"</td>\n\t\t\t\t<td>").concat(row.reaction_time,"</td>\n\t\t\t\t<td>").concat(row.idle_time,"</td>\n\t\t\t\t<td>").concat(row.resolve_time,"</td>\n\t\t\t</tr>")}).join(""),"\n\t\t</tbody>\n\t\t</table>\n\t\t</div>")},/**
		 ...
		 */registerAddRecordBtnClick:function registerAddRecordBtnClick(){var _this3=this;this.container.find(".js-sla-policy-add-record-btn").on("click",function(e){e.preventDefault(),e.stopPropagation();var index=_this3.container.find(".js-custom-row").length,row=$("<div class=\"card js-custom-row shadow-sm mb-2\" data-id=\"0\" data-js=\"container\">\n\t\t\t<div class=\"card-body\">\n\t\t\t\t<div class=\"d-flex\">\n\t\t\t\t\t<div class=\"d-block\" style=\"flex-grow:1\">\n\t\t\t\t\t\t<div class=\"row no-gutters\">\n\t\t\t\t\t\t\t<div class=\"col-5 pr-2\">\n\t\t\t\t\t\t\t\t<label>".concat(app.vtranslate("JS_BUSINESS_HOURS"),"</label>\n\t\t\t\t\t\t\t\t<select class=\"select2\" name=\"business_hours[").concat(index,"][]\" multiple data-validation-engine=\"validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]\">\n\t\t\t\t\t\t\t\t").concat(_this3.businessHours.map(function(businessHours){return "<option value=\"".concat(businessHours.id,"\">").concat(businessHours.name,"</option>")}).join(""),"\n\t\t\t\t\t\t\t\t</select>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class=\"col-2 pr-2\">\n\t\t\t\t\t\t\t\t<label>").concat(app.vtranslate("JS_REACTION_TIME"),"</label>\n\t\t\t\t\t\t\t\t<div class=\"input-group time\">\n\t\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"reaction_time[").concat(index,"]\" class=\"c-time-period\" value=\"1:d\">\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class=\"col-2 pr-2\">\n\t\t\t\t\t\t\t\t<label>").concat(app.vtranslate("JS_IDLE_TIME"),"</label>\n\t\t\t\t\t\t\t\t<div class=\"input-group time\">\n\t\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"idle_time[").concat(index,"]\" class=\"c-time-period\" value=\"1:d\">\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t<div class=\"col-2 pr-2\">\n\t\t\t\t\t\t\t\t<label>").concat(app.vtranslate("JS_RESOLVE_TIME"),"</label>\n\t\t\t\t\t\t\t\t<div class=\"input-group time\">\n\t\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"resolve_time[").concat(index,"]\" class=\"c-time-period\" value=\"1:d\">\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t\t<div class=\"row mt-2\">\n\t\t\t\t\t\t\t<div class=\"js-conditions-col col\">\n\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"rowid[").concat(index,"]\" value=\"0\" class=\"js-custom-row-id\" />\n\t\t\t\t\t\t\t\t<input type=\"hidden\" name=\"conditions[").concat(index,"]\" class=\"js-conditions-value\" value=\"{}\" data-js=\"container\">\n\t\t\t\t\t\t\t\t").concat(_this3.container.find(".js-conditions-template").html(),"\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t\t<div class=\"d-inline-flex text-right border-left\" style=\"flex-grow:0\">\n\t\t\t\t\t\t<div class=\"d-inline-block align-center\" style=\"margin:auto 0;\">\n\t\t\t\t\t\t\t<a href class=\"btn btn-danger ml-4 js-delete-row-action\"><span class=\"fas fa-trash-alt\"></span></a>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t</div>\n\t\t\t</div>\n\t\t</div>"));App.Fields.TimePeriod.register(row),_this3.registerDelBtnClick(row),App.Fields.Picklist.showSelect2ElementView(row.find(".select2")),_this3.registerConditionBuilder(row.find(".js-condition-builder").eq(0),_this3.container.find(".js-conditions-col").length),_this3.container.find(".js-custom-conditions").append(row);});},/**
