...
Vtiger_Detail_Js('ServiceContracts_Detail_Js', {}, {
  ...
  * @return  {self}
  */
	showTemplateSettings: function showTemplateSettings() {
  ...
  * @returns {String} HTML
  */
	getTemplateTableHtml: function getTemplateTableHtml(rows) {
  ...
  */
	loadTemplates: function loadTemplates() {
		...
		AppConnector.request({
			...
			if (data.success) {
				_this.container.find('.js-sla-policy-template--container').html(_this.getTemplateTableHtml(data.result));
			}
		});
	},
  ...
  */
	onPolicyTypeChange: function onPolicyTypeChange() {
  ...
  */
	onSubmit: function onSubmit(ev) {
		...
		var policyType = Number(this.container.find('[name="policy_type"]:checked').val());
		var policyId = Number(this.container.find('[name="policy_id"]:checked').val());
		...
		var params = this.container.serializeFormData();
		...
		AppConnector.request({ data: params }).done(function (data) {
			progress.progressIndicator({ mode: 'hide' });
			if (Array.isArray(data.result)) {
				data.result.forEach(function (row, index) {
					var rowElem = _this2.container.find('.js-custom-row').eq(index);
					rowElem.data('id', row.id);
					rowElem.find('.js-custom-row-id').val(row.id);
				});
			} else {
				$.each(_this2.container.find('.js-custom-row'), function (index, rowElem) {
					rowElem = $(rowElem);
					rowElem.data('id', 0);
  ...
  */
	registerAddRecordBtnClick: function registerAddRecordBtnClick() {
		var _this3 = this;

		this.container.find('.js-sla-policy-add-record-btn').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			var index = _this3.container.find('.js-custom-row').length;
			var row = $('<div class="card js-custom-row shadow-sm mb-2" data-id="0" data-js="container">\n\t\t\t<div class="card-body">\n\t\t\t\t<div class="d-flex">\n\t\t\t\t\t<div class="d-block" style="flex-grow:1">\n\t\t\t\t\t\t<div class="row no-gutters">\n\t\t\t\t\t\t\t<div class="col-5 pr-2">\n\t\t\t\t\t\t\t\t<label>' + app.vtranslate('JS_BUSINESS_HOURS') + '</label>\n\t\t\t\t\t\t\t\t<select class="select2" name="business_hours[' + index + '][]" multiple data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]">\n\t\t\t\t\t\t\t\t' + _this3.businessHours.map(function (businessHours) {
			...
			_this3.registerDelBtnClick(row);
			App.Fields.Picklist.showSelect2ElementView(row.find('.select2'));
			_this3.registerConditionBuilder(row.find('.js-condition-builder').eq(0), _this3.container.find('.js-conditions-col').length);
			_this3.container.find('.js-custom-conditions').append(row);
		});
	},
  ...
  */
	registerDelBtnClick: function registerDelBtnClick(container) {
		var _this4 = this;

		container.find('.js-delete-row-action').on('click', function (e) {
			e.preventDefault();
			e.stopPropagation();
			var row = $(e.target).closest('.js-custom-row');
			...
			AppConnector.request({
				...
				targetModule: _this4.targetModule,
				record: rowId
			}).done(function (data) {
				progress.progressIndicator({ mode: 'hide' });
  ...
  */
	onConditionsChange: function onConditionsChange(instance) {
		var index = this.conditionBuilders.indexOf(instance);
		this.conditionsBuildersContainers[index].parent().find('.js-conditions-value').val(JSON.stringify(instance.getConditions()));
	},
  ...
  * @param {Number} index
  */
	registerConditionBuilder: function registerConditionBuilder(container, index) {
		this.conditionBuilders[index] = new Vtiger_ConditionBuilder_Js(container, this.targetModule, this.onConditionsChange.bind(this));
  ...
  */
	initSlaPolicy: function initSlaPolicy() {
		var _this5 = this;
		...
		this.container.find('.js-sla-policy-type-radio').on('click', function (e) {
			return _this5.onPolicyTypeChange();
		});
		...
		$.each(this.container.find('.js-custom-conditions .js-condition-builder'), function (index, col) {
			_this5.registerConditionBuilder($(col), index);
		});
	},
	...
	registerEvents: function registerEvents() {
		...
		app.event.on('DetailView.Tab.AfterLoad', function (event, data, instance) {
