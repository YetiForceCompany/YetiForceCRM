...
$.Class("Base_TwoFactorAuthenticationModal_JS", {}, {
  ...
  * @param data
  */
	registerSubmitFrom: function registerSubmitFrom(data) {
		data.find('button[name=saveButton]').prop("disabled", true);
		data.find('input[name=user_code]').on('keyup', function (e) {
			if (e.keyCode !== 13) {
				data.find('button[name=saveButton]').prop("disabled", $(e.currentTarget).val().length === 0);
			}
		});
		data.find('input[name=user_code]').on('change', function (e) {
			data.find('button[name=saveButton]').prop("disabled", $(e.currentTarget).val().length === 0);
		});
		data.find('input[name=turn_off_2fa]').on('change', function (e) {
		...
		form.on('submit', function (e) {
			...
			AppConnector.request(form.serializeFormData()).done(function (respons) {
				if (respons.result.success) {
					...
					var wrongCode = form.find('.js-wrong-code');
