...
Vtiger_List_Js('Settings_Users_List_Js',{/*
		 ...
		 *@param userId, event
		 */restoreUser:function restoreUser(userId,e){e.stopPropagation(),Vtiger_Helper_Js.showConfirmationBox({message:app.vtranslate('LBL_RESTORE_CONFIRMATION')}).done(function(){var progressInstance=jQuery.progressIndicator({position:'html',blockInfo:{enabled:!0}}),params={module:app.getModuleName(),action:'SaveAjax',userid:userId,mode:'restoreUser'};AppConnector.request(params).done(function(response){if(response.success){progressInstance.progressIndicator({mode:'hide'}),app.showNotify({text:response.result.message,type:'success'});var url=response.result.listViewUrl;window.location.href=url;}});});},/*
