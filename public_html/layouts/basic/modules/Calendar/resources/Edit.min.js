...
if('undefined'!=typeof selected.type&&'no results'==selected.type)return !1;var recordExist=!0;if(participantsContent.find('.js-participant-row').each(function(){$(this).data('crmid')==selected.id&&(recordExist=!1);}),recordExist){var participantRow=participantsContent.find('.d-none .js-participant-row').clone(!0,!0);Vtiger_Index_Js.getEmailFromRecord(selected.id,selected.module).done(function(email){participantRow.data('crmid',selected.id),participantRow.data('email',email),participantRow.find('.js-participant-name').data('content',selected.fullLabel+email).text(selected.label),participantRow.find('.js-participant-icon .c-badge__icon').removeClass('fas fa-envelope').addClass('userIcon-'+selected.module),participantsContent.append(participantRow);});}else Vtiger_Helper_Js.showPnotify({text:app.vtranslate('JS_DUPLICATE_RECORD')+': '+selected.fullLabel,type:'info'});},close:function close(){participantsSearch.val('');}});},/**
	 ...
	 * @param {jQuery} form
	 */registerRecordPreSaveEventEvent:function registerRecordPreSaveEventEvent(form){var self=this,lockSave=!0;form.on(Vtiger_Edit_Js.recordPreSave,function(e){self.validateHolidayDate(form).done(function(isHoliday){lockSave&&isHoliday&&(e.preventDefault(),app.showConfirmModal(app.vtranslate('JS_DATES_SELECTED_HOLIDAYS'),function(){lockSave=!1,form.submit();}));});});},registerRow:function registerRow(row){row.on('click','.js-participant-remove',function(e){$(e.target).closest('.js-participant-row').remove();});},registerEvents:function registerEvents(){this.proceedRegisterEvents()&&(this.registerReminderFieldCheckBox(),this.registerRecurrenceFieldCheckBox(),this.registerFormSubmitEvent(),this.registerRecurringTypeChangeEvent(),this.registerInviteEvent(this.getForm()),this.registerAddInvitation(),this._super());}});
