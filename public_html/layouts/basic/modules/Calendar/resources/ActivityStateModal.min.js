						...
						(params.noCache = !0),
							headerInstance.getQuickCreateForm(url, 'Calendar', params).done(function (data) {
								if (
									...
									'0' == currentTarget.data('type') &&
										'undefined' != typeof subject &&
										0 < subject.length)
								) {
									data = $(data);
									var element = data.find('[name="subject"]');
									element.length && element.val(subject);
								}
				...
				if (data.success) {
					...
					if ('Detail' === viewName) {
						var thisInstance = Vtiger_Detail_Js.getInstance(),
							widget = $('.activityWidgetContainer .widgetContentBlock');
						if (widget.length) thisInstance.loadWidget(widget);
						else {
							var recentActivitiesTab = thisInstance.getTabByLabel(
								thisInstance.detailViewRecentActivitiesTabLabel
							);
							recentActivitiesTab && recentActivitiesTab.trigger('click'),
								'Calendar' === app.getModuleName() &&
									((recentActivitiesTab =
					...
					if (
						...
						'Calendar' === app.getModuleName() &&
							('Calendar' === viewName || 'CalendarExtended' === viewName))
					) {
						var calendarInstance = new window['Calendar_' + viewName + '_Js']();
						calendarInstance.loadCalendarData(), calendarInstance.getCalendarCreateView();
