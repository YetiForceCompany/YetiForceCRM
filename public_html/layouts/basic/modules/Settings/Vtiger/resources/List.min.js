...
Vtiger_List_Js('Settings_Vtiger_List_Js',{triggerDelete:function triggerDelete(event,url){event.stopPropagation();var instance=Vtiger_List_Js.getInstance();instance.DeleteRecord(url);},/**
		 * Make delete request
		 *
		 * @param   {object}  params
		 * @param   {jQuery.Deferred}  aDeferred
		 * @param   {Vtiger_List_Js}  instance
		 */makeDeleteRequest:function makeDeleteRequest(params,aDeferred,instance){AppConnector.request(params).done(function(data){var response=data.result;response&&response.success?($('#recordsCount').val(''),$('#totalPageCount').text(''),instance.getListViewRecords().done(function(){instance.updatePagination();})):Vtiger_Helper_Js.showPnotify({text:response.message?response.message:app.vtranslate('JS_ERROR')}),aDeferred.resolve(data);});},/**
		 * Delete by id
		 *
		 * @param   {number}  id
		 ...
		 */deleteById:function deleteById(id){var _this=this,showConfirmation=!(1<arguments.length&&void 0!==arguments[1])||arguments[1],aDeferred=jQuery.Deferred(),instance=Vtiger_List_Js.getInstance(),params=$.extend(instance.getDeleteParams(),{record:id});return showConfirmation?Vtiger_Helper_Js.showConfirmationBox({message:app.vtranslate('JS_DELETE_RECORD_CONFIRMATION')}).done(function(){_this.makeDeleteRequest(params,aDeferred,instance);}):this.makeDeleteRequest(params,aDeferred,instance),aDeferred.promise()}},{getDeleteParams:function getDeleteParams(){return {module:app.getModuleName(),parent:app.getParentModuleName(),action:'DeleteAjax'}},/*
		 * Function to register the list view container
		 */getListViewContainer:function getListViewContainer(){return !1==this.listViewContainer&&(this.listViewContainer=$('div.listViewPageDiv')),this.listViewContainer},/*
