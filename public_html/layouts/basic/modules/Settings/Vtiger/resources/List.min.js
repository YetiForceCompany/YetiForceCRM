...
Vtiger_List_Js('Settings_Vtiger_List_Js',{triggerDelete:function triggerDelete(event,url){event.stopPropagation();var instance=Vtiger_List_Js.getInstance();instance.DeleteRecord(url);},/**
		 ...
		 */makeDeleteRequest:function makeDeleteRequest(params,aDeferred,instance){AppConnector.request(params).done(function(data){var response=data.result;response&&response.success?($('#recordsCount').val(''),$('#totalPageCount').text(''),instance.getListViewRecords().done(function(){instance.updatePagination();})):Vtiger_Helper_Js.showPnotify({text:response.message}),aDeferred.resolve(data);});},/**
		 * Delete by id
		 *
		 * @param   {number}  id
