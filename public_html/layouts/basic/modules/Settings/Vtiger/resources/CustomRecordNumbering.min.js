...
$.Class('Settings_CustomRecordNumbering_Js',{},{form:!1,getForm:function getForm(){return !1==this.form&&(this.form=jQuery('#EditView')),this.form},/**
	 * Function to register change event for source module field
	 */registerOnChangeEventOfSourceModule:function registerOnChangeEventOfSourceModule(){var editViewForm=this.getForm(),sequenceBtn=editViewForm.find('.js-adavanced-sequence');editViewForm.find('[name="sourceModule"]').on('change',function(e){$('.saveButton').removeAttr('disabled'),sequenceBtn.addClass('d-none'),AppConnector.request({module:app.getModuleName(),parent:app.getParentModuleName(),action:'CustomRecordNumberingAjax',mode:'getModuleCustomNumberingData',sourceModule:$(e.currentTarget).val()}).done(function(data){if(data){editViewForm.find('[name="prefix"]').val(data.result.prefix),editViewForm.find('[name="leading_zeros"]').val(data.result.leading_zeros).trigger('change'),editViewForm.find('[name="reset_sequence"]').val(data.result.reset_sequence).trigger('change'),editViewForm.find('[name="postfix"]').val(data.result.postfix),editViewForm.find('[name="sequenceNumber"]').val(data.result.cur_id),editViewForm.find('[name="sequenceNumber"]').data('oldSequenceNumber',data.result.cur_id);var options='';for(var index in data.result.picklists)options+='<option value="picklist:'+index+'">'+data.result.picklists[index]+'</option>';editViewForm.find('#picklistVariables').html(options);}});});},/**
	 ...
	 */registerEventToUpdateRecordsWithSequenceNumber:function registerEventToUpdateRecordsWithSequenceNumber(){var editViewForm=this.getForm();editViewForm.find('[name="updateRecordWithSequenceNumber"]').on('click',function(){var sourceModule=editViewForm.find('[name="sourceModule"]').val();AppConnector.request({module:app.getModuleName(),parent:app.getParentModuleName(),action:'CustomRecordNumberingAjax',mode:'updateRecordsWithSequenceNumber',sourceModule:sourceModule}).done(function(data){!0===data.success?Settings_Vtiger_Index_Js.showMessage({text:app.vtranslate('JS_RECORD_NUMBERING_UPDATED_SUCCESSFULLY_FOR')+' '+editViewForm.find('option[value="'+sourceModule+'"]').text()}):Settings_Vtiger_Index_Js.showMessage(data.error.message);});});},/**
	 * Function to register change event for prefix,postfix,reset_sequence and sequence number
	 */registerChangeEvent:function registerChangeEvent(){this.getForm().find('[name="prefix"],[name="leading_zeros"],[name="sequenceNumber"],[name="postfix"],[name="reset_sequence"]').on('change',this.checkPrefix.bind(this));},registerCopyClipboard:function registerCopyClipboard(editViewForm){new ClipboardJS('#customVariableCopy',{text:function text(){return Vtiger_Helper_Js.showPnotify({text:app.vtranslate('JS_NOTIFY_COPY_TEXT'),type:'success'}),'{{'+editViewForm.find('#customVariables').val()+'}}'}}),new ClipboardJS('#picklistVariableCopy',{text:function text(){return Vtiger_Helper_Js.showPnotify({text:app.vtranslate('JS_NOTIFY_COPY_TEXT'),type:'success'}),'{{'+editViewForm.find('#picklistVariables').val()+'}}'}});},/**
	 * Check if reset sequence appears in prefix or postfix to prevent duplicate number generation
	 * @returns {boolean}
	 */checkPrefix:function checkPrefix(){var sequenceExists=!1,editViewForm=this.getForm(),value=editViewForm.find('[name="reset_sequence"]').val(),prefix=editViewForm.find('[name="prefix"]').val(),postfix=editViewForm.find('[name="postfix"]').val(),saveBtn=editViewForm.find('.saveButton');switch(value){case'Y':-1===prefix.indexOf('{{YY}}')&&-1===prefix.indexOf('{{YYYY}}')&&-1===postfix.indexOf('{{YY}}')&&-1===postfix.indexOf('{{YYYY}}')?(saveBtn.attr('disabled','disabled'),Vtiger_Helper_Js.showMessage({type:'error',text:app.vtranslate('JS_RS_ADD_YEAR_VARIABLE')})):(saveBtn.removeAttr('disabled'),sequenceExists=!0);break;case'M':-1===prefix.indexOf('{{MM}}')&&-1===prefix.indexOf('{{M}}')&&-1===postfix.indexOf('{{MM}}')&&-1===postfix.indexOf('{{M}}')?(saveBtn.attr('disabled','disabled'),Vtiger_Helper_Js.showMessage({type:'error',text:app.vtranslate('JS_RS_ADD_MONTH_VARIABLE')})):(saveBtn.removeAttr('disabled'),sequenceExists=!0);break;case'D':-1===prefix.indexOf('{{DD}}')&&-1===prefix.indexOf('{{D}}')&&-1===postfix.indexOf('{{DD}}')&&-1===postfix.indexOf('{{D}}')?(saveBtn.attr('disabled','disabled'),Vtiger_Helper_Js.showMessage({type:'error',text:app.vtranslate('JS_RS_ADD_DAY_VARIABLE')})):(saveBtn.removeAttr('disabled'),sequenceExists=!0);break;case'X':default:saveBtn.removeAttr('disabled'),sequenceExists=!0;}if(sequenceExists){var regex=/{{picklist:([a-z0-9_]+)}}/g,regexResult=(postfix+prefix).match(regex);regexResult&&1<regexResult.length?(Vtiger_Helper_Js.showMessage({type:'error',text:app.vtranslate('JS_PICKLIST_TOO_MANY')}),sequenceExists=!1,saveBtn.attr('disabled','disabled')):(saveBtn.removeAttr('disabled'),sequenceExists=!0);}return this.checkAdavancedSequenceBtn(),sequenceExists},/**
	 * Function to enable button if prefix or postfix has picklist value
	 */checkAdavancedSequenceBtn:function checkAdavancedSequenceBtn(){var editViewForm=this.getForm(),sequenceBtn=editViewForm.find('.js-adavanced-sequence'),prefix=editViewForm.find('[name="prefix"]').val(),postfix=editViewForm.find('[name="postfix"]').val(),regex=/{{picklist:([a-z0-9_]+)}}/g,regexResult=(postfix+prefix).match(regex);regexResult&&1<regexResult.length||!regexResult?sequenceBtn.addClass('d-none'):sequenceBtn.removeClass('d-none');},/**
	 ...
	 */registerEvents:function registerEvents(){var thisInstance=this,editViewForm=this.getForm();this.registerOnChangeEventOfSourceModule(),this.registerEventToUpdateRecordsWithSequenceNumber(),this.registerChangeEvent(),this.checkAdavancedSequenceBtn(),this.registerAdavancedSequenceEvent();var params=app.validationEngineOptions;params.onValidationComplete=function(editViewForm,valid){return valid&&thisInstance.saveModuleCustomNumbering(),!1},editViewForm.validationEngine('detach'),editViewForm.validationEngine('attach',params),this.registerCopyClipboard(editViewForm);}}),$(document).ready(function(){var customRecordNumberingInstance=new Settings_CustomRecordNumbering_Js;customRecordNumberingInstance.registerEvents();});
