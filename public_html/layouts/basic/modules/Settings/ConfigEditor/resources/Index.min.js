...
/* {[The file is published on the basis of YetiForce Public License 5.0 that can be found in the following directory: licenses/LicenseEN.txt or yetiforce.com]} */"use strict";jQuery.Class("Settings_ConfigEditor_Index_Js",{},{/*
		 ...
		 */registerEditViewEvents:function registerEditViewEvents(){var thisInstance=this,form=jQuery("#ConfigEditorForm"),detailUrl=form.data("detailUrl");App.Fields.Picklist.showSelect2ElementView(form.find("select.select2"),{dropdownCss:{"z-index":0}});//register validation engine
var params=app.validationEngineOptions;params.onValidationComplete=function(form,valid){if(valid){var progressIndicatorElement=jQuery.progressIndicator({position:"html",blockInfo:{enabled:!0}});return thisInstance.saveConfigEditor(form).done(function(data){var params={};data.success?(params.text=app.vtranslate("JS_CONFIGURATION_DETAILS_SAVED"),thisInstance.loadContents(detailUrl).done(function(){window.location.reload();}),Settings_Vtiger_Index_Js.showMessage(params)):progressIndicatorElement.progressIndicator({mode:"hide"});}).fail(function(){progressIndicatorElement.progressIndicator({mode:"hide"});}),valid}},form.validationEngine(params),form.on("submit",function(e){e.preventDefault();});//Register click event for cancel link
