...
showNotify:function showNotify(customParams){var params={title:app.vtranslate('JS_MESSAGE'),text:'',type:'info'};$.extend(params,customParams),Vtiger_Helper_Js.showPnotify(params);},frameProgress:!1,deleteModule:function deleteModule(container){var self=this;container.on('click','.deleteModule',function(){var forModule=$(this).attr('name');Vtiger_Helper_Js.showConfirmationBox({message:app.vtranslate('JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE')}).done(function(){self.frameProgress=$.progressIndicator({position:'html',message:app.vtranslate('JS_FRAME_IN_PROGRESS'),blockInfo:{enabled:!0}}),AppConnector.request({module:app.getModuleName(),action:'Basic',parent:app.getParentModuleName(),mode:'deleteModule',forModule:forModule}).done(function(){Vtiger_Helper_Js.showPnotify({title:app.vtranslate('JS_REMOVED_MODULE'),type:'info'}),app.openUrl('index.php?module=ModuleManager&parent=Settings&view=List');});});});},registerEvents:function registerEvents(){var thisInstance=this,container=jQuery('#moduleManagerContents');container.find('.createModule').on('click',thisInstance.createModule);var scrollbar=container.find('.js-scrollbar');//register click event for check box to update the module status
