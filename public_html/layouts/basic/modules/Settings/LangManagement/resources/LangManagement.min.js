...
var Settings_Index_Js = {
	initEvants: function initEvants() {
		...
		container.find('.js-add-languages-modal').on('click', function () {
			...
			app.showModalWindow(null, 'index.php?module=YetiForce&parent=Settings&view=DownloadLanguageModal', function () {
	...
	registerUpdateLanguageBtn: function registerUpdateLanguageBtn(container) {
		container.find('.js-update').on('click', function (e) {
			var icon = $(e.target).find('.js-update__icon'),
	...
	LoadEditLang: function LoadEditLang(e) {
	...
	initEditLang: function initEditLang(position) {
	...
	changeTranslation: function changeTranslation(e, position, mod) {
	...
	deleteTranslation: function deleteTranslation(e, position) {
		var target = $(e.currentTarget);
	...
	initEvant: function initEvant(element) {
		var options = {
	...
	ShowLangMondal: function ShowLangMondal(e) {
		var cloneModal = $('.AddNewLangMondal').clone(true, true);
		app.showModalWindow($(cloneModal));
	},
	ShowTranslationMondal: function ShowTranslationMondal(e) {
	...
	AddLangMondal: function AddLangMondal(e) {
	...
	AddTranslationMondal: function AddTranslationMondal(e) {
		var currentTarget = $(e.currentTarget);
	...
	DeleteLang: function DeleteLang(closestTrElement, e) {
		Settings_Index_Js.registerSaveEvent('delete', { 'prefix': closestTrElement.data('prefix') });
		closestTrElement.hide();
	},
	setAsDefaultLang: function setAsDefaultLang(closestTrElement, e) {
		...
		var OldTrDefaultLang = tbodyElement.find('tr[data-prefix="' + prefix + '"]');
		var buttonDelete = '';
		if (!OldTrDefaultLang.data('isDefault')) {
			buttonDelete = '<button class="btn btn-danger btn-sm marginLeftZero" data-toggle="confirmation" id="deleteItemC">' + '<span class="fas fa-trash fa-xs"></span>' + app.vtranslate('JS_DELETE') + '</button>';
		}
		OldTrDefaultLang.find('td:last').append(buttonDelete + '<button class="btn btn-success btn-sm marginLeftZero" data-toggle="confirmation" id="setAsDefault">' + '<span class="fas fa-check fa-xs"></span>' + app.vtranslate('JS_DEFAULT') + '</button>');
	...
	registerStats: function registerStats() {
	...
	showStats: function showStats(data, modules) {
		var thisInstance = this;
		var container = $('.LangManagement'),
		...
		var element = container.find('.statsData').html(html);
	...
	registerStatsEvent: function registerStatsEvent() {
		var thisInstance = this;
		jQuery('.statsData .btn').on('click', function (e) {
			...
			var url = 'index.php?module=' + app.getModuleName() + '&parent=' + app.getParentModuleName() + '&view=GetLabels' + '&langBase=' + jQuery('[name="langs_basic"]').val() + '&lang=' + element.data('lang') + '&sourceModule=' + row.data('module');
	...
	getDataCharts: function getDataCharts(shortages, max) {
		var container = $('.LangManagement'),
		    data = [],
		    chartData = {
			labels: [],
