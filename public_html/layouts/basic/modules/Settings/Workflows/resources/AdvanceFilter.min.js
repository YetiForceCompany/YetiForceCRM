...
Vtiger_AdvanceFilter_Js('Workflows_AdvanceFilter_Js', {}, {
	...
	comparatorsWithNoValueBoxMap: ['is record open', 'is record closed', 'has changed', 'is empty', 'is not empty', 'is added', 'is Watching Record', 'is Not Watching Record'],
	...
	loadConditions: function loadConditions(fieldSelect) {
		...
		for (var key in conditionList) {
			...
			if (conditionList.hasOwnProperty(key)) {
				...
				if (fieldInfo.type === 'picklist' && jQuery.inArray(conditionValue, ['is record open', 'is record closed']) !== -1 && 'undefined' !== typeof fieldInfo.field_params && 'undefined' !== typeof fieldInfo.field_params.isProcessStatusField && !fieldInfo.field_params.isProcessStatusField) {
					continue;
				}
				if (match != null) {
		...
		conditionGroups.each(function (index, domElement) {
			...
			conditions.each(function (i, conditionDomElement) {
				...
				var fieldType = fieldSelectElement.find('option:selected').data('fieldinfo').type,
				    ...
				    key = void 0,
				    field = void 0;
				if ($.inArray(fieldType, ['picklist', 'multipicklist', 'multiReferenceValue', 'categoryMultipicklist']) > -1) {
					for (key in fieldList) {
						...
						} else if (field === 'value' && valueSelectElement.is('select') && $.inArray(fieldType, ['multipicklist', 'multiReferenceValue', 'categoryMultipicklist']) > -1) {
