...
Vtiger_AdvanceFilter_Js(
	...
	{},
	{
		validationSupportedFieldConditionMap: { email: ['e', 'n'], date: ['is'], datetime: ['is'] }, //Hols field type for which there is validations always needed
		 ...
		 */ addNewCondition: function addNewCondition(conditionGroupElement) {
			var basicElement = jQuery('.basic', conditionGroupElement),
				newRowElement = basicElement.find('.js-conditions-row').clone(!0, !0);
		 ...
		 */ loadConditions: function loadConditions(fieldSelect) {
			var row = fieldSelect.closest('div.js-conditions-row'),
				...
				conditionSelected = row.find('[name="comparatorValue"]').val(),
			...
			'undefined' == typeof conditionList && ((conditionList = {}), (conditionList.none = 'None'));
			var options = '';
			for (var key in conditionList) //IE Browser consider the prototype properties also, it should consider has own properties only.
				if (conditionList.hasOwnProperty(key)) {
					...
					null == match
						? ((options += '<option value="' + conditionValue + '"'),
						  conditionValue == conditionSelected && (options += ' selected="selected" '),
						  (options += '>' + conditionLabel + '</option>'))
						: 'has changed' != conditionValue &&
						  ((options += '<option value="' + conditionValue + '"'),
						  conditionValue == conditionSelected && (options += ' selected="selected" '),
						  (options += '>' + conditionLabel + '</option>'));
				}
			return conditionSelectElement.empty().html(options).trigger('change'), conditionSelectElement;
		},
		/**
		 ...
		 */ getValues: function getValues() {
			...
			return (
				conditionGroups.each(function (index, domElement) {
					...
					if (0 >= conditions.length) return !0;
					var iterationValues = {};
					conditions.each(function (i, conditionDomElement) {
						var rowElement = jQuery(conditionDomElement),
							...
							valueSelectElement = jQuery('[data-value="value"]', rowElement); //To not send empty fields to server
						if (thisInstance.isEmptyFieldSelected(fieldSelectElement)) return !0;
						var key,
							field,
							fieldDataInfo = fieldSelectElement.find('option:selected').data('fieldinfo'),
							fieldType = fieldDataInfo.type,
							rowValues = {};
						if ('picklist' == fieldType || 'multipicklist' == fieldType) {
							for (key in fieldList)
								if (
									((field = fieldList[key]), 'value' == field && valueSelectElement.is('input'))
								) {
									...
									for (i = 0; i < valuesArr.length; i++)
										'undefined' == typeof pickListValues[valuesArr[i]]
									...
									'value' == field &&
									valueSelectElement.is('select') &&
									'multipicklist' == fieldType
								) {
									var value = valueSelectElement.val();
									rowValues[field] = null == value ? value : value.join(',');
								} else rowValues[field] = jQuery('[name="' + field + '"]', rowElement).val();
						} else for (key in fieldList) (field = fieldList[key]), (rowValues[field] = 'value' == field ? valueSelectElement.val() : jQuery('[name="' + field + '"]', rowElement).val());
						('false' == jQuery('[name="valuetype"]', rowElement).val() ||
							0 == jQuery('[name="valuetype"]', rowElement).length) &&
							(rowValues.valuetype = 'rawtext'),
							(rowValues.groupid = '0' == index ? '0' : '1'),
							rowElement.is(':last-child') && (rowValues.column_condition = ''),
							(iterationValues[columnIndex] = rowValues),
							columnIndex++;
					}),
						jQuery.isEmptyObject(iterationValues) ||
							((values[index + 1] = {}), (values[index + 1].columns = iterationValues)),
						0 < groupElement.find('div.groupCondition').length &&
		 ...
		 */ getFieldSpecificUi: function getFieldSpecificUi(fieldSelectElement) {
			...
			return -1 == jQuery.inArray(fieldInfo.comparatorElementVal, this.comparatorsWithNoValueBoxMap)
				? this._super(fieldSelectElement)
				: jQuery('');
		}
	}
),
	...
	Vtiger_Date_Field_Js(
		...
		{},
		{
			 ...
			 */ getUi: function getUi() {
				var html,
					element,
					comparatorSelectedOptionVal = this.get('comparatorElementVal'),
					dateSpecificConditions = this.get('dateSpecificConditions');
				if (0 < comparatorSelectedOptionVal.length) {
					if ('between' == comparatorSelectedOptionVal || 'custom' == comparatorSelectedOptionVal)
						return (
							(html =
								...
								'"></div>'),
							(element = jQuery(html)),
							this.addValidationToElement(element)
						);
					if (this._specialDateComparator(comparatorSelectedOptionVal))
						return (
							(html =
								'<input name="' +
								this.getName() +
								...
								this.getValue() +
								'" data-validation-engine="validate[funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" data-validator="[{"name":"PositiveNumber"}]"><input type="hidden" name="valuetype" value="' +
								this.get('workflow_valuetype') +
								'" />'),
							jQuery(html)
						);
					if (comparatorSelectedOptionVal in dateSpecificConditions) {
					...
					if ('is today' == comparatorSelectedOptionVal);
			...
			_specialDateComparator: function _specialDateComparator(comp) {
				...
				for (var index in specialComparators) if (comp == specialComparators[index]) return !0;
				return !1;
			}
		}
	),
	Vtiger_Date_Field_Js(
		...
		{},
		{
			 ...
			 */ getUi: function getUi() {
				var html, element;
				return (
					this._specialDateTimeComparator(this.get('comparatorElementVal'))
						? ((html =
								'<input name="' +
								this.getName() +
								'" type="text" value="' +
				...
				for (var optGroup in pickListValues) {
					...
					for (var option in optionGroupValues)
						(html += '<option value="' + option + '" '),
							option == selectedOption && (html += ' selected '),
							(html += '>' + optionGroupValues[option] + '</option>');
					html += '</optgroup>';
				}
				html += '</select>';
				var selectContainer = jQuery(html);
				return this.addValidationToElement(selectContainer), selectContainer;
			}
		}
	),
		...
		{},
		{
			getUi: function getUi() {
				var selectedOption = app.htmlDecode(this.getValue()),
					pickListValues = this.getPickListValues(),
					tagsArray = [];
				jQuery.map(pickListValues, function (val) {
					tagsArray.push(val);
				});
				...
				var html = '<input type="hidden" class="row select2" name="' + this.getName() + '">',
					selectContainer = jQuery(html).val(pickListValues[selectedOption]);
				return (
