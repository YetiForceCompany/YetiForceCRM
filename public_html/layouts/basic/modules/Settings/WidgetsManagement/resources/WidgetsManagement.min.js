...
jQuery.Class('Settings_WidgetsManagement_Js', {}, {
	...
	setWidgetWithFilterUsers: function setWidgetWithFilterUsers() {
		var thisInstance = this;
		var element = jQuery('[name="filter_users"]').val();
		if (element) thisInstance.widgetWithFilterUsers = JSON.parse(element);else thisInstance.widgetWithFilterUsers = [];
	},
	widgetWithFilterDate: [],
	setWidgetWithFilterDate: function setWidgetWithFilterDate() {
		var thisInstance = this;
		var element = jQuery('[name="filter_date"]').val();
		if (element) thisInstance.widgetWithFilterDate = JSON.parse(element);else thisInstance.widgetWithFilterDate = [];
	},
	restrictFilter: [],
	setRestrictFilter: function setRestrictFilter() {
		var thisInstance = this;
		var element = jQuery('[name="filter_restrict"]').val();
		if (element) thisInstance.restrictFilter = JSON.parse(element);else thisInstance.restrictFilter = [];
	},
	/**
  ...
  */
	...
	getAllFieldsInBlock: function getAllFieldsInBlock(continer) {
		var fields = [];
	...
	registerAddedDashboard: function registerAddedDashboard() {
		var thisInstance = this;
	...
	registerSelectDashboard: function registerSelectDashboard() {
		...
		$('.selectDashboard li').on('click', function (e) {
			var contentsDiv = $('.contentsDiv');
			thisInstance.getModuleLayoutEditor($('#selectedModuleName').val(), $(e.currentTarget).data('id')).done(function (data) {
				thisInstance.updateContentsDiv(contentsDiv, data);
			});
		});
	},
	registerDashboardAction: function registerDashboardAction() {
		var thisInstance = this;
  ...
  */
	registerAddBlockDashBoard: function registerAddBlockDashBoard() {
		var thisInstance = this;
		...
		contents.find('.addCustomField').on('click', function (e) {
			...
			var callBackFunction = function callBackFunction(data) {
				...
				form.validationEngine($.extend(true, {
					onValidationComplete: function onValidationComplete(form, valid) {
						if (valid) {
							if (widgets.val()) {
								...
								if (paramsForm['default_owner'] && typeof paramsForm['owners_all'] === "undefined") {
									form.find('select[name="owners_all"]').prev('div').validationEngine('showPrompt', app.vtranslate('JS_FIELD_EMPTY'), 'error', 'bottomLeft', true);
  ...
  */
	showCustomField: function showCustomField(result) {
		...
		if (typeof result['default_owner'] !== "undefined") fieldContainer.find('.widgetFilterAll').removeClass('d-none').show();
  ...
  */
	setFieldDetails: function setFieldDetails(result, form) {
	...
	registerEditFieldDetailsClick: function registerEditFieldDetailsClick() {
  ...
  */
	registerSaveFieldDetailsEvent: function registerSaveFieldDetailsEvent(form) {
  ...
  */
	...
	registerSpecialWidget: function registerSpecialWidget() {
		var thisInstance = this;
	...
	addRssWidget: function addRssWidget(element, url) {
		var thisInstance = this;
	...
	saveAfterInfo: function saveAfterInfo(data, paramsForm) {
		var thisInstance = this;
		var result = data['result'];
		var params = {};
		if (data['success']) {
	...
	addChartFilterWidget: function addChartFilterWidget(element) {
		var thisInstance = this;
		...
		app.showModalWindow(null, "index.php?module=Home&view=ChartFilter&step=step1", function (wizardContainer) {
			...
			form.on('submit', function (e) {
				...
				var selectedFilterId = form.find('.filtersId').val();
	...
	addNoteBookWidget: function addNoteBookWidget(element, url) {
		var thisInstance = this;
		element = jQuery(element);
	...
	addMiniListWidget: function addMiniListWidget(element, url) {
  ...
  */
	registerModulesChangeEvent: function registerModulesChangeEvent() {
		var thisInstance = this;
		var container = $('#widgetsManagementEditorContainer');
		...
		container.on('change', '[name="widgetsManagementEditorModules"]', function (e) {
			thisInstance.getModuleLayoutEditor($(e.currentTarget).val(), thisInstance.getCurrentDashboardId()).done(function (data) {
				thisInstance.updateContentsDiv(contentsDiv, data);
			});
		});
	},
  ...
  */
	updateContentsDiv: function updateContentsDiv(contentsDiv, data) {
  ...
  */
	getModuleLayoutEditor: function getModuleLayoutEditor(selectedModule, selectedDashboard) {
		var aDeferred = jQuery.Deferred();
		var progressIndicatorElement = jQuery.progressIndicator({
			'position': 'html',
			'blockInfo': {
