...
$.Class('Settings_LayoutEditor_Js',{},{updatedBlockSequence:{},reactiveFieldsList:[],inActiveFieldsList:!1,updatedBlockFieldsList:[],updatedBlocksList:[],blockNamesList:[],/**
...
if(form.find('.supportedType').addClass('d-none'),selectedOption.data('lengthsupported')&&(form.find('.lengthsupported').removeClass('d-none'),selectedOption.data('nolimitforlength')?lengthInput.data('validator',[{name:'WholeNumberGreaterThanZero'}]):lengthInput.data('validator',maxLengthValidator)),selectedOption.data('decimalsupported')){var decimalFieldUi=form.find('.decimalsupported');decimalFieldUi.removeClass('d-none');var decimalInput=decimalFieldUi.find('[name="decimal"]'),maxFloatingDigits=selectedOption.data('maxfloatingdigits');'undefined'!=typeof maxFloatingDigits&&(decimalInput.data('validator',decimalValidator),lengthInput.data('validator',lengthValidator)),selectedOption.data('decimalreadonly')?decimalInput.val(maxFloatingDigits).attr('readonly',!0):decimalInput.removeAttr('readonly').val('');}if(selectedOption.data('predefinedvalueexists')){var pickListUi=form.find('.preDefinedValueExists');pickListUi.removeClass('d-none');}if(selectedOption.data('picklistoption')){var pickListOption=form.find('.picklistOption');pickListOption.removeClass('d-none');}'Related1M'==selectedOption.val()&&form.find('.preDefinedModuleList').removeClass('d-none'),('Tree'==selectedOption.val()||'CategoryMultipicklist'==selectedOption.val())&&form.find('.preDefinedTreeList').removeClass('d-none'),'ServerAccess'==selectedOption.val()&&form.find('.js-server-access-list').removeClass('d-none'),'MultiReferenceValue'==selectedOption.val()&&(form.find('.preMultiReferenceValue').removeClass('d-none'),thisInstance.loadMultiReferenceFields(form));});},/**
		 ...
		 */showCustomField:function showCustomField(result){var thisInstance=this,contents=$('#layoutEditorContainer').find('.contents'),relatedBlock=contents.find('.block_'+result.blockid),fieldCopy=contents.find('.newCustomFieldCopy').clone(!0,!0),fieldContainer=fieldCopy.find('div.marginLeftZero.border1px');fieldContainer.addClass('opacity editFields').attr('data-field-id',result.id).attr('data-block-id',result.blockid),fieldContainer.find('.deleteCustomField, .saveFieldDetails').attr('data-field-id',result.id),fieldContainer.find('.fieldLabel').html(result.label+' ['+result.name+']'),fieldContainer.find('#relatedFieldValue').val(result.name).prop('id','relatedFieldValue'+result.id),fieldContainer.find('.copyFieldLabel').attr('data-target','relatedFieldValue'+result.id),thisInstance.registerCopyClipboard(),result.customField||fieldContainer.find('.deleteCustomField').remove(),-1==$.inArray(result.type,['string','phone','currency','url','integer','double'])&&fieldContainer.find('.maskField').remove();var block=relatedBlock.find('.blockFieldsList'),sortable1=block.find('.js-sort-table1'),length1=sortable1.children().length,sortable2=block.find('.js-sort-table2'),length2=sortable2.children().length;// Deciding where to add the new field
