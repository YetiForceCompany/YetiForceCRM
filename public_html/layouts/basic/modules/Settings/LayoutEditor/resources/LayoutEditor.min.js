...
$.Class("Settings_LayoutEditor_Js",{},{container:!1,updatedBlockSequence:{},inActiveFieldsList:!1,updatedBlockFieldsList:[],updatedBlocksList:[],blockNamesList:[],/**
...
var lastElementInLeftContainer=leftSideContainer.children(":last");rightSideContainer.append(lastElementInLeftContainer);}},/**
		 ...
		 */saveCustomField:function saveCustomField(blockId,form){var _this4=this,saveButton=form.find(":submit");this.addCustomField(blockId,form).done(function(data){var result=data.result,params={};data.success?(app.hideModalWindow(),params.text=app.vtranslate("JS_CUSTOM_FIELD_ADDED"),Settings_Vtiger_Index_Js.showMessage(params),_this4.showCustomField(result)):(app.showNotify({title:513==data.error.code?form.find(".fieldLabelForm").text():form.find(".fieldNameForm").text(),type:"error",text:data.error.message}),saveButton.removeAttr("disabled"));});},/**
...
lengthInput.data("validator",maxLengthValidator),form.find("[name=\"fieldType\"]").on("change",function(e){var currentTarget=$(e.currentTarget),lengthInput=form.find("[name=\"fieldLength\"]"),selectedOption=currentTarget.find("option:selected");if(form.find(".supportedType").addClass("d-none"),selectedOption.data("lengthsupported")&&(form.find(".lengthsupported").removeClass("d-none"),selectedOption.data("nolimitforlength")?lengthInput.data("validator",[{name:"WholeNumberGreaterThanZero"}]):lengthInput.data("validator",maxLengthValidator)),selectedOption.data("decimalsupported")){var decimalFieldUi=form.find(".decimalsupported");decimalFieldUi.removeClass("d-none");var decimalInput=decimalFieldUi.find("[name=\"decimal\"]"),maxFloatingDigits=selectedOption.data("maxfloatingdigits");"undefined"!=typeof maxFloatingDigits&&(decimalInput.data("validator",decimalValidator),lengthInput.data("validator",lengthValidator)),selectedOption.data("decimalreadonly")?decimalInput.val(maxFloatingDigits).attr("readonly",!0):decimalInput.removeAttr("readonly").val("");}if(selectedOption.data("predefinedvalueexists")){var pickListUi=form.find(".preDefinedValueExists");pickListUi.removeClass("d-none");}if(selectedOption.data("picklistoption")){var pickListOption=form.find(".picklistOption");pickListOption.removeClass("d-none");}"Related1M"==selectedOption.val()&&form.find(".preDefinedModuleList").removeClass("d-none"),("Tree"==selectedOption.val()||"CategoryMultipicklist"==selectedOption.val())&&form.find(".preDefinedTreeList").removeClass("d-none"),"ServerAccess"==selectedOption.val()&&form.find(".js-server-access-list").removeClass("d-none"),"MultiReferenceValue"==selectedOption.val()&&(form.find(".preMultiReferenceValue").removeClass("d-none"),thisInstance.loadMultiReferenceFields(form));});},/**
		 ...
		 */showCustomField:function showCustomField(result){var thisInstance=this,contents=this.container.find(".contents"),relatedBlock=contents.find(".block_"+result.blockid),fieldCopy=contents.find(".newCustomFieldCopy").clone(!0,!0),fieldContainer=fieldCopy.find(".js-custom-field");fieldContainer.addClass("opacity editFields").attr("data-field-id",result.id).attr("data-block-id",result.blockid),fieldContainer.find(".deleteCustomField, .saveFieldDetails").attr("data-field-id",result.id),fieldContainer.find(".fieldLabel").html(result.label+"<span class=\"ml-3 font-weight-normal\">["+result.name+"]</span>"),fieldContainer.find("#relatedFieldValue").val(result.name).prop("id","relatedFieldValue"+result.id),fieldContainer.find(".copyFieldLabel").attr("data-target","relatedFieldValue"+result.id),thisInstance.registerCopyClipboard(),result.customField||fieldContainer.find(".deleteCustomField").remove(),-1==$.inArray(result.type,["string","phone","currency","url","integer","double"])&&fieldContainer.find(".maskField").remove();var block=relatedBlock.find(".blockFieldsList"),sortable1=block.find(".js-sort-table1"),length1=sortable1.children().length,sortable2=block.find(".js-sort-table2"),length2=sortable2.children().length;length1>length2?sortable2.append(fieldCopy.removeClass("d-none newCustomFieldCopy")):sortable1.append(fieldCopy.removeClass("d-none newCustomFieldCopy")),thisInstance.makeFieldsListSortable();},/**
		 ...
		 */registerInactiveFieldsEvent:function registerInactiveFieldsEvent(){var thisInstance=this,contents=this.container.find(".contents");contents.on("click",".js-inactive-fields-btn",function(e){var currentTarget=$(e.currentTarget);currentTarget.closest(".editFieldsTable");//If there are no hidden fields, show pnotify
...
thisInstance.showHiddenFields(form),form.on("submit",function(e){thisInstance.createReactivateFieldslist(form),app.hideModalWindow(),e.preventDefault();});};app.showModalWindow(inActiveFieldsContainer,function(data){"function"==typeof callBackFunction&&callBackFunction(data);},{width:"1000px"});}});},/**
		 ...
		 */registerDeleteCustomBlockEvent:function registerDeleteCustomBlockEvent(){var thisInstance=this,contents=this.container.find(".contents");contents.on("click",".js-delete-custom-block-btn",function(e){var currentTarget=$(e.currentTarget),table=currentTarget.closest("div.editFieldsTable"),blockId=table.data("blockId"),message=app.vtranslate("JS_LBL_ARE_YOU_SURE_YOU_WANT_TO_DELETE");Vtiger_Helper_Js.showConfirmationBox({message:message}).done(function(){thisInstance.deleteCustomBlock(blockId);}).fail(function(){});});},/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){return this.validateValue(this.getElement().val())},validateValue:function validateValue(fieldValue){var _this7=this,r=!0;return $.each(fieldValue,function(i,val){/[\<\>\"\,\#]/.test(val)&&(_this7.setError(app.vtranslate("JS_SPECIAL_CHARACTERS")+" < > \" , # "+app.vtranslate("JS_NOT_ALLOWED")),r=!1);}),r}});
