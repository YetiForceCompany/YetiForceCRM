...
var lastElementInLeftContainer=leftSideContainer.children(":last");rightSideContainer.append(lastElementInLeftContainer);}},/**
		 ...
		 */saveCustomField:function saveCustomField(blockId,form){var saveButton=form.find(":submit");this.addCustomField(blockId,form).done(function(data){var result=data.result,params={};data.success?(app.hideModalWindow(),params.text=app.vtranslate("JS_CUSTOM_FIELD_ADDED"),Settings_Vtiger_Index_Js.showMessage(params),window.location.reload()):(app.showNotify({title:513==data.error.code?form.find(".fieldLabelForm").text():form.find(".fieldNameForm").text(),type:"error",text:data.error.message}),saveButton.removeAttr("disabled"));});},/**
...
thisInstance.showHiddenFields(form),form.on("submit",function(e){thisInstance.createReactivateFieldslist(form),app.hideModalWindow(),e.preventDefault();});};app.showModalWindow(inActiveFieldsContainer,function(data){"function"==typeof callBackFunction&&callBackFunction(data);},{width:"1000px"});}});},/**
		 ...
		 */registerAddInventoryField:function registerAddInventoryField(){var _this4=this,thisInstance=this,container=thisInstance.getInventoryViewLayout();container.find(".addInventoryField").on("click",function(e){var currentTarget=$(e.currentTarget),selectedModule=_this4.container.find("[name=\"layoutEditorModules\"]").val(),blockId=currentTarget.closest(".inventoryBlock").data("block-id"),progress=$.progressIndicator();app.showModalWindow(null,"index.php?module=LayoutEditor&parent=Settings&view=CreateInventoryFields&mode=step1&sourceModule="+selectedModule+"&block="+blockId,function(modalContainer){app.showScrollBar(modalContainer.find(".well"),{height:"300px"}),thisInstance.registerStep1(modalContainer,blockId),progress.progressIndicator({mode:"hide"});});});},/**
		 ...
		 * @return true if validation is successfull
		 * @return false if validation error occurs
		 */validate:function validate(){return this.validateValue(this.getElement().val())},validateValue:function validateValue(fieldValue){var _this6=this,r=!0;return $.each(fieldValue,function(i,val){/[\<\>\"\,\#]/.test(val)&&(_this6.setError(app.vtranslate("JS_SPECIAL_CHARACTERS")+" < > \" , # "+app.vtranslate("JS_NOT_ALLOWED")),r=!1);}),r}});
