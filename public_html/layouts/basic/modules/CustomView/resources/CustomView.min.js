 ...
 *************************************************************************************/"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}function _defineProperties(target,props){for(var descriptor,i=0;i<props.length;i++)descriptor=props[i],descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor);}function _createClass(Constructor,protoProps,staticProps){return protoProps&&_defineProperties(Constructor.prototype,protoProps),staticProps&&_defineProperties(Constructor,staticProps),Object.defineProperty(Constructor,"prototype",{writable:!1}),Constructor}function _defineProperty(obj,key,value){return key in obj?Object.defineProperty(obj,key,{value:value,enumerable:!0,configurable:!0,writable:!0}):obj[key]=value,obj}var CustomView=/*#__PURE__*/function(){function CustomView(url){var _this=this;_classCallCheck(this,CustomView);var progressIndicatorElement=$.progressIndicator({position:"html",blockInfo:{enabled:!0}});app.showModalWindow(null,url,function(){_this.modalContainer=$(".js-filter-modal__container"),_this.advanceFilterInstance=new Vtiger_ConditionBuilder_Js(_this.modalContainer.find(".js-condition-builder-view .js-condition-builder"),_this.modalContainer.find("#sourceModule").val()),_this.advanceFilterInstance.registerEvents(),CustomView.registerAdvancedConditionsEvents(_this.modalContainer),_this.columnSelectElement=!1,_this.registerEvents(),progressIndicatorElement.progressIndicator({mode:"hide"});});}/** @type {Vtiger_ConditionBuilder_Js} Condition builder object */return _createClass(CustomView,[{key:"loadDateFilterValues",value:function loadDateFilterValues(){var selectedDateFilter=$("#standardDateFilter option:selected"),currentDate=selectedDateFilter.data("currentdate"),endDate=selectedDateFilter.data("enddate");$("#standardFilterCurrentDate").val(currentDate),$("#standardFilterEndDate").val(endDate);}/**
	 ...
	 */},{key:"getContentsContainer",value:function getContentsContainer(){return !1==this.modalContainer&&(this.modalContainer=$(".js-filter-modal__container")),this.modalContainer}/**
...
select2Element.validationEngine("hide");var result=form.validationEngine("validate");if(!0==result){//handled standard filters saved values.
var stdfilterlist={};return ""!=$("#standardFilterCurrentDate").val()&&""!=$("#standardFilterEndDate").val()&&"none"!=$("select.standardFilterColumn option:selected").val()&&(stdfilterlist.columnname=$("select.standardFilterColumn option:selected").val(),stdfilterlist.stdfilter=$("select#standardDateFilter option:selected").val(),stdfilterlist.startdate=$("#standardFilterCurrentDate").val(),stdfilterlist.enddate=$("#standardFilterEndDate").val(),$("#stdfilterlist").val(JSON.stringify(stdfilterlist))),$("#advfilterlist").val(JSON.stringify(_this2.advanceFilterInstance.getConditions())),form.find("#advancedConditions").val(JSON.stringify(CustomView.getAdvancedConditions(form))),$("[name=\"duplicatefields\"]").val(JSON.stringify(_this2.getDuplicateFields())),$("input[name=\"columnslist\"]",_this2.getContentsContainer()).val(JSON.stringify(_this2.getSelectedColumns())),_this2.saveAndViewFilter(),!1}app.formAlignmentAfterValidation($(e.currentTarget));});}/**
	 ...
	 */},{key:"registerEvents",value:/**
	 * Register events
	 */function registerEvents(){var _this3=this;this.registerIconEvents(),App.Fields.Tree.register(this.getContentsContainer()),App.Tools.Form.registerBlockToggle(this.getContentsContainer()),this.registerColorEvent(),this.registerDuplicatesEvents();var select2Element=App.Fields.Picklist.showSelect2ElementView(this.getColumnSelectElement());this.registerSubmitEvent(select2Element),$(".stndrdFilterDateSelect").datepicker(),$("#standardDateFilter").on("change",function(){_this3.loadDateFilterValues();}),$("#CustomView").validationEngine(app.validationEngineOptions),this.registerDisableSubmitOnEnter();}}],[{key:"registerCustomViewAdvCondEvents",value:function registerCustomViewAdvCondEvents(listViewContainer){var _this4=this;listViewContainer.on("click",".js-custom-view-adv-cond-modal",function(){var _AppConnector$request,customViewAdvCond=listViewContainer.find(".js-custom-view-adv-cond"),advancedConditions=customViewAdvCond.val();advancedConditions&&(advancedConditions=JSON.parse(advancedConditions)),AppConnector.request((_AppConnector$request={module:app.getModuleName(),view:"CustomViewAdvCondModal"},_defineProperty(_AppConnector$request,"module",app.getModuleName()),_defineProperty(_AppConnector$request,"advancedConditions",advancedConditions),_AppConnector$request)).done(function(data){data&&app.showModalWindow(data,function(modalContainer){App.Tools.Form.registerBlockToggle(modalContainer),_this4.registerAdvancedConditionsEvents(modalContainer),modalContainer.find("[name=\"saveButton\"]").on("click",function(){customViewAdvCond.val(JSON.stringify(_this4.getAdvancedConditions(modalContainer))),app.hideModalWindow(),"undefined"!=typeof app.pageController.getListViewRecords&&app.pageController.getListViewRecords();});});}).fail(function(textStatus,errorThrown){app.showNotify({title:app.vtranslate("JS_ERROR"),text:errorThrown,type:"error"});});});}/**
