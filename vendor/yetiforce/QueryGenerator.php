...
class QueryGenerator
{
	...
	public function getQueryField($fieldName)
	{
		...
		if (empty($field)) {
			...
			throw new \App\Exceptions\AppException(\App\Language::translate('LBL_NOT_FOUND_FIELD_MODEL'));
		}
		$className = '\App\QueryField\\' . ucfirst($field->getFieldDataType()) . 'Field';
		if (!class_exists($className)) {
			Log::error('Not found query field condition');
			throw new \App\Exceptions\AppException(\App\Language::translate('LBL_NOT_FOUND_QUERY_FIELD_CONDITION'));
		}
