<?php
/*+***********************************************************************************************************************************
 * The contents of this file are subject to the YetiForce Public License Version 1.1 (the "License"); you may not use this file except
 * in compliance with the License.
 * Software distributed under the License is distributed on an "AS IS" basis, WITHOUT WARRANTY OF ANY KIND, either express or implied.
 * See the License for the specific language governing rights and limitations under the License.
 * The Original Code is YetiForce.
 * The Initial Developer of the Original Code is YetiForce. Portions created by YetiForce are Copyright (C) www.yetiforce.com. 
 * All Rights Reserved.
 *************************************************************************************************************************************/
$languageStrings = array(
	'LBL_MOBILE_KEYS'	=>	'Ключи мобильных приложений',
	'LBL_MOBILE_KEYS_DESCRIPTION'	=>	'Создание и управление ключами мобильных приложений.',
	'LBL_USERNAME'	=>	'Пользователь',
	'LBL_SERVICE'	=>	'Приложение',
	'LBL_KEY'	=>	'Ключ',
	'LBL_WHO_CAN_DIAL'	=>	'Ответственный',
	'LBL_TOOLS'	=>	'Действия',
	'LBL_DELETE_KEY'	=>	'Удалить ключ',
	'LBL_ADD_KEY'	=>	'Добавить ключ',
	'LBL_SELECT_USER'	=>	'Ответственный:',
	'LBL_SELECT_SERVICE'	=>	'Выберите приложение:',
	'LBL_SUCCESS_SAVE_KEY'	=>	'Ключ успешно сохранен',
	'LBL_ERROR_SAVE_KEY'	=>	'Ошибка сохранения ключа',
	'LBL_DUPLICATE_USER_SERVICES'	=>	'Ключ для этого приложения уже существует.',
	'LBL_KEY_HAS_BEEN_REMOVED'	=>	'Ключ успешно удален',
	'LBL_SUCCESS_SAVE_CHANGES'	=>	'Изменения успешно сохранены',

 	// Mobile services
	'LBL_MOBILE_HISTORYCALL'	=>	'История звонков',
	'LBL_MOBILE_PUSHCALL'	=>	'Автоматический набор номера',	'LBL_MOBILE_TEST'	=>	'Тест',	
);
$jsLanguageStrings = array(
	'Adding a Key'	=>	'Добавить ключ',
);