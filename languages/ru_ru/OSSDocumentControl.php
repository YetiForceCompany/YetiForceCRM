<?php
/*+***********************************************************************************************************************************
 * The contents of this file are subject to the YetiForce Public License Version 1.1 (the "License"); you may not use this file except
 * in compliance with the License.
 * Software distributed under the License is distributed on an "AS IS" basis, WITHOUT WARRANTY OF ANY KIND, either express or implied.
 * See the License for the specific language governing rights and limitations under the License.
 * The Original Code is YetiForce.
 * The Initial Developer of the Original Code is YetiForce. Portions created by YetiForce are Copyright (C) www.yetiforce.com. 
 * All Rights Reserved.
 * Contributor(s): 
 * VERSION YetiForceCRM: 1.1.0 RC
 *************************************************************************************************************************************/
$languageStrings = [
	'LBL_VIEW_EDIT2'	=>	'Добавление условий',
	'LBL_NEW_TPL'	=>	'Добавить документ',
	'OSSDocumentControl'	=>	'Менеджер документов',
	'LBL_MODULE_NAME'	=>	'Название модуля',
	'LBL_NO_PROJECT_TPL_ADDED'	=>	'Документов нет',
	'TPL'	=>	'Документы',
	'OSSProjectTemplates_manual'	=>	'Шаблоны проектов',
        
	'LBL_CREATING_TPL'	=>	'Создание документа',
	'LBL_FILL_BASE_DATA'	=>	'Информация',
	'ADD_CONDITIONS'	=>	'Добавить условие',
	'LBL_CONDITION_ALL'	=>	'Все условия',
	'LBL_CONDITION_ALL_DSC'	=>	'Все условия в этом разделе должны быть выполнены',
	'LBL_CONDITION_OPTION'	=>	'Любое условие',
	'LBL_CONDITION_OPTION_DSC'	=>	'Одно из условий в этом разделе должно быть выполнено',
	'FOLDER_LIST'	=>	'Выберите папку',
	'DOC_NAME'	=>	'Название документа',
	'DOC_REQUIRED'	=>	'Требовать документ',
	'DOC_ORDER'	=>	'Сортировать',
	'NEXT'	=>	'Далее',
	'CANCEL'	=>	'Отменить',
	'BACK'	=>	'Назад',
    
	'LIST_DOCUMENTS'	=>	'Список документов',
	'DOC_REQ'	=>	'Обязательный документ',
	'FOLDER'	=>	'Папка',
	'ADD_DOCUMENT'	=>	'Добавить документ',
	'ATTACHED'	=>	'Вложение',
    
	'NOT_ALLOW_TO_SAVE'	=>	'Вы не можете сохранить запись, так как необходимые документы не прикреплены',
	'LIST_REQ_DOC'	=>	'Пожалуйста, приложите следующие документы',
];
$jsLanguageStrings = [
	'LBL_VIEW_EDIT2'	=>	'Добавление условий',
];