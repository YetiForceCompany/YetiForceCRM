      ...
      <q-page class="row">
        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 fixed-center">
          <div class="card-shadow q-pa-xl column">
            ...
            <q-banner v-if="$store.state.App.Core.Users.message" :class="[msgClass, 'q-mt-lg', 'text-white']">
              <p>{{ $store.state.App.Core.Users.message }}</p>
            </q-banner>
          </div>
        </div>
      </q-page>
 ...
 */
const moduleName = 'App.Core.Users.Layouts.Login'
export default {
  name: moduleName,
  data() {
    return {
  ...
  computed: {
    ...
    msgClass: function() {
      return {
        'bg-positive': this.$store.App.Core.Users.messageType === 'success',
        'bg-negative': this.$store.App.Core.Users.messageType === 'error',
        'bg-warning': this.$store.App.Core.Users.messageType === ''
      }
    }
  },
  methods: {
