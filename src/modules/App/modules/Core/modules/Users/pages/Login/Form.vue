...
<template>
  <div>
    <form
      ...
      :autocomplete="$store.state.App.Core.Users.loginPageRememberCredentials ? 'on' : 'off'"
    >
      <q-input
        type="text"
        ref="user"
      ...
      <q-select
        v-if="$store.state.App.Core.Users.languageSelection"
        v-model="language"
        :options="$store.state.App.Core.Language.langs"
      ...
      <q-select
        v-if="$store.state.App.Core.Users.layoutSelection"
        v-model="layout"
      ...
      <router-link
        v-if="$store.state.App.Core.Users.forgotPassword"
...
export default {
  ...
  data() {
    return {
      user: '',
      password: '',
      language: this.$store.state.App.Core.Language.defaultLanguage,
      layout: ''
    }
  },
  methods: {
