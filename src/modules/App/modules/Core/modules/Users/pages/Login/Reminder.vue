...
<template v-if="$store.state.App.Core.Users.forgotPassword && showReminderForm">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if (this.$refs.reminderUsers.hasError || this.$refs.reminderEmail.hasError) {
        ...
        this.$store.dispatch(actions.App.Core.Users.remind, {
  ...
  mounted() {
    if (!this.$store.state.App.Core.Users.forgotPassword || this.$store.state.App.Core.Users.isBlockedIp) {
