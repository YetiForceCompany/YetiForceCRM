...
<template v-if="$store.state.App.Users.forgotPassword && showReminderForm">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if (this.$refs.reminderUsers.hasError || this.$refs.reminderEmail.hasError) {
        ...
        this.$store.dispatch(actions.App.Users.remind, {
  ...
  mounted() {
    if (!this.$store.state.App.Users.forgotPassword || this.$store.state.App.Users.isBlockedIp) {
