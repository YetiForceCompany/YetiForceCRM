...
<template>
  <div>
    <form
      ...
      :autocomplete="$store.state.App.Users.loginPageRememberCredentials ? 'on' : 'off'"
    >
      <q-input
        type="text"
        ref="user"
      ...
      <q-select
        v-if="$store.state.App.Users.languageSelection"
        v-model="language"
        :options="$store.state.App.Language.langs"
      ...
      <q-select
        v-if="$store.state.App.Users.layoutSelection"
        v-model="layout"
      ...
      <router-link
        v-if="$store.state.App.Users.forgotPassword"
...
export default {
  ...
  data() {
    return {
      user: '',
      password: '',
      language: this.$store.state.App.Language.defaultLanguage,
      layout: ''
    }
  },
  methods: {
    onSubmit() {
      ...
      if (this.$refs.user.hasError || this.$refs.password.hasError) {
        ...
        this.$store.dispatch(actions.App.Users.login, {
          username: this.user,
