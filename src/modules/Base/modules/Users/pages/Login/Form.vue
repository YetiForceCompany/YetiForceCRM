...
<template>
  <div>
    <form
      ...
      :autocomplete="$store.state.Base.Users.loginPageRememberCredentials ? 'on' : 'off'"
    >
      <q-input
        type="text"
        ref="user"
      ...
      <q-select
        v-if="$store.state.Base.Users.languageSelection"
        v-model="language"
        :options="$store.state.Base.Language.langs"
      ...
      <q-select
        v-if="$store.state.Base.Users.layoutSelection"
        v-model="layout"
      ...
      />
      <router-link
        v-if="$store.state.Base.Users.forgotPassword"
...
export default {
  ...
  data() {
    return {
      user: '',
      password: '',
      language: this.$store.state.Base.Language.defaultLanguage,
      layout: ''
    }
  },
  methods: {
    onSubmit() {
      ...
      if (this.$refs.user.hasError || this.$refs.password.hasError) {
        ...
        this.$store.dispatch(actions.Base.Users.login, {
          username: this.user,
