      ...
      <q-page class="row">
        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-3 fixed-center">
          <div class="card-shadow q-pa-xl column">
            ...
            <q-banner
              v-if="$store.state.Base.Users.message"
              :class="[msgClass, 'q-mt-lg', 'text-white']"
            >
              <p>{{ $store.state.Base.Users.message }}</p>
            </q-banner>
          </div>
        </div>
      </q-page>
...
export default {
  ...
  computed: {
    ...
    msgClass: function() {
      return {
        'bg-positive': this.$store.Base.Users.messageType === 'success',
        'bg-negative': this.$store.Base.Users.messageType === 'error',
        'bg-warning': this.$store.Base.Users.messageType === ''
      }
    }
  },
  methods: {
