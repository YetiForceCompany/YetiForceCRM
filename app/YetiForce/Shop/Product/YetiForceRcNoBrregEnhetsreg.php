...
/**
 * YetiForce shop YetiForceRcNoBrregEnhetsreg class.
 */
class YetiForceRcNoBrregEnhetsreg extends \App\YetiForce\Shop\AbstractBaseProduct
{
	/** {@inheritdoc} */
	...
	{
		$message = $status = true;
		if (\App\YetiForce\Register::getProducts('YetiForceRcNoBrregEnhetsreg')) {
			[$status, $message] = \App\YetiForce\Shop::checkWithMessage('YetiForceRcNoBrregEnhetsreg');
		} else {
			if (
				(new \App\Db\Query())->from('vtiger_links')->where(['linktype' => 'EDIT_VIEW_RECORD_COLLECTOR', 'linklabel' => 'NoBrregEnhetsregisteret'])->exists()
				 || (new \App\Db\Query())->from('com_vtiger_workflowtasks')->where(['like', 'task', '%\NoBrregEnhetsregisteret";%', false])->exists()
			) {
