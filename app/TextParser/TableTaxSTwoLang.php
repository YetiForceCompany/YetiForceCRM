			...
			if (in_array('tax', $columns) && in_array('taxmode', $columns)) {
							...
							<thead>
								<tr>
									<th colspan="2" class="tBorder noBottomBorder tHeader">
										<strong>' . \App\Language::translate('LBL_TAX_SUMMARY', $this->textParser->moduleName) . '/ ' . \App\Language::translate('LBL_TAX_SUMMARY', $this->textParser->moduleName, 'en_us') . '</strong>
									</th>
								</tr>
							</thead>
							<tbody>';
				foreach ($taxes as $key => &$tax) {
					...
					$html .= '<tr>
								...
								<td class="textAlignRight tBorder">' . \CurrencyField::convertToUserFormat($tax, null, true) . ' ' . $currencySymbolRate['symbolRate']['symbol'] . '</td>
							</tr>';
				}
				$html .= '<tr>
							...
							<td class="textAlignRight tBorder">' . \CurrencyField::convertToUserFormat($tax_AMOUNT, null, true) . ' ' . $currencySymbolRate['symbolRate']['symbol'] . '</td>
						 </tr>
						</tbody>
					</table>
				</div>';
