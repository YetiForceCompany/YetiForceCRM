...
class HTTP_Session_Container_MDB2 extends HTTP_Session_Container
{
	...
	{
		$query = sprintf("SELECT data FROM %s WHERE id = %s && expiry >= %d", $this->options['table'], $this->db->quote(md5($id), 'text'), time());
	...
	public function write($id, $data)
	{
		if ((false !== $this->crc) &&
			...
			$query = sprintf("UPDATE %s SET expiry = %d WHERE id = %s", $this->options['table'], time() + ini_get('session.gc_maxlifetime'), $this->db->quote(md5($id), 'text'));
		} else {
			// Check if table row already exists
			$query = sprintf("SELECT COUNT(id) FROM %s WHERE id = %s", $this->options['table'], $this->db->quote(md5($id), 'text'));
			$result = $this->db->queryOne($query);
			if (MDB2::isError($result)) {
				$this->db->raiseError($result->code, PEAR_ERROR_DIE);
				return false;
			...
			if (0 == intval($result)) {
				...
				$query = sprintf("INSERT INTO %s (id, expiry, data) VALUES (%s, %d, %s)", $this->options['table'], $this->db->quote(md5($id), 'text'), time() + ini_get('session.gc_maxlifetime'), $this->db->quote($data, 'text'));
			} else {
				// Update existing row
				$query = sprintf("UPDATE %s SET expiry = %d, data = %s WHERE id = %s", $this->options['table'], time() + ini_get('session.gc_maxlifetime'), $this->db->quote($data, 'text'), $this->db->quote(md5($id), 'text'));
			}
		}
	...
	{
		$query = sprintf("DELETE FROM %s WHERE id = %s", $this->options['table'], $this->db->quote(md5($id), 'text'));
		$result = $this->db->query($query);
		if (MDB2::isError($result)) {
			$this->db->raiseError($result->code, PEAR_ERROR_DIE);
			return false;
	...
	public function replicate($targetTable, $id = null)
	{
		...
		// Check if table row already exists
		$query = sprintf("SELECT COUNT(id) FROM %s WHERE id = %s", $targetTable, $this->db->quote(md5($id), 'text'));
		$result = $this->db->queryOne($query);
		if (MDB2::isError($result)) {
			$this->db->raiseError($result->code, PEAR_ERROR_DIE);
			return false;
		...
		if (0 == intval($result)) {
			$query = sprintf("INSERT INTO %s SELECT * FROM %s WHERE id = %s", $targetTable, $this->options['table'], $this->db->quote(md5($id), 'text'));
		} else {
			// Update existing row
			$query = sprintf("UPDATE %s dst, %s src SET dst.expiry = src.expiry, dst.data = src.data WHERE dst.id = src.id && src.id = %s", $targetTable, $this->options['table'], $this->db->quote(md5($id), 'text'));
		}
