...
{strip}
	...
	{if count($FIELDS) neq 0}
		...
		<table class="table blockContainer suppliesItemTable">
			<thead>
				<tr>
					<th style="min-width: 50px">&nbsp;&nbsp;</th>
					{foreach item=FIELD from=$FIELDS[1]}
						<th colspan="{$FIELD->get('colspan')}" class="col{$FIELD->getName()} textAlignCenter">{vtranslate($FIELD->get('label'), $SUPMODULE)}</th>
					{/foreach}
				</tr>
			</thead>
			<tbody>
