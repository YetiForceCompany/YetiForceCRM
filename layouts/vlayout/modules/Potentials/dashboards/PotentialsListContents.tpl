...
{if count($DATA) gt 0 }
	<div style="padding:5px;">
		<div class="row-fluid">
			<div class="span4"><strong>{vtranslate('Potential Name', $RELATED_MODULE)}</strong></div>
			<div class="span4"><strong>{vtranslate('Sales Stage', $RELATED_MODULE)}</strong></div>
			<div class="span4"><strong>{vtranslate('Related To', $RELATED_MODULE)}</strong></div>
		</div>
		{foreach item=ROW from=$DATA}
			<div class="row-fluid">
				<div class="span4"><a class="moduleColor_{$RELATED_MODULE}" href="index.php?module={$RELATED_MODULE}&view=Detail&record={$ROW.potentialid}">{$ROW.potentialname}</a></div>
