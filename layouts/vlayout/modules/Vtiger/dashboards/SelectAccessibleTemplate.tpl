...
{/if}
<select class="widgetFilter width90" id="owner" name="owner" title="{vtranslate('LBL_OWNER')}">
	...
	{if array_key_exists( $CURRENTUSER->getId(), $ACCESSIBLE_USERS ) && in_array('all', $ACCESS_OPTIONS.available)}
		<option value="all" {if $ACCESS_OPTIONS.default eq 'all'} title="{vtranslate('LBL_ALL')}" selected {/if}>{vtranslate('LBL_ALL')}</option>
	{/if}
		...
		<optgroup label="{vtranslate('LBL_USERS')}">
			{foreach key=OWNER_ID item=OWNER_NAME from=$ACCESSIBLE_USERS}
				{if $OWNER_ID neq {$CURRENTUSER->getId()}}
					<option title="{$OWNER_NAME}" value="{$OWNER_ID}">{$OWNER_NAME}</option>
				{/if}
			{/foreach}
		</optgroup>
	{/if}
