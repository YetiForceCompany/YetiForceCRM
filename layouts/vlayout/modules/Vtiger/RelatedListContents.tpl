									...
									<span class="actionImages">
										{if $RELATED_MODULE_NAME eq 'Calendar'}
											...
											{if $IS_EDITABLE && in_array($RELATED_RECORD->get('activitystatus'),$CURRENT_ACTIVITY_LABELS)}
												<a class="markAsHeld showModal" data-url="{$RELATED_RECORD->getActivityStateModalUrl()}"><span title="{vtranslate('LBL_MARK_AS_HELD', $MODULE)}" class="glyphicon glyphicon-ok alignMiddle"></span></a>&nbsp;
												{/if}
												{if $DETAILVIEWPERMITTED eq 'yes'}
