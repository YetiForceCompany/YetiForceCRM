...
{strip}
	<li class="hovernav menuLabel {$CLASS} {if !$HASPOPUP}hasParentMenu{/if}" data-id="{$MENU.id}" role="menuitem" tabindex="{$TABINDEX}" 
		...
		<a class="{if $PARENT_MODULE == $MENU.id}active{/if}" {if $CHILDS|@count neq 0}data-toggle="collapse" aria-expanded="false" role="button"{/if} href="#menu{$MENU.id}" aria-controls="menu{$MENU.id}">
