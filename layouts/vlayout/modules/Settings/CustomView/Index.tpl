				...
				<tbody>
					{foreach from=$MODULE_MODEL->getCustomViews() item=item key=key}
						<tr data-cvid="{$key}" data-mod="{$item['entitytype']}">
							...
							<td>
								<button class="btn btn-primary marginLeftZero update" data-cvid="{$key}" data-editurl="{$MODULE_MODEL->GetUrlToEdit($item['entitytype'],$key)}">{vtranslate('Edit',$QUALIFIED_MODULE)}</button>
								&nbsp;&nbsp;
								{if $item['cn'] neq 1}
