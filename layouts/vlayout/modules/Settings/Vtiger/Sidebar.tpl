	...
	{foreach item=MENU from=$SETTINGS_MENUS}
		<div class="quickWidget">
			<div class="accordion-heading accordion-toggle quickWidgetHeader" data-parent="#settingsQuickWidgetContainer" data-target="#Settings_sideBar_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($MENU->getLabel())}"
				...
				<h6 class="title paddingLeft10px widgetTextOverflowEllipsis" title="{vtranslate($MENU->getLabel(), $QUALIFIED_MODULE)}">{vtranslate($MENU->getLabel(), $QUALIFIED_MODULE)}</h6>
			...
			{foreach item=MENUITEM from=$MENU->getMenuItems()}
				<div class="{if $MENUITEM->getId() eq $SELECTED_FIELDID} selectedMenuItem selectedListItem{/if}" style="padding:7px;border-top:0px;">
					<div class="row menuItem"  data-actionurl="{$MENUITEM->getPinUnpinActionUrl()}">
						<a href="{$MENUITEM->getUrl()}" data-id="{$MENUITEM->getId()}" class="col-md-9 menuItemLabel" data-menu-item="true" >{vtranslate($MENUITEM->get('name'), Settings_Vtiger_MenuItem_Model::getModuleNameFromUrl($MENUITEM->getUrl()))}</a>
