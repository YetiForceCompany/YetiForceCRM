	...
	<div id="condition_all">
			{foreach from=$REQUIRED_CONDITIONS key=cnd_key item=cnd_item name=field_select}
				<div class="row conditionRow marginBottom10px" id="cnd_num_{$smarty.foreach.field_select.index}">
					<span class="col-md-4">
						<select data-num="{$smarty.foreach.field_select.index}" class="select2 chzn-select chzn-done row field-select" data-placeholder="{vtranslate('LBL_SELECT_FIELD',$QUALIFIED_MODULE)}">
					...
					<span class="col-md-3">
						<select data-num="{$smarty.foreach.field_select.index}" class="select2 row" name="comparator">
