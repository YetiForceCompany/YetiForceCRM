	...
	<div class="reportContents">
		<form class="form-horizontal recordEditView" id="report_step1" method="post" action="index.php">
			...
			<div class="well contentsBackground">
				...
				<div class="row marginBottom5px paddingTop10">
					<div class="col-md-3"><span class="redColor">*</span>{\App\Language::translate('PRIMARY_MODULE',$MODULE)}</div>
					<div class="col-md-7">
						<select class="col-md-6 chzn-select form-control" title="{\App\Language::translate('PRIMARY_MODULE',$MODULE)}" id="primary_module" name="primary_module">
							<optgroup>
				...
				<div class="row marginBottom5px paddingTop10">
					...
					<div class="col-md-7">
						...
						<select class="col-md-6 select2-container form-control" id="secondary_module" multiple name="secondary_modules[]" title="{\App\Language::translate('LBL_SELECT_RELATED_MODULES',$MODULE)}" data-placeholder="{\App\Language::translate('LBL_SELECT_RELATED_MODULES',$MODULE)}">
				...
				<div id="scheduleBox" class='well contentsBackground {if $SCHEDULEDREPORTS->get('scheduleid') eq ''} d-none {/if}'>
					<div class="row" style="padding:5px 0px;">
						...
						<div class="col-md-4">
							<select class="chzn-select form-control" title="{\App\Language::translate('LBL_FILE_TYPE',$MODULE)}" name="scheduleFileType">
								<option value="CSV" {if $SCHEDULEDREPORTS->get('scheduleFileType') eq 'CSV'}selected{/if}>{\App\Language::translate('LBL_CSV', $MODULE)}</option>
								<option value="EXCEL" {if $SCHEDULEDREPORTS->get('scheduleFileType') eq 'EXCEL'}selected{/if}>{\App\Language::translate('LBL_EXCEL', $MODULE)}</option>
							</select>
						</div>
					</div>
					<div class='row' style="padding:5px 0px;">
						<div class='col-md-3 marginBottom5px' style='position:relative;top:5px;'>{\App\Language::translate('LBL_RUN_REPORT', $MODULE)}</div>
						<div class='col-md-4'>
							{assign var=scheduleid value=$SCHEDULEDREPORTS->get('scheduleid')}
							<select class='chzn-select form-control' id='schtypeid' title="{\App\Language::translate('LBL_SCHEDULE_REPORTS',$MODULE)}" name='schtypeid'>
								<option value="1" {if $scheduleid eq 1}selected{/if}>{\App\Language::translate('LBL_DAILY', $MODULE)}</option>
								<option value="2" {if $scheduleid eq 2}selected{/if}>{\App\Language::translate('LBL_WEEKLY', $MODULE)}</option>
								<option value="5" {if $scheduleid eq 5}selected{/if}>{\App\Language::translate('LBL_SPECIFIC_DATE', $QUALIFIED_MODULE)}</option>
								<option value="3" {if $scheduleid eq 3}selected{/if}>{\App\Language::translate('LBL_MONTHLY_BY_DATE', $MODULE)}</option>
								<option value="4" {if $scheduleid eq 4}selected{/if}>{\App\Language::translate('LBL_YEARLY', $MODULE)}</option>
							</select>
						</div>
					</div>
					...
					<div class='row {if $scheduleid neq 2} d-none {/if}' id='scheduledWeekDay' style='padding:5px 0px;'>
						...
						<div class='col-md-4'>
							...
							<select style='width:230px;' multiple class='chosen form-control' title="{\App\Language::translate('LBL_ON_THESE_DAYS', $MODULE)}" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name='schdayoftheweek' id='schdayoftheweek'>
					...
					<div class='row {if $scheduleid neq 3} d-none {/if}' id='scheduleMonthByDates' style="padding:5px 0px;">
						...
						<div class='col-md-4'>
							...
							<select style="width: 281px !important;" multiple class="chosen-select col-md-6 form-control" title="{\App\Language::translate('LBL_ON_THESE_DAYS', $MODULE)}" data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name='schdayofthemonth' id='schdayofthemonth' >
					...
					<div class='row {if $scheduleid neq 4} d-none {/if}' id='scheduleAnually' style='padding:5px 0px;'>
						...
						<div class='col-md-2'>
							<div style='padding-bottom:5px;'>{\App\Language::translate('LBL_SELECTED_DATES', $MODULE)}</div>
							<div>
					...
					<div class='row' id='scheduledTime' style='padding:5px 0px 10px 0px;'>
						<div class='col-md-3 marginBottom5px' style='position:relative;top:5px;'>
							<span class="redColor">*</span>{\App\Language::translate('LBL_AT_TIME', $MODULE)}
						</div>
					...
					<div class='row' id='recipientsList' style='padding:5px 0px 10px 0px;'>
						<div class='col-md-3 marginBottom5px' style='position:relative;top:5px;'>
							<span class="redColor">*</span>{\App\Language::translate('LBL_SELECT_RECIEPIENTS', $MODULE)}
						</div>
						<div class='col-md-4'>
							...
							<select multiple data-placeholder="{\App\Language::translate('LBL_SELECT_OPTION')}" title="{\App\Language::translate('LBL_SELECT_RECIEPIENTS', $MODULE)}" class="chosen-select col-md-6 form-control" id='recipients' name='recipients' data-validation-engine="validate[required,funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" style="width: 281px !important;">
								...
								<optgroup label="{\App\Language::translate('Roles', 'Roles')}">
									{foreach key=ROLE_ID item=ROLE_OBJ from=$ROLES}
										{assign var=ROLEID value="ROLE::{$ROLE_ID}"}
										<option value="{$ROLEID}" {if is_array($recipients) && in_array($ROLEID, $recipients)} selected {/if} data-picklistvalue= '{$ROLE_OBJ->get('rolename')}'>{\App\Language::translate($ROLE_OBJ->get('rolename'))}</option>
									{/foreach}
								</optgroup>
							</select>
						</div>
					...
					<div class='row' id='specificemailsids' style='padding:5px 0px 10px 0px;'>
						<div class='col-md-3 marginBottom5px' style='position:relative;top:5px;'>
							{\App\Language::translate('LBL_SPECIFIC_EMAIL_ADDRESS', $MODULE)}
						</div>
						<div class='col-md-4'>
							{assign var=specificemailids value=\App\Json::decode($SCHEDULEDREPORTS->get('specificemails'))}
							<input id="specificemails" style="width: 281px !important;" class="col-md-6 form-control" title="{\App\Language::translate('LBL_SPECIFIC_EMAIL_ADDRESS', $MODULE)}" type="text" value="{$specificemailids}" name="specificemails" data-validation-engine="validate[funcCall[Vtiger_MultiEmails_Validator_Js.invokeValidation]]"></input>
						</div>
					</div>
					<div class="row">
						<div class='col-md-3 marginBottom5px'>
							<span class=''>{\App\Language::translate('LBL_NEXT_TRIGGER_TIME', $MODULE)}</span>
						</div>
						<div class='col-md-4'>
