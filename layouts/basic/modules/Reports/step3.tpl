...
{strip}
	<form class="form-horizontal recordEditView" id="report_step3" method="post" action="index.php">
		...
		<input type="hidden" name="selected_fields" id="seleted_fields" value='{\includes\utils\Json::encode($REPORT_MODEL->get('selected_fields'))}' />
		<input type="hidden" name="selected_sort_fields" id="selected_sort_fields" value={\includes\utils\Json::encode($REPORT_MODEL->get('selected_sort_fields'))} />
		<input type="hidden" name="selected_calculation_fields" id="calculation_fields" value={\includes\utils\Json::encode($REPORT_MODEL->get('calculation_fields'))} />
		...
		<input type="hidden" name="schdayoftheweek" value={\includes\utils\Json::encode($REPORT_MODEL->get('schdayoftheweek'))}>
		<input type="hidden" name="schdayofthemonth" value={\includes\utils\Json::encode($REPORT_MODEL->get('schdayofthemonth'))}>
		<input type="hidden" name="schannualdates" value={\includes\utils\Json::encode($REPORT_MODEL->get('schannualdates'))}>
		<input type="hidden" name="recipients" value={\includes\utils\Json::encode($REPORT_MODEL->get('recipients'))}>
        <input type="hidden" name="specificemails" value={\includes\utils\Json::encode($REPORT_MODEL->get('specificemails'))}>
