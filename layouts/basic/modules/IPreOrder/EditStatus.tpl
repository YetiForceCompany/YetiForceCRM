...
{strip}
	...
	{assign var=OPEN_RECORD value=Users_Privileges_Model::isPermitted($MODULE_NAME, 'OpenRecord', $ID)}
...
<div class="modal-footer">
	<div class="pull-left">
		<div class="btn-toolbar">
			{if $OPEN_RECORD || $RECORD->isViewable()}
				<div class="btn-group">
					<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						{vtranslate('LBL_CHANGE_STATUS',$MODULE_NAME)} <span class="caret"></span>
					</button>
					<ul class="dropdown-menu">
						{foreach  item=ITEM from=Vtiger_Util_Helper::getPickListValues('ipreorder_status')}
							{if in_array($ITEM, $RESTRICTS_ITEM) || $ITEM eq $RECORD->get('ipreorder_status')} {continue} {/if}
			...
			{foreach from=$RESTRICTS_ITEM item=ITEM}
				{if $RECORD->get('ipreorder_status') neq $ITEM}
					<div class="btn-group">
