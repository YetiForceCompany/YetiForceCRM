...
jQuery.Class("SSingleOrders_Modal_Js", {}, {
	registerChangeStatus: function () {
		...
		jQuery('#sSingleOrdersModal .changeStatus').on('click', function (e) {
		...
		AppConnector.request(params).then(
				function (data) {
					if (data.success) {
						...
						if (viewName === 'Detail') {
							if (app.getModuleName() == 'SSingleOrders') {
	...
	registerHelpInfo: function () {
		var form = jQuery('#sSingleOrdersModal');
	...
	registerEvents: function () {
		...
		jQuery('#sSingleOrdersModal .convert').each(function () {
...
jQuery(document).ready(function (e) {
	var instance = new SSingleOrders_Modal_Js();
