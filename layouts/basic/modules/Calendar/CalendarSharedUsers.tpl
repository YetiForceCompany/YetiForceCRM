...
<div name='calendarViewTypes'>
	...
	<div id="calendarview-feeds" style="margin-left:10px;">
		...
		<div class=" modal fade addViewsToCalendar">
					...
					<div class="modal-body">
						<form class="form-horizontal">
							...
							<div class="form-group addCalendarViewsList">
								<label class="control-label">{\App\Language::translate('LBL_SELECT_USER_CALENDAR', $MODULE)}</label>
								<div class="controls">
							...
							<div class="form-group editCalendarViewsList">
								...
								<div class="controls">
									<select class="select2 form-control" name="editingUsersList" style="min-width: 250px;">
										<option value="{$CURRENT_USER_ID}" data-user-color="{$SHAREDUSERS_INFO[$CURRENT_USER_ID]['color']}">{\App\Language::translate('LBL_MINE',$MODULE)}</option>
										{foreach key=USER_ID item=USER_NAME from=$SHAREDUSERS}
											{if $SHAREDUSERS_INFO[$USER_ID]['visible'] != '0'}
												<option value="{$USER_ID}">{$USER_NAME}</option>
											{/if}
		...
		<label class="addedCalendars" style="text-shadow: none">
			...
			&nbsp;<span class="label" style="text-shadow: none; background-color: {$SHAREDUSERS_INFO[$CURRENT_USER_ID]['color']};">{\App\Language::translate('LBL_MINE',$MODULE)}</span>
			&nbsp;<i class="glyphicon glyphicon-pencil editCalendarColor cursorPointer actionImage" title="{\App\Language::translate('LBL_EDIT_COLOR',$MODULE)}"></i>
		</label>
