...
{strip}
	...
	<hr>
	<label>{\App\Language::translate('LBL_SELECT_OPTION_TO_SEARCH', 'DataAccess')}:</label>
	<div class="form-group text-center">
		<div class="radio-inline">
			<label>
				...
				{\App\Language::translate('LBL_SET_CUSTOM_CONDITIONS', 'DataAccess')}
			</label>
		</div>
		<div class="radio-inline">
			<label>
				...
				{\App\Language::translate('LBL_CURRENT_EVENTS', 'DataAccess')}
			</label>
		</div>
		<div class="radio-inline">
			<label>
				...
				{\App\Language::translate('LBL_PAST_EVENTS', 'DataAccess')}
			</label>
		</div>
	</div>
	<div class="form-group marginLeftZero marginRightZero statusContainer{if $STATUS_TYPE} hide{/if}">
		<label for="status" class="">{\App\Language::translate('LBL_SELECT_STATUS', 'DataAccess')}:</label>
		<select multiple id="status" name="status" class="form-control select2">
			{foreach item=ITEM from=Calendar_Module_Model::getComponentActivityStateLabel()}
				<option value="{$ITEM}" {if in_array($ITEM, $STATUS)}selected {/if}>{\App\Language::translate($ITEM, 'Calendar')}</option>
			{/foreach}
		</select>
	</div>		
	<hr><br />
	<div class="row">
		<div class="col-md-4 padding-bottom1per">
			<label class="padding-bottom1per"><strong>{\App\Language::translate('LBL_MAXIMUM_NUMBER_EVENTS_PER_DAY', 'DataAccess')}:</strong></label>
		...
		<div class="col-md-12 padding-bottom1per checkbox">
			<label for="lockSave" class="">
				...
				{\App\Language::translate('LBL_LOCKS_SAVE', 'DataAccess')}
			</label >
		</div>
