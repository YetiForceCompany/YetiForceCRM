	...
	<div class="rowRelatedRecord" data-id="{$RELATED['id']}" data-module="{$RELATED['module']}">
		...
		<div class="pull-right rowActions">
			{if Users_Privileges_Model::isPermitted('Calendar','CreateView')}
				<button class="addRelatedRecord" data-module="Calendar" title="{vtranslate('LBL_ADD_CALENDAR',$MODULE_NAME)}">
					<span class="userIcon-Calendar" aria-hidden="true"></span>
				</button>
			{/if}
			{if Users_Privileges_Model::isPermitted('ModComments','CreateView')}
				<button class="addRelatedRecord" data-module="ModComments" title="{vtranslate('LBL_ADD_MODCOMMENTS',$MODULE_NAME)}">
					<span class="glyphicon glyphicon-comment" aria-hidden="true"></span>
				</button>
			{/if}
			{if in_array($RELATED['module'], ['HelpDesk','Project']) &&  Users_Privileges_Model::isPermitted('HelpDesk','CreateView')}
				<button class="addRelatedRecord" data-module="HelpDesk" title="{vtranslate('LBL_ADD_HELPDESK',$MODULE_NAME)}">
			...
			{/if}
			<button class="removeRecord " title="{vtranslate('LBL_REMOVE_RELATION',$MODULE_NAME)} {$RELATED['label']}">
