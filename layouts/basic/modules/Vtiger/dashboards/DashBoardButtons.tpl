...
{strip}
	...
	<div class="dashboardHeading d-flex ml-auto mb-2 mt-sm-2 pr-sm-1 u-remove-dropdown-icon-down-lg u-w-xs-down-100">
		...
		{if $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(),'CreateDashboardChartFilter')}
			<button class="btn btn-outline-dark js-show-modal ml-1 js-popover-tooltip" title="{\App\Language::translate('LBL_ADD_CHART_FILTER','_DashBoard')}" data-content="{\App\Purifier::encodeHtml(\App\Language::translate('LBL_ADD_CHART_FILTER_DESC','_DashBoard'))}"
				...
				<span class="d-none d-xxl-inline ml-2">{\App\Language::translate('LBL_ADD_CHART_FILTER','_DashBoard')}</span>
			</button>
		{/if}
		<a class="btn btn-outline-dark ml-1 js-popover-tooltip js-post-action" href="index.php?module={$MODULE_MODEL->getName()}&action=Widget&mode=clear" title="{\App\Purifier::encodeHtml(\App\Language::translate('LBL_CLEAR_DEVICE_CONF','_DashBoard'))}" data-content="{\App\Purifier::encodeHtml(\App\Language::translate('LBL_CLEAR_DEVICE_CONF_DESC','_DashBoard'))}" data-js="popover">
