...
{strip}
	...
	<div class="dashboardHeading d-flex ml-auto mb-2 mt-sm-2 pr-sm-1 u-remove-dropdown-icon-down-lg u-w-xs-down-100">
		...
		{/if}
		{if isset($SPECIAL_WIDGETS['Mini List']) && $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(),'CreateDashboardFilter')}
			<button class="btn btn-outline-dark js-add-filter ml-1 js-popover-tooltip" title="{\App\Purifier::encodeHtml(\App\Language::translate('LBL_ADD_FILTER','Dashboard'))}" data-content="{\App\Language::translate('LBL_ADD_FILTER_DESC','Dashboard')}"
		...
		{/if}
		{if isset($SPECIAL_WIDGETS['ChartFilter']) && $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(),'CreateDashboardChartFilter')}
			<button class="btn btn-outline-dark js-show-modal ml-1 js-popover-tooltip" title="{\App\Language::translate('LBL_ADD_CHART_FILTER','Dashboard')}" data-content="{\App\Language::translate('LBL_ADD_CHART_FILTER_DESC','Dashboard')}"
		...
		{/if}
		<a class="btn btn-outline-dark ml-1 js-popover-tooltip js-post-action" href="index.php?module={$MODULE_MODEL->getName()}&action=Widget&mode=clear" title="{\App\Language::translate('LBL_CLEAR_DEVICE_CONF','Dashboard')}" data-content="{\App\Purifier::encodeHtml(\App\Language::translate('LBL_CLEAR_DEVICE_CONF_DESC','Dashboard'))}" data-js="popover">
