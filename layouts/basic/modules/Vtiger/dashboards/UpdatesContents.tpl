		...
		{if $UPDATES neq false}
			{foreach item=UPDATE_ROW from=$UPDATES}
				...
				{if $MODEL_NAME == 'ModTracker_Record_Model'}
					...
					{assign var=MOD_NAME value=$UPDATE_ROW->getParent()->getModule()->getName()}
					...
					{if $PROCEED}
						<div class="d-flex">
							...
							<div class="w-100 ml-1">
								...
								{else if ($UPDATE_ROW->isRelationLink() || $UPDATE_ROW->isRelationUnLink())}
									{assign var=RELATION value=$UPDATE_ROW->getRelationInstance()}
									...
									<div>
										<strong>{$USER->getName()}&nbsp;</strong>
										{\App\Language::translate($UPDATE_ROW->getStatusLabel(), 'ModTracker')}&nbsp;
							...
							{else}
								<div>
									<strong>{$USER->getName()}&nbsp;</strong>{\App\Language::translate($UPDATE_ROW->getStatusLabel(), 'ModTracker')}
									<a class="u-cursor-pointer" {if stripos($DETAILVIEW_URL, 'javascript:')===0} onclick='{$DETAILVIEW_URL|substr:strlen("javascript:")}' {else} onclick='window.location.href = "{$DETAILVIEW_URL}"' {/if}>
										&nbsp;{$PARENT->getName()}
									</a>
								</div>
				...
				{else if $MODEL_NAME == 'ModComments_Record_Model'}
					...
					<div class="d-flex">
						...
						<div class="w-100 ml-1">
							{assign var=COMMENT_TIME value=$UPDATE_ROW->getCommentedTime()}
							...
							<div>
								<strong>{$UPDATE_ROW->getCommentedByModel()->getName()}</strong> {\App\Language::translate('LBL_COMMENTED')} {\App\Language::translate('LBL_ON')} <a class="u-text-ellipsis" href="{$UPDATE_ROW->getParentRecordModel()->getDetailViewUrl()}">{$UPDATE_ROW->getParentRecordModel()->getName()}</a>
							</div>
							<div class='font-x-small'><span>"{nl2br($UPDATE_ROW->getDisplayValue('commentcontent'))}"</span></div>
						</div>
					</div>
					{/if}
						{/foreach}
