...
<div>
	...
	<div class="row mailRow" data-mailId="{$key}">
		<div class="col-md-12" style="font-size:x-small;">
			...
			<h5 style="margin-left:2%;">{$item->get('subject')} {if count($item->get('attachments')) > 0}<img alt="{\App\Language::translate('LBL_ATTACHMENT')}" class="float-right" src="{\App\Layout::getLayoutFile('modules/OSSMailView/attachment.png')}" />{/if}<h5>
		</div>
