...
{strip}
	...
	<input class="tpl-Edit-Field-Base form-control {if $FIELD_MODEL->isNameField()}nameField{/if}" name="{$FIELD_MODEL->getFieldName()}" id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE)}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}{if $FIELD_MODEL->get('maximumlength') or $FIELD_MODEL->get('maxlengthtext')}maxSize[{if $FIELD_MODEL->get('maximumlength') < $FIELD_MODEL->get('maxlengthtext')}{$FIELD_MODEL->get('maximumlength')}{else}{$FIELD_MODEL->get('maxlengthtext')}{/if}],{/if}funcCall[Vtiger_InputMask_Validator_Js.invokeValidation]]" value="{$FIELD_VALUE}"
