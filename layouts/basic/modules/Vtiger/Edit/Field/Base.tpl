...
{strip}
	...
	<input name="{$FIELD_MODEL->getFieldName()}" value="{$FIELD_VALUE}" class="form-control {if $FIELD_MODEL->isNameField()}nameField{/if}" {' '}
		...
		data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true}required,{/if}{if $FIELD_MODEL->get('maximumlength')}maxSize[{$FIELD_MODEL->getMaxValue()}],{/if} funcCall[Vtiger_InputMask_Validator_Js.invokeValidation]]" {if $FIELD_MODEL->isNameField()}autocomplete="username" {/if}{' '}
