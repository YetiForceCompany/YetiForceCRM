...
{strip}
	...
	{if $FIELD_MODEL->getUIType() eq '71'}
		<div class="input-group">
			{if $SYMBOL_PLACEMENT neq '1.0$'}
				<span class="input-group-append"><span class="input-group-text js-currency"> {$USER_MODEL->get('currency_symbol')}</span></span>
			{/if}
	...
	{else if ($FIELD_MODEL->getUIType() eq '72') && ($FIELD_MODEL->getName() eq 'unit_price')}
		<div class="input-group">
			{if $SYMBOL_PLACEMENT neq '1.0$'}
				<span class="input-group-append row"><span class="input-group-text js-currency">{$BASE_CURRENCY_SYMBOL}</span></span>
			{/if}
