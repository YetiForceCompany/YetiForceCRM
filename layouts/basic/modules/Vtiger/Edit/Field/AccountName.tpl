...
{strip}
	...
	<div class="row">
		...
		<div class="col pl-0 {if $LEGAL_FORM !== 'PLL_NATURAL_PERSON'}d-none{/if} js-first-name" data-js="container">
			<input value="{\App\Purifier::encodeHtml($EXPLODED_FIELD_VALUE['first'])}" class="form-control" type="text" title="{\App\Language::translate('First Name', $MODULE_NAME)}" {' '}
		...
		<div class="col px-0 {if $LEGAL_FORM !== 'PLL_NATURAL_PERSON'}d-none{/if} js-last-name" data-js="container">
			<input value="{\App\Purifier::encodeHtml($EXPLODED_FIELD_VALUE['last'])}" class="form-control" type="text" title="{\App\Language::translate('Last Name', $MODULE_NAME)}" {' '}
