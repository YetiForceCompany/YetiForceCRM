...
{strip}
	...
	{if !empty($FIELD_MODEL->get('fieldvalue'))}
		...
		{assign var=FIELD_VALUE_RAW value=$FIELD_MODEL->get('fieldvalue')}
	{else}
		{assign var=FIELD_VALUE_RAW value=''}
		{assign var=FIELD_VALUE value=[['e' => '', 'o' => 0]]}
	{/if}
	{assign var=ITEMS_COUNT value=count($FIELD_VALUE)}
	<div class="js-multi-email">
		<input type="hidden" name="{$FIELD_MODEL->getFieldName()}" value="{\App\Purifier::encodeHtml($FIELD_VALUE_RAW)}" class="js-multi-email-value" />
		<div class="row">
