...
{strip}
	...
	<div class="border rounded {if $WIDTHTYPE eq 'wide'}px-2 py-2{else}px-1 py-1{/if} d-flex align-items-center clearfix c-multi-image js-multi-image">
		...
		<button type="button" class="align-top d-inline mr-1 btn {if $WIDTHTYPE eq 'narrow'}btn-xs{else}btn-sm{/if} btn-primary js-multi-image__file-btn" tabindex="{$TABINDEX}" data-js="click">
			<i class="fa fa-plus"></i>&nbsp;<span>{\App\Language::translate('BTN_ADD_FILE', $MODULE_NAME)}</span>
		</button>
		<div class="d-inline js-multi-image__result d-flex align-items-center c-multi-image__btn-{$WIDTHTYPE}" data-js="container" data-name="{$FIELD_MODEL->getFieldName()}"></div>
