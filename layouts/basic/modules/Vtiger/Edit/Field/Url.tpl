...
{strip}
	...
	<div class="tpl-Edit-Field-Url">
		<input id="{$MODULE}_editView_fieldName_{$FIELD_MODEL->getName()}" type="text"
			   ...
			   class="validate[custom[url]] form-control" name="{$FIELD_MODEL->getFieldName()}"
			   ...
			   {if !empty($SPECIAL_VALIDATOR)}data-validator={\App\Json::encode($SPECIAL_VALIDATOR)}{/if} {if $FIELD_MODEL->isEditableReadOnly()}readonly="readonly"{/if} autocomplete="off"/>
	</div>
{/strip}
