				...
				<optgroup label="{\App\Language::translate($BLOCK_NAME)}">
					{foreach key=OWNER_ID item=OWNER_NAME from=$OWNERS}
						<option id="{\App\Layout::getUniqueId('shared-owner')}" value="{$OWNER_ID}" data-picklistvalue="{$OWNER_NAME}"
							...
							data-state="{$ACTIVE}" data-template="<span class='c-favorite-user-template'>{$OWNER_NAME}<span class='js-select-option-actions o-filter-actions noWrap float-right'><span data-js='click|class:icons' class='mx-1 js-select-option-event{if $ACTIVE == 'active'} fas fa-star{else} far fa-star{/if}'></span></span></span>"{/if}>
							{$OWNER_NAME}
						</option>
					{/foreach}
				</optgroup>
