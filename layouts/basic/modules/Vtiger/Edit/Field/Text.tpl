...
{strip}
	...
	<div class="tpl-Edit-Field-Text">
		{if $smarty.post.view neq 'QuickCreateAjax' && $smarty.post.view neq 'MassActionAjax'}
		...
		{if $FIELD_MODEL->getUIType() eq '19' || $FIELD_MODEL->getUIType() eq '20' || $FIELD_MODEL->getUIType() eq '300' }
			...
			<textarea name="{$FIELD_MODEL->getFieldName()}"
					  ...
					  class="col-md-11 form-control {if $FIELD_MODEL->getUIType() eq '300'}js-ckeditor{/if} {if $FIELD_MODEL->isNameField()}nameField{/if} {if !empty($PARAMS['class'])}{$PARAMS['class']}{/if}"
