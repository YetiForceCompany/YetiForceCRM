			...
			{if AppConfig::performance('SEARCH_OWNERS_BY_AJAX')} 
				{foreach from=$SEARCH_VALUES item=OWNER_ID}
					{if !empty($OWNER_ID)}
						...
						<option value="{$OWNER_ID}" data-picklistvalue="{\App\Purifier::encodeHtml($OWNER_NAME)}" selected="selected" data-userId="{$OWNER_ID}">
			...
			{else}
				<optgroup label="{\App\Language::translate('LBL_USERS')}">
					{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEUSER_LIST}
						<option value="{$OWNER_ID}" data-picklistvalue= '{\App\Purifier::encodeHtml($OWNER_NAME)}' {if in_array($OWNER_ID,$SEARCH_VALUES)} selected {/if}
				...
				{if count($ALL_ACTIVEGROUP_LIST) gt 0}
					<optgroup label="{\App\Language::translate('LBL_GROUPS')}">
						{foreach key=OWNER_ID item=OWNER_NAME from=$ALL_ACTIVEGROUP_LIST}
							<option value="{$OWNER_ID}" data-picklistvalue="{\App\Purifier::encodeHtml($OWNER_NAME)}" {if in_array(trim($OWNER_ID),$SEARCH_VALUES)} selected {/if}>
								{\App\Purifier::encodeHtml($OWNER_NAME)}
							</option>
						{/foreach}
					</optgroup>
