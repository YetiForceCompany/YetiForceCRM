{strip}
	...
	********************************************************************************/
	...
	<html lang="{$HTMLLANG}">
		...
		<body class="{if AppConfig::module('Users', 'IS_VISIBLE_USER_INFO_FOOTER')}user-info--active{/if}" data-language="{$LANGUAGE}" data-skinpath="{$SKIN_PATH}" data-layoutpath="{$LAYOUT_PATH}" {$USER_MODEL->getBodyLocks()}>
