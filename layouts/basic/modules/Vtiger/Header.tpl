...
<body class="{if App\Config::module('Users', 'IS_VISIBLE_USER_INFO_FOOTER')}user-info--active{/if}{if App\Config::performance('LIMITED_INFO_IN_FOOTER')} limited-footer--active{/if}" data-language="{$LANGUAGE}" data-skinpath="{$SKIN_PATH}" data-layoutpath="{$LAYOUT_PATH}" {$USER_MODEL->getBodyLocks()}>
