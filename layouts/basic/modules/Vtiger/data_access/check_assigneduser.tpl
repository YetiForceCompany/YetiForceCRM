...
<div class="row">
	<div class="col-md-12 padding-bottom1per">
		<h5 class="padding-bottom1per"><strong>{\App\Language::translate('LBL_SELECT_USER_OR_GROUP', 'DataAccess')}:</strong></h5>
		<select multiple name="field" class="marginLeftZero col-md-6 chzn-select">
			<optgroup label="{\App\Language::translate('LBL_USERS')}">
				<option value="currentUser" {foreach item=USER from=$FIELD}{if $USER eq 'currentUser' } selected {/if}{/foreach}>{\App\Language::translate('LBL_CURRENT_USER', 'DataAccess')}</option>
	...
	<div class="marginLeftZero col-md-12 padding-bottom1per">
		<h5 class="padding-bottom1per"><strong>{\App\Language::translate('Message', 'DataAccess')}:</strong></h5>
