{strip}
	...
	<div class="row">
		<div class="col-md-12 padding-bottom1per">
			<h5 class="padding-bottom1per"><strong>{\App\Language::translate('Select status', 'DataAccess')}:</strong></h5>
			<select multiple name="status" class="marginLeftZero col-md-6 select2">
				{foreach item=item key=key from=$CONFIG['status']}
					<option value="{$item}" {if $item == $status} selected {/if}{if $item != null && is_array($status) && in_array( $item, $status)} selected {/if} >{\App\Language::translate($item, 'Calendar')}</option>
				{/foreach}
			</select>
		</div>
		<div class="marginLeftZero col-md-12 padding-bottom1per">
			<h5 class="padding-bottom1per"><strong>{\App\Language::translate('Subject tasks', 'DataAccess')}:</strong></h5>
			<input type="text" name="name" class="marginLeftZero col-md-6 " value="{$name}">
		</div>
		<div class="marginLeftZero col-md-12 padding-bottom1per">
			<h5 class="padding-bottom1per"><strong>{\App\Language::translate('Message if the task does not exist', 'DataAccess')}:</strong></h5>
