...
{strip}
	...
	<nav class="Pagination" aria-label="Page navigation">
		<ul class="js-pagination-list pagination m-0" data-total-count="{$LISTVIEW_COUNT}">
					...
					{if $PAGE_INDEX eq $PAGE_COUNT || $PAGE_INDEX eq $PAGIN_TO}
						{if $PAGE_COUNT > 5}
							<li class="page-item {if $PAGE_COUNT eq 1} disabled {/if}">
								...
								<ul class="js-page-jump-drop-down dropdown-menu listViewBasicAction"
									...
									<li class="dropdown-item">
										<div>
											...
											<div class="js-total-page col-md-2 recentComments pushUpandDown2per textAlignCenter"
												 id="totalPageCount">{$PAGE_COUNT}</div>
										</div>
									</li>
								</ul>
							</li>
			...
			{if $PAGE_INDEX <= $PAGE_COUNT}
				<li class="js-set-page pageNumber{if $PAGE_NUMBER eq $PAGE_COUNT} active disabled{/if}" data-id="{$PAGE_COUNT}">
					<a class="page-link" href="#">{$PAGE_COUNT}</a>
				</li>
			{/if}
			<li class="js-next-page page-item {if (!$PAGING_MODEL->isNextPageExists())}disabled{/if}"
				id="{$VIEWNAME}ViewNextPageButton"
				data-js="click">
