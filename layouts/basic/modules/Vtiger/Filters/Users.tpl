	...
	{if !empty($FILTER_DATA)}
		...
		<div class="js-filter__container">
			<div class="form-row d-flex align-items-center m-0">
				<input type="checkbox" class="alignMiddle js-select-all mr-2"
					{if $SELECT_ALL_BY_DEFAULT eq '1' } checked {/if}
					title="{\App\Language::translate('LBL_SELECT_ALL', $MODULE_NAME)}"
					autocomplete="off">
				<h6 class="boxFilterTitle mt-2">{\App\Language::translate('LBL_SELECT_CALENDAR',$MODULE_NAME)}</h6>
			</div>
								...
								<div class="mr-2">
									<input value="{$OWNER_ID}" type="checkbox" id="ownerId{$OWNER_ID}"
										...
										{if (empty($HISTORY_USERS) && $USER_MODEL->getId() eq $OWNER_ID)
																		|| (!empty($HISTORY_USERS) && in_array($OWNER_ID, $HISTORY_USERS))
