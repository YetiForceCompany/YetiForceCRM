	...
	<div class="detailViewContainer">
		<div class="row detailViewTitle">
										...
										<div class="btn-toolbar">
											<span class="btn-group ">
												{foreach item=LINK from=$DETAILVIEW_LINKS['DETAILVIEWBASIC']}	
													{include file='ButtonLink.tpl'|@\App\Layout::getTemplatePath:$MODULE BUTTON_VIEW='detailViewBasic'}
												{/foreach}
											</span>
											{if $DETAILVIEW_LINKS['DETAILVIEW']|@count gt 0}
												<span class="btn-group">
													{foreach item=LINK from=$DETAILVIEW_LINKS['DETAILVIEW']}
														{include file='ButtonLink.tpl'|@\App\Layout::getTemplatePath:$MODULE BUTTON_VIEW='detailView'}
													{/foreach}
												</span>
											{/if}
										</div>
			...
			{if !empty($DETAILVIEW_LINKS['DETAIL_VIEW_HEADER_WIDGET'])}
				...
				{/foreach}
			{/if}
			{include file="DetailViewHeaderTitle.tpl"|\App\Layout::getTemplatePath:$MODULE}
		</div>
		<div class="detailViewInfo row">
			{include file="RelatedListButtons.tpl"|\App\Layout::getTemplatePath:$MODULE}
