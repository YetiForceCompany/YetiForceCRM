...
{strip}
	<div class="btn-group {if $BUTTON_VIEW|strrpos:'listView' !== false && $USER_MODEL->get('rowheight') eq 'narrow'}btn-group-sm{/if}{if isset($CLASS)} {$CLASS}{/if}">
		...
		>
			...
			{/if}
			{if $LABEL neq '' && $MODE == 'responsive'}
				<span class="d-{$BREAKPOINT}-none">{\App\Language::translate($LABEL, $BTN_MODULE)}</span>
			{/if}
			{if $LABEL neq '' && $LINK->get('showLabel') == 1}
