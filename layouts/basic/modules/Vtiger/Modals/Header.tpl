...
{strip}
	...
	<div class="modal js-modal-data {if $LOCK_EXIT}static{if !empty($BLUR_BACKGROUND)} modal-backdrop-filter{/if}" data-keyboard="false{/if}" tabindex="-1" data-js="data" role="dialog" {foreach from=$MODAL_VIEW->modalData key=KEY item=VALUE} data-{$KEY}="{$VALUE}" {/foreach}>
		<div class="modal-dialog {$MODAL_VIEW->modalSize}{if empty($MODAL_DRAGGABLE)} js-no-drag{/if}" role="document">
