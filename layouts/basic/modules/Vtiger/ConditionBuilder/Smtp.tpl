	...
	<div class="tpl-Base-ConditionBuilder-Smtp">
		...
		<select class="js-picklist-field select2 form-control js-condition-builder-value"
			...
			{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
				<option value="{\App\Purifier::encodeHtml($PICKLIST_NAME)}" title="{\App\Purifier::encodeHtml($PICKLIST_VALUE['name'])}" {if in_array($PICKLIST_NAME, $FIELD_VALUES)} selected {/if}>{\App\Purifier::encodeHtml($PICKLIST_VALUE['name'])}</option>
			{/foreach}
		</select>
	</div>
{/strip}
