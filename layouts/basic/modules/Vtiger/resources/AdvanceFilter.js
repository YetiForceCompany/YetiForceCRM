...
jQuery.Class("Vtiger_AdvanceFilter_Js", {
	...
	loadConditions: function (fieldSelect) {
		...
		for (var key in conditionList) {
			...
			if (jQuery.inArray(fieldInfo.type, ['owner', 'picklist', 'modules', 'tree', 'inventoryLimit', 'categoryMultipicklist']) != -1 && jQuery.inArray(conditionList[key], ['s', 'ew', 'c', 'k']) != -1) {
				continue;
			}
	...
	loadFieldSpecificUi: function (fieldSelect) {
		...
		if ($.inArray(fieldModel.getType(), ['multipicklist', 'sharedOwner', 'multiReferenceValue']) > -1) {
			fieldName = fieldName + "[]";
		} else if ((fieldModel.getType() == 'picklist' || fieldModel.getType() == 'owner' || fieldModel.getType() == 'modules' || fieldModel.getType() == 'inventoryLimit')
	...
	getValues: function () {
		...
		conditionGroups.each(function (index, domElement) {
			...
			conditions.each(function (i, conditionDomElement) {
				...
				} else if ($.inArray(fieldType, ['picklist', 'multipicklist', 'modules', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit']) > -1) {
					for (var key in fieldList) {
						var field = fieldList[key];
						...
						} else if (field == 'value' && valueSelectElement.is('select') && ($.inArray(fieldType, ['picklist', 'multipicklist', 'modules', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit']) > -1)) {
...
Vtiger_Field_Js('AdvanceFilter_Field_Js', {}, {
	...
	getModuleName: function () {
		...
		if ($.inArray(type, ['picklist', 'multipicklist', 'owner', 'modules', 'date', 'datetime', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit']) > -1) {
