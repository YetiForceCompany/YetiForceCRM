...
jQuery.Class("Vtiger_AdvanceFilter_Js", {
	...
	loadConditions: function (fieldSelect) {
		...
		for (var key in conditionList) {
			...
			if (jQuery.inArray(fieldInfo.type, ['owner', 'picklist', 'modules', 'tree', 'inventoryLimit', 'categoryMultipicklist', 'languages', 'currencyList', 'fileLocationType']) != -1 && jQuery.inArray(conditionList[key], ['s', 'ew', 'c', 'k']) != -1) {
				continue;
			}
	...
	getValues: function () {
		...
		conditionGroups.each(function (index, domElement) {
			...
			conditions.each(function (i, conditionDomElement) {
				...
				} else if ($.inArray(fieldType, ['picklist', 'multipicklist', 'modules', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit', 'posList', 'languages', 'currencyList', 'taxes', 'fileLocationType']) > -1) {
					for (var key in fieldList) {
						var field = fieldList[key];
						...
						} else if (field == 'value' && valueSelectElement.is('select') && ($.inArray(fieldType, ['picklist', 'multipicklist', 'modules', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit', 'posList', 'languages', 'currencyList', 'taxes', 'fileLocationType']) > -1)) {
...
Vtiger_Field_Js('AdvanceFilter_Field_Js', {}, {
	...
	getModuleName: function () {
		...
		if ($.inArray(type, ['picklist', 'multipicklist', 'owner', 'modules', 'date', 'datetime', 'sharedOwner', 'multiReferenceValue', 'inventoryLimit', 'posList', 'languages', 'currencyList', 'taxes', 'fileLocationType']) > -1) {
