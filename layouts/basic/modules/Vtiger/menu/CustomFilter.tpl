{strip}
	...
	<li class="tpl-menu-CustomFilter c-menu__item js-menu__item nav-item menuCustomFilter .modCT_{$MENU.mod}"
		...
		<a class="nav-link {if $HASCHILDS=='true'} collapsed{/if}{if $ICON} hasIcon{/if}{if isset($MENU['hotkey'])} hotKey{/if}{if $HASCHILDS == 'true'} js-submenu-toggler is-submenu-toggler{/if}"
		...
		rel="noreferrer noopener" {if $HASCHILDS == 'true'} aria-haspopup="true" aria-expanded="false"
			...
			{if !empty($MENU['showentries'])}<span class="js-count" data-url="{$MENU['dataurl']}" data-js="container"></span>{/if}
			{if $HASCHILDS == 'true'}
