...
{*<!-- {[The file is published on the basis of YetiForce Public License 2.0 that can be found in the following directory: licenses/License.html or yetiforce.com]} -->*}
	...
	<li class="menuShortcut {if !$HASCHILDS}hasParentMenu{/if}" data-id="{$MENU.id}" role="menuitem" tabindex="{$TABINDEX}" {if $HASCHILDS}aria-haspopup="{$HASCHILDS}"{/if}>
		<a class="{if isset($MENU['hotkey'])}hotKey{/if} {if $MENU['active']}active{/if}{if $ICON} hasIcon{/if}" {if isset($MENU['hotkey'])} data-hotkeys="{$MENU['hotkey']}"{/if} href="{$MENU['dataurl']}" {if $MENU.newwindow eq 1 OR $MENU['name'] eq 'LBL_YETIFORCE_SHOP'}target="_blank" {/if} rel="noreferrer">
			{$ICON}
			<span class="menuName">
