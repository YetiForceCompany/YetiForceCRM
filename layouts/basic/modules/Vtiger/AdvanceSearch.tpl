		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="actions modal-footer">
					<a class="cancelLink pull-right btn btn-warning" type="reset" id="advanceSearchCancel" data-dismiss="modal"><span class="glyphicon glyphicon-remove iconTextMargin"></span>{\App\Language::translate('LBL_CANCEL', $MODULE)}</a>
					<button class="btn btn-info pull-right" id="advanceSearchButton" {if $SOURCE_MODULE eq 'Home'} disabled="" {/if}  type="submit"><span class="glyphicon glyphicon-search iconTextMargin"></span><strong>{\App\Language::translate('LBL_SEARCH', $MODULE)}</strong></button>
					{if $SAVE_FILTER_PERMITTED}
						<button class="btn hide btn-success pull-right" {if $SOURCE_MODULE eq 'Home'} disabled="" {/if} id="advanceSave">
							<span class="glyphicon glyphicon-ok iconTextMargin"></span><strong>{\App\Language::translate('LBL_SAVE_FILTER', $MODULE)}</strong>
						</button>
						{if \App\Privilege::isPermitted($SOURCE_MODULE, 'CreateCustomFilter')}
							<button class="btn btn-success pull-right" {if $SOURCE_MODULE eq 'Home'} disabled="" {/if} id="advanceIntiateSave">
								<span class="glyphicon glyphicon-ok iconTextMargin"></span><strong>{\App\Language::translate('LBL_SAVE_AS_FILTER', $MODULE)}</strong>
							</button>
						{/if}
