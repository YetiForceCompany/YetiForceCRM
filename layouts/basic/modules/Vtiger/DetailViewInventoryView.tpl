...
{strip}
	...
	{if count($FIELDS) neq 0}
					...
					<tr>
						...
						{foreach item=FIELD from=$FIELDS[0]}
							<th>
								...
								{include file=$FIELD_TPL_NAME|@\App\Layout::getTemplatePath:$MODULE_NAME ITEM_VALUE=$INVENTORY_ROWS[0][$FIELD->get('columnname')]}
							</th>
						{/foreach}
					</tr>
				</thead>
					...
					<tr>
						{foreach item=FIELD from=$FIELDS[1]}
							<td {if in_array($FIELD->getName(), $FIELDS_TEXT_ALIGN_RIGHT)}class="textAlignRight"{/if}>
								{assign var="FIELD_TPL_NAME" value="inventoryfields/"|cat:$FIELD->getTemplateName('DetailView',$MODULE_NAME)}
								{include file=$FIELD_TPL_NAME|@\App\Layout::getTemplatePath:$MODULE_NAME ITEM_VALUE=$INVENTORY_ROW[$FIELD->get('columnname')]}
							</td>
						{/foreach}
					</tr>
				{/foreach}
		...
		{include file='DetailViewInventorySummary.tpl'|@\App\Layout::getTemplatePath:$MODULE_NAME}
	{/if}
{/strip}
