...
{strip}
	...
	<div class="tpl-List-Field-UserRole picklistSearchField">
		<select class="select2noactive select2 listSearchContributor form-control" title="{\App\Language::translate($FIELD_MODEL->getFieldLabel(), $MODULE)}" name="{$FIELD_MODEL->getFieldName()}" multiple{/strip} {strip}
			...
			{if AppConfig::performance('SEARCH_ROLES_BY_AJAX')}
				{foreach from=$SEARCH_VALUES item=PICKLIST_VALUE}
					<option value="{$PICKLIST_VALUE}" selected>{\App\Purifier::encodeHtml(\App\PrivilegeUtil::getRoleName($PICKLIST_VALUE))}</option>
				{/foreach}
			{else}
