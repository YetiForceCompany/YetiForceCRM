				...
				<form class="form-horizontal" id="changeOwner" name="changeOwner" method="post" action="index.php">
					<div class="modal-body tabbable">
						<div class="form-group">
							<div class="col-sm-4 control-label">{\App\Language::translate('LBL_ASSIGNED_TO', $MODULE)}</div>
						...
						<div class="form-group">
							<div class="col-sm-4 control-label">{\App\Language::translate('LBL_SELECT_RELATED_MODULES',$MODULE)}</div>
							<div class="col-sm-7 controls"> 
										...
										<optgroup label="{\App\Language::translate('LBL_RELATIONSHIPS_BASED_ON_FIELDS')}">
											{foreach item=RELATED from=$REL_BY_FIELDS}
												{if !in_array($RELATED, $SKIP_MODULES)}
													<option value="{$RELATED.name}::0::{$RELATED.field}">{\App\Language::translate($RELATED.name, $RELATED.name)} - {\App\Language::translate($RELATED.field)} [M:1]</option>
												{/if}
											{/foreach}
										</optgroup>
									{/if}
									{if $REL_BY_RELATEDLIST}
										<optgroup label="{\App\Language::translate('LBL_RELATIONSHIPS_BASED_ON_MODULES')}">
											{foreach item=RELATED from=$REL_BY_RELATEDLIST}
												{if !in_array($RELATED, $SKIP_MODULES)}
													<option value="{$RELATED.name}::{$RELATED.type}">{\App\Language::translate($RELATED.name, $RELATED.name)} [{if $RELATED.type == 1}1:M{else}M:M{/if}]</option>
												{/if}
											{/foreach}
										</optgroup>
									{/if}
