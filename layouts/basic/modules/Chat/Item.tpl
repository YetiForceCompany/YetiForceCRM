...
{strip}
	...
	<div class="tpl-Chat-Item js-chat-item o-chat__item chatItem {if $USER_ID == $ROW['userid']} active flex-row {else} flex-row-reverse {/if} my-1 d-flex align-items-center"
		...
		<div class="o-chat__messages col-9 p-3 ownerCBg_{$ROW['userid']}  {if $USER_ID == $ROW['userid']} active float-right  {else} float-left {/if}">{\App\Purifier::decodeHtml($ROW['messages'])}</div>
	</div>
{/strip}
