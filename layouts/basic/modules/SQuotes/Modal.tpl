...
{strip}
	...
	{assign var=OPENQUOTES value=Users_Privileges_Model::isPermitted($MODULE_NAME, 'OpenQuotes', $ID)}
	{assign var=CLOSEQUOTES value=Users_Privileges_Model::isPermitted($MODULE_NAME, 'CloseQuotes', $ID)}
...
<div class="modal-footer">
	<div class="pull-left">
		<div class="btn-toolbar">
			{if $OPENQUOTES || ($EDITVIEW_PERMITTED && !$LOCKEDIT)}
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						{foreach  item=ITEM from=Vtiger_Util_Helper::getPickListValues('squotes_status')}
							{if in_array($ITEM, $RESTRICTS_ITEM) || $ITEM eq $RECORD->get('squotes_status')} {continue} {/if}
			...
			{foreach from=$RESTRICTS_ITEM item=ITEM}
				{if $CLOSEQUOTES && $RECORD->get('squotes_status') neq $ITEM}
					<div class="btn-group">
