	...
	{if $SELECTED_PICKLIST_FIELDMODEL}
		...
		<div class="tab-content layoutContent py-3 themeTableColor overflowVisible">
			<div class="tab-pane fade show active" role="tabpanel" id="allValuesLayout">
				<div class="row">
					<div class="col-12 btn-group flex-wrap flex-md-nowrap" role="group">
						{if $SELECTED_PICKLIST_FIELDMODEL->isEditable()}
							{if $SELECTED_PICKLIST_FIELDMODEL->isRoleBased()}
								<button class="btn btn-primary" id="assignValue">
							...
							<button class="btn btn-info" id="importItem">{\App\Language::translate('LBL_IMPORT_VALUE',$QUALIFIED_MODULE)}</button>
							<button class="btn btn-warning" id="renameItem">{\App\Language::translate('LBL_EDIT',$QUALIFIED_MODULE)}</button>
							{if $SELECTED_PICKLIST_FIELDMODEL->isRoleBased()}
								<button class="btn btn-secondary" id="processStatusItem">
						...
						{/if}
						<button class="btn btn-success" disabled="" id="saveSequence">{\App\Language::translate('LBL_SAVE_ORDER',$QUALIFIED_MODULE)}</button>
					</div>
					<div class="col-12">
						<ul class="my-1 my-md-3">
							<li>{\App\Language::translate('LBL_DRAG_ITEMS_TO_RESPOSITION',$QUALIFIED_MODULE)}</li>
							<li>{\App\Language::translate('LBL_SELECT_AN_ITEM_TO_RENAME_OR_DELETE',$QUALIFIED_MODULE)}</li>
							<li>{\App\Language::translate('LBL_TO_DELETE_MULTIPLE_HOLD_CONTROL_KEY',$QUALIFIED_MODULE)}</li>
						</ul>
					</div>
					<div class="col-12">
						<div class="table-responsive">
							<table id="pickListValuesTable" class="table table-bordered">
								...
								<tbody>
									<input type="hidden" value="{\App\Layout::getImagePath('drag.png')}" id="dragImagePath"/>
				...
				<div id="importViewContents" class="d-none">
					{include file=\App\Layout::getTemplatePath('ImportView.tpl', $QUALIFIED_MODULE)}
				</div>
			</div>
			{if $SELECTED_PICKLIST_FIELDMODEL->isRoleBased()}
