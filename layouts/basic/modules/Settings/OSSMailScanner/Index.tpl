...
{if ( $CHECKCRON[1]['frequency'] * 2) > $CHECKCRON[0]['frequency']}
	<div class="alert alert-block alert-warning">
		<button type="button" class="close" data-dismiss="alert">×</button>
		<h4 class="alert-heading">{\App\Language::translate($MODULE_NAME, $MODULE_NAME)}
			- {\App\Language::translate('Alert_active_crontime', $MODULE_NAME)}</h4>
...
<ul id="tabs" class="nav nav-tabs nav-justified my-2 mr-0" data-tabs="tabs">
	<li class="nav-item"><a class="nav-link active" href="#tab_accounts"
							data-toggle="tab">{\App\Language::translate('E-mail Accounts', $MODULE_NAME)} </a></li>
	<li class="nav-item"><a class="nav-link" href="#tab_actions"
							data-toggle="tab">{\App\Language::translate('Actions', $MODULE_NAME)}</a></li>
	<li class="nav-item"><a class="nav-link" href="#tab_email_search"
							data-toggle="tab">{\App\Language::translate('General Configuration', $MODULE_NAME)}</a>
	</li>
	<li class="nav-item"><a class="nav-link" href="#tab_record_numbering"
							data-toggle="tab">{\App\Language::translate('Record Numbering', $MODULE_NAME)}</a></li>
	<li class="nav-item"><a class="nav-link" href="#exceptions"
							data-toggle="tab">{\App\Language::translate('LBL_EXCEPTIONS', $MODULE_NAME)}</a></li>
</ul>
	...
	<div class='editViewContainer tab-pane active' id="tab_accounts">
		...
		{if $ERRORNOMODULE}
			<div class="alert alert-block alert-warning">
				...
				<h4 class="alert-heading">{\App\Language::translate('OSSMail', 'OSSMail')}
					- {\App\Language::translate('Alert_no_module_title', $MODULE_NAME)}</h4>
		...
		{if $ACCOUNTS_LIST eq false}
			<div class="alert alert-block alert-warning">
				<button type="button" class="close" data-dismiss="alert">×</button>
				<h4 class="alert-heading">{\App\Language::translate('OSSMail', 'OSSMail')}
					- {\App\Language::translate('Alert_no_accounts_title', $MODULE_NAME)}</h4>
					...
					<tr id="row_account_{$row['user_id']}" style="{cycle values="'',background-color: #f9f9f9"}">
							...
							<select id="user_list_{$row['user_id']}" data-user="{$row['user_id']}"
								...
								<optgroup label="{\App\Language::translate('User list', $MODULE_NAME)}">
									{if $row['crm_user_id'] eq '0'}
										<option value="0"
												id="user_list_none">{\App\Language::translate('None', $MODULE_NAME)}</option>
									{/if}
						...
						<td class='scanerMailActionsButtons'>
							<div class="btn-toolbar">
								<div class="btn-group">
									<button title="{\App\Language::translate('LBL_SHOW_ACCOUNT_DETAILS', $MODULE_NAME)}"
											type="button" data-user-id="{$row['user_id']}"
											class="btn btn-light expand-hide">
										<span class="fas fa-chevron-down"></span>
									</button>
									<button title="{\App\Language::translate('LBL_EDIT_FOLDER_ACCOUNT', $MODULE_NAME)}"
											type="button" data-user="{$row['user_id']}"
											class="btn btn-light editFolders">
										<span class="fas fa-folder-open"></span>
									</button>
									<button title="{\App\Language::translate('LBL_DELETE_ACCOUNT', $MODULE_NAME)}"
											type="button" data-user-id="{$row['user_id']}"
							...
							<span class="js-empty-folders-alert badge badge-danger{if !empty($FOLDERS)} d-none{/if}">{\App\Language::translate('ERR_NO_CONFIGURATION_FOLDERS', $MODULE_NAME)}</span>
						</td>
					</tr>
							...
							<div>
								<table class="table">
									...
									<tr>
										...
										<th colspan="2"
											style="color: black; background-color: #d3d3d3;">{\App\Language::translate('identities_del', $MODULE_NAME)}</th>
									</tr>
									</thead>
										...
										<tr style="{cycle values="'',background-color: #f9f9f9"}">
											...
											<td colspan="2" style="text-align: center;">
												<button data-id="{$item['identity_id']}" type="button"
														class="btn btn-danger identities_del">{\App\Language::translate('identities_del', $MODULE_NAME)}</button>
											</td>
										</tr>
									{/foreach}
								</table>
	...
	<div class='editViewContainer tab-pane marginTop20' id="tab_email_search">
		<div class="card mb-2">
			...
			<div class="card-body">
				...
				<h4>{\App\Language::translate('Alert_info_tab_email_search', $MODULE_NAME)}</h4></div>
		...
		<div class="card">
			...
			<div class="card-body">
				<div class="alert alert-info">
				<h4>{\App\Language::translate('LBL_CONFTAB_CHANGE_TICKET_STATUS', $MODULE_NAME)}</h4></div>
				<form class="form-horizontal">
					<div class="form-group col-sm-12">
						<div class="radio">
							<label>
								...
								<strong>{\App\Language::translate('LBL_NO_ACTION', $MODULE_NAME)}</strong>
							</label>
						</div>
						<div class="radio">
							<label>
						...
						<div class="radio">
							<label>
								<input type="radio" name="conftabChangeTicketStatus" class="conftabChangeTicketStatus"
											...
											data-active="1"{/if}>
								<strong>{\App\Language::translate('LBL_CREATE_TICKET', $MODULE_NAME)}</strong>
							</label>
						</div>
					</div>
				</form>
