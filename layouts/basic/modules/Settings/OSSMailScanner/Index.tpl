...
<div id="my-tab-content" class="tab-content marginTop20">
								...
								<select id="user_list_{$row['user_id']}" data-user="{$row['user_id']}" name="user_list_{$row['user_id']}" class="form-control select2">
									<optgroup label="{\App\Language::translate('User list', 'OSSMailScanner')}">
										{if $row['crm_user_id'] eq '0'}
											<option value="0" id="user_list_none">{\App\Language::translate('None', 'OSSMailScanner')}</option>
										{/if}
									...
									<table class="table">
										<thead>
											<tr>
												...
												<th colspan="2" style="color: black; background-color: #d3d3d3;">{\App\Language::translate('identities_del', 'OSSMailScanner')}</th>
											</tr>
										</thead>
										{foreach item=item from=$IDENTITYLIST[$row['user_id']]}
											<tr style="{cycle values="'',background-color: #f9f9f9"}">
												<td>{$item['name']}</td>
												<td>{$item['email']}</td>
												<td colspan="2" style="text-align: center;"><button data-id="{$item['identity_id']}" type="button" class="btn btn-danger identities_del">{\App\Language::translate('identities_del', 'OSSMailScanner')}</button></td>
											</tr>
										{/foreach}
									</table>
								</div>
    ...
    <div class='editViewContainer tab-pane marginTop20' id="tab_email_search">
		...
		<hr/>
        <div class="alert alert-info"><h4>{\App\Language::translate('Alert_info_tab_email_search', 'OSSMailScanner')}</h4></div>
			...
			<select multiple id="email_search" name="email_search" class="select2 form-control">
				{foreach item=item key=key from=$EMAILSEARCH}
					{if $last_value neq $item['name']}
						...
						{/if}
						<option value="{$item['key']}" {if in_array($item['key'], $EMAILSEARCHLIST) } selected="selected"{/if}>{\App\Language::translate($item['name'], $item['name'])} - {\App\Language::translate($item['fieldlabel'], $item['name'])}</option>
		...
		<hr/>
        <div class="alert alert-info"><h4>{\App\Language::translate('LBL_CONFTAB_CHANGE_TICKET_STATUS', 'OSSMailScanner')}</h4>	</div>
		<form class="form-horizontal">
	...
	<div class='editViewContainer tab-pane marginTop20' id="exceptions">
		...
		<div class="form-group">
			...
			<div>
				<select multiple id="crating_mails" name="crating_mails" class="select2 form-control test" data-placeholder="{\App\Language::translate('LBL_WRITE_AND_ENTER','OSSMailScanner')}">
		...
		<div class="form-group">
			...
			<div>
				<select multiple id="crating_tickets" name="crating_tickets" class="select2 form-control" data-placeholder="{\App\Language::translate('LBL_WRITE_AND_ENTER','OSSMailScanner')}">
