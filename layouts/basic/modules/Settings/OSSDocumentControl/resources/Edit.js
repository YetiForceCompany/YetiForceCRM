...
jQuery(document).ready(function ($) {
	...
	$('#myModal .okay-button').click(function () {
		var disabled = $('#confirm_unistall').attr('disabled');
	...
	$('#status').change(function () {
		$('#confirm_unistall').attr('disabled', !this.checked);
		if (this.is(':checked')) {
			$('#confirm_unistall').attr('href', 'index.php?module=OSSDocumentControl&parent=Settings&view=Uninstall');
		} else
			$('#confirm_unistall').attr('href', '#');
	});
});
		...
		jQuery("#next_step").on('click', function (e) {
			...
			this.addCondiion = function () {
				...
				jQuery('button.add_condition').on('click', function () {
					...
					requestParams.data = {module: 'OSSDocumentControl', parent: 'Settings', view: 'Condition', base_module: baseModule, num: numCondition}
			...
			this.validateCondition = function (type) {
				...
				jQuery('#' + type + ' .conditionRow').each(function () {
					var row = jQuery(this);
					var fieldInfo = jQuery(row).find('.field-select option:selected').data('info');
			...
			this.toJson = function (type) {
				...
				jQuery('#' + type + ' .conditionRow').each(function () {
					var tab = {};
					tab.field = jQuery(this).find('select.field-select').val();

					tab.name = jQuery(this).find('select[name="comparator"]').val();

					var fieledType = jQuery(this).find('select.field-select option:selected').data('info')['type'];

					if (fieledType == 'time') {
						if (jQuery(this).find('select.field-select option:selected').data('info')['time-format'] == '12') {
			...
			this.filterTpl = function () {
				jQuery('#moduleFilter').on('change', function () {
					var val = jQuery(this).val(),
							requestParam = {};
					requestParam.data = {module: 'OSSDocumentControl', parent: 'Settings', view: 'ListDoc', base_module: val}
			...
			this.comparatorHasChanged = function () {
				...
				jQuery('[name="comparator"]').on('change', function () {
					...
					if (jQuery.inArray(val, conditionToShowInput) !== -1) {
						...
						var fieldInfo = jQuery(this).parents('.conditionRow').find('.field-select option:selected').data('info'),
