...
{strip}
	...
	<div>
			...
			<table class="locksTable table table-bordered">
				...
				<tbody>
					{foreach item=LOCK key=ID from=$LOCKS}
						{include file='LocksItem.tpl'|@\App\Layout::getTemplatePath:$QUALIFIED_MODULE SELECT=true}
					{/foreach}
				</tbody>
			</table>
		</div>
		...
		<table class="table table-bordered cloneItem hide">
			...
			{include file='LocksItem.tpl'|@\App\Layout::getTemplatePath:$QUALIFIED_MODULE SELECT=false}
		</table>
	</div>
{/strip}
