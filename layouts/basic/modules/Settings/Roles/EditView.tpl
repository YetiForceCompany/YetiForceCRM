                ...
                <div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						<select id="allowassignedrecordsto" class="row select2 form-control" name="allowassignedrecordsto">
							<option value="1" {if $RECORD_MODEL->get('allowassignedrecordsto') == '1'}selected="true"{/if}>{\App\Language::translate('LBL_ALL_USERS', $QUALIFIED_MODULE)}</option>
							<option value="2" {if $RECORD_MODEL->get('allowassignedrecordsto') == '2'}selected="true"{/if}>{\App\Language::translate('LBL_USERS_WITH_SAME_OR_LOWER_LEVEL', $QUALIFIED_MODULE)}</option>
							<option value="3" {if $RECORD_MODEL->get('allowassignedrecordsto') == '3'}selected="true"{/if}>{\App\Language::translate('LBL_USERS_WITH_LOWER_LEVEL', $QUALIFIED_MODULE)}</option>
							<option value="4" {if $RECORD_MODEL->get('allowassignedrecordsto') == '4'}selected="true"{/if}>{\App\Language::translate('LBL_JUST_ME', $QUALIFIED_MODULE)}</option>
							<option value="5" {if $RECORD_MODEL->get('allowassignedrecordsto') == '5'}selected="true"{/if}>{\App\Language::translate('LBL_FROM_PANEL', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
                </div>
                <div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						<select id="assignedmultiowner" class="row select2 form-control" name="assignedmultiowner">
							...
							<option value="4" {if $RECORD_MODEL->get('assignedmultiowner') == '4'}selected="true"{/if}>{\App\Language::translate('LBL_JUST_ME', $QUALIFIED_MODULE)}</option>
							<option value="5" {if $RECORD_MODEL->get('assignedmultiowner') == '5'}selected="true"{/if}>{\App\Language::translate('LBL_FROM_PANEL', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
                </div>
				<div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						...
						<select class="select2" multiple="true" id="profilesList" name="profiles[]" data-placeholder="{\App\Language::translate('LBL_CHOOSE_PROFILES',$QUALIFIED_MODULE)}" data-validation-engine="validate[required]" style="width: 800px">
							{foreach from=$ALL_PROFILES item=PROFILE}
								{if $PROFILE->isDirectlyRelated() eq false}
									<option value="{$PROFILE->getId()}" {if isset($ROLE_PROFILES[$PROFILE->getId()])}selected="true"{/if}>{\App\Language::translate($PROFILE->getName(),'Settings::Profiles')}</option>
								{/if}
							{/foreach}
						</select>
					</div>
				...
				<div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						<select id="listRelatedRecord" class="row select2 form-control" name="listRelatedRecord">
							<option value="0" {if $RECORD_MODEL->get('listrelatedrecord') == '0'}selected="true"{/if}>{\App\Language::translate('LBL_INACTIVE', $QUALIFIED_MODULE)}</option>
							<option value="1" {if $RECORD_MODEL->get('listrelatedrecord') == '1'}selected="true"{/if}>{\App\Language::translate('LBL_ONLY_PARENT', $QUALIFIED_MODULE)}</option>
							<option value="2" {if $RECORD_MODEL->get('listrelatedrecord') == '2'}selected="true"{/if}>{\App\Language::translate('LBL_ACCORDING_TO_HIERARCHY', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
				</div>
				<div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						<select id="previewRelatedRecord" class="row select2 form-control" name="previewRelatedRecord">
							<option value="0" {if $RECORD_MODEL->get('previewrelatedrecord') == '0'}selected="true"{/if}>{\App\Language::translate('LBL_INACTIVE', $QUALIFIED_MODULE)}</option>
							<option value="1" {if $RECORD_MODEL->get('previewrelatedrecord') == '1'}selected="true"{/if}>{\App\Language::translate('LBL_ONLY_PARENT', $QUALIFIED_MODULE)}</option>
							<option value="2" {if $RECORD_MODEL->get('previewrelatedrecord') == '2'}selected="true"{/if}>{\App\Language::translate('LBL_ACCORDING_TO_HIERARCHY', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
				</div>
				<div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						...
						<select id="permissionsRelatedField" class="row select2 form-control" name="permissionsRelatedField[]" multiple >
							...
							<option value="3" {if in_array('3', $PERMISSIONS_RELATED_FIELD)}selected="true"{/if}>{\App\Language::translate('LBL_PERMITTED_BY_READ_ACCESS', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
				</div>
				<div class="row">
					...
					<div class="col-md-7 fieldValue">
						...
						<select id="modulesList" class="row modules select2 form-control" multiple="true" name="searchunpriv[]">
							{foreach from=Vtiger_Module_Model::getAll([0],[],true) key=TABID item=MODULE_MODEL}
								<option value="{$MODULE_MODEL->getName()}" {if in_array($MODULE_MODEL->getName(), $SEARCH_MODULES)}selected="true"{/if}>{\App\Language::translate($MODULE_MODEL->getName(), $MODULE_MODEL->getName())}</option>
							{/foreach}
						</select>
					</div>
				</div>
				...
				<div class="row mb-2">
					...
					<div class="col-md-7 fieldValue">
						<select id="clendarallorecords" class="row select2 form-control" name="clendarallorecords">
							<option value="1" {if $RECORD_MODEL->get('clendarallorecords') == '1'}selected="true"{/if}>{\App\Language::translate('LBL_CLENDAR_ALLO_RECORDS_1', $QUALIFIED_MODULE)}</option>
							<option value="2" {if $RECORD_MODEL->get('clendarallorecords') == '2'}selected="true"{/if}>{\App\Language::translate('LBL_CLENDAR_ALLO_RECORDS_2', $QUALIFIED_MODULE)}</option>
							<option value="3" {if $RECORD_MODEL->get('clendarallorecords') == '3'}selected="true"{/if}>{\App\Language::translate('LBL_CLENDAR_ALLO_RECORDS_3', $QUALIFIED_MODULE)}</option>
						</select>
					</div>
				</div>
				<hr>
