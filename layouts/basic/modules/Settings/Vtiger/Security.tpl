			...
			<tbody>
				{foreach from=Settings_ConfReport_Module_Model::getSecurityConf() key=key item=item}
					<tr {if $item.status}class="danger"{/if}>
						<td>
							<label>{$key}</label>
							{if isset($item.help) && $item.status}<a href="#" class="popoverTooltip float-right" data-trigger="focus" data-placement="rigth" data-content="{App\Language::translate($item.help, 'Settings::ConfReport')}"><i class="fa fa-info-circle"></i></a>{/if}
						</td>
