		...
		{if !empty($DOC_TPL_LIST)}
			{foreach from=$DOC_TPL_LIST item=item key=key}
				<tr class="listViewEntries" data-id="{$item.id}">
					...
					<td onclick="location.href = jQuery(this).data('url')" data-url="index.php?module={$MODULE_NAME}&parent=Settings&view=Edit&tpl_id={$item.id}">{vtranslate($item.summary, $QUALIFIED_MODULE)}</td>
