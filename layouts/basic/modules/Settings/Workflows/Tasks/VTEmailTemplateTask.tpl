	...
	<div class="well" id="VtVTEmailTemplateTaskContainer">
		<div class="">
			<div class="row padding-bottom1per">
				<span class="col-md-4 control-label">{vtranslate('LBL_SMTP', $QUALIFIED_MODULE)}</span>
				<div class="col-md-4">
			...
			<div class="row padding-bottom1per">
				<span class="col-md-4 control-label">{vtranslate('EmailTempleteList', $QUALIFIED_MODULE)}</span>
				<div class="col-md-4">
					<select class="chzn-select form-control" name="template" data-validation-engine='validate[required]'>
						...
						{foreach from=App\Mail::getTempleteList($SOURCE_MODULE,'PLL_RECORD') key=key item=item}
							<option {if $TASK_OBJECT->template eq $item['id']}selected=""{/if} value="{$item['id']}">{vtranslate($item['name'], $QUALIFIED_MODULE)}</option>
						{/foreach}	
					</select>
				</div>
			</div>
			...
			<div class="row padding-bottom1per">
				<span class="col-md-4 control-label">{vtranslate('Select e-mail address', $QUALIFIED_MODULE)}</span>
				<div class="col-md-4">
					<select class="chzn-select form-control" name="email" data-placeholder="{vtranslate('LBL_SELECT_FIELD',$QUALIFIED_MODULE)}" multiple  data-validation-engine="validate[required]">
						...
						{foreach item=FIELDS key=BLOCK_NAME from=$TEXT_PARSER->getRecordVariable('email')}
							<optgroup label="{\App\Language::translate($BLOCK_NAME, $SOURCE_MODULE)}">
								{foreach item=ITEM from=$FIELDS}
									<option value="{$ITEM['var_value']}" data-label="{$ITEM['var_label']}" {if in_array($ITEM['var_value'],$TASK_OBJECT->email)}selected=""{/if}>
