	 ...
	 <select class="select2 form-control" name="attachments"
	 	...
	 	{foreach from=$DOCUMENTS_MULTI_REFERENCE_FIELD key=FIELD_NAME item=FIELD_MODEL}
		 	<option value="{$FIELD_NAME}::allAttachments"
		 		{if isset($TASK_OBJECT->attachments) && $TASK_OBJECT->attachments === "{$FIELD_NAME}::allAttachments"}selected="selected" {/if}>
		 	...
		 	<option value="{$FIELD_NAME}::latestAttachments" {if isset($TASK_OBJECT->attachments) && $TASK_OBJECT->attachments === "{$FIELD_NAME}::latestAttachments"}selected="selected" {/if}>
