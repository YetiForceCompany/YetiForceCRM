...
{strip}
	<!-- tpl-Settings-MailSignature-Edit -->
						...
						<div class="c-panel__body p-2 js-block-content {if !$EXPAND_BLOCKS}d-none{/if}">
							<div class="form-group row mb-0">
								{foreach from=$FIELDS item=FIELD_MODEL key=FIELD_NAME name=field}
									...
									{/if}
									<div class="{if $FIELD_MODEL->get('col-md')}{$FIELD_MODEL->get('col-md')}{else}col-md-4{/if} mb-2 js-field-container">
										...
										<div class="fieldValue{if $FIELD_MODEL->getFieldDataType() eq 'booelean'} ml-2 align-top d-inline-block{else} m-auto{/if}">
											{if $FIELD_MODEL->get('uitype') === 300}
												...
												<textarea class="form-control js-editor" name="{$FIELD_NAME}" id="{$FIELD_NAME}" data-purify-mode="Html" data-js="ckeditor">{$FIELD_MODEL->get('fieldvalue')}</textarea>
											{else}
