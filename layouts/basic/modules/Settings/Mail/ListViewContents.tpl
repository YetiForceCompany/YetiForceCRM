			...
			<tr class="listViewEntries" data-id="{$ENTRY_ID}"
			 ...
			 >
					...
					<td>{if $LISTVIEW_ENTRY->get('status') eq 0}
						<input type="checkbox" value="{$ENTRY_ID}" class="listViewEntriesCheckBox" title="{vtranslate('LBL_SELECT_SINGLE_ROW')}" />
						{/if}
					</td>
				{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
								...
								<div class="pull-right actions">
									<span class="actionImages">
										{foreach item=RECORD_LINK from=$LISTVIEW_ENTRY->getRecordLinks()}
											...
											<a class="{$RECORD_LINK->getButtonClass()}" {if stripos($RECORD_LINK_URL, 'javascript:')===0} onclick="{$RECORD_LINK_URL|substr:strlen("javascript:")};if(event.stopPropagation){ldelim}event.stopPropagation();{rdelim}else{ldelim}event.cancelBubble=true;{rdelim}" {else} href='{$RECORD_LINK_URL}' {/if}>
