	...
	<div class="contents row">
		<div class="col-md-12">
			...
			<div class="marginBottom10px">
				<button type="button" class="btn btn-success addInventory addButton" data-url="{$RECORD_MODEL->getCreateUrl()}" data-type="0"><i class="fa fa-plus"></i>&nbsp;&nbsp;<strong>{\App\Language::translate('LBL_ADD', $QUALIFIED_MODULE)} {\App\Language::translate($PAGE_LABELS.title_single, $QUALIFIED_MODULE)}</strong></button>
			</div>
						...
						<tr class="opacity" data-id="{$RECORD->getId()}">
							...
							<td class="textAlignCenter {$WIDTHTYPE}"><input type="checkbox" class="status" {if !$RECORD->getStatus()}checked{/if} />
								<div class="pull-right actions">
									<a class="editInventory cursorPointer" data-url="{$RECORD->getEditUrl()}"><span title="{\App\Language::translate('LBL_EDIT', $MODULE)}" class="fa fa-pencil alignBottom"></span></a>&nbsp;
