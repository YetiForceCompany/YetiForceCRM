	...
	<div class="">
		<div class='widget_header row '>
			...
			<div class="col-6 text-right">
				<div class="row">
					<div class="col">
						{\App\Language::translate('LBL_LAST_START',$QUALIFIED_MODULE)}:&nbsp; <b>{$LAST_CRON['laststart']}</b>
					</div>
				</div>
				<div class="row">
					<div class="col">
						...
						<b>
							{if $LAST_CRON['duration']==='running'}<i class="fa fa-spinner fa-spin" title="{\App\Language::translate('LBL_IS_RUNNING',$QUALIFIED_MODULE)}"></i>
							{elseif $LAST_CRON['duration']==='timeout'}<i class="fa fa-exclamation-triangle text-danger" title="{\App\Language::translate('LBL_HAD_TIMEOUT',$QUALIFIED_MODULE)}"></i>
		...
		<div class="listViewActionsDiv row">
			<div class="{if $SUPPORTED_MODULE_MODELS}col-md-5{else}col-md-8{/if} btn-toolbar">
				{foreach item=LINK from=$LISTVIEW_LINKS['LISTVIEWBASIC']}
					...
					{/if}
					<button type="button" title="{if $LINK->getLabel()}{$LABEL}{/if}" class="btn{if $LINK->getClassName()} {$LINK->getClassName()}{else} btn-light{/if} {if $LINK->get('modalView')}showModal{/if}" 
							{if $LINK->getUrl()}
