				...
				<div style="border-radius: 4px 4px 0px 0px;background: white;" class="editFieldsTable block_1 marginBottom10px border1px">
					...
					<table class="table tableRWD table-bordered ">
						...
						{foreach item=HOLIDAY from=$HOLIDAYS}
							<tr class="holidayElement" data-holiday-id="{$HOLIDAY['id']}" data-holiday-type="{$HOLIDAY['type']}" data-holiday-name="{$HOLIDAY['name']}" data-holiday-date="{\App\Fields\Date::currentUserDisplayDate($HOLIDAY['date'])}">
								<td>
									<span class="fieldLabel marginLeft20">{\App\Fields\Date::currentUserDisplayDate($HOLIDAY['date'])}</span>
								</td>
								<td>
							...
							<div class="modal-body">
								<div class="form-group">
									...
									<div class="col-sm-6 controls">
										<input type="text" name="holidayDate" class="dateField form-control" data-date-format="{$CURRENTUSER->column_fields['date_format']}" value="{\App\Fields\Date::currentUserDisplayDate(date('Y-m-d'))}" required >
