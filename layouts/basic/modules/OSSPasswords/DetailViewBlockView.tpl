...
{strip}
	...
	{if $BLOCKS_HIDE}
		<div class="detailViewTable">
			<div class="card mb-2" data-label="{$BLOCK_LABEL}">					
				...
				<div class="col-md-12 card-body blockContent py-0 px-3 {if $IS_HIDDEN} d-none{/if}">
					{assign var=COUNTER value=0}
					<div class="form-row fieldRow">
						{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
							...
							{if $FIELD_MODEL->getUIType() eq "69" || $FIELD_MODEL->getUIType() eq "105"}
								{if $COUNTER neq 0}
									{if $COUNTER eq 2}
									</div><div class="px-0 form-row">
										{assign var=COUNTER value=0}
									{/if}
								{/if}
								<div class="col-md-6 form-row px-0">
							...
							{else}
								...
								{/if}
								<div class="col-md-6 form-row px-0">
									<div class="fieldLabel fieldRow col-sm-6 {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldLabel_{$FIELD_MODEL->getName()}">
										<label class="muted small font-weight-bold float-sm-left float-md-right float-lg-right">
											{\App\Language::translate({$FIELD_MODEL->getFieldLabel()},{$MODULE_NAME})}
										</label>
									</div>
									<div class="fieldValue fieldRow col-sm-6 {$WIDTHTYPE}" id="{$MODULE}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->getUIType() eq '19' or $FIELD_MODEL->getUIType() eq '20'} {assign var=COUNTER value=$COUNTER+1} {/if} {if $FIELD_MODEL->getName() eq 'password'}onclick="showPasswordQuickEdit('{$smarty.get.record}');" {/if}>
