...
<!-- tpl-MailIntegration-Detail-Row -->
	<li class="list-group-item list-group-item-action py-0 px-2 js-row-click" data-id="{$ROW['id']}" data-module="{$ROW['module']}">
	  <div class="d-flex w-100 align-items-center">
				<a class="modCT_{$ROW['module']} js-record-link js-popover-tooltip--record small u-text-unset" href="{$URL}index.php?module={$ROW['module']}&view=Detail&record={$ROW['id']}" target="_blank">
			...
			<div class="ml-auto btn-group btn-group-sm" role="group" aria-label="record actions">
				...
				{/if}
				{if !empty($ROW['isRelatedToDocuments']) && \App\Privilege::isPermitted('Documents', 'DetailView')}
					<button class="js-add-attachments btn u-text-unset js-popover-tooltip" data-type="1" data-js="popover" data-content="{\App\Language::translate('LBL_ADD_DOCUMENTS', $MODULE_NAME)}">
						<span class="yfm-Documents" aria-hidden="true"></span>
					</button>
				{/if}
				{if $REMOVE_RECORD}
					<button class="js-remove-record btn u-text-unset js-popover-tooltip" data-js="popover" data-content="{\App\Language::translate('LBL_REMOVE_RELATION',$MODULE_NAME)} {$ROW['label']}">
