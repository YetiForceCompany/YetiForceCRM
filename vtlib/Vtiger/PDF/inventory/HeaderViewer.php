...
class Vtiger_PDF_InventoryHeaderViewer extends Vtiger_PDF_HeaderViewer
{
	...
	function display($parent)
	{
		...
		if ($this->model) {
			...
			$pdf->MultiCell($headerColumnWidth, $contentHeight, $modelColumn0['summary'], 0, 'L', 0, 1, $headerFrame->x, $headerFrame->y + $imageHeightInMM + 2);

			$pdf->SetFont('freeserif', '');
			$contentHeight = $pdf->GetStringHeight($modelColumn0['content'], $headerColumnWidth);
			$pdf->MultiCell($headerColumnWidth, $contentHeight, $modelColumn0['content'], 0, 'L', 0, 1, $headerFrame->x, $pdf->GetY());
			...
			$pdf->SetFont('freeserif', 'B');
			$pdf->MultiCell($contentWidth * 2.0, $contentHeight, $this->model->get('title'), 0, 'R', 0, 1, $contentX - $contentWidth, $headerFrame->y + 2);
			...
			foreach ($modelColumn2 as $label => $value) {
				if (is_array($value)) {
					...
					foreach ($value as $l => $v) {
						$pdf->MultiCell($headerColumnWidth - $offsetX, 7, sprintf('%s: %s', $l, $v), 1, 'C', 0, 1, $headerFrame->x + $headerColumnWidth * 2.0 + $offsetX, $pdf->GetY() + $offsetY);
						$offsetY = 0;
					}
				} else {
					$offsetY = 1;
					...
					if ($label == 'Shipping Address') {
						...
						$pdf->MultiCell($headerColumnWidth - $offsetX, 7, $label, 1, 'L', 1, 1, $headerFrame->x + $headerColumnWidth * 2.0 + $offsetX, $pdf->GetY() + $offsetY - $height - $offsetX - 4.0);

						$pdf->SetFont('freeserif', '');
						$pdf->MultiCell($headerColumnWidth - $offsetX, 7, $value, 1, 'L', 0, 1, $headerFrame->x + $headerColumnWidth * 2.0 + $offsetX, $pdf->GetY());
					} else {
						$pdf->MultiCell($headerColumnWidth - $offsetX, 7, $label, 1, 'L', 1, 1, $headerFrame->x + $headerColumnWidth, $pdf->GetY() + $offsetY);

						$pdf->SetFont('freeserif', '');
						$pdf->MultiCell($headerColumnWidth - $offsetX, 7, $value, 1, 'L', 0, 1, $headerFrame->x + $headerColumnWidth, $pdf->GetY());
					}
				}
			}
			$pdf->setFont('freeserif', '');
