...
class ReportRun extends CRMEntity
{
	...
	public static function getInstance($reportid)
	{
		if (!isset(self::$instances[$reportid])) {
			self::$instances[$reportid] = new self($reportid);
		}
	...
	public function getColumnSQL($selectedfields)
	{
		...
		if ($selectedfields[4] == 'C') {
			...
			$concatSql = \vtlib\Deprecated::getSqlForNameInDisplayFormat(['last_name' => $targetTableName . '.last_name', 'first_name' => $targetTableName . '.first_name'], 'Users');
			...
			$moduleInstance = CRMEntity::getInstance($module);
			$this->queryPlanner->addTable($moduleInstance->table_name);
		} elseif (stristr($selectedfields[0], 'vtiger_crmentity') && ($selectedfields[1] == 'shownerid')) {
	...
	public function getFilterComparedField($field)
	{
		...
		$field_query = $adb->pquery('SELECT tablename,columnname,typeofdata,fieldname,uitype FROM vtiger_field WHERE tabid = ? && fieldname= ?', [$tabid, $fieldname]);
		$fieldtablename = $adb->queryResult($field_query, 0, 'tablename');
		$fieldcolname = $adb->queryResult($field_query, 0, 'columnname');
					...
					if ($fieldcolname != '' && $comparator != '') {
						...
						if (isset($valuearray) && count($valuearray) > 1 && $comparator != 'bw') {
							...
							for ($n = 0; $n < $countValueArray; ++$n) {
								if (($selectedfields[0] == 'vtiger_users' . $this->primarymodule || $selectedfields[0] == 'vtiger_users' . $this->secondarymodule) && $selectedfields[1] == 'user_name') {
									...
									if (is_numeric($valuearray[$n]) || empty($valuearray[$n])) {
										...
										$advcolsql[] = 'trim(' . \vtlib\Deprecated::getSqlForNameInDisplayFormat(['last_name' => "$tableName.last_name", 'first_name' => "$tableName.first_name"], 'Users') . ')' .
							...
							if (is_numeric($value) || empty($value)) {
								...
								$fieldvalue = 'trim(' . \vtlib\Deprecated::getSqlForNameInDisplayFormat(['last_name' => "$tableName.last_name", 'first_name' => "$tableName.first_name"], 'Users') . ')' .
	...
	public function getStandardCriterialSql($reportid)
	{
		...
		for ($i = 0; $i < $noofrows; ++$i) {
			$fieldcolname = $adb->queryResult($result, $i, 'datecolumnname');
			$datefilter = $adb->queryResult($result, $i, 'datefilter');
			$startdate = $adb->queryResult($result, $i, 'startdate');
			$enddate = $adb->queryResult($result, $i, 'enddate');
		...
		while ($reportsortrow = $adb->fetchArray($result)) {
			...
			if ($fieldcolname != 'none') {
				...
				if (stripos($selectedfields[1], 'cf_') == 0 && stristr($selectedfields[1], 'cf_') === true) {
					...
					$sqlvalue = '`' . $adb->sqlEscapeString(App\Purifier::decodeHtml($selectedfields[2])) . '` ' . $sortorder;
				} else {
	...
	public function getSelectedOrderbyList($reportid)
	{
		...
		for ($i = 0; $i < $noofrows; ++$i) {
			$fieldcolname = $adb->queryResult($result, $i, 'columnname');
			$sortorder = $adb->queryResult($result, $i, 'sortorder');
	...
	public function getReportsNonAdminAccessControlQuery($module, $scope = '')
	{
		...
		if ($is_admin === false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 && $defaultOrgSharingPermission[$tabId] == 3) {
			...
			if (!empty($sharedTabId)) {
				...
				for ($i = 0; $i < $rows; ++$i) {
					$ids[] = $db->queryResult($result, $i, 'id');
				}
				if (!empty($ids)) {
			...
			if ($result) {
				...
				do {
					...
					for ($i = 0; $i < $y; ++$i) {
						...
						if ($fieldvalue == '') {
							$fieldvalue = '-';
						} elseif ($fld->name == $this->primarymodule . '__LBL_ACTION' && $fieldvalue != '-') {
	...
	public function getAccessPickListValues()
	{
		...
		for ($i = 0; $i < $countResult; ++$i) {
			...
			$tabid = $adb->queryResult($result, $i, 'tabid');
			...
			for ($j = 0; $j < $countMulSelResult; ++$j) {
				$fldvalue = $adb->queryResult($mulselresult, $j, $fieldname);
	...
	public function writeReportToCSVFile($fileName, $filterlist = '')
	{
		...
		$fp = fopen($fileName, 'w+');
		fwrite($fp, chr(239) . chr(187) . chr(191)); //UTF-8 byte order mark
		if (isset($arr_val)) {
			// Header
