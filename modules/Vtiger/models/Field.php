...
class Vtiger_Field_Model extends vtlib\Field
{
	...
	{
		$moduleEntityNameFields = $this->getModule()->getNameFields();
	...
	{
		if (!$this->isEditEnabled() || ( ((int) $this->get('displaytype')) !== 1 && ((int) $this->get('displaytype')) !== 10 ) || $this->isReadOnly() === true || $this->get('uitype') === 4) {
			return false;
		}
		return true;
	}
	...
	public function getFieldInfo()
	{
		...
		switch ($fieldDataType) {
			...
			case 'userCreator':
			case 'sharedOwner':
				if (!AppConfig::performance('SEARCH_OWNERS_BY_AJAX') || in_array(AppRequest::get('module'), ['CustomView', 'Workflows', 'PDF', 'MappedFields', 'DataAccess']) || AppRequest::get('mode') === 'showAdvancedSearch') {
