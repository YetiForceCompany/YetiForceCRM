...
function replaceProductTable2($pdftype, $id, $templateid,$content, $tcpdf) {
    ...
    $total_gross_amount = $associated_products[1]['final_details']['hdnSubTotal'];
    $total_discount_new = $associated_products[1]['final_details']['discountTotal_final'];
    ...
    if ($printColumnWithCalculatedTax == TRUE) {
        ...
        $currency_rate_result = $CurrencyUpdate->getCurrencyRate($newdate, $currency_id);

        $num_rate = $adb->num_rows($currency_rate_result, true);
        $currency_rate = '0.00';
        if ($num_rate != 0) {
            $currency_rate = (float) $adb->query_result($currency_rate_result, 0, 'kurs');
            $newdate = date('Y-m-d', strtotime($adb->query_result($currency_rate_result, 0, "data_faktyczna_kursu")));
        } else {
            $CurrencyUpdate->getCurrency($newdate);
            $currency_rate_result = $CurrencyUpdate->getCurrencyRate($newdate, $currency_id);
            $num_rate = $adb->num_rows($currency_rate_result);
            if ($num_rate != 0) {
                $currency_rate = (float) $adb->query_result($currency_rate_result, 0, 'kurs');
            }
        }
    ...
    foreach ($product_line as $item) {
        ...
        if ($final_details['taxtype'] == 'group') {
            ...
            $tax_pln = $item['Tax'] * $currency_rate;
    ...
    if ($displays_a_summary_of_invoices == TRUE) {
		$product_table .= '<tr ><td colspan="'.$mainolspan.'" style="border: 0.2mm solid black; background-color: #e8e8e8;"><small><b>' . $languageStrings["SUMMARY"] . '</b></small></td></tr>';
        if ($enable_summary_rebate) {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["Discount Amount"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($total_discount_new, 2, $dec_sep, $group_sep) . ' (' . $currency_symbol . ')</small></td></tr>';
        }
        if ($enable_summary_taxpercentage && $final_details['taxtype'] == 'group') {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["The percentage of tax"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . $grup_tax_percent . ' (%)</small></td></tr>';
        }
        if ($enable_summary_taxamount && $final_details['taxtype'] == 'group') {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["The amount of tax"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($grup_tax, 2, $dec_sep, $group_sep) . ' (' . $currency_symbol . ')</small></td></tr>';
        }
        if ($enable_summary_topay) {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["Grand Total"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($grand_total, 2, $dec_sep, $group_sep) . ' (' . $currency_symbol . ')</small></td></tr>';
        }
        $product_table .= "</table>";
    }
