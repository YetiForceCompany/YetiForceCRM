...
function replaceProductTable2($pdftype, $id, $templateid,$content, $tcpdf) {
    ...
    if ($displays_a_summary_of_invoices == TRUE) {
        ...
        if ($enable_summary_rebate) {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["Discount Amount"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($rabat_calkowity, 2, $dec_sep, $group_sep) . ' (' . $symbol_waluty . ')</small></td></tr>';
        }
        ...
        if ($enable_summary_taxamount && $final_details['taxtype'] == 'group') {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["The amount of tax"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($grup_tax, 2, $dec_sep, $group_sep) . ' (' . $symbol_waluty . ')</small></td></tr>';
        }
        if ($enable_summary_topay) {
			$product_table .= '<tr><td colspan="4" style="border: 0.2mm solid black; text-align:right;"><small><b>' . $languageStrings["Grand Total"] . '</b> :</small></td><td colspan="'.$colspan.'" style="border: 0.2mm solid black; text-align:right;"><small>' . number_format($grand_total, 2, $dec_sep, $group_sep) . ' (' . $symbol_waluty . ')</small></td></tr>';
        }
        $product_table .= "</table>";
    }
