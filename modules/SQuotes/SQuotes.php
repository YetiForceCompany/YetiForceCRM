...
class SQuotes extends Vtiger_CRMEntity
{
	...
	function vtlib_handler($moduleName, $eventType)
	{
		...
		if ($eventType == 'module.postinstall') {
			$moduleInstance = CRMEntity::getInstance('SQuotes');
			$moduleInstance->setModuleSeqNumber("configure", 'SQuotes', 'S-Q', '1');
			...
			if ($modcommentsModuleInstance && file_exists('modules/ModTracker/ModTracker.php')) {
				...
				$tabid = Vtiger_Functions::getModuleId('SQuotes');
	...
	{
		$adb = PearDatabase::getInstance();
		$actions = ['OpenQuotes', 'CloseQuotes'];
