...
function _1_created_HelpDesk($user_id, $mail_detail, $folder, $return)
{
	...
	$EmailNumPrefix = OSSMailScanner_Record_Model::findEmailNumPrefix('HelpDesk', $mail_detail['subject']);
	$result = $adb->pquery("SELECT ticketid FROM vtiger_troubletickets where ticket_no = ? ", array($EmailNumPrefix), true);
	if ($adb->num_rows($result) == 0) {
		$contact_id = OSSMailScanner_Record_Model::findEmail($mail_detail['fromaddress'], 'Contacts', false);
		$parent_id = OSSMailScanner_Record_Model::findEmail($mail_detail['fromaddress'], 'Accounts', false);
		$HelpDeskInstance = CRMEntity::getInstance('HelpDesk');
		$HelpDeskInstance->column_fields['assigned_user_id'] = $assigned_user_id;
		$HelpDeskInstance->column_fields['ticket_title'] = $mail_detail['subject'];
		...
		$HelpDeskInstance->column_fields['description'] = strip_tags($mail_detail['body']);
