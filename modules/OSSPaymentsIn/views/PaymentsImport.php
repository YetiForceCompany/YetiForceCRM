 ...
 * All Rights Reserved.
 *************************************************************************************************************************************/
class OSSPaymentsIn_PaymentsImport_View extends Vtiger_Index_View {
	function process(Vtiger_Request $request) {

		$type = array();
		$bank = array();
		foreach(new DirectoryIterator('modules/OSSPaymentsIn/helpers') as $file){
			if(!$file->isDot()){
		...
		$bank[] = 'Default';
		foreach(new DirectoryIterator('modules/OSSPaymentsIn/helpers/subclass') as $file){
			if(!$file->isDot()){
				if( strpos($file->getFilename(), '.php') !== false){
