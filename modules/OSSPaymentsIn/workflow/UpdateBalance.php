...
function UpdateBalance($entityData) {
	...
	if ($moduleName == 'OSSPaymentsIn' || $moduleName == 'OSSPaymentsOut') {
...
function UpdateInvoice( $salesId ) {
	...
	$sql = 'SELECT SUM(`paymentsvalue`) as suma FROM `vtiger_osspaymentsin` WHERE `salesid` = ? AND `osspaymentsin_status` = ?;';
	$result = $adb->pquery($sql, array($salesId, $StatusBalance), false);
	$paymentsIn = $adb->query_result($result, 0, 'suma');

	$sql = 'SELECT SUM(`paymentsvalue`) as suma FROM `vtiger_osspaymentsout` WHERE `salesid` = ? AND `osspaymentsout_status` = ?;';
	$result = $adb->pquery($sql, array($salesId, $StatusBalance), false);
