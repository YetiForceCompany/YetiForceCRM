...
class Settings_Search_Module_Model extends Settings_Vtiger_Module_Model
{
	...
	public static function UpdateLabels($params)
	{
		...
		$sql .= $sqlExt;
		$sql .= ' INNER JOIN vtiger_crmentity_label ON vtiger_crmentity_label.crmid = vtiger_crmentity.crmid';
		$sql .= ' INNER JOIN vtiger_crmentity_search ON vtiger_crmentity_search.crmid = vtiger_crmentity.crmid';
		$sql .= " SET vtiger_crmentity_label.label = CONCAT_WS(' ' $entityColumnName), vtiger_crmentity_search.searchlabel = CONCAT_WS(' ' $searchColumnName)";
