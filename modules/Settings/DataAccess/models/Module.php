...
class Settings_DataAccess_Module_Model extends Vtiger_Module_Model
{
	...
	public static function saveActionConfig($ID, $action, $form_data, $aid = false)
	{
		...
		Vtiger_Loader::includeOnce("~~modules/{$actionArray[0]}/data_access/{$actionArray[1]}.php");
	...
	public static function showConfigDataAccess($tpl_id, $actionsName, $baseModule)
	{
		...
		Vtiger_Loader::includeOnce("~~modules/{$actionsNameA[0]}/data_access/{$actionsNameA[1]}.php");
	...
	{
		Vtiger_Loader::includeOnce('~~modules/Settings/DataAccess/helpers/DataAccess_Conditions.php');
	...
	public static function executeAction($module, $param, $data)
	{
		...
		if ($data) {
			foreach ($data as $row) {
				...
				if (file_exists($file)) {
					Vtiger_Loader::includeOnce("~~$file");
