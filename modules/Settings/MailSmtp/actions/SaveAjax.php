...
class Settings_MailSmtp_SaveAjax_Action extends Settings_Vtiger_IndexAjax_View
{
	...
	public function updateSmtp(Vtiger_Request $request)
	{
		...
		if (isset($testMailer['result']) && $testMailer['result'] !== false) {
			...
			$recordModel->set('default', $data['default']);
			...
			$recordModel->set('authentication', $data['authentication']);
			...
			$recordModel->set('individual_delivery', $data['individual_delivery']);
			$recordModel->save();

			$result = ['success' => true, 'url' => $recordModel->getDetailViewUrl()];
		} else {
			$result = ['success' => false, 'message' => \App\Language::translate('LBL_MAIL_TEST_FAILED')];
		}
