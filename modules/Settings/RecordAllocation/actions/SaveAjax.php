...
class Settings_RecordAllocation_SaveAjax_Action extends Settings_Vtiger_Save_Action
{
	...
	public function save(\App\Request $request)
	{
		...
		$oldValues = array_merge(isset($oldValues[$data['userid'][0]]['users']) ? (array) $oldValues[$data['userid'][0]]['users'] : [], isset($oldValues[$data['userid'][0]]['groups']) ? (array) $oldValues[$data['userid'][0]]['groups'] : []);
		...
		$newValues = array_merge(isset($newValues[$data['userid'][0]]['users']) ? (array) $newValues[$data['userid'][0]]['users'] : [], isset($newValues[$data['userid'][0]]['groups']) ? (array) $newValues[$data['userid'][0]]['groups'] : []);
