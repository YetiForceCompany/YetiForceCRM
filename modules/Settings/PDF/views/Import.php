...
class Settings_PDF_Import_View extends Settings_Vtiger_Index_View
{
	...
	public function process(Vtiger_Request $request)
	{
		...
		if ($request->has('upload') && $request->get('upload') == 'true') {
			...
			//$xmlSize		= $_FILES['imported_xml']['size'];
			$xmlError = $_FILES['imported_xml']['error'];
			$extension = end(explode('.', $xmlName));
			$imagePath = '';
			$base64Image = false;

			$pdfRecordModel = Settings_PDF_Record_Model::getCleanInstance();
						...
						foreach ($fieldValue as $columnKey => $columnValue) {
							switch ($columnKey) {
								...
								default:
									...
									$pdfRecordModel->set($columnKey, $value);
							}
						}
					}
				}
				$pdfRecordModel->import();

				if ($pdfRecordModel->getId() && $imagePath != '' && $base64Image) {
					$targetDir = Settings_PDF_Module_Model::$uploadPath;
					$imageExt = end(explode('.', basename($imagePath)));
					$imageData = base64_decode($base64Image);
					$newFilePath = $targetDir . $pdfRecordModel->getId() . '.' . $imageExt;

					$pdfRecordModel->set('watermark_image', $newFilePath);
					$pdfRecordModel->save(8);

					file_put_contents($newFilePath, $imageData);
				}
				$viewer->assign('PDF_MODEL', $pdfRecordModel);
