...
class Calendar_Invitees_Action extends Vtiger_Action_Controller
{
	...
	public function find(Vtiger_Request $request)
	{
		...
		$query = "SELECT label, crmid, setype FROM vtiger_crmentity INNER JOIN vtiger_entityname ON vtiger_crmentity.setype = vtiger_entityname.modulename WHERE vtiger_crmentity.setype IN ('$modules') AND vtiger_crmentity.searchlabel LIKE ? AND vtiger_crmentity.deleted = 0 AND vtiger_entityname.turn_off = ?";
