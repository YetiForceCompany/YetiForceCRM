...
function vtTaskEdit($adb, $current_language, $app_strings)
{
	global $theme;
	...
	$smarty->assign('edit', $request->has('task_id'));
	if ($request->has('task_id')) {
		$task = $tm->retrieveTask($request->get('task_id'));
		$taskClass = get_class($task);
		$workflowId = $task->workflowId;
	} else {
		$workflowId = $request->get('workflow_id');
		$taskClass = $request->getForSql('task_type');
	...
	$smarty->assign("returnUrl", $request->get('return_url'));
	$smarty->assign("task", $task);
	$smarty->assign("taskType", $taskClass);
	$smarty->assign("saveType", $request->get('save_type'));
...
vtTaskEdit($adb, $current_language, $app_strings);
