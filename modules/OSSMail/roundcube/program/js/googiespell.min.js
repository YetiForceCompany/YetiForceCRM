...
this.cnt_errors_fixed=0;this.setStateChanged("checking_spell");this.orginal_text="";!b&&this.main_controller&&this.appendIndicator(this.spell_span);this.error_links=[];this.ta_scroll_top=this.text_area.scrollTop;this.ignore=a;this.hideLangWindow();if(""==$(this.text_area).val()||a)this.custom_no_spelling_error?this.custom_no_spelling_error(this):this.flashNoSpellingErrorState(),this.removeIndicator();else{this.createEditLayer(this.text_area.offsetWidth,this.text_area.offsetHeight);this.createErrorWindow();
$("body").append(this.error_window);try{netscape.security.PrivilegeManager.enablePrivilege("UniversalBrowserRead")}catch(c){}this.main_controller&&$(this.spell_span).unbind("click");this.orginal_text=$(this.text_area).val()}};this.parseResult=function(a){var b=/\w+="(\d+|true)"/g,c=/\t/g;a=a.match(/<c[^>]*>[^<]*<\/c>/g);var d=[];if(null==a)return d;for(var e=0,g=a.length;e<g;e++){var k=[];this.errorFound();k.attrs=[];for(var f,h,m=a[e].match(b),l=0;l<m.length;l++)f=m[l].split(/=/),h=f[1].replace(/"/g,
""),k.attrs[f[0]]="true"!=h?parseInt(h):h;k.suggestions=[];f=a[e].replace(/<[^>]*>/g,"").split(c);for(h=0;h<f.length;h++)""!=f[h]&&k.suggestions.push(f[h]);d.push(k)}return d};this.processData=function(a){this.results=this.parseResult(a);this.results.length&&(this.showErrorsInIframe(),this.resumeEditingState())};this.createErrorWindow=function(){this.error_window=document.createElement("div");$(this.error_window).addClass("googie_window popupmenu").attr("googie_action_btn","1")};this.isErrorWindowShown=
