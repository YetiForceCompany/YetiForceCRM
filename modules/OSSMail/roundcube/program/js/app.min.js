...
!0)};this.add_onload=function(a){this.onloads.push(a)};this.init=function(){var a;this.task=this.env.task;if(409==this.env.server_error||bw.dom&&bw.xmlhttp_test()&&!(bw.mz&&1.9>bw.vendver||bw.ie&&7>bw.vendver)){this.env.blankpage||(this.env.blankpage=this.assets_path("program/resources/blank.gif"));for(a in this.gui_containers)this.gui_containers[a]=$("#"+this.gui_containers[a]);for(a in this.gui_objects)this.gui_objects[a]=rcube_find_object(this.gui_objects[a]);if(this.env.x_frame_options)try{if("deny"==
...
"save-pref","compose","undo","about","switch-task","menu-open","menu-close","menu-save",!0);this.set_button(this.task,"sel");this.env.permaurl&&this.enable_command("permaurl","extwin",!0);switch(this.task){case "mail":this.enable_command("list","checkmail","add-contact","search","reset-search","collapse-folder","import-messages",!0);this.gui_objects.messagelist&&(this.message_list=new rcube_list_widget(this.gui_objects.messagelist,{multiselect:!0,multiexpand:!0,draggable:!0,keyboard:!0,column_movable:this.env.col_movable,
...
function(a){g.drag_end(a)}).addEventListener("expandcollapse",function(a){g.msglist_expand(a)}).addEventListener("column_replace",function(a){g.msglist_set_coltypes(a)}).addEventListener("listupdate",function(a){g.triggerEvent("listupdate",a)}).init(),$(this.message_list.thead).on("click","a.sortcol",function(a){return g.command("sort",$(this).attr("rel"),this)}),this.enable_command("toggle_status","toggle_flag","sort",!0),this.enable_command("set-listmode",this.env.threads&&!this.is_multifolder_listing()),
this.command("list"),$(this.gui_objects.qsearchbox).val(this.env.search_text).focusin(function(){g.message_list.blur()}));this.set_button_titles();this.env.message_commands="show reply reply-all reply-list move copy delete open mark edit viewsource print load-attachment download-attachment show-headers hide-headers download forward forward-inline forward-attachment change-format".split(" ");"show"==this.env.action||"preview"==this.env.action?(this.enable_command(this.env.message_commands,this.env.uid),
this.enable_command("reply-list",this.env.list_post),"show"==this.env.action&&this.http_request("pagenav",{_uid:this.env.uid,_mbox:this.env.mailbox,_search:this.env.search_request},this.display_message("","loading")),this.env.blockedobjects&&(this.gui_objects.remoteobjectsmsg&&(this.gui_objects.remoteobjectsmsg.style.display="block"),this.enable_command("load-images","always-load",!0)),"preview"==this.env.action&&this.is_framed()&&(this.enable_command("compose","add-contact",!1),parent.rcmail.show_contentframe(!0))):
...
f)}catch(b){}}).contents().on("mouseup",f);this.triggerEvent("init",{task:this.task,action:this.env.action});for(a in this.onloads)if("string"===typeof this.onloads[a])eval(this.onloads[a]);else if("function"===typeof this.onloads[a])this.onloads[a]();this.start_refresh();this.start_keepalive()}else this.goto_url("error","_code=0x199")};this.log=function(a){window.console&&console.log&&console.log(a)};this.command=function(a,b,d,e){var f,h,n,k=!1;!d||!d.blur||e&&rcube_event.is_keyboard(e)||d.blur();
if(this.busy&&("reset-search"!=a||"search"!=this.last_command)&&!a.match(/^menu-/))return!1;if(d&&d.href&&0>String(d.href).indexOf("#")&&rcube_event.get_modifier(e))return!0;if(!this.commands[a])return this.is_framed()&&parent.rcmail.command(a,b),!1;if("mail"==this.task&&"compose"==this.env.action&&0>$.inArray(a,this.env.compose_commands)&&!this.env.server_error){if(!this.env.is_sent&&this.cmp_hash!=this.compose_field_hash()&&!confirm(this.get_label("notsentwarning")))return!1;this.remove_compose_data(this.env.compose_id);
this.compose_skip_unsavedcheck=!0}this.last_command=a;if("function"===typeof this.command_handlers[a])return f=this.command_handlers[a](b,d,e),void 0!==f?f:d?!1:!0;if("string"===typeof this.command_handlers[a])return f=window[this.command_handlers[a]](b,d,e),void 0!==f?f:d?!1:!0;this.triggerEvent("actionbefore",{props:b,action:a,originalEvent:e});f=this.triggerEvent("before"+a,b||e);if(void 0!==f){if(!1===f)return!1;b=f}f=void 0;switch(a){case "login":this.gui_objects.loginform&&this.gui_objects.loginform.submit();
break;case "logout":case "mail":case "addressbook":case "settings":this.switch_task(a);break;case "about":this.redirect("?_task=settings&_action=about",!1);break;case "permaurl":if(d&&d.href&&d.target)return!0;this.env.permaurl&&(parent.location.href=this.env.permaurl);break;case "extwin":if("compose"==this.env.action){if(e=this.gui_objects.messageform,h=this.open_window(""))this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0,$("input[name='_action']",e).val("compose"),e.action=this.url("mail/compose",
{_id:this.env.compose_id,_extwin:1}),e.target=h.name,e.submit()}else this.open_window(this.env.permaurl,!0);break;case "change-format":e=this.env.permaurl+"&_format="+b;"preview"==this.env.action&&(e=e.replace(/_action=show/,"_action=preview")+"&_framed=1");this.env.extwin&&(e+="&_extwin=1");location.href=e;break;case "menu-open":b&&"attachmentmenu"==b.menu&&(h=this.env.attachments[b.id],this.enable_command("open-attachment",h&&this.env.mimetypes&&0<=$.inArray(h,this.env.mimetypes)));this.show_menu(b,
b.show||void 0,e);break;case "menu-close":this.hide_menu(b,e);break;case "menu-save":return this.triggerEvent(a,{props:b,originalEvent:e}),!1;case "open":if(h=this.get_single_uid())return d.href=this.url("show",{_mbox:this.get_message_mailbox(h),_uid:h}),!0;break;case "close":this.env.extwin&&window.close();break;case "list":b&&""!=b&&this.reset_qsearch(!0);"compose"==this.env.action&&this.env.extwin?window.close():"mail"==this.task?(this.list_mailbox(b),this.set_button_titles()):"addressbook"==this.task&&
...
1:0,flagged:d.flagged?1:0,has_children:d.has_children?1:0,depth:d.depth?d.depth:0,unread_children:d.unread_children?d.unread_children:0,parent_uid:d.parent_uid?d.parent_uid:0,selected:this.select_all_mode||this.message_list.in_selection(a),ml:d.ml?1:0,ctype:d.ctype,mbox:d.mbox,flags:d.extra_flags});var f,h,g,k="",m="",l="",p="",q=this.message_list,v=q.rows;g=this.env.messages[a];var s=this.html_identifier(a,!0),t="message"+(d.seen?"":" unread")+(d.deleted?" deleted":"")+(d.flagged?" flagged":"")+
(g.selected?" selected":""),w={cols:[],style:{},id:"rcmrow"+s,uid:a};f="msgicon";null===this.env.status_col&&(f+=" status",d.deleted?(k+=" deleted",m+=this.get_label("deleted")+" "):d.seen?0<d.unread_children&&(k+=" unreadchildren"):(k+=" unread",m+=this.get_label("unread")+" "));d.answered&&(k+=" replied",m+=this.get_label("replied")+" ");d.forwarded&&(k+=" forwarded",m+=this.get_label("forwarded")+" ");g.selected&&!q.in_selection(a)&&q.selection.push(a);this.env.threading&&(g.depth?(l+='<span id="rcmtab'+
s+'" class="branch" style="width:'+15*g.depth+'px;">&nbsp;&nbsp;</span>',v[g.parent_uid]&&!1===v[g.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||v[g.parent_uid]&&v[g.parent_uid].expanded)?(w.style.display="none",g.expanded=!1):g.expanded=!0,t+=" thread expanded"):g.has_children&&(void 0===g.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&g.unread_children)&&(g.expanded=!0),p='<div id="rcmexpando'+w.id+'" class="'+(g.expanded?"expanded":
"collapsed")+'">&nbsp;&nbsp;</div>',t+=" thread"+(g.expanded?" expanded":"")),d.unread_children&&d.seen&&!g.expanded&&(t+=" unroot"));l+='<span id="msgicn'+w.id+'" class="'+f+k+'" title="'+m+'"></span>';w.className=t;b.subject&&(f=d.mbox==this.env.drafts_mailbox?"compose":"show",m={_mbox:d.mbox},m[d.mbox==this.env.drafts_mailbox?"_draft_uid":"_uid"]=a,b.subject='<a href="'+this.url(f,m)+'" onclick="return rcube_event.keyboard_only(event)" onmouseover="rcube_webmail.long_subject_title(this,'+(g.depth+
...
(d=d.opener());!d&&window.parent&&(d=parent.rcmail);d&&d.message_list&&(!1===d.set_message(a,"unread",!1)&&d.set_message(a+"-"+b,"unread",!1),0<d.env.unread_counts[b]&&(d.env.unread_counts[b]-=1,d.set_unread_count(b,d.env.unread_counts[b],"INBOX"==b&&!d.is_multifolder_listing())))};this.show_contentframe=function(a){var b,d,e=this.env.contentframe;if(e&&(b=this.get_frame_element(e)))if(!a&&(d=this.get_frame_window(e)))0>d.location.href.indexOf(this.env.blankpage)&&(d.stop?d.stop():d.document.execCommand("Stop"),
...
100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(m={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(e,m);for(a in k)this.init_address_input_events($("[name='_"+k[a]+"']"),m);g||(a=this.env.top_posting?0:h.value.length,"select-one"==d.prop("type")&&this.change_identity(d[0]),this.set_caret_pos(h,a),a&&$(h).scrollTop(h.scrollHeight));this.env.save_localstorage&&this.compose_restore_dialog(0,g);""==e.val()?b=e:""==f.val()?
...
"E"==f.charAt(0)&&0>this.env.contactdata[f].indexOf("@")&&d.length)){var g=f.substr(1);this.group2expand[g]={name:this.env.contactdata[f],input:d.get(0)};this.http_request("group-expand",{_source:this.env.source,_gid:g},!1)}b.length&&d.length&&(f=d.val(),h=RegExp(RegExp.escape(e)+"\\s*$"),f&&!h.test(f)&&(f+=e+" "),d.val(f+b.join(e+" ")+e+" "),this.triggerEvent("add-recipient",{field:a,recipients:b}));return b.length};this.check_compose_input=function(a){var b=$("[name='_to']"),d=$("[name='_cc']"),
...
g.env.identities[e][this]:"",m=d&&g.env.identities[d]?g.env.identities[d][this]:"",l=$('[name="_'+this+'"]'),p=l.val();b&&p&&(a=RegExp("\\s*"+RegExp.escape(b)+"\\s*"),p=p.replace(a,""));a=RegExp(h+"\\s*"+h,"g");p=String(p).replace(a,f);a=RegExp("^[\\s"+h+"]+");p=p.replace(a,"");m&&-1==p.indexOf(m)&&-1==p.indexOf(m.replace(/"/g,""))&&(p&&(a=RegExp("["+h+"\\s]+$"),p=p.replace(a,"")+f+" "),p+=m+f+" ");(b||m)&&l.val(p).change()});this.editor.change_signature(d,b);this.env.identity=d;this.triggerEvent("change_identity");
...
if(a&&(h=a.length))for(e=0;e<h&&0<l;e++)n="object"===typeof a[e]?a[e].display||a[e].name:a[e],k="object"===typeof a[e]?a[e].type:"",f=e+this.env.contacts.length,$("<li>").attr("id","rcmkSearchItem"+f).attr("role","option").html('<i class="icon"></i>'+this.quote_html(n.replace(RegExp("("+RegExp.escape(m)+")","ig"),"##$1%%")).replace(/##([^%]+)%%/g,"<b>$1</b>")).addClass(k||"").appendTo(b).mouseover(function(){g.ksearch_select(this)}).mouseup(function(){g.ksearch_click(this)}).get(0)._rcm_id=f,l-=1;
b.childNodes.length&&($(this.ksearch_input).attr("aria-haspopup","true").attr("aria-expanded","true").attr("aria-owns","rcmKSearchpane"),this.ksearch_pane.show(),this.env.contacts.length||this.ksearch_select($("li:first",b).get(0)));h&&(this.env.contacts=this.env.contacts.concat(a));this.ksearch_data.id==d&&this.ksearch_data.num--}};this.ksearch_click=function(a){this.ksearch_input&&this.ksearch_input.focus();this.insert_recipient(a._rcm_id);this.ksearch_hide()};this.ksearch_blur=function(){this.ksearch_timer&&
...
"",b)})};this.folder_selector=function(a,b){var d=this.folder_selector_element;if(!d){var e=[],f=this.env.delimiter,h=$('<ul class="toolbarmenu">'),n=document.createElement("a"),d=$('<div id="folder-selector" class="popupmenu"></div>');n.href="#";n.className="icon";$.each(this.env.mailboxes_list,function(){var a=0,b=0,d=g.env.mailboxes[this],h=d.id,q=$(n.cloneNode(!1)),v=$("<li>");d.virtual?q.addClass("virtual").attr("aria-disabled","true").attr("tabindex","-1"):q.addClass("active").data("id",d.id);
...
this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}};this.print_dialog=function(){bw.safari?setTimeout("window.print()",10):window.print()}}rcube_webmail.long_subject_title=function(g,u){if(!g.title){var a=$(g);a.width()+15*(u||0)>a.parent().width()&&(g.title=rcube_webmail.subject_text(g))}};
