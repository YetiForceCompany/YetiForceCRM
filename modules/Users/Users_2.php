...
class Users extends CRMEntity
{
	...
	{
		$db = \App\Db::getInstance();
		$userModel = \App\User::getUserModel($this->id);
		$setUpdater = false;
		...
		if ($this->mode !== 'edit') {
			$isExists = (new \App\Db\Query())
				->from('vtiger_users')
				->where(['user_name' => $this->column_fields['user_name'], 'email1' => $this->column_fields['email1']])
				->exists();
			if ($isExists) {
...
//				\App\Privilege::setAllUpdater();
//			}
			$isAdmin = $this->column_fields['is_admin'] === 'off' ? false : true;
			if ($isAdmin !== $userModel->getDetail('is_admin')) {
				$setUpdater = true;
			}
			if ($userModel->getDetail('roleid') !== $this->column_fields['roleid']) {
		...
		$prevReminderInterval = $userModel ? $userModel->getDetail('reminder_interval') : null;
...
//		$this->saveHomeStuffOrder($this->id);
		...
		$this->resetReminderInterval($prevReminderInterval);
...
//		echo '<pre>', var_dump($setUpdater,$userModel->getDetail('record_id'),$this->column_fields['roleid'],$userModel->getDetail('roleid') !== $this->column_fields['roleid']); echo '</pre>';
		if($setUpdater){
			
			\App\Privilege::setAllUpdater();
//			echo '<pre>', var_dump($setUpdater); echo '</pre>'; exit;
		}
	}
